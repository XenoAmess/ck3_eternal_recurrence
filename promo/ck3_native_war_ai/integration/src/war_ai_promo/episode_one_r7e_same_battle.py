"""Render Episode 1 R7E with an evidence-first opening and brown-gold visual system."""
...
from .visuals import BG, PANEL, INK, MUTED, GOLD, FAINT_RULE, make_canvas
...
def _opening_stage(index: int, source_frame: Path, target_frame: Path, output: Path) -> None:
    """Show both adjacent native frames before introducing the calculation cards."""
    canvas = make_canvas()
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((0, 0, 2559, 126), fill=PANEL)
    draw.rectangle((0, 126, 2559, 133), fill=GOLD)
    headings = (
        "墨西拿 · 罗贝尔守方",
        "第 5 日 1,222   →   第 6 日 1,156",
        "数字跳得很快，账却是一笔一笔记下的",
    )
    _text(draw, (46, 30), headings[index], 52 if index != 1 else 55,
          fill=INK if index < 2 else GOLD, bold=True)
    for position, frame, label in (
        (24, source_frame, "第 5 日 · 守方 1,222"),
        (1288, target_frame, "第 6 日 · 守方 1,156"),
    ):
        with Image.open(frame) as image:
            picture = image.convert("RGB").resize((1248, 936), Image.Resampling.LANCZOS)
        if position == 1288 and index == 0:
            picture = ImageEnhance.Brightness(picture).enhance(.35)
        canvas.paste(picture, (position, 154))
        draw.rectangle((position - 3, 151, position + 1251, 1092), outline=GOLD, width=3)
        draw.rounded_rectangle((position + 18, 175, position + 562, 243), radius=4,
                               fill=PANEL, outline=FAINT_RULE, width=2)
        _text(draw, (position + 34, 184), label, 31,
              fill=INK if position == 24 or index else MUTED, bold=True)
    draw.rectangle((0, 1120, 2559, 1439), fill=BG)
    draw.rectangle((0, 1120, 2559, 1126), fill=GOLD)
...
def _stage(row: dict, index: int, source_frame: Path, target_frame: Path, output: Path) -> None:
    if row["id"] == "E1-F01":
        _opening_stage(index, source_frame, target_frame, output)
        return
    canvas = make_canvas()
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((0, 0, 2559, 72), fill=PANEL)
    draw.rectangle((0, 72, 2559, 79), fill=GOLD)
    _text(draw, (34, 7), row["visual_title"], 46, fill=INK, bold=True)
    ...
    draw.rectangle((20, 88, 1372, 1104), outline=GOLD, width=3)
    draw.rounded_rectangle((40, 102, 1290, 180), radius=4,
                           fill=PANEL, outline=FAINT_RULE, width=2)
    _text(draw, (57, 110), f"原版实机定格 · 原片 {frame_second} 秒", 31,
          fill=GOLD, bold=True)
    _text(draw, (57, 149), FRAME_LABELS[frame_second], 26, fill=INK)
    draw.rounded_rectangle((1394, 92, 2538, 1104), radius=4,
    ...
    for claim_index, claim in enumerate(row["visual_lines"]):
        ...
        draw.rounded_rectangle((1424, top, 2506, top + 243), radius=4,
                               fill="#453724" if active else "#29211A",
                               outline=GOLD if active else FAINT_RULE, width=2)
        _text(draw, (1445, top + 13), f"{claim_index + 1:02d}", 31,
              fill=GOLD if active else MUTED, bold=True)
        ...
        for line_index, line in enumerate(wrapped):
            _text(draw, (1510, first_line_y + line_step * line_index), line, claim_size,
                  fill=INK if active else MUTED, bold=active)
    # Episode 0 keeps the subtitle bed plain brown; no footer copy under captions.
    draw.rectangle((0, 1120, 2559, 1439), fill=BG)
    draw.rectangle((0, 1120, 2559, 1126), fill=GOLD)
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open("xb") as stream:
...
def _paired_clip(row: dict, source_video: Path, target: Path,
    ...
    if row["id"] == "E1-F01":
        frame_durations = [7 + fade, 10 + fade, duration - 17]
        offset_1, offset_2 = 7, 17
    else:
        frame_duration = (duration + 2 * fade) / 3
        frame_durations = [frame_duration] * 3
        offset_1 = frame_duration - fade
        offset_2 = 2 * offset_1
    filters = [
        f"[{index}:v]trim=duration={frame_durations[index]:.6f},settb=AVTB,setpts=PTS-STARTPTS,format=yuv420p[s{index}]"
...
def compose(config, run, *, config_path, run_path, workdir,
    ...
    if config.project_id != "ck3-war-ai-episode-01-r7e-same-battle":
        raise ValueError("Wrong ProjectConfig for original Messina recut")
    inputs = load(_artifact(run, run_path, "r7e-production-inputs"))
    rows = inputs["cues"]
    ...
    return PipelineInvocation(
        PipelineDraft(config, tuple(segments), Path("episode-01-r7e-messina-unmixed.mp4"),
                      "episode-01-r7e-messina-unmixed-v1", "video/mp4"),
