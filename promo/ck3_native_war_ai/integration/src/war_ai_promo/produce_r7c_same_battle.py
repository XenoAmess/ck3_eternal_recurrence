"""Preserve and produce the muted-palette R7C Messina EdgeTTS film."""
...
def main() -> None:
    ...
    sources = [
        ...
        ("r7c-script", args.script.resolve(strict=True), "raw", "script"),
        ("r7c-timeline", args.timeline.resolve(strict=True), "raw", "timeline"),
    ...
    command(cli + ["build", str(manifest), "--workdir", str(attempt / "build"),
                   "--composer", "war_ai_promo.episode_one_r7c_same_battle:compose", "--offline-tts"],
            logs, "build")
    candidates = list((attempt / "build").rglob("episode-01-r7c-messina-unmixed.mp4"))
    if len(candidates) != 1:
        raise ValueError(f"Expected one R7 unmixed film; got {len(candidates)}")
    unmixed = candidates[0]
    mixed = attempt / "episode-01-r7c-theme-mixed-before-chapters.mp4"
    ...
    receipt = {
        "schema": "ck3-war-ai-episode-01-r7c-build.v1",
        ...
        "palette": "muted iron gray, deep burgundy and aged brass; blank subtitle strip",
