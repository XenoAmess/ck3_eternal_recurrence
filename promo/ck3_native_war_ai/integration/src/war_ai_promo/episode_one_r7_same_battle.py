"""Render Episode 1 R7 with the original Messina battle beside every calculation."""
...
def _stage(row: dict, index: int, source_frame: Path, target_frame: Path, output: Path) -> None:
    canvas = Image.new("RGB", (2560, 1440), "#111821")
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((0, 0, 2559, 72), fill="#221b18")
    draw.rectangle((0, 72, 2559, 79), fill="#B89357")
    _text(draw, (34, 7), row["visual_title"], 46, fill="#F5E9D7", bold=True)
    ...
    draw.rectangle((20, 88, 1372, 1104), outline="#B89357", width=4)
    draw.rounded_rectangle((40, 102, 1290, 180), radius=12, fill="#19242A")
    _text(draw, (57, 110), f"原版实机定格 · 原片 {frame_second} 秒", 31,
          fill="#E1C894", bold=True)
    _text(draw, (57, 149), FRAME_LABELS[frame_second], 26, fill="#EDE8DF")
    draw.rounded_rectangle((1394, 92, 2538, 1104), radius=18,
                           fill="#221D1B", outline="#B89357", width=3)
    ...
    for claim_index, claim in enumerate(row["visual_lines"]):
        ...
        draw.rounded_rectangle((1424, top, 2506, top + 243), radius=15,
                               fill="#312923" if active else "#252523",
                               outline="#D4A95F" if active else "#655D51", width=3)
        _text(draw, (1445, top + 13), f"{claim_index + 1:02d}", 31,
              fill="#E1C894" if active else "#998B78", bold=True)
        ...
        for line_index, line in enumerate(wrapped):
            _text(draw, (1510, first_line_y + line_step * line_index), line, claim_size,
                  fill="#FCF5E7" if active else "#A39A8E", bold=active)
    draw.rectangle((0, 1120, 2559, 1439), fill="#10151C")
    draw.rectangle((0, 1120, 2559, 1126), fill="#B89357")
    _text(draw, (35, 1142), "画面：原始战局录像；兵力为游戏界面取整。右侧定点数来自同场原生回执。",
          28, fill="#DDCBAA")
...
def compose(config, run, *, config_path, run_path, workdir,
    ...
    if config.project_id != "ck3-war-ai-episode-01-r7-same-battle":
    ...
    return PipelineInvocation(
        PipelineDraft(config, tuple(segments), Path("episode-01-r7-messina-unmixed.mp4"),
                      "episode-01-r7-messina-unmixed-v1", "video/mp4"),
