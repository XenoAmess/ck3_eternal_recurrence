"""Preserve and produce the evidence-first R7E Messina film."""
...
def main() -> None:
    ...
    if not inputs_path.is_file():
        raise FileNotFoundError("R7E speech inputs missing")
    if any((attempt / name).exists() for name in ("automation", "build", "theme-mix", "build-receipt.json")):
        raise FileExistsError("R7E production already started; use a new attempt")
    inputs = json.loads(inputs_path.read_text(encoding="utf-8"))
    rows = inputs["cues"]
    if len(rows) != 23 or inputs["provider"] != "edge" or not 1200 <= inputs["actual_duration_seconds"] <= 2400:
        raise ValueError("Expected 23 R7E EdgeTTS cues of 20–40 minutes")
    script_rows = json.loads(args.script.read_text(encoding="utf-8"))["cues"]
    if len(script_rows) != len(rows) or any(
        ...
        for original, prepared in zip(script_rows, rows)
    ):
        raise ValueError("Frozen speech inputs do not match the R7E script")
    ...
    logs.mkdir()
    sources = [
        ("r7e-production-inputs", inputs_path, "derived", "production-inputs"),
        ("r7e-script", args.script.resolve(strict=True), "raw", "script"),
        ("r7e-timeline", args.timeline.resolve(strict=True), "raw", "timeline"),
    ...
    command(cli + ["build", str(manifest), "--workdir", str(attempt / "build"),
                   "--composer", "war_ai_promo.episode_one_r7e_same_battle:compose", "--offline-tts"],
            logs, "build")
    candidates = list((attempt / "build").rglob("episode-01-r7e-messina-unmixed.mp4"))
    if len(candidates) != 1:
        raise ValueError(f"Expected one R7E unmixed film; got {len(candidates)}")
    unmixed = candidates[0]
    mixed = attempt / "episode-01-r7e-theme-mixed-before-chapters.mp4"
    ...
    receipt = {
        "schema": "ck3-war-ai-episode-01-r7e-build.v1",
