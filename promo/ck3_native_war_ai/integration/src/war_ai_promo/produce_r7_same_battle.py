"""Preserve and produce the Messina-bound Episode 1 R7 EdgeTTS film."""
...
def main() -> None:
    ...
    sources = [
        ...
        ("r7-script", args.script.resolve(strict=True), "raw", "script"),
        ("r7-timeline", args.timeline.resolve(strict=True), "raw", "timeline"),
    ...
    command(cli + ["build", str(manifest), "--workdir", str(attempt / "build"),
                   "--composer", "war_ai_promo.episode_one_r7_same_battle:compose", "--offline-tts"],
            logs, "build")
    candidates = list((attempt / "build").rglob("episode-01-r7-messina-unmixed.mp4"))
    if len(candidates) != 1:
        raise ValueError(f"Expected one R7 unmixed film; got {len(candidates)}")
    unmixed = candidates[0]
    mixed = attempt / "episode-01-r7-theme-mixed-before-chapters.mp4"
    ...
    receipt = {
        "schema": "ck3-war-ai-episode-01-r7-build.v1",
