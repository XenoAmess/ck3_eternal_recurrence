...
新 attempt 可通过 `--capture-spec` 接入真实 GREEN bundle 的连续片段，具体见 [实机集成](README.capture-integration.md)。准备器 5 项、集成 4 项聚焦测试通过，成功媒体测试使用明确标注的合成素材；尚未完成 GREEN 游戏录像验收。实机分支移除默认尾帧克隆和帧率转换，不通过补画面凑时。新 attempt 保存导演案 v3 和新主张台账。
