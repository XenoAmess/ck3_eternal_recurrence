...
第 1 集 R6.2 的[实机对拍审计](episode-01-battle-win-probability/r62-gameplay-alignment-audit.md)发现：公式卡与实机分别正确并不足以构成“对着实际战况计算”。之后的战争机制片在冻结文案前必须为每段计算建立同战斗、同日边界、同兵团的镜头账，核对可见 UI 与原始回执；无法绑定的跨案例算术移出实战时间线。剪辑完成后再按最终 MP4 时间轴逐段复核，不以素材库存中“存在实机录像”代替成片实际使用。
