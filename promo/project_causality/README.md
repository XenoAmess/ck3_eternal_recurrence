# 《project因果律》8:20 宣传片工程

> 副标题：伪天司的辉煌愿景  
> 状态：无音乐预演片已生成；未完成配乐、连续人工审片、签核或发布  
> 冻结日：2026-09-19（Asia/Shanghai）

本目录把体系级导演案落实为精确 `8:20` 的生产工程。目标是生成一支简体中文主旁白、简体中文主字幕、英文副字幕的本地正式候选，
同时保持产品实机、验收证据、生成概念图和目标愿景之间的边界。

## 当前冻结

| 项目 | 冻结值 |
|---|---|
| 主仓 commit | `909317ddc5d03b1283aacfb428b7803945175a03` |
| CK3 exact build | `1.19.0.6` |
| 片长 | `500.000 s` / `15000` frames at 30 fps |
| 画幅 | `2560×1440` |
...
`2026-09-19-r1` 是用于校验结构、旁白、字幕和事实边界的本地预演，不是正式成片。它使用已存在的实机静帧、家徽编辑器生产页
GET-only 录屏、历史有界自动玩家片段和生成章节图；命令行、发布回读及部分静帧仍可在精剪时换成干净动态素材。预演已自动验证为
15,000 个视频帧，但仍必须经过项目
所有者 1× 连续观看，之后才可据此调整旁白与镜头。
