...
| 04 — Principle / Evidence Has Weight | `35:50–42:30` | 实际 `04:33.600` | 从 `04:10` 续写 `03:00` | 不续写 | `07:10` |
...
| Track | 入选文件 | 实际时长 | 选择依据 |
|---|---|---:|---|
| 00 | `00-prologue-a.wav` | `04:29.960` | 动态略大、结构留白更清楚，适合冷开场 |
| 01 | `01-spell-a.wav` | `05:59.880` | 比 B 略克制，较适合作为长旁白下的产品展示底床 |
| 02 | `02-robert-b.wav` | `05:59.920` | A 只生成到 `04:19.720`；B 能覆盖既定续写点并保持战役推进 |
| 03 | `03-method-a.wav` | `05:59.560` | 完整覆盖六分钟，段落反差最清晰，适合工程流程章 |
| 04 | `04-principle-a.wav` | `04:33.600` | 两版都未达到六分钟；A 更安静、峰值余量更大，符合全片最稀疏章节 |
| 05 | `05-vision-a.wav` | `05:59.640` | 比 B 低 `0.8 LUFS`，保留终章上扬感但更不抢旁白 |

技术审计见 `artifacts/project-causality/2026-09-20-r14/music-audit/`。第二轮只按 `suno-continuation-round.md` 操作；不要续写未入选候选。
...
- 覆盖：`35:50–42:30`
- 第一轮母带原设 Duration：`06:00`；入选版实际生成到 `04:33.600`
- 第二阶段：从母带 `04:10` Continue，Duration：`03:00`
- Weirdness：20%
- Style Influence：88%
...
母带入选后，从 `04:10` 续写 `03:00`，Style Box 与 Exclude Styles 不变：
...
入选母带已经确定。严格按 `suno-continuation-round.md` 续写：01=`05:30`，02=`03:45`，03=`06:00` 后再 `02:00`，04=`03:00`，05=`06:00`。所有单次 Duration 均不超过 `06:00`。每一步都先在 A/B 中选出入选版本，再从该版本继续；不要同时从两个候选分叉续写。
