...
- 从本机 1.19.0.6、简中、2560×1440、non-debug 的 15 份历史主菜单截图冻结的像素探针。四枚探针覆盖左侧菜单、
  按钮边框与固定建筑背景；模态变暗或页面替换必须令分类变为 `unknown`。画面中部是轮换背景，2026-08-22 实机均值
  `(61.90,47.43,43.75)` 已证明其不能被冻结为纯黑，因此不再用中央黑块作分类条件。
