...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。最新 run 完成 `159/160` turns、61 gameplay 与 19 checkpoints，durable checkpoint 已到 `53256000 / history=5183 / EE6D1B37...06D85`。CharacterID `29829` / episode 不变且存活；cleanup/tree-gone 全绿，现停在独立 GEN-027 | 从最新 checkpoint 用显式 committed-route canary 冷恢复；只提升真实 B0/B1，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024 resolved；GEN-025 live pending；GEN-026 static-ready；GEN-027 static-ready；G1 未完成 |
...
| GEN-006 | B1 | 自然死亡与结算 | strict runner 在死亡后继续等待琉焰卿 Mod 的 committed settlement 与必要 record persistence；只接受本次执行且 `ready=true`、`commit_serial=1`、source CharacterID/settlement/cross-run record/no-heir/cleanup 全部匹配的 `death-terminal`。`terminal-settlement.json.one_life_settlement.final_score` 是权威“人生分数”，并须等于顶层 `score` 与 `recorded_episode.score`。08-28 最新 checkpoint 已到 `53256000`，角色仍活且没有匹配 terminal/committed settlement | 继续 production 长跑；观测自然死亡，等待 committed scoring，生成匹配 CharacterID `29829` 且三处人生分数一致的 `terminal-settlement.json`，并以全部 qualification gates GREEN 正常终止 | aggregate static-ready；等待当前 formal continuation 与自然 terminal |
| GEN-007 | B2/B3 | 战斗质量与吞吐 | ordinary active combat 已用 exact-build decision-epoch sentinel 在 production selector 运行 speed 3；不再逐日暂停/RQ。全军 watch 的 terminal primitive 已实机证明 speed 5 到真实终点时零中间暂停、零 running rich query、零过冲。reinforcement assigned/join、异常 terminal、forecast 与双 `4x` crush 样本仍未全闭合 | G1 立即使用普通战 speed 3；player-won pursuit 已具 production selector，但须等真实胜局复验。双 `4x` crush 只在真实 qualifying checkpoint 做完整 `1,2,3,4,5,5,4,3,2,1` 平衡矩阵；`[1,5,5,1]` 仅是吞吐子投影，不再把未出现样本阻断长跑 | ordinary speed-3 production-live loop；full-watch speed-5 terminal production-live primitive；crush selector live pending |
...
| GEN-027 | B1→吞吐 P0 | embark route 边界的 arrival timeline unavailable；旧路线又要求逐日 query/pause | formal run `20260828T064918Z-one-generation-38aa5830` 在 `53256000` 的共享首边命中 `progress=0 / cached speed=0 / recalculated speed=0`，`0x2247320` 会错误扣 `0xffffffff`，故必须拒绝。run 为 `159/160`、61 gameplay、19 checkpoints，report/blocker `F8F834A3...E9D1A / E409EC42...19A9B`，cleanup 全绿 | reader 只在该 exact 边界用 `0x22475E0` 算首边，再把 index-1 shallow tail 交回 `0x2247320`；同时以独立显式 `committed-route-sentinel-advance-army-<subject>-to-<target>-until-<date>`、speed 3、完整 watch、`combat_count=0` 取代逐日 query/pause。首次 cold canary 必须零 external pause/RQ/overshoot，并严格回传相同 scope/subject/target | narrow ETA fallback + explicit route sentinel static-ready；native 39/39、Python 1482/skipped3/990 subtests；fresh canary pending |
