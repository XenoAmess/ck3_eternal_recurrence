...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。`9186bfa3` 从旧 anchor cold 恢复并完成 `20/20` turns，实机跨过 GEN-030/031 后推进到 `53279256 / history=6159 / 0DF9CB66...69C`；CharacterID `29829` / episode `native-29829-ee172aa720db` 不变且存活，cleanup 全绿 | 从最新 checkpoint 直接继续正式 `50000 / 604800 / cadence 3 / speed 3` 长跑；只提升真实 B0/B1，普通 bound 直接续跑，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027/028/029/030/031 resolved；G1 未完成，待续跑与自然 terminal |
...
| GEN-006 | B1 | 自然死亡与结算 | strict runner 在死亡后继续等待琉焰卿 Mod 的 committed settlement 与必要 record persistence；只接受本次执行且 `ready=true`、`commit_serial=1`、source CharacterID/settlement/cross-run record/no-heir/cleanup 全部匹配的 `death-terminal`。`terminal-settlement.json.one_life_settlement.final_score` 是权威“人生分数”，并须等于顶层 `score` 与 `recorded_episode.score`。08-28 最新 checkpoint 已到 `53279256`，角色仍活且没有匹配 terminal/committed settlement | 继续当前 production 长跑；观测自然死亡，等待 committed scoring，生成匹配 CharacterID `29829` 且三处人生分数一致的 `terminal-settlement.json`，把该分数明确写入报告，并以全部 qualification gates GREEN 正常终止 | aggregate static-ready；等待自然 terminal 与 committed 人生分数 |
