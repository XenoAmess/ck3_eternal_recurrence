...
| M4 real faction intervention | private budgeted gift policy static-ready, public OFF | normal production faction strategy after paused legality and gift binding | masterda3fbed pure private faction_gift_policy_v1 | native FACTION6/16 fields are typed inputs; focused normal/-O5/5; no real gift/opinion/faction next turn | no faction cold restore | Bind to one targeting landed member in a real paused two-year window; m4_faction_min | temp branch refs cleaned; integrated clean clone physical recursive deletion auto-review rejected |
