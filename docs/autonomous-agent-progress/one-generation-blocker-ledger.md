...
| Delivery gate / capability | Status | Formal entry | Artifact / source | Actual game and next-turn evidence | Restore evidence | B0/B1 gap / owner | Branch cleanup |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Primary-defender exact-capital contact transition | production capability RED; existing native primitive, scoped Python projection/ordering repair in progress | formal `g2_preview_operator.py run` -> `native_auto_run` | master `351d4dfb`; report `7D0843ED...8524`; operator `FFE1BB5A...68B2F`; final driver `0E4A859A...7CDC`; external seal `D4993C4D...BCB1` | R864 hold branch was selected twice. At the next decision frame enemy184549393 targeted capital45 over `[8747,23,8749,45]`; generic `native_war_no_safe_target` stopped before the stationary horizon query, with no failing-turn command | h1179/date53281008 `8CF78229...29CB` is the sole anchor. It includes the confirmed h1174 raise. h1180-h1183 are discarded; no unconfirmed action and no duplicate-raise permission | Project/query the existing target=current contact horizon only for strict primary-defender fresh-capital frames; contact-free -> proof-bound one-day advance, unavoidable -> existing strict transition, stale/unavailable/route-away -> RED. No native ABI/schema/MCP change; `/root/war_recovery_decision`, `/root` | implementation worktree active; R865 PID64384/injector reclaimed; authority remains preview GO, GEN-034 3/4, Council 1/4, G2 1/8 |
