...
| GEN-034 | B1（G2） | Raiktor 特殊战争可合法投降，但 CB-specific actual terms 尚未形成可执行闭环 | frozen continuation 的 CharacterID `29829` / WarID `50331699` 为 primary attacker，`raiktor_claim_cb`、1281 日、战分 `-50`；surrender validator/available/auto-accept/`would_accept_now` 全真。gold、F/prestige、PoW、favor-hook 四域已有 production-live 只读 wire；`6be8831` 已将 truce 的非负 `evaluated_days` 接入同一 public terms wire，但 persisted expiry 固定为 `false/null`，且尚无 paused/live shape artifact；generic war-bound current/cleanup 与 claims-base+六动态域聚合仍由 `c0455b6 / 94d367e / 5e2dfea` 保持 static/fixture-ready。`630ff35` 又闭合 conservative pairwise policy core，但当前仍缺 generic 同帧 input、campaign dominance certificate、owner budget profile 与 white-peace evaluation，故 policy 仍 underdetermined、无 action literal。source attribution、pre soldiers、proven loss 仍为 false | 完成 truce duration 的 paused/live shape probe，接入 generic war-bound/aggregate 的同帧 production reader/MCP，补 campaign certificate、显式 budget profile 与 white-peace comparison，再将 policy 接到 typed surrender；普通 `claim_cb` 不变，旧 crash reader继续禁用。随后一次 CK3 启动完成双查询、唯一 submit、六域 postcondition与 postwar checkpoint；若策略要求 source-specific loss，必须先闭合 attribution/pre/loss，不能用 authored 3000 猜测。入口：[Raiktor 三方退出策略专文](../ck3-native-ai/raiktor-three-way-exit-policy.md)；[四域 terms runner](../../ck3_autonomous_player/native_bridge/research/run_war_termination_terms_live_acceptance.py)、[policy/four-domain contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_continue_vs_surrender_policy_v1_contract.json)、[GEN-034 boundary](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_gen034_boundary_v1.json)、[六域 source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_surrender_six_domain_v1_source_contract.json)、[truce source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_surrender_truce_v1_source_contract.json)、[war-bound source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_war_bound_regiment_v1_source_contract.json) | **GEN-034 unresolved**；四域 read-only production-live；truce duration public/query-wired 但 paused/live pending；六域 aggregate 与 policy core static/fixture-ready/not-live；generic public wire、full-exit policy、MCP action 与 live matrix pending |
