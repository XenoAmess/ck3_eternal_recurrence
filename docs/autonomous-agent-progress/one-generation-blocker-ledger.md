...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。最新 durable checkpoint 仍为 `53266608 / history=5814 / 1FADF1C0...B34D`；CharacterID `29829` / episode 不变且存活。GEN-028 已跨过，GEN-029 已从 timeout 假设精确收敛为 player-event boundary 漏接，重复复现 cleanup/tree-gone 全绿 | 最小解除 GEN-029 后从最新 checkpoint 直接恢复正式 `50000 / 604800 / cadence 3` 长跑；只提升真实 B0/B1，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027/028 resolved；GEN-029 open；G1 未完成 |
...
| GEN-006 | B1 | 自然死亡与结算 | strict runner 在死亡后继续等待琉焰卿 Mod 的 committed settlement 与必要 record persistence；只接受本次执行且 `ready=true`、`commit_serial=1`、source CharacterID/settlement/cross-run record/no-heir/cleanup 全部匹配的 `death-terminal`。`terminal-settlement.json.one_life_settlement.final_score` 是权威“人生分数”，并须等于顶层 `score` 与 `recorded_episode.score`。08-28 最新 checkpoint 已到 `53266608`，角色仍活且没有匹配 terminal/committed settlement | 继续 production 长跑；观测自然死亡，等待 committed scoring，生成匹配 CharacterID `29829` 且三处人生分数一致的 `terminal-settlement.json`，并以全部 qualification gates GREEN 正常终止 | aggregate static-ready；等待 GEN-029 后的 formal continuation 与自然 terminal |
| GEN-007 | B2/B3 | 战斗质量与吞吐 | ordinary active combat 与 committed route 已用 speed-3 sentinel production-live。新 stationary-objective canary 连续完成 20 个 7 日 arm，共 140 日；每臂零 external/intermediate pause、零 external RQ/overshoot，post-stop 全 matched。speed-3 arm 平均 `6.012s`，含 query/planner/checkpoint 的稳态持续吞吐为 `62.53 日/分钟` | 该结果已越过项目所有者最新指定的 `>=60 日/分钟` hard gate，但仅约 4.2% 余量；G1 保留已验证 speed 3，性能线程继续做 stationary/route speed-5 matrix 与最小 gate，冲刺 120。player-won pursuit 仍等真实胜局；双 `4x` crush 只在 qualifying checkpoint 做完整 `1,2,3,4,5,5,4,3,2,1` 矩阵 | ordinary/route speed-3 production-live；stationary speed-3 20-arm live canary、待 GEN-029 后晋级；speed-5 terminal primitive live；60 hard 已过、120 stretch 未达 |
...
| GEN-029 | B1 harness + 吞吐 P0 | stationary sentinel 漏接 ordinary player-decision boundary，导致事件已阻断日期却继续空等 | `90d3cf79`、`71e3b7c1` 与 `e8cec411` 三次都从 `53266944` 到同一 `53267040` 后不再推进；把 wait 从 30 秒扩到 60 秒没有改变日期。driver-state 最终帧存在 `active_event instance_id=47`、8 options，native revisions 正常前进；CPU/日志采样也排除持续重算。根因是事件阻断日期但 global paused/sentinel pause 尚未满足旧等待谓词 | active event 或 pending interaction 出现时立即作为真实 player-decision boundary，有界 pause/stabilize并交回既有 policy；不得当作 sentinel terminal，也不得改变战争意愿。保留其它 sentinel final-stage/overshoot/post-stop；撤销已被反证的专用 60 秒 wait，随后从同一 anchor cold replay | open；exact root cause live-confirmed，最小 policy-neutral 修复施工中 |
