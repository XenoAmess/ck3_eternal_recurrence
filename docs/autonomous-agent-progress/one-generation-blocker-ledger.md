...
| Primary-defender post-battle native-rally hold | production capability RED; strict Python repair in progress | formal `g2_preview_operator.py run` -> `native_auto_run` | R866 report `EEB33B41...4403`; R867 report `EA07E451...4CBA0`; h1340 `BE4E0C73...13C5A`; RED seal `F8B1B47B...B2198` | Exact contact entered real combat; battle-control/decision epochs consumed its terminal; old Army234881216 disappeared; fresh raise created Army184549472 at8750 and next turn consumed gathering->regular. Turn80 submitted nothing | h1340/date53282424 contains the confirmed fresh raise and regular replacement army; h1341 is read-only; no unconfirmed action and repeat raise forbidden | Existing hold dispatch only accepts exact capital45. Add strict durable raise-tail/ArmyID/province-bound safe rally hold, retaining all event/multi-war/unsafe/contact/exit fail-closed gates; no move into occupied capital and no blind surrender; `/root/war_recovery_decision` | implementation branch/worktree active; source master remains `bf8930ac`; CK3/injector zero |
