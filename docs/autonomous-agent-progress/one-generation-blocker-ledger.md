...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。最新 formal continuation 完成 `485/486` turns、148 gameplay 与 49 checkpoints，durable checkpoint 已到 `53263584 / history=5735 / 4620F2EE...AFA99A`。CharacterID `29829` / episode 不变且存活；cleanup/tree-gone 全绿，现停在 GEN-028 | 最小解除 GEN-028 后从最新 checkpoint 直接恢复正式 `50000 / 604800 / cadence 3` 长跑；只提升真实 B0/B1，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027 resolved；GEN-028 open；G1 未完成 |
...
| GEN-006 | B1 | 自然死亡与结算 | strict runner 在死亡后继续等待琉焰卿 Mod 的 committed settlement 与必要 record persistence；只接受本次执行且 `ready=true`、`commit_serial=1`、source CharacterID/settlement/cross-run record/no-heir/cleanup 全部匹配的 `death-terminal`。`terminal-settlement.json.one_life_settlement.final_score` 是权威“人生分数”，并须等于顶层 `score` 与 `recorded_episode.score`。08-28 最新 checkpoint 已到 `53263584`，角色仍活且没有匹配 terminal/committed settlement | 继续 production 长跑；观测自然死亡，等待 committed scoring，生成匹配 CharacterID `29829` 且三处人生分数一致的 `terminal-settlement.json`，并以全部 qualification gates GREEN 正常终止 | aggregate static-ready；等待 GEN-028 后的 formal continuation 与自然 terminal |
| GEN-007 | B2/B3 | 战斗质量与吞吐 | ordinary active combat 已用 exact-build decision-epoch sentinel 在 production selector 运行 speed 3；当前 episode 两臂连续推进 `15+24` 日到 terminal，未因 phase/winner 粗变化暂停。committed route 的 5 臂共推进 44 日并在接触同日停表。两条路径都不再逐日暂停/RQ。全军 watch 的 terminal primitive另已实机证明 speed 5 到真实终点时零中间暂停、零 running rich query、零过冲 | G1 立即使用普通战与 committed route speed 3；进一步把普通战争暂停压到真正 policy invalidation。player-won pursuit 已具 production selector但须等真实胜局复验；双 `4x` crush 只在真实 qualifying checkpoint 做完整 `1,2,3,4,5,5,4,3,2,1` 平衡矩阵 | ordinary/route speed-3 production-live loop；full-watch speed-5 terminal production-live primitive；crush zero-pause selector research/live pending |
...
| GEN-028 | B1 + 吞吐 P0 | 全局 warscore 下降被错误绑定为玩家当前省战败，且多战争重复逐日终战 RQ | formal run `20260828T082919Z-one-generation-ca52af74` 为 `485/486`、148 gameplay、49 checkpoints。history `5739` 中 WarID `33554565` 因远端 occupation 从 `50→16`；Army `201326874` regular@objective `2635`，同省敌军为零。`_recent_war_tactics` 仍写入 `2635` 的 90 日 cooldown，随后 route candidate 被标 blocked。同期三场战争形成 `337 query / 148 gameplay` | 本地 defeat memory 现只在真实同省非撤退 hostile/contact 时写入；negative termination 仅复用 7 日且在 `claim_cb` 第 365 日提前到期，stale positive 永不授权动作。完整 unit `1488 passed / 3 skipped / 1006 subtests`；从 `53263584 / 4620F2EE...AFA99A` cold replay，stationary objective speed-3 sentinel 紧随其后做吞吐 canary | static-ready；等待合并后的 cold replay 才可 resolved/live |
