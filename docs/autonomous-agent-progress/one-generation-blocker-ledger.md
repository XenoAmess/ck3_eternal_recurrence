...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。GEN-029 cold GREEN 后最新 durable checkpoint 为 `53270568 / history=5921 / 98B99BBB...560B9`；CharacterID `29829` / episode 不变且存活。正式 run `cbbd3fab` 已按 `50000 / 604800 / cadence 3 / speed 3` 从该点恢复 | 继续当前正式长跑；只提升真实 B0/B1，普通 bound 直接续跑，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027/028/029 resolved；正式 G1 running；未完成 |
