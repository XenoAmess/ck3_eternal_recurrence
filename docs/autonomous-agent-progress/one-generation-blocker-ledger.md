...
| GEN-034 | B1（G2） | Raiktor 特殊战争可合法投降，但 CB-specific actual terms 尚未形成可执行闭环 | frozen continuation 的 CharacterID `29829` / WarID `50331699` 为 primary attacker，`raiktor_claim_cb`、1281 日、战分 `-50`；surrender validator/available/auto-accept/`would_accept_now` 全真。gold、F/prestige、PoW、favor-hook 四域已有 production-live 只读 wire；`6be8831` 已将 truce 的非负 `evaluated_days` 接入同一 public terms wire，但 persisted expiry 固定为 `false/null`。一次 pre-reset live 已将失败收窄为 `root_shape_drift`；随后唯一 staged private live 又精确证明首个失败检查为 `root_capacity_mismatch`：root vtable RVA `0x44CF030` 与预期一致，实际 `capacity/count=13/12`，旧合同为 `19/14`，因此尚未调用 duration evaluator。distinct root-child private live 完整枚举 12/12 个 vtable，证明 scripted-effect RVA `0x44CF0F8` 在 indices `6/7/9/10/11` 共出现五次；候选级 live 又得到 selector/default shape：`6=8/null`、`7=0/4/4`、`9=0/1/1`、`10=0/1/1`、`11=0/2/2`。sole-child live 已区分剩余两条路径：index 9 为 `Context(1/1)→0x44D1E18`，index 10 为 `0x44D1D50(1/1)→Context`；exact Context live 进一步确认两条 Context 均为 `scope/capacity/count=1/1/1`，但 child0 分别是 `0x44D1E18(1/1)` 与 `0x41E36D0(6/6)`，均非 Truce vtable `0x4461CA8`。generic war-bound current/cleanup 与 claims-base+六动态域聚合仍由 `c0455b6 / 94d367e / 5e2dfea` 保持 static/fixture-ready。`630ff35` 又闭合 conservative pairwise policy core，但当前仍缺 generic 同帧 input、campaign dominance certificate、owner budget profile 与 white-peace evaluation，故 policy 仍 underdetermined、无 action literal。source attribution、pre soldiers、proven loss 仍为 false | 下一包只沿新观察到的两种 shape：index 9 在 `0x44D1E18(1/1)` 下再读唯一 child0 vtable；index 10 仅枚举 `0x41E36D0(6/6)` 的六个 child vtable，找到精确 `0x4461CA8` 即停止。不得回访其它 root index 或重复 Context prefix，不得把旧 index/RVA 猜成 production 合同。唯一化后才修改 stale shape 合同并重验 duration。之后接入 generic war-bound/aggregate 的同帧 production reader/MCP，补 campaign certificate、显式 budget profile 与 white-peace comparison，再将 policy 接到 typed surrender；普通 `claim_cb` 不变，旧 crash reader继续禁用。最终一次 CK3 启动完成双查询、唯一 submit、六域 postcondition与 postwar checkpoint；若策略要求 source-specific loss，必须先闭合 attribution/pre/loss，不能用 authored 3000 猜测。入口：[Raiktor 三方退出策略专文](../ck3-native-ai/raiktor-three-way-exit-policy.md)；[私有 shape live 证据](../ck3-native-ai/g2-truce-private-live-capture-2026-09-02.md)；[四域 terms runner](../../ck3_autonomous_player/native_bridge/research/run_war_termination_terms_live_acceptance.py)、[policy/four-domain contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_continue_vs_surrender_policy_v1_contract.json)、[GEN-034 boundary](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_gen034_boundary_v1.json)、[六域 source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_surrender_six_domain_v1_source_contract.json)、[truce source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_surrender_truce_v1_source_contract.json)、[war-bound source contract](../../ck3_autonomous_player/native_bridge/research/fixtures/raiktor_war_bound_regiment_v1_source_contract.json) | **GEN-034 unresolved**；四域 read-only production-live；truce reader private live 已将剩余路径收窄为 index9 `0x44D1E18(1/1)` 与 index10 `0x41E36D0(6/6)`，但 Truce/duration/expiry 仍 not-live；六域 aggregate 与 policy core static/fixture-ready/not-live；generic public wire、full-exit policy、MCP action 与 live matrix pending |
