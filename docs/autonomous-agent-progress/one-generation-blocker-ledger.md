...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。最新 canary 完成 `83/84` successful turns、20 gameplay、6 checkpoints；durable checkpoint 已到 `53266608 / history=5814 / 1FADF1C0...B34D`。CharacterID `29829` / episode 不变且存活；cleanup/tree-gone 全绿，GEN-028 已跨过，现停在 GEN-029 harness timeout | 最小解除 GEN-029 后从最新 checkpoint 直接恢复正式 `50000 / 604800 / cadence 3` 长跑；只提升真实 B0/B1，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027/028 resolved；GEN-029 open；G1 未完成 |
...
| GEN-029 | B1 harness + 吞吐 P0 | 7 日 stationary sentinel 的固定 30 秒等待窗在真实性能抖动时过早终止 | canary `20260828T092300Z-one-generation-90d3cf79` 前 20 臂各 7 日全绿；第 21 臂从 `53266944` 在 30 秒内只推进到 `53267040`（4 日），尚未到 target `53267112`，于是报 `timed out before a native sentinel pause`。这不是 semantic stop；managed cleanup/tree-gone 全绿，durable anchor `53266608 / history=5814 / 1FADF1C0...B34D` 可恢复 | 只对 typed stationary 1..7 日 arm 提供有界扩展 wait；最终仍必须由 native pause/status、exact target、zero overshoot 与 fresh post-stop 验证。随后从最新 anchor cold replay；speed3 稳态 `62.53` 已过 60 hard，speed5/等效削减继续作为扩大余量与 120 stretch 优化 | open；最小 timeout 修复施工中，性能线程持续优化 |
