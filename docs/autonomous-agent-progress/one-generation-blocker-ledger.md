...
|---|---|---|---|---|---|
| GEN-001 | B0 | 一代人 supervised runner | strict `one_generation` 合同已在正常交互桌面多次 production-live。GEN-030 修复后 cold replay 又完成 2 gameplay 并推进到 `53278752`；本次未重现 event 51，随后因 GEN-031 war-query same-frame binding 停止。最新 durable checkpoint 仍为 `53278416 / history=6136 / 164A3960...02DE4`；CharacterID `29829` / episode 不变且存活 | 最小解除 GEN-031 后从最新 checkpoint 继续正式长跑；只提升真实 B0/B1，普通 bound 直接续跑，直到自然死亡、琉焰卿 committed scoring 与匹配 `terminal-settlement.json` | runner production-live；GEN-023/024/027/028/029 resolved；GEN-030 static-ready/live pending；GEN-031 open；G1 未完成 |
...
| GEN-006 | B1 | 自然死亡与结算 | strict runner 在死亡后继续等待琉焰卿 Mod 的 committed settlement 与必要 record persistence；只接受本次执行且 `ready=true`、`commit_serial=1`、source CharacterID/settlement/cross-run record/no-heir/cleanup 全部匹配的 `death-terminal`。`terminal-settlement.json.one_life_settlement.final_score` 是权威“人生分数”，并须等于顶层 `score` 与 `recorded_episode.score`。08-28 最新 checkpoint 已到 `53270568`，角色仍活且没有匹配 terminal/committed settlement | 继续当前 production 长跑；观测自然死亡，等待 committed scoring，生成匹配 CharacterID `29829` 且三处人生分数一致的 `terminal-settlement.json`，并以全部 qualification gates GREEN 正常终止 | aggregate static-ready；formal continuation running，等待自然 terminal |
| GEN-007 | B2/B3 | 战斗质量与吞吐 | ordinary active combat 与 committed route 已用 speed-3 sentinel production-live。stationary-objective 先完成 20 个 7 日 arm、140 日 `62.53 日/分钟`；GEN-029 修复后的 `6421f80c` 又完成 `100/100` turns，在事件边界正确停表并继续，共推进 165 日，含 75 query 与 8 checkpoint 的 turn-loop 为 `63.232 日/分钟` | speed 3 已重复越过 `>=60 日/分钟` hard gate；G1 保留已验证 speed 3。量化上 89.50% 墙钟在 native advance，性能线程继续做完全相同战争合同下的 stationary/route speed-4/5 matrix，冲刺 120。player-won pursuit 与双 `4x` crush 仍只等自然 qualifying checkpoint | ordinary/route speed-3 production-live；stationary speed-3 cold live GREEN、晋级处理中；60 hard 重复通过、120 stretch 未达 |
...
| GEN-030 | B1 harness | player decision 与 native sentinel 在 deadline 同日转移状态时仍盲目 cancel | formal `cbbd3fab` 在 `53278752` 同时观察 event 51 与七日 deadline；frame 已 paused，arm generation 50，但 cancel 在验证仍为同代后返回 `requires an armed generation`，证明状态已离开 armed。由于旧路径尚未 query status，本 artifact 不能单独排除 failed。失败前 `205/206` turns、52 gameplay、16 checkpoints；cleanup/tree-gone GREEN，最新 anchor `53278416 / 164A3960...02DE4` | `07f37a4` 已实现 status-first：同代 armed 走 cancel→idle；同代正常 triggered 必须通过完整 ordinary stop/date/tick/trigger/pause/overshoot 验证后免 cancel，并仍以 player-decision 交回事件策略；failed/无证明 idle 拒绝。cold replay 未重现 event 51，尚等自然同日分支 live | static-ready；Python full GREEN；exact same-day live pending，不阻止 GEN-031 解除后继续 G1 |
| GEN-031 | B1 native capability | war-termination options query 未绑定已发布 paused revision | cold `33876238` 在 `53278752 / native:25 / revision=26` 先成功查 WarID `83886203`，后查 `134217852` 时返回 row 与 published active row 的 side/primary-leader/player-relative-score 至少一项不一致；WarID 已证明一致。旧 native handler 忽略已发送 expected revision 且无 admission/completion snapshot sandwich；旧 failure artifact 没保留原始 response，不伪造具体 mismatch 值。report/blocker `71C0416A...4166 / 9D9B85C7...619D`，cleanup GREEN，anchor 仍 `164A3960...02DE4` | 复用 application-main read-only mailbox，消费现有 expected revision，只在 `previous==admission==completion` 时提交 query result/序列；明确 stale/changed 零写入拒绝走既有一次 whole-turn replan，持续漂移或其他四字段 mismatch 仍 RED 并保留最小 diff。不改战争或终战 policy | open；native/Python 最小修复施工中 |
