...
| GEN-013 | B1→B3 | 长跑 query/history 复制与持久化写放大 | 真实冻结 state 为 79,517,587 bytes；旧 threatened-siege 规划同帧执行 167 条查询，用户观测 121 条查询约 26 分钟。`a8ff95f` first-safe、`7cb0b75` 只读批量持久化及 `79b8d2a → e0688c7 → 9ff04ae` transcript/history 去复制已完成同 `A8DD...AD76CF` checkpoint live A/B；最终 12-turn 运行段 `24.684s`，route query 约 `0.050–0.068s`，2 checkpoints 与 cleanup 全绿。13:00 后正式 G1 另实证单目标 war-entry assessment 内部错误地全场扫描 Character×CB；target-only 修复已 `static-ready` | 原复制/持久化 B1 已满足。war-entry 热点只将 same-frame 声明复核从 `O(all characters × CB)` 收窄为 `O(CB)`，不改合法性、策略 cadence 或 durable barrier；待同 checkpoint 冷启 live A/B 后再升 production-live | 2026-08-28 原 query/copy B1 resolved；war-entry target-only static-ready / live pending |
