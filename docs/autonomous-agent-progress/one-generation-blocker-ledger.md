...
| Delivery gate / capability | Status | Formal entry | Artifact / source | Actual game and next-turn evidence | Restore evidence | B0/B1 gap / owner | Branch cleanup |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Ordinary defensive-war continuity with no title objective | production capability RED; scoped Python repair in progress | formal `g2_preview_operator.py run` -> `native_auto_run` | source master `ee03bad5`; R864 report `53970EED...9624E`; operator `DD50F71E...6F349`; final driver `12686AFD...9D553`; RED seal `785B7850...6B7525` | 17/18 successful, +207d; natural primary-defender `vassalization_cb`; termination query rejected day0 surrender policy; one raise and independent gathering->regular result at exact capital; enemy gathering/no route; planner then stopped with zero failing-turn command | h1167/date53280600 checkpoint `01980CE6...9E5D` is before war/tail and independently unpolluted. Cold restore must truncate h1168-h1176 and fresh-query; no unconfirmed action, no old WarID/ArmyID/reply reuse | Add strict primary-defender empty-objective exact-capital bounded hold, test normal/`-O`, then cold-replay h1167 through changed tactical state and new paired checkpoint; `/root/war_recovery_decision` implementation, `/root` integration/live owner | implementation branch/worktree active; R864 CK3 PID117108/injector fully reclaimed |
