...
| GEN-007 | B2/B3 | 战斗质量与吞吐 | ordinary active combat 与 committed route 已用 speed-3 sentinel production-live。新 stationary-objective canary 连续完成 20 个 7 日 arm，共 140 日；每臂零 external/intermediate pause、零 external RQ/overshoot，post-stop 全 matched。speed-3 arm 平均 `6.012s`，含 query/checkpoint 的持续吞吐仅 `62.53 日/分钟`，证明 3 速达不到新的端到端 100 门 | G1 保留已验证 speed 3；性能线程按同合同优先做 stationary/route speed-5 matrix 与最小 gate。player-won pursuit 仍等真实胜局；双 `4x` crush 只在 qualifying checkpoint 做完整 `1,2,3,4,5,5,4,3,2,1` 矩阵 | ordinary/route speed-3 production-live；stationary speed-3 20-arm live canary、待 GEN-029 后晋级；speed-5 terminal primitive live；100 d/min 未满足 |
...
| GEN-029 | B1 harness + 吞吐 P0 | 7 日 stationary sentinel 的固定 30 秒等待窗在真实性能抖动时过早终止 | canary `20260828T092300Z-one-generation-90d3cf79` 前 20 臂各 7 日全绿；第 21 臂从 `53266944` 在 30 秒内只推进到 `53267040`（4 日），尚未到 target `53267112`，于是报 `timed out before a native sentinel pause`。这不是 semantic stop；managed cleanup/tree-gone 全绿，durable anchor `53266608 / history=5814 / 1FADF1C0...B34D` 可恢复 | 只对 typed stationary 1..7 日 arm 提供有界扩展 wait；最终仍必须由 native pause/status、exact target、zero overshoot 与 fresh post-stop 验证。随后从最新 anchor cold replay，并测端到端 `>=100 日/分钟`；speed3 实测仅 `62.53`，需 speed5 或等效削减 | open；最小 timeout 修复施工中，性能线程持续优化 |
