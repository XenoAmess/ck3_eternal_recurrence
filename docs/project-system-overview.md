...
当前版本必须区分两件事：361 张政策卡已经进入真实配置与账本链，不等于 361 项都各自拥有一套完整小游戏。下一阶段的案卷、期限、
资源守恒、HC、晋升包和 38 个领域状态机属于更深的 Runtime 扩展，不应被旧版本宣传提前宣称完成。
...
## 1.5 《XenoAmess 的生活质量》：独立实用工具 Mod

该产品面向行政制、贤能制和天朝制玩家，当前提供两项默认关闭、可随时切换的功能：
...
- 在“决议”面板的专用分组中分别开启或关闭。
- 只对符合政府条件的独立真人统治者显示。
- 它代表“小而明确、默认不侵入、随时可回退”的工具型产品路线。

## 1.6 《食人赋能》：成长、记录与兼容性样本
...
- 获得虔诚并降低压力；
- 生成不同档次的遗骨宝物；
- 记录受害者、生前地位、所得能力和食用日期；
- 通过累计吞食人数成长为“食人魔王”；
- 支持单独吞食和批量吞食。

该 Mod 当前还承担一个重要的工程角色：它真实暴露了整文件覆写、发布副本、Workshop 缓存、加载顺序、debug 热重载与冷启动之间的
兼容性问题。当前仓库对它既保存产品源码，也保存静态分析和单体执行失效的根因报告。对外使用前应特别关注与其他覆写
`00_prison_interactions.txt` 或宫廷 GUI 的 Mod 冲突；已有分析不能冒充完整实机验收。

## 1.7 CK3 自动游玩智能体：正在成长的玩家能力
...
- 不能把视觉固定剧本冒充通用世界发现；
- 不能把静态的一代人 runner 冒充已经完成自然死亡的一代或跨继承长期自治。
...
## 1.8 `open_kaishek`：开发者可见的开源脚本工具链
...
- 纯 Java CLI 和未来的 Quarkus 集成服务。

当前边界是早期静态实现：parser、profile、validator、IR、Runtime 原语和 synthetic 361 切片已经形成基线；但这不能被描述为完整 CK3
Runtime，也不能被描述为已经完成大范围差分认证。未知 opcode、版本不匹配或缺失字段必须显式返回 `UNSUPPORTED`。

## 1.9 Promo、Workshop、Release 与能力展示
...
脚本、分镜、shot list、实机 marks、视觉审计和成片门禁；自动玩家则通过月度 show-off 汇总截至截止日的完整真实能力。
