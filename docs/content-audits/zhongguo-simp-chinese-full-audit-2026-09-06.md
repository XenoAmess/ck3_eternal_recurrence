...
- 最新整改结论：**以第 19 节为当前状态。626 张玩家可见事件已逐卡复读并按最终分片 SHA-256 封账；决议、互动、京察入口与 GUI 也已复核。机器失败 0、人工静态未修 0，当前为 STATIC GREEN；动态插值、排版与游戏内语境仍为 LIVE PENDING。**
...
本节是第 19 节最终复读前的阶段状态。第 19 节已完成逐卡人工语义复审；修复后全量 5 速实机语境、
动态插值和排版验收仍未完成。后者是“未验”，不能用 `machine_failures = 0` 或 STATIC GREEN 冒充最终 LIVE GREEN。

## 19. 626 张可见事件逐卡人工复审与最终封账（2026-09-06，STATIC GREEN）

第 18 节的机器清单不能替代人工语义判断。本轮把 16 个 event ledger 分片按两片一组并发逐卡阅读，
同时回查真实 event、option、tooltip 与 effect。审计过程中又确认 9 个写成
`type = character_event hidden = yes` 的同栏隐藏调度事件曾被解析器误计为可见；提交 `c66a1ef` 修复解析后，
当前真实范围为 **626 个 visible / 383 个 hidden / 1,009 个总事件定义**。这 626 张玩家可见卡已全部逐卡复读；
每卡同时核对最终 title、desc、实际绑定按钮、tooltip、event 与 effect/业务账簿消费端。非事件的决议、互动、
京察入口与 GUI 玩家文案也已完成独立复核。

| 分片 | 人工覆盖 | 终审新增问题簇 | 已修 | 未修 | 最终 SHA-256 前缀 | 状态 |
|---|---:|---:|---:|---:|---|---|
| 001–002 | 80/80 | 4 | 4 | 0 | `6a0296b8248f` / `5604cebb423d` | PASS |
| 003–004 | 80/80 | 5 | 5 | 0 | `5758ef9b94a0` / `1f3adddd9a8f` | PASS |
| 005–006 | 80/80 | 1 | 1 | 0 | `dc2c0db15ad4` / `2585a151bd81` | PASS |
| 007–008 | 80/80 | 7 | 7 | 0 | `712b12c58aa2` / `3e6b733d6ee7` | PASS |
| 009–010 | 80/80 | 0 | 0 | 0 | `b950090fb1ab` / `57907e456f02` | PASS |
| 011–012 | 80/80 | 9 | 9 | 0 | `ad31715719b8` / `efe84798a26d` | PASS |
| 013–014 | 80/80 | 1 | 1 | 0 | `fdd12e8f993f` / `5bbad16f39cc` | PASS |
| 015–016 | 66/66 | 14 | 14 | 0 | `bec43dd7bce0` / `03da335cf099` | PASS |
| **事件合计** | **626/626** | **41** | **41** | **0** | 16 个分片均已绑定 | **PASS** |

非事件终审覆盖：决议 7/7、互动 6/6、京察入口本地化 22/22、GUI 5/5；终审新增并修复 ratio-off
规则说明 1 簇，未修 0。另确认 `zg361we.274.b/.c` 及两个 tooltip 无任何可见入口，已从生成本地化中删除，
不再把死 key 留作含糊的 `review`。

最终人审清单为 `docs/content-audits/zg361-copy-human-review.json`。它精确绑定 16 个最终 event shard、
626 条记录与输入 digest；任一事件/简中源或 shard 字节变化都会把账本自动降回 `review`，不能靠机器扫描自行提升。
当前账本结论：`ledger_status=pass`、`machine_checks_status=pass`、`human_semantic_review_status=pass`、
`machine_failures=0`、静态未修 0。**审计已完成；CK3 实机排版、动态人物/数值插值和真实语境仍为 LIVE PENDING，
须在下一阶段按默认 5 速验收。**

### 19.1 最终静态回归

- `py -m unittest discover -s mod_zhongguo_style/tools -p "test*.py"`：**1,831/1,831 GREEN**，63.021 秒。
- `py -O -m unittest discover -s mod_zhongguo_style/tools -p "test*.py"`：**1,831/1,831 GREEN**，64.277 秒。
- `validate_local.py` normal / `-O`：GREEN。
- copy ledger generator normal / `-O`：626 visible、4,994 final loc keys、0 machine failures、human semantic review PASS。
- Workforce/endgame generator normal / `-O`：146 文件 current；361 mechanism generator normal / `-O`：208 文件 current。

以上闭合的是静态内容审计。下一步恢复 CK3 单实例、默认 5 速的实机验收；动态插值、窗口换行/截断和当时剧情语境
必须以实机截图与日志为准，不提前写成 LIVE GREEN。
