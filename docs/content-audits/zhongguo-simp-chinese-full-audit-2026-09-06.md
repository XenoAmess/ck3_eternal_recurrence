...
- 最新整改结论：**以第 19 节为当前状态。机器门禁仍为 0 失败，但 626 张玩家可见事件的逐卡人工语义审计重新发现多批按钮/effect、主体、因果、术语和无价值 tooltip 问题；当前重新标记为 STATIC RED / 并发整改中，5 速实机继续暂停。**
...
## 19. 626 张可见事件逐卡人工复审重新开单（2026-09-06，整改中）
...
当前真实范围为 **626 个 visible / 383 个 hidden / 1,009 个总事件定义**。这 626 张玩家可见卡已经全部纳入
人工逐卡范围；非事件的决议、互动、活动与 GUI 玩家文案另行并发复核。

| 分片 | 人工覆盖 | 当前结论 | 整改状态 |
|---|---:|---|---|
| 001–002 | 80/80 | B2 威望/保护承诺、HC 按钮与业务投影、CP 主体及知情选择等重新开单 | 并发修复中 |
| 003–004 | 80/80 | PP 签收/同意、死按钮、本人拒绝披露、硬编码分数，CP 主体与 tooltip 等重新开单 | 并发修复中 |
| 005–006 | 80/80 | 机制 1–71 的完整账簿后果、黑话/病句，以及 PP/Compensation 主体与措辞重新开单 | 并发修复中 |
| 007–008 | 80/80 | 机制 72–151 的政策/个案时态、虚列路线、内部术语与主体不清重新开单 | 并发修复中 |
| 009–010 | 80/80 | 机制 152–231 的标题错配、病句、对象漂移及 74 条无新增信息 tooltip 重新开单 | 并发修复中 |
| 011–012 | 80/80 | 机制 232–311 的数学/开发术语、标题和按钮信息不足重新开单 | 并发修复中 |
| 013–014 | 71 个真实可见项；另识别并排除 9 个隐藏调度项 | 机制 312–361 与 P3 的直译、病句、标题/按钮不自解释重新开单 | 并发修复中 |
| 015–016 | 75/75（解析修正前分片） | P3/WF 主体写反、按钮与 effect 相反、重要费用遗漏、D+1 被误写成 30 日等重新开单 | 并发修复中；最终分片将重排复核 |

因此当前“已修 / 未修”结论已再次更新：第 18 节列出的 39 个旧命中仍保持已修，但本节新增人工命中尚未
全部闭合，**整体文案状态为 STATIC RED**。所有分片修复合入后必须统一重生成 event/localization 与 copy ledger，
对哈希变化的分片再做人工差异复读，并跑串行 normal/`-O` 全量；在这些步骤完成前不得恢复 CK3。
