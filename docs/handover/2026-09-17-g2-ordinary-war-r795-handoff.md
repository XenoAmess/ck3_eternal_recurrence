...
- R846 在修复后的新进程再次自然出现 `prison_notification.2002`，完成 query → 单次 typed option → 独立 disappearance → 下一 turn → history693 checkpoint；20/20、5 gameplay、15 query，report `63CFCDD4...34D1`，checkpoint `9893F8C1...C08C`，driver `8315217A...F83`。已退役的外层 shell 捕获层因正常 stderr 的 stop-file 提示返回过一次外层 code1，完整 JSON `ok=true`、CLI 逻辑与进程回收证据保留；不据此掩盖或重跑实机。
