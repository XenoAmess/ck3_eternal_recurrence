...
当前 GPU 覆盖范围是背景候选选择。语义元素局部优化和逐块残差绘制仍由 CPU renderer/scorer 执行；因此这证明“GPU 已实际参与一段批量
候选搜索”，不证明整个拟合已经 GPU 化，也不把本项标成 WP4 全部完成。
...
后续仍需把批量 GPU 搜索扩展到语义/local 与大规模 tile 候选，补充真实 GPU/浏览器进程内存证据，并在更多取消阶段验证资源及时释放。
