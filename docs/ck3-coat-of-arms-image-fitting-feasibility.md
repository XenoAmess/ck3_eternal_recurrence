...
- 只生成原生实机矩阵已接受字段的 serializer 和 128 KiB 门禁；
...
从最佳背景开始，每轮尝试增加一个原生 emblem instance，只保留若干低损失且代码结构不同的候选。若改善低于阈值、达到时间预算、达到层数限制或序列化接近 128 KiB，立即停止。复杂度惩罚确保微小像素改善不会换来几十个难以编辑的图层。
