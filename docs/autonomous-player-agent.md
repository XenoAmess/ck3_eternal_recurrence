...
- 已完成首个 opening 交易闭环：从主菜单到地图共七次真实按钮提交均为 2/2，接受契约并选择首轮可见祝福
  【兵棋的余局】与低损失咒痕【千面的哑剧】，最终双帧确认 `map_hud` 并完成进程树清理。
- 已完成第一条地图内状态读取：第八次真实按钮提交打开玩家角色页，双帧确认罗贝尔本人、配偶、玩家继承人和 7 名臣属。
- 尚未实机接线：规则页、通用 HUD/事件状态机、角色发展与宫廷治理动作，以及独立 policy 进程与字段白名单。
