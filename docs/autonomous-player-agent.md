...
| B. UI 驱动底座（进行中） | 窗口分类、OCR/像素反证、快捷键优先控制、地图/HUD/事件通用恢复 | `20260822T172415Z-opening-d20b8adb` 已实测用 F2/F3/F4/F8 读取领地、军事、内阁和决议面板；下一步从可见领地状态执行首个经济建设动作 |
...
5. `opening-smoke` 已实测选择 1066 罗贝尔、完成首轮祝福/咒痕、读取玩家页、选择军事【权威重心】，并在
   `20260822T170117Z-opening-eaeef47a` 以速度 5 推进 451 日，识别【诺曼人的西西里】与四页连续疾病事件链，
   逐页按可见收益评分并用 `Shift+N` 选择，回到地图后暂停。动作执行优先遵循原版
   `game/gui/shortcuts.shortcuts`：F1、Esc、Enter、`5`、Space 与 `Shift+事件选项序号`；OCR 负责确认画面和
   选项语义，鼠标仅保留没有可靠直达键的控件；该 run 的 16 个键盘动作与 6 个鼠标动作全部得到正确后置画面。
   地图等待只跑事件栏裁剪 OCR，命中候选后才持久化两张全屏确认帧。下一步扩大事件布局覆盖，并接入地图经营动作。
6. 地图面板同样优先使用原版快捷键。提交 `75642ca`、环境
   `e3f8504582f19b19cd0ed10b3d3b04303c519064e20be6eddd8546631df66135` 的实机
   `20260822T172415Z-opening-d20b8adb` 在一个普通事件后用 F2/F3/F4/F8 依次打开、双帧读取并关闭
   【我的领地】【军事】【内阁】【决议】；26 个动作中 20 个为键盘。OCR 看到 254 金、约 2203 兵、4.9 月收入、
   五席内阁和当前决议列表，最终暂停于 `map_hud` 并完整清理。该后续建设动作已由提交 `c16e22f`、环境
   `86d5e8847bd0591dfa4f9811544f7eb979b4da1d79c63254fca50b79e8f2f3a4` 的实机
   `20260822T190656Z-opening-bfa943a7` 完成：F2 打开领地面板后，执行器进入阿普利亚伯爵领的普通空建筑槽，
   从三个可见候选中选择成本 150、赋税 `+0.50/月` 的【等级1：简易牧场】，点击【修建】并以地产行
   【个月内完工】确认工程已经开始。31 个动作中 22 个使用原生键盘路径；仅无可靠快捷键的 9 个控件使用鼠标，
   最终返回暂停的 `map_hud` 并完整清理。
...
- 已完成持续 gameplay session：同一 CK3 PID 内热加载策略，不因普通 Python/OCR/决策修改重启游戏。2026-08-23 的
  `20260823T032552Z-dev-session-b4ded92f` 已恢复原生存档、处理事件和时间推进，并生成四个原生 CK3 检查点。
- 已完成当前生命的王朝决策输入：实机读取配偶、玩家继承人、7 名子女、26 名宗族、19 名廷臣、9 名封臣；继承页识别
  分割继承制和将失去 2 个头衔；婚姻链以原生快捷键接受丹麦订婚。这里只优化玩家仍活着时的联盟与领地稳定，不做死后继承人玩法。
- 已完成一场真实战争闭环：宣战、集结、定位巴勒莫堡垒、围城推进、战争分数复查、100% 强制要求、胜利窗确认和解散军队。
  旧的裸坐标进军命令已改为已证明可用的目标搜索与悬停确认路径。
- 已实现单代死亡终局：识别原生继承窗、把【继续扮演】仅作为 mod 结算投递载体、确认【轮回终结】、提取分数并返回主菜单；
  `heir_gameplay_actions=0`。无继承人则走原生继承窗注入的【退出到菜单】。该链已用历史实机结算截图和完整模拟流程验证，当前常驻
  非 debug 存档尚未自然死亡，所以仍缺一次自然死亡的端到端实机验收。
- 已实现跨局策略记忆：只读取智能体自己的 session 结果，把战争胜利、订婚、分割风险、最终检查点和死亡分数写入
  `strategy/one-life-history.json`，下一局据此排序开局扩张、婚姻和领地保全。`strategy-review` 是纯读取命令，在常驻进程热加载实测
  从约 6 秒的旧 UI 路径降为约 11 ms，不再绑定窗口或运行 OCR。
- 已接入单步自主循环 `auto-turn`：规划器从实际 command history 选择基线存档、王朝/继承读取、婚姻联盟、巴勒莫战争、解散军队、
  里程碑存档或轻量生命推进；普通推进每回合只开放约 10 秒现实时间窗口，结束必回暂停，累计三回合自动原生保存。实机同一 PID
  已从 1068-01-08 推进至 1069-01-21，识别并处理真实事件【锈迹斑斑的工具】，且自动切换到周期存档
  `阿普利亚公爵，罗贝尔_1069_01_21.ck3`。普通事件、盟友参战和死亡终端现由一次合并预览 OCR 同时覆盖；同一常驻 PID 的实机
  `life-advance` 从 41.3 秒降到 26.75 秒。
- 首个自主推进暴露真实玩法故障：旧 180 秒等待在无普通事件时抛错却把速度 5 留在运行态，期间漏掉丹麦【召集加入战争】外交信函，
  盟友关系实际下降 20。现已把无事件改成正常有界推进、显式暂停；外交信函有独立预览与稳定识别，策略选择 `Shift+2` 履行当前生命
  联盟。此修复仅针对已发生的玩法损失。
- `auto-turn` 在生命推进中看到稳定死亡终局时立即停止时间推进；下一步只路由既有 `death-terminal` 单代结算，不执行继承人玩法。
  下一价值切片是盟友战争实际参战、动态战争目标比较、婚姻候选评分和当前生命内的分割损失缓解。任何新安全工作必须先有会影响
  真实游玩的可复现故障。
- 常驻会话支持 `auto-run N`（1..20）：在一个命令内连续调用同一单步规划器，并把每个子回合的选择、动作和结果保留在 session
  report。实机第一批两回合连续处理【锈迹斑斑的工具】的两个链式事件；第二批验证跨批次历史后先保存
  `阿普利亚公爵，罗贝尔_1069_05_08.ck3`，再推进至 1069-06-25。普通错误停止整批；可见事件打断与死亡终局才允许转入明确恢复步骤。
