...
| B. UI 驱动底座（进行中） | 窗口分类、OCR/像素反证、快捷键优先控制、地图/HUD/事件通用恢复 | `20260822T155740Z-opening-b33c1328` 已实测 11 个键盘动作、6 个鼠标动作并推进时间；下一步处理首个真实游戏事件 |
...
5. `opening-smoke` 已实测选择 1066 罗贝尔、完成首轮祝福/咒痕、读取玩家页、选择军事【权威重心】，并在
   `20260822T155740Z-opening-b33c1328` 以速度 5 推进 17 日后暂停。动作执行优先遵循原版
   `game/gui/shortcuts.shortcuts`：F1、Esc、Enter、`5`、Space 与 `Shift+事件选项序号`；OCR 负责确认画面和
   选项语义，鼠标仅保留没有可靠直达键的控件；该 run 的 11 个键盘动作与 6 个鼠标动作全部得到正确后置画面。
   下一步直接处理首个真实游戏事件。
