...
| B. UI 驱动底座（进行中） | 窗口分类、OCR/像素反证、快捷键优先控制、地图/HUD/事件通用恢复 | `20260822T162844Z-opening-33bdd96f` 已实测 12 个键盘动作、6 个鼠标动作，推进 52 日并处理首个普通事件；下一步连续处理多个事件并选择收益更高的选项 |
...
5. `opening-smoke` 已实测选择 1066 罗贝尔、完成首轮祝福/咒痕、读取玩家页、选择军事【权威重心】，并在
   `20260822T162844Z-opening-33bdd96f` 以速度 5 推进 52 日，识别普通事件【诺曼人的西西里】的三个可见选项，
   用 `Shift+1` 选择第一项后确认事件消失并暂停。动作执行优先遵循原版
   `game/gui/shortcuts.shortcuts`：F1、Esc、Enter、`5`、Space 与 `Shift+事件选项序号`；OCR 负责确认画面和
   选项语义，鼠标仅保留没有可靠直达键的控件；该 run 的 12 个键盘动作与 6 个鼠标动作全部得到正确后置画面。
   下一步把单事件竖切扩成连续事件循环，并用可见效果评分代替固定第一项。
