...
| B. UI 驱动底座（进行中） | 窗口分类、OCR/像素反证、可靠点击、地图/HUD/事件通用恢复 | `20260822T111130Z-opening-a27391b9` 已真实完成八次按钮提交并从地图打开玩家角色页，读取配偶、继承人与臣属状态；下一步执行首个角色发展或宫廷治理动作 |
...
5. `opening-smoke` 使用已冻结的隔离 profile 规则，已实测选择 1066 罗贝尔、点击【开始】、接受【终末之契】、完成首轮
   祝福/咒痕并进入地图。规则页视觉复核仍是独立待办，但不再阻塞功能竖切；下一步直接接首个地图经营动作。
