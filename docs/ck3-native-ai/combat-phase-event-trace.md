...
从上述第 6 日不可变存档再次独立冷启动、暂停且不推进日期的只读 attempt
`episode01-day06-maim-next-input-attempt-040`，查询 CombatID `16777218` 的
原版 `combat-simulation-inputs-v3`。原版 save SHA-256
`9ACACDE3E2D1987180EFE5FFDDC32B092146E6116779AF0D4BEBC7C97B9B7F9A`
与上段的后存档逐字节绑定；新 run 的 paused snapshot、battle control、v3、
capture cleanup 各有独立 SHA。[逐对象投影报告](../../ck3_autonomous_player/src/xar_autoplayer/simulation/data/ck3_1_19_0_6_episode01_messina_knight_maim_next_input.json)
SHA-256 `961DA3BC152903316214CEC3CA820669F83161A3A04C6EC3D6235C239E164100`，
[只读复算工具](../../ck3_autonomous_player/tools/project_native_knight_maim_next_input.py)
校验前后 run、源/后存档、追加事件和游戏进程清理状态。

| 同身份原版 v3 输入 | 第 5 日事件前 | 第 6 日复载后 | 本案解释 |
| --- | ---: | ---: | --- |
| 兵团/骑士人数 | 51 / 24 | 51 / 24 | 致残者仍为骑士，不从战斗名册移除 |
| 骑士 `34333` 的有效勇武 | 11 | 7 | `one_legged + wounded_1` 写回后原版重新求值；基础勇武在存档中仍为 3 |
| 骑士效能 | 175,000 / 100,000 | 175,000 / 100,000 | 本案效能乘数 1.75 不变 |
| 其 61 号兵团有效伤害 raw | 96,250,000 | 61,250,000 | 除以 100,000 为游戏数值 962.5→612.5 |
| 其 61 号兵团有效坚韧 raw | 19,250,000 | 12,250,000 | 除以 100,000 为游戏数值 192.5→122.5 |
| 61 号兵团当前人数 | 1 | 1 | 变的是单个骑士的属性，不是人数 |

本案公式为 `伤害=有效勇武×骑士效能×50`，因此 `11×1.75×50=962.5`、
`7×1.75×50=612.5`；`坚韧=有效勇武×骑士效能×10`，因此
`11×1.75×10=192.5`、`7×1.75×10=122.5`。前后 23 名其他骑士的 v3 行逐对象相同，
50 个其他兵团的有效属性行相同。另有 12 个兵团的 `current_soldiers`
随同一日正常伤亡改变；职业兵士计数变化也会改变 `counter.current_chunk_raw`，
