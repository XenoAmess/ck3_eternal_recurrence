...
[implementation-confirmed, research-only] `phase_event_feedback.PhaseEventFeedbackResearchKernel` 现把同一 root 的五日到期判定、stock load-order trigger/整数权重、原生 `draw31` 选择、已选 AST effect 和下一到期日权重重算连成逐日 `BattleTransitionKernel` 切片。基于 production-shaped v3 静态 fixture 的聚焦测试中，root `36108` 首次选中正权重 `commander_wounded`，伤情写回 `100000`，下一到期日 `commander_killed` 权重从 `0` 变为 `2`；下一次选中 `commander_none` 时只消费选择 draw，状态 SHA 不变。未选中事件不执行；选中非空事件缺精确 effect draw tape 即报错。这仅证明**同角色未来候选权重**的离线反馈，尚未对上原版逐日 effect trace。kernel 的 outcome 固定 `NO_RESOLUTION`、伤亡 `0`，manifest 的 loaded phase effect 和 original trace 门仍为 false，`planner_usable=false`、`active_attack_allowed=false`。

```mermaid
flowchart LR
    D["同一 root 到期日"] --> S["stock 顺序 trigger/chance 与 draw31"]
    S --> E{"选中非空 effect?"}
    E -->|yes| A["AST + 显式 effect draw tape"]
    E -->|empty| N["只消费 selection draw"]
    A --> R["下一到期日重算权重"]
    N --> R
    R -. "跨 root/side、同日贡献刷新、原版 trace 未闭合" .-> U["研究切片；无胜率/进攻资格"]
    classDef unknown stroke-dasharray: 6 4,fill:#fff4e5,stroke:#b36b00;
    class U unknown;
```

[live-input + offline analysis] R0207 的原始 `probe/combat-v3-inputs.json` SHA-256 为 `8621F26CFC14841444C81A7377723D0B87945C10289C0741E6BAACD0DA55A196`，来自冻结索引 SHA-256 `1A8499E91F645C68CBA2BE4052AB6E5523DBDC6711045EF6C89AC9D23BA77B24`。该帧有 24 个候选 root（2 commander、22 knight）；9 类行在至少一个 root 上 trigger valid 且整数权重为正：`commander_none/wounded/maimed`、`knight_none/becomes_incapable/wounded/maimed/killed/qualify_for_accolade`。按 13 行 stock AST 的 effect 依赖表映射，已有账本的 **15/15 类 effect** 都落在至少一个这样的行上。这只是事件行**可能**被选中的上界：分支未必执行，effect 也未必回流本场胜率。逐类战斗期回流与战后候选边界见 [combat-phase-feedback-battle-horizon.md](combat-phase-feedback-battle-horizon.md)。
