...
The formal entrance remains default OFF. The operator's explicit `run --private-lifestyle-formal-trial` forwards the agent's bounded `native-auto-run --allow-private-lifestyle-formal-trial` flag; an ordinary run omits both flags. R0129's h2145/raw53392392 paired checkpoint is a later candidate source: its same-date post-checkpoint campaign-root observation reports `feudal_government`, and the next formal turn has no war, army, event or pending interaction. This is scene screening only. The checkpoint's current focus and native final legality still require a fresh paused read after official cold restore, and the M4 DLL must be rebuilt and pinned separately from R0128's older readback DLL. No typed action, receipt or next-turn consumption is claimed from this screening.
