...
此事件由信仰月度解锁广播，R0129 当帧无战争，不属于“战争中的圣战/大圣战”窄例外。项目所有者随后**明确授权**通用 exact-build 已证明选项及 after 均无玩法效果、且当前帧只有一个合法选项的通知确认机制处理 R0129 `.0011`；授权不包含 faith/doctrine/tenet/fervor 研究或信仰策略。工作包 `WAR-R0129-GENERIC-NOTICE` 只扩共享 registry consumer 与该事件的数据变体：以冻结脚本哈希证明所有 authored 选项及共同 after 的效果边界，在同一 paused frame 严格匹配 event identity、ROOT、完整作用域、唯一 shown+enabled native index 和正确 typed step。原生空 effect indicator 不当作证明；未知定义、来源或投影继续拒绝。normal/`-O` 聚焦重放仅证明静态候选，R0129 产品 RED 要等正式 typed 动作、独立后置、下一 turn 与配对恢复后才能关闭。旧 R0118 WarID251658364 的战争 RED 另行保留。

工作包登记：`WAR-R0129-GENERIC-NOTICE`｜B0 为 R0129 registry 选项消费者｜负责人 war-precommit agent｜隔离 Z 盘工作树 `Z:/workspace/g2_r0129_generic_notice_20260922`、分支 `codex/g2-r0129-generic-notice-20260922`｜rebase 基线 master `f6e9edda913ac634d8285b5253b4c706fdaadcf6`、R0129 原始报告 SHA 如上｜修改边界为共享 registry policy、`.0011` source-reviewed 数据、原始帧聚焦测试和本文｜不占 CK3；正式 h2145 恢复与唯一实例由协调者排队。原始帧、来源失配、选项/作用域漂移及既有 registry 测试在 normal/`-O` 各 **52/52**；这只给出 `static-ready` 候选，typed/live 证据待正式轮次。
