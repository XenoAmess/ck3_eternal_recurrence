...
- [static-ready complete analysis + five production-live primitives + two retained RED] [vanilla-event-knowledge-registry.md](vanilla-event-knowledge-registry.md)
  已组合默认 `159 contracts / 159 analysis / 7 observation keys` 的 exact-build 原版事件表并发布离线
  ...
  production-live primitive；长跑当前停于第二次合法出现的 `epidemic_events.5009` instance `871` park12 RED，旧合同的
  单次上限与原版十年 cooldown 冲突。
