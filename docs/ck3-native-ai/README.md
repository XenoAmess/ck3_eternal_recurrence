...
- [static-ready complete analysis + mixed production-live primitives] [vanilla-event-knowledge-registry.md](vanilla-event-knowledge-registry.md)
  本包组合默认 `175 contracts / 175 analysis / 175 observation metadata rows` 的 exact-build 原版事件表，其中
  `27` 个 key 含非 legacy 的 paused/live observation；既有迁移基线与
  embedded bucket 仍保持冻结，R384 新增 `pay_homage.0101` 的 exact-build Smooth 合同和选择前 RED。离线只读
  `ck3_query_vanilla_event_knowledge_v1` 保持既有 schema，production runtime 当前消费 `321` 条事件合同。R372 的 `TGP0160`、
  ...
  并用热更新合同越过 `bp1_house_feud.0014` 的 relation-scope 形态；当前暂停在新事件 `bp1_yearly.1040` 的选择前 RED，
  exact-build 树与 authored3/native2 路线见 [bp1-yearly-bathhouse-friend.md](bp1-yearly-bathhouse-friend.md)。
  上述状态不表示 175 条全部 live；
