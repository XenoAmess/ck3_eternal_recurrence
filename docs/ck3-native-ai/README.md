...
- [static-ready; paused production live pending] [Private typed death-succession modal Close](death-succession-modal-continue-v1.md) reacquires the exact controller on the owning thread, calls its validated Close vslot once, and requires a later independent predicate/root observation plus formal date advancement. ACK is never material success; the public registry and MCP tool list remain unchanged.
