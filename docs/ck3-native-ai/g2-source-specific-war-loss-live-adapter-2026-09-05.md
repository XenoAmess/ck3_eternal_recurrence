...
## R459 qualified source-specific outcome

R459 unique PID `123140` completed the source-first normal lifecycle on exact build `1.19.0.6`: six source executions, two stable terms queries, a durable checkpoint, one accepted `surrender-war-33554473`, stable postwar cleanup, and final process cleanup. The checkpoint is `68,603,154` bytes with SHA-256 `FAA32578602EF546D991C364D196292C70C2D491FBCC6D4558FAB31444E14E78`; the source save remains `89D15B8A...D4DD`.

The captured source set measured 3000 soldiers both at event creation and immediately before termination. The exact bound generations are destroyed after surrender, post-termination soldiers are zero, and `proven_surrender_boundary_loss=3000`. The persisted native truce relation reports `evaluated_days=1825` and `expiry_date_raw=53227656` twice on the same paused postwar frame. All ticket, session, successor, one-action, cleanup, and expiry checks are GREEN.

This promotes only `source_specific_loss_ready` and `comparison_input_ready`. The report hash is `87ADB7E1...4B34`; the normalized offline policy intake is `43B0A053...4FE1` with observation `BA139830...F462`. Campaign dominance, owner budget, and same-frame white-peace comparison remain missing, so three-way comparison, decision, action, automatic surrender, and `GEN-034` stay false.
