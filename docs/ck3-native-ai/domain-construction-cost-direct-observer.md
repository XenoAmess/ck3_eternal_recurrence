...
The new private source is statically verified in normal `/Od` Debug and optimized `/O2` Release with `/W4 /WX`. It has **no paused live cost receipt**, no public MCP/query registration, no open_kaishek public-adapter change and no material building outcome. R739's legal samples cannot by themselves satisfy the two-game-year construction gate. The next bounded live check is a fresh production paused frame on the exact agent/native pair, followed by mapping the spend slots to real actor balances and the existing typed submit/receipt path. If cost source fails in that frame, retain its failure and query the exact stock callsite rather than submit an action with unknown cost.
