...
后续 R0133 已完成五候选私有联盟投影的同帧实机读回，但抽样五行均不会尝试形成联盟，且 R0133 没有同帧战争/现金/补给/长期承诺。新的[单帧联合调度与只读缺口](m5-single-frame-dispatch-2026-09-22.md)把完整 episode 身份和一次资源预留加入纯分析层；不能把本页旧 R736 战争读数跨实例拼入 R0133。
...
1. R0133 已在独立 M5 `xar_on` 候选上完成现有 `query-first-heir-candidate-alliance-projection-v1-private` 的 paused 五候选读回；八个 pair 均未达到会尝试形成联盟的条件。其 pair 仅是原生投影，不直接产生收益或联盟承诺。新候选与 live 身份见[单帧联合调度记录](m5-single-frame-dispatch-2026-09-22.md)；不可与战争 `xar_off` 恢复配对混用。
