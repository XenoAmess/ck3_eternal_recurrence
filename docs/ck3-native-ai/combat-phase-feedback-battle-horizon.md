...
本次新构建的并行 v3 输入查询另报 `native_phase_definition_context_culture_parameter_unavailable:knights_slightly_more_prone_to_injury`，因此这条完整采集**不能直接充当 v3 事件模拟输入**。这项 source/context 缺口需单独修复并对拍，不能借七边界采集成功而跳过。
...
2. 以新取得的**非空原版事件七边界**为基线，补齐 actual selected row/root、随机分支、全部写集与后续 tick；先修 v3 的 `knights_slightly_more_prone_to_injury` context 缺口，再调用已合入的 `execute_phase_event_trial_sequence` 做同一 root 的 effect 对拍，随后补跨 root/side 写回。七边界的角色死亡与兵团退出观察不得代替这些尚缺的因果证据。
