...
- [static-ready] 默认扁平 registry 当前为 **162 个 unique vanilla event key**：20 个 vanilla shard、57 个 manager-original、79 个 embedded-original 合并为 156 个 unique key，再加独立的 `tgp_movement_events.0160`、`tgp_dynastic_cycle_events.0001`、`tgp_dynastic_cycle_events.0020`、`epidemic_events.1064`、`vassal_interaction.0040` 与 `trait_specific.4001`。该数量由 registry/migration 测试冻结；以后代码和测试同步调整时，以测试中的当前期望值为准。
- [static-ready analysis / mixed live evidence] 当前同时发布 **162 条 analysis** 与 **12 个 observation keys**。缺少已冻结 source hash 的旧结论只按既有合同注释、docs/tests 标为 migration-only，不编造 hash；`TGP0160`、`great_holy_war.0011`、`TGP0020`、`TGP0001` 与 `epidemic_events.1064` 已完成 production runner 的真实 drain/advance，`stress_threshold.1721`、`epidemic_events.5009` 的第二次合法出现以及 R374 的 `natural_disaster.7031`、`ep3_story_cycle_admin_eunuch.1001`、`tribute_mission.1005`、`vassal_interaction.0040`、`trait_specific.4001` 均保留真实 RED observations。
...
当前状态表示 registry、默认数据组合和只读查询可以被静态消费者使用，不表示 162 条记录都已有独立 production-live exemplar，也不表示 CK3 自动玩家已经具备完整事件效用判断。production runtime 当前组合 `301` 条事件合同；共享 registry 的 production-live 标签严格只落在九条已完成选择与 advance 的切片。TGP0001、epidemic1064、`.7031`、`.1001`、`.1005` 与 `.0040` 均在同一 PID 热恢复后完成 reviewed selection；后四者的 observations 保留选择前 RED，而 R374 随后的 drains 已 GREEN。park4/park5 已闭合；当前 park6 的原版 `trait_specific.4001` 已完成选择前共享迁移，但尚未提交选择，不改变 T0 产品进度。
...
R372/R374 的日期、instance 和人物 ID 不进入新建或已触达迁移后的通用 timeline contract。当前 MCP v1 在同一个 knowledge envelope 中以彼此独立的 `contract`、`analysis`、`observations` 字段返回这三层；所有 162 条记录已有 analysis，只有十二条拥有 observation metadata。元数据可查询，但不会混入选择合同或被物化成当前人物约束。

R372 已在同一 PID/session 上把 `TGP0160`、`great_holy_war.0011`、`tgp_dynastic_cycle_events.0020`、`tgp_dynastic_cycle_events.0001` 与 `epidemic_events.1064` 从共享记录解析到真实 option submission，并验证旧 instance 消失或 advance；R374 又把 `.7031` authored3/native2、`.1001` authored2/native1、`.1005` authored6/native5 与 `.0040` authored1/native0 依次同 PID drain 并验证 instances `978`、`988`、`1007`、`1038` advance，因此九条均为 `production-live primitive`。`stress_threshold.1721` observation 保留前一次真实 RED：共享模块 reload 实际已经生效，错误是 submission 阶段再次按 base contract 解析，导致提交了 base route；这不是 reload failure。`.5009` 的第一次交付曾真实 GREEN，第二次交付则证明旧 `max_occurrences=1` 错误。`.7031`、`.1001`、`.1005`、`.0040` observations 保留的是选择前 RED，随后 live drains 已 GREEN；`.1005` 的通用合同只新增实测 rejected-eunuch 十六 scope shape，既有精确 variants 不变。`.0040` 合同仅接受 exact-build 审阅证明的 7-scope 信件 shape。`.4001` 的选择前 observation 对应严格 create-witch+secret 3-scope shape，尚不计入 live。九条 GREEN 不能外推为其余 153 条记录已经 live。
...
T0 的 migration parity 测试逐 bucket 对照旧合同，并冻结 `20/57/79` 数量、156 个迁移 unique key，以及两条 prebootstrap 有意 overlap；再加 TGP0160、TGP0001、TGP0020、epidemic1064、vassal-interaction0040 与 trait-specific4001，默认合同与 analysis 均为 162。后续 T0 consumer 应按实际产品路径查询或物化所引用的记录，并继续用产品自己的 window、occurrence、source checkpoint 和业务后置条件验收。
...
- 迁移测试保证 legacy buckets 逐值相等、默认合同与 analysis 均精确覆盖 162 key，并拒绝意外 duplicate/conflict；默认组合故意不加入两条 prebootstrap context profile。
...
默认 162 key 是当前按需积累的复用资产，不是覆盖率目标。Registry 不要求在继续 T0、运行其它 mod、CI 或发布前枚举全部 `361` 个场景或 `626` 个 definition。
