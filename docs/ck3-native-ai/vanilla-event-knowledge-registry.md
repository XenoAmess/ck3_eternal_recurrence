...
当前状态表示 registry、默认数据组合和只读查询可以被静态消费者使用，不表示 160 条记录都已有独立 production-live exemplar，也不表示 CK3 自动玩家已经具备完整事件效用判断。production runtime 当前组合 `299` 条事件合同；共享 registry 的 production-live 标签严格只落在七条已完成选择与 advance 的切片。TGP0001、epidemic1064、`.7031` 与 `.1001` 均在同一 PID 热恢复后完成 reviewed selection；`.7031`、`.1001` observations 保留选择前 RED，而 R374 随后的 drains 已 GREEN。当前只停在 `tribute_mission.1005` instance `1007` 动作前 RED。
...
R372 已在同一 PID/session 上把 `TGP0160`、`great_holy_war.0011`、`tgp_dynastic_cycle_events.0020`、`tgp_dynastic_cycle_events.0001` 与 `epidemic_events.1064` 从共享记录解析到真实 option submission，并验证旧 instance 消失或 advance；R374 又把 `.7031` authored3/native2 与 `.1001` authored2/native1 依次同 PID drain 并验证 instances `978`、`988` advance，因此七条均为 `production-live primitive`。`stress_threshold.1721` observation 保留前一次真实 RED：共享模块 reload 实际已经生效，错误是 submission 阶段再次按 base contract 解析，导致提交了 base route；这不是 reload failure。`.5009` 的第一次交付曾真实 GREEN，第二次交付则证明旧 `max_occurrences=1` 错误。`.7031`、`.1001` observations 保留的是选择前 RED，随后 live drains 已 GREEN；当前 `.1005` observation 仍只记录动作前 RED，不宣称已选中。`.1005` 的通用合同只新增实测 rejected-eunuch 十六 scope shape，既有精确 variants 不变。七条 GREEN 不能外推为其余 153 条记录已经 live。
