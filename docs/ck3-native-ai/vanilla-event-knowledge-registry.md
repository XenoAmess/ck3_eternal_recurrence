...
- [static-ready] 本包默认扁平 registry 为 **165 个 unique vanilla event key**。原冻结迁移基线不变：20 个 vanilla shard、57 个 manager-original、79 个 embedded-original 合并为 156 个 unique key，再加当时八个独立记录，仍是 `156 + 8 = 164`，embedded bucket 仍为 `79`。本次 `.1101` 作为该基线之后的第九个独立记录追加，因此当前总数为 165；数量由 registry/migration 测试冻结。
- [static-ready analysis / mixed live evidence] 本包同时发布 **165 条 analysis** 与 **15 个 observation keys**。缺少已冻结 source hash 的旧结论只按既有合同注释、docs/tests 标为 migration-only，不编造 hash；`TGP0160`、`great_holy_war.0011`、`TGP0020`、`TGP0001`、`epidemic_events.1064` 及 R374 后续七个共享事件已完成 production runner 的真实 drain/advance。`stress_threshold.1721`、`epidemic_events.5009` 的第二次合法出现和已迁移条目的选择前 RED observations 均继续保留；第十四、十五个 observation 分别是 `faction_demand.2001` park8 与 `faction_demand.1101` park9 的选择前 RED，后续 GREEN 也不得删除。
...
当前状态表示 registry、默认数据组合和只读 MCP 可以被静态消费者使用，不表示 165 条记录都已有独立 production-live exemplar，也不表示 CK3 自动玩家已经具备完整事件效用判断。production runtime 本包组合 `304` 条事件合同；共享 registry 的 production-live 标签严格只落在十二条已完成选择与 advance 的切片。park7 的 `death_management.1007` 是第十一条；park8 的 `faction_demand.2001` 已在 R374 同一 PID `51852` / generation `1` 选择 authored3/native2，instance `1050 -> null`、snapshot `native:1847 -> native:1848`、revision `1848 -> 1849` 且 postcondition GREEN，成为第十二条。因此十二条 GREEN 之外还有 `153` 条非 live 记录。`.1101` 的 MCP list/call 与 T2 `NO-CODE-CHANGE` 审计已 GREEN，但提交推送和 live retry 尚待；该增量不改变 T0 产品进度。
...
R372/R374 的日期、instance 和人物 ID 不进入新建或已触达迁移后的通用 timeline contract。当前 MCP v1 的既有 knowledge envelope 以彼此独立的 `contract`、`analysis`、`observations` 字段返回这三层；本包全部 165 条记录已有 analysis，十五条拥有 observation metadata。元数据不会混入选择合同或被物化成当前人物约束。`.1101` 已进入默认静态组合，实际 MCP 查询返回 `available` 且三层投影均可严格 JSON 往返。

R372 已在同一 PID/session 上把 `TGP0160`、`great_holy_war.0011`、`tgp_dynastic_cycle_events.0020`、`tgp_dynastic_cycle_events.0001` 与 `epidemic_events.1064` 从共享记录解析到真实 option submission，并验证旧 instance 消失或 advance；R374 又把 `.7031` authored3/native2、`.1001` authored2/native1、`.1005` authored6/native5、`.0040` authored1/native0、`.4001` authored2/native1、`.1007` authored1/native0 与 `.2001` authored3/native2 依次同 PID drain 并验证 instances `978`、`988`、`1007`、`1038`、`1040`、`1046`、`1050` advance，因此十二条均为 `production-live primitive`。`.1007` 的动作后 snapshot `native:1779 -> native:1780`、revision `1780 -> 1781`；`.2001` 的动作后 snapshot `native:1847 -> native:1848`、revision `1848 -> 1849`，两者 postcondition 均 GREEN。`stress_threshold.1721` observation 保留前一次真实 RED：共享模块 reload 实际已经生效，错误是 submission 阶段再次按 base contract 解析，导致提交了 base route；这不是 reload failure。`.5009` 的第一次交付曾真实 GREEN，第二次交付则证明旧 `max_occurrences=1` 错误。选择前 RED observations 不因后续 GREEN 删除。
