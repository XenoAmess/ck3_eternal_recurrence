...
```json
{
  ...
  "player_health": {"raw": 275000, "scale": 100000},
...
`player_health` 在 available frame 中同样不可为空，表示 exact-build `Character.GetHealth` 的 signed Q100000 结果；调用失败或
Character generation 漂移会整帧返回 `player_health_unavailable`，两次采样不一致仍返回 `state_changed`。原生链、函数体哈希和
最低策略阈值见[玩家健康观测](player-health-v1.md)。
