...
状态：`WP4 in_progress / real-fit-budget and cancel-restart sub-gates passed`  
...
| ---: | ---: | ---: | ---: | --- | ---: |
| 128 | 8 / 8 | 4,120 | 4 | `no_improvement` | 7,340 ms |
| 1,024 | 250 / 250 | 3,992 | 6 | `no_improvement` | 7,519 ms |
| 10,000 | 1,824 / 1,824 | 15,640 | 7 | `no_improvement` | 14,644 ms |
...
绘制阶段取消延迟 52 ms；旧 Worker 被 run ID 作废后，新的 128 预算 run 完成且 receipt 为 128，没有旧结果污染。压力序列的
JavaScript heap 增量为 33,108,085 bytes；非 GET 请求为 0。高分辨率接缝 receipt 在三个预算下均为 96/230/512 px 零泄漏。
...
本子门禁仍不等于 WP4 完成：暂停/恢复和版本化 checkpoint、上下文丢失恢复、GPU 真正承担批量候选搜索、更多取消阶段及参考设备总进程/
GPU 内存证据仍未完成。
