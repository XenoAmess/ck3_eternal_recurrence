...
- 后续候选已实现 typed foreground-loss 证据，尚未再次运行 CK3：作出拒绝判定的同一次 sample 记录 checkpoint、capture sequence、UTC/monotonic、input tick、target HWND/PID/TID，以及 actual raw/root HWND、root HWND 对应的 PID/TID、class、rect、topmost；actual process image/creation 只在持柄与 HWND 复核都成功时证明，否则显式 unknown。判定前仍运行既有的只读 WMI/唯一窗口认证；判定样本后的 enrichment 不读取标题、不再做 WMI/全桌面枚举，也不做窗口 mutation、输入、sleep 或重试。artifact 独立 fsync 后发布唯一 `foreground_lost` 主 WAL，report/event/manifest/public replay 三向绑定；发布失败保持 provisional。旧 `49f9b8bd` 不回填且仍可原样 RED replay。本次 runtime 变更使上一组资格失效，下一次 one-shot 前必须重新提交、prepare，并在新 environment 下取得 ordinary v2/crash GREEN。
