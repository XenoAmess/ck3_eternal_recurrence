...
effect 文件中的条件容器按已识别的 trigger side 校验；不增加 runtime
certification。对 76-file 二期语料的离线预验耗时约 `3.144s`，parser 为
...
语义，已改为有限产品观察窗口内可重复。commit `2ba288b` 推送并通过 Official Runner 后，同一 PID 已将
instance `1035` authored1/native0 安全 drain，且验证 old instance 消失。该修复不进入 shared vanilla registry，也没有把
typed-RED cycle 写成成功结果。

继续 drain `zg361.40` #1036 与 `epidemic_events.5009` #1037 后，原版 `vassal_interaction.0040`
#1038 的 exact-build 定义与 interaction 调用链表明：封臣获得宣称、玩家获得关系修正早已在自动接受的
interaction 结算时发生；信件唯一 authored1/native0 只重现 tooltip，本身不再执行 gameplay effect。合同、analysis
和 observation 只加该实见的严格 7-scope shape。commit `eeea8a6` 推送且 Official Runner GREEN 后，R374 保持
PID `51852` / generation `1` 选择 authored1/native0，old/new instance `1038/null`，并以
`postcondition_verified=true` 将该条提升为 production-live。

随后 `debate_event.5110` #1039 以 authored2/native1 GREEN drain；当前 park6 为原版
`trait_specific.4001` #1040 / date `53583192`。root/player 为 `32904`，三个 scopes 为
...
- 截图坐标必须按 `docs/desktop-coordinate-mapping.md` 使用 `tools/desktop_coordinate_map.py` 换算并留下点击回执；禁止从聊天预览目测后手算固定倍率。2026-08-22 `20260822T130230Z-opening-cb15ab30` 的历史 RED 证明只修一个轴会点中错误控件；其中 `2560×1440 → 2048×1152` 和 `1.25` 只描述该次实证，不构成当前或未来屏幕/预览的默认参数。
...
### 现行强制文件边界政策（2026-09-05）

单文件过大直接视为必须修复的文件边界缺陷，不再等待加载性能 RED、不再讨论是否拆分，也不再生成未拆分的新候选。scripted effect 必须按用途分组，目标每文件 `1–10` 个顶层定义，原则上不得超过 `20`；超过 20 的例外必须同时记录不可拆理由和该精确文件的实机证据。generator、静态测试、production projection 与 release 清单必须覆盖全部分片，旧聚合 owner 不得与分片并存。现行 361 合同见 `mod_zhongguo_style/docs/361-effect-file-boundary-contract.md`。

下节仅保留 2026-09-03 至 09-05 的历史实验与 artifact，不再作为“是否拆分”的决策流程。

### 历史：加载性能 RED 的文件边界 A/B（已废止为决策依据）
...
Phase2 full discovery 的普通 Python 与 `python -O` 两轮，在共享同一产品源树时必须串行执行；若确需并发，
必须分别使用隔离工作树和输出根。已复现的现象是：两轮单独执行均通过，并发执行却会随机报告 legacy
monolith 或 retired residue 回流。原因是部分 residue-rejection 夹具会在产品源树中短暂创建预期应被拒绝的
旧单体文件或退役残留，另一进程恰好扫描到这些临时文件后便产生伪 RED。解决方式是串行运行这两轮 full
discovery，并等待前一轮完成夹具清理后再启动后一轮；该约束只针对共享产品源树的全量 discovery，不影响
互不覆盖文件族的独立文案审计与整改继续并发。2026-09-06 按此方式串行重跑，普通模式与 `-O` 模式均为
**1,796/1,796 GREEN**。这属于已证实的测试夹具隔离问题，不是产品或 CK3 加载 RED。
...
### R281：VFS mount lifecycle 成功、loader terminal 仍缺失（2026-09-08）

R281 在同一 source save、product projection 与 `continue-last-save` 编排上，只增加 default-OFF/read-only 的 VFS mount lifecycle observer。无启动预检前后 CK3/injector 槽均为空，source save SHA-256 仍为 `C7E5CA119F4DB4D9BFE738859DD5E32DEC6F9F1F804D91F9B26ED5D256E219DA`。同次 MSVC Release `/Z7 + /DEBUG:FULL + /MAP` bundle 的 DLL / injector / PDB / MAP SHA-256 为 `64D0BE6D980FBFFF1B22075F2DB5D71230B68B4ACD31756E434C2E310BDCBB64` / `74EA234451208BA8FAA8A75722B917C19A550E09DF5D0725F6A73FEA19A89A88` / `727A7F21097C9CCBEB0C094309504509C36354DDC32799C1EDD03DCCF132F065` / `1FF47D0D7785B0C46F109CEA3AEA3991CC7975C63D4D13C4320AD03499C32301`。

PID `128368`、generation `1` 不再于 `ck3+0x3BE33A9` 崩溃。最终 heartbeat 证明 D2 `installed=true / mask=15 / failure=0`：core init 调用一次并返回 true；34 次 publisher entry 全部有对应 return，`success=34 / failure=0 / correlation-miss=0 / overwrite=0`。目标 `paths.settings` lookup 时 manager head 已非零且无 read fault；既有 producer observer 同时证明两次目标 lookup 各自返回非 NULL，两个 parser 各进入一次，insert 共 38 次，`0x583` 前后各一次且 `result_inserted=true`，表从 count `30` 增长到 `31`。因此 R280 的双 NULL、空表与 exact crash shape 在本轮没有复现；本轮不能据此声称 observer 修复了竞态，只能证明该次 VFS 初始化与 publisher 路径成功。

游戏随后完成 303 条 `Database Node Init Time` callback，末项为 `CJominiInGameMusicDatabase`，无 crash bundle、无 runner-classified fatal error；但 164 秒静默后仍没有 Frontend、Load Save、In Game 或 native-ready terminal。runner 在 `299.729s` 以 `loader_terminal_missing_after_database_callbacks` RED 收口，并在总 elapsed `310.696s` 后受管停止：cleanup `GREEN`、tree gone、最终 CK3/injector 槽为空。report / loader gate / cleanup / session-state / debug.log / error.log SHA-256 为 `144065DBB0EE523012283971943A5DD2E801D723C7D6D118A61F408272FCFAFB` / `10CE0A86522D2B98AE05E92BA332293C3137E0A8100C0F94A0735A1860B716AC` / `C009FFF47CEC7678060311C05E89AF5D59631CE864B74BF05422BFFBDDCE4553` / `C39836B9295612284B70DBA627BE0D70C2B482BE3FE0FC30AF976691C245B012` / `A92BD1308BD1226A811DF5E1D409A9D596FAC832C621B54CF23AFDFA747E9B2E` / `1FAA4E6B45DB1C876E19CDD1518598AD55BF6F03BE64B0CF1AEFDF8BEABA5AAE`。

R281 只把 startup blocker 从 VFS crash 前移到 database callbacks 之后的 terminal-missing 边界；没有 map/mailbox、存档续接、玩法输入或事件 postcondition，因此不提升 scene、stage、full-tree 或宣传 readiness。下一次启动不得原样重复 R281；必须先用已有 exact-build completion observer 静态闭合一个能区分 completion publish、outer return 与 terminal consumer 的只读联合观测形态。

### R282：completion/consumer 联合观测进入 Frontend（2026-09-08）

R282 复用现有 default-OFF observer，将 D2 与 producer identity、completion consumer correlation、wrapper entry/consumer edge 合入同一 `/Z7 + /DEBUG:FULL + /MAP` build；全量 CTest `102/102` GREEN。bundle DLL / injector / PDB / MAP SHA-256 为 `07390F5DE17CAE35B5A7C798EF308AD7FCE45A8FB844B685D9B307B15E950DF3` / `74EA234451208BA8FAA8A75722B917C19A550E09DF5D0725F6A73FEA19A89A88` / `AEDB2842863B3035F62620F740052945FCD01525EB3C2B91386A4D217745974F` / `090462A0522395FAA15607DADDE58B9FE755E38AF3B494B04AE56D00E0C01A26`。

唯一 `continue-last-save` PID `23608` 再次得到 core-init true、34/34 publisher success，并完成 database `303/303`、fatal 0，随后明确进入 Frontend。completion raw hit `19115`，其中 state 2 为 `13461`；producer selected count `1`。publish 后 wrapper 重入 `186870` 次，两条 consumer edge 分别命中 `2466` / `184404` 次，但 retained selected task 的 consumer identity match 仍为 `0`。这证明 wrapper/consumer 在 publish 后持续运行，且 Frontend 可达；selected task 没进入 completion consumer 仍是一个私有调度诊断，不能冒充 Frontend readiness 的必要条件。
...
### R284：fresh opener 归零、exact chancellor drain 与 agenda RED（2026-09-08）

R284 从 `04ac889` 构建 `phase2-full-release-r284-04ac889`，共 1,031 文件。source/product tree SHA-256 为 `4960EDEE4CD2B0A748830D3C94F0753B727C977BEE4DD95A446C76CA7F8AFD30`；projection manifest / release manifest SHA-256 为 `63E30B4F5514C02B6A4897149F3DC84A1BF38E06A9BB860DA2AB902D656365D5` / `30AD063924464156B0F2F82BBBDD5C5A4D09E7127CCC7A3F2B2D9B561325ED19`。source autosave SHA-256 保持 `C7E5CA119F4DB4D9BFE738859DD5E32DEC6F9F1F804D91F9B26ED5D256E219DA`；state/artifact fresh、空进程槽与外层 no-launch preflight 均在启动前闭合。

final PID `45100` 在 warm-up 完整释放后显式载入 autosave，loader `303/303`、fatal `0`、paused/map/mailbox。initial client 依次完成 `debate_event.5110`、`zg361p2c.2`、`zg361.40`、`death_management.1001` 与 `ep3_decisions_event.2001`；R283 的 `This scope doesn't support variables` 及 B1 opener 文件签名都为 `0`。因此 `04ac889` 只对该旧族升级为 fresh-live 修复，不代表 B1/full-tree 整体 GREEN。

strict boundary 为 `chancellor_task.1002`，instance `207`、date `53209248`、root `32904`，保存 `councillor=29346`、`councillor_liege=32904`、`target=34838`，唯一 visible/enabled native option 为 `0`。CK3 1.19.0.6 原版 event source SHA-256 `EAF95612E4AEC6BF0CEDBC1ACA1C66C8087DD280BC42A60296F824437A5A46EB`；唯一 option 只执行 `scope:target cancel_truce_one_way = root`。独立合同同时绑定三 scope、target/councillor 不同、root 与 option shape；提交 `8e467fc` 后 generation `2` reconnect 精确验证 old instance 关闭。
...
### B1 持久 roster 的 exact-tuple prune（R492）
