...
| `ordered_in_list` 明明有多项，却只执行一次；或动态列表报 `ordered_in_list max bigger than list` | `ordered_*` 没写 `max` 时默认只取排名第一项，不等价于 `every_in_list`；而用过时/未过滤计数作为 `max` 时，实际合格列表可能更短 | 全量排序迭代必须保留显式 `max`；对会因过滤或 weak scope 清理而缩短的列表，在同一 `ordered_in_list` 加 `check_range_bounds = no`，并在排序前先 prune。2026-08-28 CK3 1.19.0.6 实测确认缺 `max` 只产生 1 行；2026-09-05 R84 又证明为压制短列表报错而删掉 `max` 会让后续全表读到大量未赋值 `zg361_b1_local_rank` |
