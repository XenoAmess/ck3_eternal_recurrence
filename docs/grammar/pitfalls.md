...
| 同一个 `limit` / `AND` / `OR` 中先写 `has_variable = x`，后写 `var:x = ...`，仍报 fetch-variable、unset-scope 与 invalid-comparison | CK3 trigger block 的同级条件不按书写顺序短路。Phase2 r8 对 23 个 B1 subject 的三个旧存档可选字段各报一次，共 `23 × 3 = 69` 条；`Z:\p2s10a\runner-report.json` 又在新 seed 的首个 B2 result 中冻结 93 条，其中 57 条来自 19 个未建立的 policy-debt active，余下是 remand/metric/PIP/object first-use 读取。两轮都已排除入口重复执行和 loader 性能 RED | 用 `trigger_if = { limit = { <整组 has_variable> } <全部 var: 比较> }` + `trigger_else = { always = no }`（或等价双层 effect `if`）建立真正的 lazy 边界；允许“缺失或不等于”时，也要把 `var:` 比较放进以同字段 `has_variable` 为 limit 的 `trigger_if`。原版同形证据见 CK3 1.19.0.6 `game/events/dlc/tgp/tgp_china_career_events.txt:207-218` 与 `game/common/scripted_effects/00_councillor_effects.txt:418-428`；B1 r9 已实机消除 69 条，B2 修复仍待同产品树实机复验。2026-09-04 实测 |
