...
| `List target 'x' is used but is never set. Setting it in an unused scripted trigger or effect does not count` | CK3 1.19.0.6 加载器会剔除三类不能到达读取点的写入：仅在 scripted effect、`always = no` event 分支、以及在业务读取前已 remove/clear 的 event-root 写入。三次实机 RED 依次为 `Z:\zg361_phase2_b1_live_20260831_0921`、`Z:\zg361_phase2_b1_live_20260831_0950`、`Z:\zg361_phase2_b1_live_20260831_1002` | 在实际消费者 event 中记录原 `has_variable_list`，加入专用 `flag:` target 并让它存活至业务读取；字符迭代必须用 `this != flag:anchor` 过滤，调用字符专用 effect 前则先移除 anchor。读取后：原容器存在就定向 `remove_list_variable`，原容器不存在还要 `clear_variable_list`，同时恢复成员与容器存在性。原版 `petition_liege_events.txt` 证明 flag target 可加入 variable list，`ep3_laamp_decision_events.txt` 证明可定向删除 flag target，`game/tests/event_target_lists_tests.txt` 证明 targeted remove 与 clear 后 `has_variable_list = no` |
