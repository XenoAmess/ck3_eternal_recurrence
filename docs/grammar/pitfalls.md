...
| `ordered_in_list` 明明有多项，却只执行一次；或动态列表报 `ordered_in_list max bigger than list` | `ordered_*` 没写 `max` 时默认只取排名第一项，不等价于 `every_in_list`；固定容量或过时计数也不是当前列表长度，运行时列表更短便会报错 | 全量排序迭代必须保留显式 `max`，并优先写成 `max = list_size:<同一个实际列表>`；只有排序内过滤或 weak scope 清理仍会令有效范围继续缩短时才加 `check_range_bounds = no`，且排序前先 prune。2026-08-28 CK3 1.19.0.6 实测确认缺 `max` 只产生 1 行；2026-09-05 R84 证明删 `max` 会令后续全表读取未赋值 rank；2026-09-06 R100 又在共同上级 bank 的 7 席固定上限、实际短列表上连续复现 3 次，四个 full-walk 均改为各自 live `list_size` |
