...
`ck3_query_engine_diagnostics_v1` 只读 `error.log`、`debug.log`、`game.log`、`system.log`、`setup.log`
和固定命名的 crash package。单日志上限 64 MiB；调用方只能调小/调大已设上界内的 tail、milestone、
...
gameplay/native 两个工具随现有 per-user stdio server 注册：
...
验证两者可见，但不读取真实存档/日志。新注册后必须新开 MCP client session，再以实际 listing 确认。
...
- 三个工具均为 readback，不是动作 ACK。`manifest_exists`、进程 PID 或 RPC 成功不等于 Steam 已更新、
