...
预览与 CK3 当前实际显示是否相差过大”。它不进入 GitHub Pages 或编辑器生产运行链路。
...
3. 把窗口绑定到 native bridge 报告的精确 CK3 PID 和连接代次；只接受前台、未最小化、无遮挡、
   `2560×1440` 的完整客户区。
...
该工具不做 OCR，不发送键盘或鼠标输入，不激活窗口，也不上传 framebuffer。返回的原生 crop
只写入本地 append-only 验收 artifact。网页正式版本仍只在浏览器内处理用户图片。
