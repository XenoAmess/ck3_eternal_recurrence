# 《一个 C 是怎样被生产出来的》二期制度版导演处理稿

状态：**正式并行成片方案（institution-led cut）/ director treatment / 非 live / 非 builder input**。这不是人物版的替代稿或备选废案，而是必须独立制作、审阅和交付的第二条正式路线。

本文只冻结二期宣传片的叙事、镜头与声音方向，不授权录制、TTS、字幕渲染、合成、发布，也不改变
`phase2-promo-project.json`、authoring ledger、八段 capture contract 或任何 readiness 结论。正式台词和入片主张仍须由通过有效
footage intake 的同一 canonical seed/save lineage、逐段可见观测、provider postcondition、claims audit 与两轮 1× 人审共同决定；允许的 span 间干净重启不得被剪成一场不间断实录。
