...
| 3 | `phase2_manager_governance` | R539 证明旧 AI-owned seed 入口与现行玩家限定产品冲突，旧素材不可用 | 恢复 hash-bound Stage 10 v7 玩家经理 source，运行 `run_stage10_player_subject`，在真实 `zg361mg.120` 保留画面；campaign-root、manager-governance 与 current-event query 可读 | 直属上级 owner / 玩家经理 subject 的 F case 到达 `state=5 / active=false`；经过时间或命令 ACK 不能代替终态 |
