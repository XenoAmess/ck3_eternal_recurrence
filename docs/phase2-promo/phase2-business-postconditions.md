...
状态：**mixed live；T0 `50%`、stage `8/11`、source `3/4` 为历史 coverage 快照，P2 `LOCKED`**。R303 已取得 projects/metrics
private-candidate provider 的真实 paused GREEN，R313 已捕获其 schema-2 source checkpoint；R326 又完成 incidents/operations
真实 `.50` checkpoint 与 strict receipt，并生成 schema-v3 three-of-four 前缀。该 source coverage 只缺 `capture_cross_cycle_endgame`。
默认 capability 边界不变，其他 span 仍不得由 fixture 或 ACK 提升 readiness，footage 仍为 `0/8`。

> **2026-09-10 policy revision：**下列四个 provider 合同继续保留为可复用业务语义和 coverage 资产，但“四门全部
> GREEN 才能签收 P1”的旧口径已 `POLICY_SUPERSEDED`。现行 P1 只取其中与 B1 fix、AF5 authored 42/native 41、Central
> stage 9/10/11 和代表性终态 cold-restore 回读直接对应的结果，再加 gameplay-window error scan、managed cleanup 与
> final-candidate L0，共九项机器门。projects/metrics、source `4/4`、第三次 `.356`、完整三周期和 footage 均不得扩成 P1
> blocker；P1 签收前 footage 仍属 P2 并保持锁定。
...
### 四个强语义合同的历史 receipt/coverage 状态

| 合同 | 当前状态 | 非阻塞 coverage 剩余 |
|---|---|---|
...
| cross-cycle/endgame | `SOURCE_AND_POSTCONDITION_PENDING` | 完整三周期、`.356`、owner `.361`、subject-side carried debt 与下一周期 charter/default 继续记为 coverage；只有 stage 9/10/11 真实终态本身属于现行 P1 硬项 |

这四个合同继续用于 canonical product 诊断、其他 mod/游玩智能体复用与真实 encountered RED 闭环，但不再作为一个“四项必须
全部完成”的 P1 总门。任何未列入现行九项机器门的缺口只能记录为 coverage，不得制造 P1 歧义或扩大 blocker。当前 R326
three-of-four artifact 为
...
关闭各自静态候选，不替代现行九项中的 live 硬证据。

`strict 4/361` 与 `definitions 106/626` 仅作非阻塞 backlog；它们不是 P1/P2 门。T0-P1 未签收，T0-P2
继续 `LOCKED`。
