...
> **1.0.0 已发布（2026-08-21）：** 付费自定义廷臣 v2 提供七页生成目录、0–120 岁与六项 0–100 基础能力、动态文化/信仰及可选同家族。最终 85 文件候选的完整 release-gating CK3 套件、非 debug 普通/铁人终局、L0 与确定性构建均为 GREEN；九语言、干净截图、UI 裁切和缩略图已完成人工签核。Steam 强制重下载缓存与正式 manifest 已完成逐文件验证，完整证据见 [发布验收报告](../release-qa-v1.0.0.md)。
...
6. 可从决议菜单选择征服者、织网者、圣徒、家主、贤王或享乐者契约。对应 CK3 行为提供增量进度和分数，3/6/10 进度会触发反馈并永久保存该契约 PB；详见 [契约与成长机制](../contracts-and-progression.md)
...
**算分规则**：详见 [完整算分规则](../scoring-rules.md)（逐条分列；游戏内死亡结算事件会展示当局逐项实况数值与完整公式）。

截图来源、裁切范围与发布用途见 [主产品截图合同](../../workshop/main_screenshots.md)。

## 原理

利用引擎全局持久化的 `tutorial.txt` 课程完成列表作为只增位存储，把真实分数向下量化为分层余烬位阶（粒度 1→1000 递增，上限 166,600，可扩展）。课程用 `trigger_transition` 自动完成，无需玩家点击；读取经由 customizable_localization → request 门控 GUI state → scripted_gui 桥接导入存档。详见 [跨存档持久化](../cross-save-persistence.md)。
...
本页只维护主 Mod 的玩家合同。开发、测试和发布规则分别见：

- [项目总入口](../../README.md)
- [仓库执行规则](../../AGENTS.md)
- [测试与实机验收](../testing-workflow.md)
- [Workshop 发布流程](../workshop-publishing.md)
- [咒、术、道与辉煌愿景](../project-system-overview.md)

源码：<https://github.com/XenoAmess/ck3_eternal_recurrence>
