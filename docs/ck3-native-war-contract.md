...
4. 玩家是进攻方、本方 primary war leader 且存在 exact `war_objective_province_ids`：从战争刚开始的 0 分起就按 DLL 的 DFS 顺序围攻 exact 目标；Python 只做稳定去重，不按 ProvinceID 重排。敌军当前省或 route 终点威胁某一目标时，跳到下一个未完成且未受威胁的 exact 目标。
