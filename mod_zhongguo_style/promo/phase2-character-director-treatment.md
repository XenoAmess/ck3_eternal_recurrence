...
对应的另一条独立成片路线是 `phase2-director-treatment.md`（制度群像版）。两版共享二期的真实实机素材合同、十章结构和八个 canonical span，但采用不同的叙事主角、旁白组织和剪辑节奏；最终应分别导出、分别审阅，不得只把同一条片换标题交付两次。
...
- 八段素材必须满足拍摄时有效的 `zg361_phase2_footage_intake` 合同。当前 authoring ledger 仍要求同一 managed run 时，按更严格规则执行；未来若验证后的合同允许同一 canonical lineage 下分段重启，剪辑也不得把重启伪装为单次不间断实录。
