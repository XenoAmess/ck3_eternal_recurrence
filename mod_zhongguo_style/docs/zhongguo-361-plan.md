...
版本：v1.1（2026-08-28，实施与审计基线）
状态：**一至三期主链已实现；本轮实机结果见 §14**
命名空间：产品脚本 `zg361_`；事件 `namespace = zg361`；外部验收夹具 `zga_`

> §4–§13 保留了早期设计与实施偏差记录。当它们与 §14 或 README 冲突时，以后两者的当前实现口径为准。
...
每位天朝制公爵及以上领主定期给直属官员打分排名，发榜、奖惩、校准、淘汰；玩家可以同时管理下属榜并在上司榜中受评。

## 2. 决策确认记录（所有者拍板）

| 决策点 | 结论 |
|---|---|
| AI 适用性 | **AI 天朝制公爵及以上领主也执行完整考核**（AGENTS.md 默认玩家限定约束的第二个已授权例外，首个为 ox_here）。AI 静默结算，事件只弹玩家 |
| AI 末位淘汰策略 | **激进版**：所有 AI 天朝制公爵及以上领主均可对连续 3.25 的直属官员执行末位处置，裁决取决于好感、能力、家族势力、派系、性格、财政等因素（详见第 5 节权重表） |
...
## 3. 原版机制映射（调研证据，CK3 1.19 / 溥天之下〔All Under Heaven〕）
...
- **考核单元**：每位天朝制公爵及以上领主（不要求独立）+其直属在任官员。
  天朝总督均可受评，直属伯爵/男爵显式纳入；伯爵/男爵因 `tier_duchy` 下限只受评、不能考核别人。
- **周期**：默认每年一次，挂 `yearly_playable_pulse`（各领主生日年度 tick，天然错峰）。
  游戏规则 `zg361_review_frequency`：年度（默认，阿里梗）/ 三年（明代大计梗）。
- **新人保护**：考核者完成至少一轮结算、建立 `zg361_review_baseline_initialized` 基线后，后来出现且没有 merit 快照的新官仍参加首次考核并出现在榜单，可得 3.75/3.5，但不进入本轮 3.25 候选；末位名额顺延给排名最靠后的旧官，旧官不足时允许缺额。模组首次启用时的既有官员属于建账基线，不是全员“新入职”，首轮必须照常执行强制分布。该规则对应现实“新员工容易成为低绩效候选人”痛点的反向保护，但不得借初始化掏空刚性 361。
...
```
# 1. 建 cohort 并打分（首次建账的存量官员正常强排；建账后新官才豁免一次 3.25）
every_vassal = {
...
- 末位比例由游戏规则或单一“调整末位比例”决议提供：刚性 10% / 松绑 5% / 不设强制末位 0%。决议打开三档廷议窗口并标明当前生效口径，新口径从下一次配额计算起施行。
- 只有一至两人的直属考核池仍按 KPI 稳定排名，但因没有足够同侪，不进入强制分布、校准与互评；全员按 3.5 走正常结算、榜单、告身和年度幂等链。0 人才不结算。
- 同分不再膨胀档位：稳定名单顺序写唯一名次，头档精确占位；末位从最差的非新人向前取人，老员工不足时允许真实新官首轮的 3.25 配额缺额。首次建账没有“全员新人”例外。
...
| **3.25 待改进** | 领主好感−20、`monthly_merit_mult −0.5`、每月威望−0.5、俸禄−25% | merit 扣减 + PIP；有国库者罚 50 国库，所有人扣 25 个人金币；申诉改判原数退回；考绩不佳 opinion 提供合法夺爵理由；`streak_bottom += 1` |
...
玩家作为 AI 天朝制直属上司的封臣被判淘汰时，收到淘汰通知事件，获得一次性翻盘窗口：

- **最后申诉**：消耗影响力 + 人情 hook，按真实百分位重裁（翻案成功 → 降岗留用）；
- **散尽家财**：支付大额金币/国库 → 改降岗留用（"花钱保命"）；
- **掀桌起兵**：加入或创建反直属上司的独立派系（"绩效逼反"，给出反抗出口）；
...
- **末位比例决议**：决议列表只保留一个配置入口；廷议窗口内选择刚性 10% / 松绑 5%（2020 改革梗，
  “361 的打分要坚持，刚性末位比例需要松绑”）/ 不设强制末位 0%，并显示当前生效档位。调整消耗威望。
- **角色互动**：
...
3. 玩家作为 AI 天朝制直属上司的封臣收到结果事件；连续两次 3.25 的玩家封臣收到淘汰事件且四个出口可用；
4. AI 天朝制公爵及以上领主对连续 3.25 的直属官员按权重表执行处置（构造低好感低能力用例断言夺爵发生）；
5. error.log 0 条 `zg361` 错误。

### 二期 · 博弈层

交付：校准会议事件链 + 单入口末位比例三档廷议 + 四个角色互动 + 野狗/小白兔双轨事件 + 完整末位淘汰链 UI。
...
4. **merit 快照边界**：首次建账时，无快照只让增长项按零处理，所有存量官员仍参与 361；考核者完成一轮结算后，后来无快照的新官才“正常入榜但免一次 3.25”。第一次京察也必须给每名直属官员送达绩效。
...
10. 新增“首轮试用期”语义：完成考核基线后才加入且无 merit 快照的官员正常参评、可得 3.75/3.5，但免于被强制分布为 3.25；同轮写入快照供下次计算成长。2026-08-30 修正了旧实现把首次启用时全体存量官员误判为新人的初始化缺陷。

### 13.1 查漏补缺闭环（2026-08-28 第二轮自审）

首轮实施后对照 §4/§5 重新逐条核对，补齐以下遗漏：

- AI 裁决分补上**军力对比**（直属官员兵力 > 考核者 20% → −50；> 50% → 处置强制锁致仕）、
  **考核者直辖未满**（+20 夺爵回血动机）、**淘汰者垫底**（名次 = cohort 末位 → +50）三个因子，
...
- 考核者性格因子采用 sadistic/deceitful/ambitious/arbitrary/callous 与 compassionate/forgiving/trusting；
...
- **末位比例单入口决议**（刚性 10% / 松绑 5% / 不设强制末位 0%）：确认决议后通过玩家专用 GUI bridge
  打开 `zg361.54` 三选项廷议，当前档位置灰；选择仍写领主变量 `zg361_ratio_override`，阈值计算优先于游戏规则。
  调整消耗威望 75，冷却 1 年，AI 不用。
...
- **玩家"掀桌起兵"出口**：zg361.6 新增选项 d——建立独立派系对抗直属上司（`can_create_faction` /
...
- **京察大计活动**（`activity_zg361_jingcha`）：治所单阶段、免费、定期弹出的半强制考核。
  玩家只要有至少一名合法直属官员（包括 1–2 人小组）就默认应举办；拒办则被直属上司降好感，并留下上司下次评你时的一次性重大 KPI 扣分。
  AI 默认履责并后台结算；独立最高领主拒办不伪造自我评分。
...
本节保留为当时的静态施工记录；当前实机状态以 §14 和独立测试报告为准。

## 14. 当前权威基线与实机闭环（2026-08-29）

本节覆盖前文中已经过时的范围和验收描述。

### 14.1 权威制度边界

- 考核者：所有在任、在世的**天朝制公爵及以上领主**，不要求独立。
- 被考核者：考核者的直属在任官员，包含天朝总督、伯爵与男爵。
- 伯爵和男爵只被考核，不能主持考核；公爵及以上可同时考核下属并接受自己的上司考核。
- 本 mod 是默认 AI 禁止规则的第二个所有者授权例外：AI 天朝制公爵及以上走完整后台考核；可见事件仍只对玩家弹出。

### 14.2 当前交付

- 持久双榜 GUI：右上角按钮、“我考核的官员”与“我所在的受评队列”两视角、榜头统计、角色行、PIP/晋升状态。
- 个人结果与主持汇总分流：主持者只看自己主持的名册结果；玩家被上司考核时，告身明确显示上司、3.75/3.5/3.25 档位、KPI 和同组名次。
- 3.25 四重问责：地方国库 `-50`、个人金币 `-25`、贤能 `-60`、一年俸禄预算收入 `-25%`；本轮申诉翻案会退回固定罚没并停止未来扣薪。
- 免费半强制京察：拒办且有上司时产生 `-20` 上司好感和下一次上司考核的一次性 KPI `-50`；独立最高领主只扣威望 `200`。
- 轮次/领主绑定、榜单冻结与受评榜镜像，防止延迟事件、调任和淘汰错误作用于新轮次。

### 14.3 实机结论

2026-08-29 在 CK3 `1.19.0.6`、精确 EXE SHA-256
`2D00FF3101EF70B566F2FCBAE292F09263199C80E9DC8F139B82D7D96F83DB86` 上完成隔离验收，结果 GREEN：

- 非独立 AI 天朝制公爵级领主成功进入真实考核并完成公示；
- 宋帝管理榜生成 23 行（7 / 14 / 2），GUI 标题、统计、列头和角色行均经画面 OCR；
- 玩家切换为直属官员后，由 AI 宋帝走真实考核链，告身实显 `3.75`、`KPI 78`、`第 2／23 名`；
- 产品相关运行诊断为空，源树/运行树未变，真实用户存储未变。

完整证据、哈希与未覆盖矩阵见 `docs/testing-report-2026-08-29.md`。活动 300 日逾期、申诉/转封/存读档、长名单滚动等分支仍需后续专项实机，不能由本次 GREEN 外推。
