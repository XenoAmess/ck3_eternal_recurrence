...
- OCR/键鼠当时只获项目所有者授权作为临时兼容路径。2026-08-29 写下该要求时，正规 MCP 终态确实只有 `research / requirement-only` 合同；后来落地的实现与 2026-08-30 实机证据必须按各自提交和 artifact 另行归因，不能倒写成当时已经具备。当前正式路径已由 MCP 按 stable landed-title key 解析并定位地图，并在最终合批 attempt 达到 `fixture-live`；能力合同见根知识库 `docs/ck3-native-title-map-navigation-contract.md`。历史 OCR 结果仍不能冒充 MCP 证据。
...
2026-08-29 末，项目所有者将正规路径优先级提升到 MCP。上述 OCR 状态机只保留为历史兼容与故障取证，不再作为正式宣传片导航路径。2026-08-30 最终合批 attempt `zga_20260830_0930_clean_2fa2ac8_mcp` 已在同一受管 PID 完成 `c_bianzhou`、`b_kaifeng`、重复 `already_centered`、unknown-key typed RED 和最终汴州镜头；该 MCP 矩阵内 OCR、屏幕/像素判断、窗口激活、键盘、鼠标与剪贴板调用计数均为 0。CK3 前台线程仍独立签为并保持 US English HKL `0x04090409`，但 HKL 不再充当头衔导航的依赖或成功证据。
...
- 存读档矩阵不能盲目要求“整份 attempt 单 PID”。当前 `ck3_restore_checkpoint` 明确定义为重启受管 pure-native session；正式证明应允许一次受控 PID/connection generation 变化，并核对恢复前后的 date、player、案卷、期限、receipt 与冻结 policy。最省启动成本的形态是首进程只负责开案和保存，恢复后的第二进程连续完成全部期限与多周期；不能为了满足单 PID 口号而跳过真实 load，也不能把一次 restore 拆成许多重复启动。
...
- 源素材的完整视觉审计不覆盖合成器后加的角标、章节牌、字幕和其他 overlay。2026-08-30 的 97.033333 秒源帧审阅卷已以 1× 通过五项人工签核，retry03 也通过了当时的技术媒体门，但合成后仍出现 450 px 定宽证据角标裁切。因此固定宽度角标必须在渲染前按实际字体和文本测量并做边界 preflight；任何合成器视觉变更都必须使缓存键失效。自动技术校验与均匀抽帧只是辅助门，最终发布 MP4 还必须由人以 1× 从头到尾完整播放，并将签核绑定到 MP4 的精确 bytes、时长和 SHA-256。
