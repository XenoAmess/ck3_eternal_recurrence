...
- [ ] 一次整批运行完成 361/361：每个编号的 BEGIN/PASS 遥测恰好出现一次，0 duplicate，0 missing；14 本组织账与选择状态均通过。
...
- [ ] 报告明确声明覆盖边界：361/361 是每号参考路线的真实引擎执行，不是 1,083 个 A/B/C 分支逐一人工点击；未跑的自然 300 日等待、跨三年存读档或角色截图矩阵不得写成已实测。
