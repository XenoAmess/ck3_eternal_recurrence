...
因此，该轮只证明所辖页、驾驶舱切换/返回、标题栏 X、模态背景和 1 条所辖人物行。本人榜入口、本人页签与本人榜人物行仍无当前版本的 deliberate-click L3；驾驶舱也缺少“关闭榜单 → 重开 → 再进入”的独立闭环。后来八轮还反复证明 Escape 没有关掉榜单，所以不得继续用“三个页签和全部关闭路径均已有实机证据”的口径。下一轮仍会在同一次启动里批量重跑，不借用局部通过冒充整批 GREEN。

## 7. 2026-08-29 新一轮阻塞审计与修复

只读审计对照当前 GUI SHA-256 `B50D3122907AAF25A6E82BF1C60528095E85DBAA749526656F960C1813EA03AB` 与最新同字节实机投影，发现三个产品缺口：
...
权威生成器现已作最小修复：增加 ledger-only 第三个互斥入口并默认打开 `system` 页；modal 可由 managed、received 或 ledger 任一状态维持；显示门补齐 struggle/tutorial；两处 shortcut 改为原版无引号形式。生成投影与九项静态测试已经通过。Escape 的根因虽然与原版语法差异高度吻合，但在下一次 CK3 同批复验前仍只标 `static-ready`，不能提前写成 L3。

当前实机证据边界保持：所辖入口及右窗抑制、所辖页、驾驶舱切换/返回、X、backdrop、1/80 所辖人物行已有 L3；本人入口/页签/80 行、深滚动、ledger-only、Escape、struggle/tutorial、Pause/Outliner/Barbershop、其他分辨率仍待同批或代表性实机覆盖。
