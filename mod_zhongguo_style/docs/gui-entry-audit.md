...
因此，考核榜常驻入口、原版右窗抑制、动态“决议”入口、三个页签、标题栏 X、模态背景以及代表性人物行均已有真实 CK3 证据；现存 RED 不再来自这些 GUI 按钮。尚未取得的是“切换到受评玩家后本人页签 + 3.25”的同批闭环，因为随后京察录制时序提前触发了个人结果，且固定吕居简实际依法得到 `3.5`。这两项属于录制夹具/目标选择问题，下一轮修复后仍会重新跑完整按钮矩阵，不借用本轮局部通过冒充整批 GREEN。
