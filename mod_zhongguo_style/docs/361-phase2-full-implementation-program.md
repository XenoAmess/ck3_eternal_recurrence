...
依赖门仍按 `Phase 0 → B1 → B2 → B3 → B4 → B5 → B6 → B7 → B8` 判断 readiness；后批不得绕过未闭合的前置资源或状态合同。
这不限制 Agent 子线程并行施工。能以独立生成器、文件所有权、运行现场或依赖边界隔离的模块应主动并行，并尽快直接收口到
`master`；“单人项目、谨慎开分支”只约束 Git 分支数量，不得再误读成少开 Agent 线程。共享 kernel、中央 hook 与唯一 GUI
表面由根线程串行整合，避免双写和回合冲突。
