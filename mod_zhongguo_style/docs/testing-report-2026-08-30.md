...
原因是 CK3 trigger 的同层 AND 条件不保证短路；把 `has_variable` 和 `var:` 比较平铺书写，仍会在 tooltip 预求值时读取不存在的变量。以上两项当前为 **static-ready**，需下一次同批 CK3 运行确认项目诊断归零及后半段新人门通过。
...
- 本报告只把 `7 / 14 / 2` 升级为 live-confirmed；GUI、京察、后半段新人和正式宣传素材仍等待下一次完整 GREEN。
