...
当前轮次 R629 的人工体验在 #107“继任准备度阶梯”实机发现：A/B/C 虽然会调用 manager/core/consumer、写入准备度并继续 D+1 案卷，但原生自动预览无法展开这些自定义 effect，三项 hover 因而统一显示“没有直接影响”。生成器现为 44 项逐案裁决的 A/B/C 各绑定一条 authored `custom_tooltip`，逐项说明按钮所登记的直接状态、次日续链和分域多数结算；七项真实双付款同时说明上司国库/私财各付 5、当事人两账各收 5，C 路说明下周期制度债与本局停提。#107 另明确准备度记录在两个考核周期后到期复核；#116 的 C 路明确保留 90 日放人时钟且不消耗一次 60 日延期。组合办案方式卡的 A/B/C/D 也披露批量范围、失败回退和首张关键裁决的次日时点。业务 effect、玩家/AI 权限和案卷状态机均未改变。

R629 只证明旧版本的呈现缺陷真实存在；新增 tooltip 尚未进入该运行实例。它在 fresh 产品中完成选项 hover 抽检前仍是 **static-ready**，不得写成 live 修复。
...
- 玩家事件与 AI 静默边界；
- 44×3 条逐案效果 tooltip、组合办案方式 4 条 tooltip、#107 两周期复核说明与七项双付款披露；
