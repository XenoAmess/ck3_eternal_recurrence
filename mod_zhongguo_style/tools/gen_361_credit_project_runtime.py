...
MECHANISMS = (
    m(26, "e", 1, "effort_ledger", "Work and Visibility Are Different Ledgers", "实绩与可见度分账",
      "The capacity ledger shows delivery, reporting and relationship work drawing from one balance. Only delivery has produced hard output so far.",
      "容量账显示交付、汇报与关系事务正在争用同一份余额；目前只有交付工时产出了实际成果。",
    ...
    m(27, "e", 2, "signed_contribution", "Allocate One Hundred Percent of the Credit", "把百分之百贡献分配清楚",
      "The subject, direct manager and cross-department contributor all claim the same result. The file has no complete ten-thousand-basis-point allocation yet, and none of them has personally signed one.",
      "受评者、直属上司与跨部门贡献者都对同一成果提出了主张；案卷尚无合计一万基点的完整归属表，三人也都没有亲自签字。",
      "Allocate seventy percent to the subject, twenty to the manager and ten cross-department.", "Allocate fifty percent to the subject, thirty to the manager and twenty cross-department.", "Give management and coordination more weight.",
      "受评者七成、直属上司两成、跨部门一成。", "受评者五成、直属上司三成、跨部门两成。", "提高管理与协调贡献的权重。"),
    m(28, "e", 3, "credit_claim", "Credit Claim and Audit Reversal", "抢功申诉与审计回拨",
      "The direct manager has challenged the frozen allocation and claimed more credit. Any accepted transfer would have to leave the total unchanged; rejected changes cannot rewrite the baseline.",
      "直属上司对冻结归属表提出了加功主张；任何获准转移都不能改变总量，未获支持的改账也不能污染原底稿。",
      "Transfer five hundred basis points from the subject to the manager.", "Reverse the five-hundred-point transfer after evidence review.", "Reject the unsupported claim without transfer.",
      "从受评者向直属上司转移五百基点。", "审证后撤销五百基点转移，恢复原份额。", "驳回无证据主张，不发生转移。"),
    m(29, "e", 4, "metric_audit", "Metric Packaging and Audit", "指标包装与审计",
      "The headline metric has jumped, but the review has not established its source and the later cost is still missing from the file.",
      "账面指标突然上升，复核却尚未查清收益来源，后续成本也仍未入账。",
      "Confirm genuine improvement: ten short-term gain and no delayed cost.", "Record metric gaming: twenty short-term gain and eight delayed cost.", "Record fraud and claw the gain back.",
      "确认真实改善：短期收益十，无延迟成本。", "认定指标博弈：短期收益二十、延迟成本八。", "记录造假并追回全部短期收益。"),
    m(30, "e", 1, "resource_race", "One Project Wins the Capacity Race", "资源赛马只产生一个赢家",
      "Three proposals are competing for one project slot. No hours have yet been reserved in the finite capacity ledger.",
      "三份提案正在争夺唯一项目席位，有限容量账本里还没有为任何一项预留工时。",
    ...
    m(31, "e", 2, "sponsor_credit", "Sponsor Credit Has a Cap and Expiry", "恩主信用有上限也会到期",
      "A sponsor has offered an introduction, but the available sponsor balance is finite and expires. The offer contains no new delivery evidence.",
      "恩主愿意出面引荐，但可用信用有限且会到期；这份人情没有附带任何新的交付证据。",
      ...
      "透明授信二十点，不动余额并增加五点可见度。", "动用十点恩主信用，增加十点可见度。", "不用恩主信用，只靠证据。"),
    m(54, "i", 1, "report_build", "Reporting Consumes Delivery Capacity", "汇报会挤占交付容量",
      "The report packet has not been built, and every hour spent on it will come out of the remaining project capacity without adding delivery output.",
      "汇报材料尚未制作；投入其中的每个小时都会从项目剩余容量中扣除，却不会增加交付产出。",
    ...
    m(55, "i", 4, "attention_read", "Routing Is Not Reading", "送达不等于阅读",
      "The report has reached management, but neither of the two deep-read slots has been spent. Delivery alone therefore proves no one has read it.",
      "材料已经送到管理层，两个深读席位却都尚未使用；仅有送达记录，不能证明任何人真正读过。",
    ...
    m(56, "i", 2, "forwarded_credit", "Forward Without Quietly Taking Credit", "逐级上报不能悄悄截功",
      "The frozen contribution book is ready to be forwarded, while one manager has requested a share change. Any such change would have to preserve the total.",
      "冻结贡献底稿即将上送，其中一名上司同时提出改份额；无论是否调整，归属总量都不能变化。",
    ...
    m(57, "i", 2, "version_signature", "Freeze the Report Version", "冻结汇报版本",
      "Several edits now compete as the report of record. The attribution must total ten thousand basis points before one becomes the subject-authored frozen version; no fresh personal signature exists.",
      "多份修改稿都在争当正式版本；归属合计到一万基点后才能冻结为受评者署名稿，目前并没有新增的本人签字。",
      "Freeze the first attribution version.", "Freeze the second attribution version after evidence review.", "Freeze version three with the cross-department appendix.",
      "冻结第一版归属表。", "审证后冻结第二版归属表。", "带跨部门附件冻结第三版。"),
    m(58, "i", 3, "report_route", "Freeze Material Before Routing", "先冻结版本，再路由",
      "The material is frozen and awaiting dispatch. A recipient record will show where it went, but no attention slot is consumed until someone reads it.",
      "材料已经冻结并等待送出；收件记录只能证明去向，真正阅读之前不会消耗注意力席位。",
    ...
    m(60, "i", 4, "idea_arbitration", "The Frozen Version Owns the Idea", "创意归属服从冻结版本",
      "Two parties claim the same idea. The frozen author record and cross-department provenance disagree on how much each contributed.",
      "两方同时认领同一项创意，冻结作者记录与跨部门来源对各自贡献给出了冲突线索。",
    ...
    m(61, "i", 1, "report_policy", "Choose the Reporting Regime First", "先定汇报制度",
      "No common reporting rule exists. Teams now submit packets of very different length, making their drain on project capacity unpredictable.",
      "眼下没有统一汇报规则，各团队送来的材料长短悬殊，对项目容量的消耗也因此无法预估。",
    ...
    m(62, "j", 2, "matrix_conflict", "Two Lines, One Recorded Choice", "两条汇报线，只能留下一个选择",
      "Two managers have issued incompatible priorities. Following both would leave two contradictory commitments and no single accountable instruction.",
      "两名上司下达了互不相容的目标；若同时照办，案卷只会留下两项矛盾承诺，没有唯一责任指令。",
    ...
    m(63, "j", 1, "matrix_weights", "Lock Solid and Dotted Weights", "锁定实线与虚线权重",
      ...
      "实线七成、虚线三成。", "实线与虚线各占五成。", "实线四成、虚线六成。"),
    m(64, "j", 3, "manager_handoff", "A Manager Handoff Needs Both Owners Named", "换上司必须列明新旧责任人",
      "A role handoff is pending. Future responsibility may move to a successor, but historical cases remain with the old manager; neither manager has personally consented here.",
      "职司交接已经提上日程；未来责任可以转给继任者，历史案卷仍归旧上司，两人目前都没有亲自表示同意。",
    ...
    m(65, "j", 2, "parachute_staffing", "An Airborne Manager Brings a Staff Pack", "空降主管与旧部包",
      "The incoming manager wants to fill a ten-person team with former staff. Every imported seat displaces retained institutional memory, and the favoritism threshold is close.",
      "新任主管准备把旧部带进十人团队；每占一个席位都会挤掉组织记忆，任人唯亲的审计阈值也已逼近。",
    ...
    m(68, "j", 4, "portable_history", "History Travels; Authorship Does Not", "履历可携带，历史作者不改写",
      "A transferred official arrives with prior ratings and an open PIP. The receiving team has not decided which history to use as a prior; none of it may consume current quota or re-own old cases.",
      "一名调入者带着旧评级与未结绩效改进计划到岗；接收团队尚未确定哪些历史只作先验，它们都不能占用本期名额或改写旧案责任人。",
    ...
    m(129, "r", 2, "promotion_queue", "Promotion Is a Queue With Real Slots", "晋升是有真实槽位的队列",
      "The promotion queue already has an eligibility order, but this cycle has only one award slot. The subject has not yet been placed against that opening.",
      "晋升队列已经排定资格先后，本周期却只有一个授予槽；受评者尚未与这个空缺完成匹配。",
    ...
    m(130, "r", 3, "talent_transfer", "Do Not Dump a Hidden PIP", "不能把隐藏 PIP 倾倒给别组",
      "A transfer request carries an open PIP and a claim of role mismatch. The file lacks a complete chain from disclosure and role evidence to trial outcome and source-manager accountability.",
      "一宗调动申请同时带着未结绩效改进计划与错岗主张；披露、岗位证据、试用结果和原上司责任尚未连成完整链条。",
    ...
    m(131, "r", 1, "project_track", "Exploration and Commitment Are Different Tracks", "探索项目与承诺项目分轨",
      "Results are still unknown, but the project file has no registered track or matching success rule. Exploration and firm commitment would be judged differently.",
      "结果尚未出现，项目案卷也没有登记类型和对应成功口径；探索任务与承诺任务本来就不该用同一把尺。",
    ...
    m(132, "r", 4, "stop_loss", "A Timely Stop Can Earn Credit", "及时止损也可以算功",
      "The project is still consuming capacity while evidence for stopping accumulates. The file does not yet separate the business outcome from the timeliness of the individual judgment.",
      "项目仍在消耗容量，支持止损的证据却不断累积；案卷尚未把业务结果与个人判断是否及时分开。",
    ...
    m(133, "r", 5, "postmortem", "Learning and Liability Need Separate Tracks", "复盘学习与具名责任分轨",
      "The postmortem contains both a system defect and a possible named violation. Treating them as one finding would either punish everyone or erase proven accountability.",
      "复盘同时发现系统缺陷与一项可能成立的具名违规；混成一个结论，要么造成连坐，要么抹掉已有责任证据。",
