...
def render_m360_source_cleanup() -> str:
    """Remove the prior manager-owned cohort source before a new publish.

    Absence is the typed N/A result.  In particular, an invalid or structurally
    ineligible cycle must not leave the preceding cycle's immutable tuple
    looking current, and it must not replace missing business facts with zero.
    """
    ...
    lines = [
        f"\tremove_variable = zg361_b1_m360_source_{field}"
        for field in scalar_fields
    ]
    lines.extend(
        f"\tremove_variable = zg361_b1_m360_source_forced_{slot}_{field}"
        for slot in range(1, M360_FORCED_CAP + 1)
        for field in slot_fields
    )
    return "\n".join(lines)


def render_m360_forced_slot_writes() -> str:
    """Freeze final forced-C identities in stable processing order."""

    source_fields = (
        ("processing_order", "zg361_b1_processing_order"),
        ("m357_receipt_id", "zg361_b1_m357_external_receipt_id"),
        ("m357_receipt_hash", "zg361_b1_m357_external_receipt_hash"),
        ("b1_owner", "zg361_b1_result_adapter_b1_owner"),
        ("b1_subject", "zg361_b1_result_adapter_b1_subject"),
        ("b1_cycle", "zg361_b1_result_adapter_b1_cycle"),
        ("b1_case", "zg361_b1_result_adapter_b1_case"),
        ("result_owner", "zg361_b1_result_adapter_result_owner"),
        ("result_subject", "zg361_b1_result_adapter_result_subject"),
        ("result_cycle", "zg361_b1_result_adapter_result_cycle"),
        ("result_case", "zg361_b1_result_adapter_result_case"),
    )
    blocks: list[str] = []
    for slot in range(1, M360_FORCED_CAP + 1):
        writes = [
            f"\t\t\t\tset_variable = {{ name = zg361_b1_m360_source_forced_{slot}_character value = prev }}"
        ]
        writes.extend(
            f"\t\t\t\tset_variable = {{ name = zg361_b1_m360_source_forced_{slot}_{target} value = prev.var:{source} }}"
            for target, source in source_fields
        )
        blocks.append(
            "\t\t\tif = {\n"
...
def render_m360_source_producer() -> str:
    """Project one real published B1 cohort for Workforce #360.

    This is deliberately a source-only adapter.  Central selects three
    distinct managers and derives their cohort IDs; Workforce owns the later
    collective submission.  B1 publishes nothing unless the full processing
    list, full agenda, final quota and every #357 receipt agree exactly.
    """

    return r'''# Product source for Workforce #360.  The manager owns this immutable
# projection; Central may join it, but callers cannot supply or rewrite facts.
zg361_b1_publish_m360_cohort_source_effect = {
__M360_SOURCE_CLEANUP__
	set_variable = { name = zg361_b1_m360_work_valid value = 1 }
	set_variable = { name = zg361_b1_m360_work_member_count value = 0 }
	set_variable = { name = zg361_b1_m360_work_forced_count value = 0 }
	set_variable = { name = zg361_b1_m360_work_abs_c value = 0 }
	set_variable = { name = zg361_b1_m360_work_missing_fields value = 0 }
	set_variable = { name = zg361_b1_m360_work_tuple_mismatch value = 0 }
	set_variable = { name = zg361_b1_m360_work_m357_hash_sum value = 0 }
	# Before mark-published this status is absent and Central must WAIT.  Once a
	...
	if = {
		...
		if = {
			limit = {
				has_variable = zg361_b1_agenda_mode
				has_variable = zg361_pending_325_n
			}
			if = {
				limit = { var:zg361_b1_agenda_mode = 3 }
				set_variable = { name = zg361_b1_m360_source_status value = 2 }
				set_variable = { name = zg361_b1_m360_source_reason value = 1 }
			}
			else_if = {
				limit = { var:zg361_pending_325_n = 0 }
				set_variable = { name = zg361_b1_m360_source_status value = 2 }
				set_variable = { name = zg361_b1_m360_source_reason value = 2 }
			}
			else_if = {
				limit = { var:zg361_pending_325_n > 6 }
				set_variable = { name = zg361_b1_m360_source_status value = 2 }
				set_variable = { name = zg361_b1_m360_source_reason value = 4 }
			}
		}
	}
	if = {
		limit = {
			...
			has_variable = zg361_b1_agenda_n
			has_variable = zg361_b1_agenda_hash
			has_variable = zg361_b1_agenda_new_hash
			has_variable = zg361_b1_agenda_mode
			has_variable = zg361_b1_agenda_header_object_available
			has_variable = zg361_b1_agenda_header_owner
			has_variable = zg361_b1_agenda_header_subject
			has_variable = zg361_b1_agenda_header_cycle
			has_variable = zg361_b1_agenda_header_case
			has_variable = zg361_b1_agenda_header_state
			has_variable = zg361_b1_agenda_closed_case
			has_variable = zg361_b1_agenda_reviewed_n
			has_variable = zg361_b1_agenda_skipped_n
			has_variable = zg361_b1_quota_conservation_valid
			has_variable = zg361_b1_quota_recount_bottom
			has_variable = zg361_pending_325_n
		}
		set_variable = {
		...
		set_variable = {
			...
			value = { value = var:zg361_b1_agenda_reviewed_n add = var:zg361_b1_agenda_skipped_n }
		}
		if = {
			limit = {
				var:zg361_b1_cycle_state = 8
				var:zg361_b1_closure_state = 4
				var:zg361_b1_processing_n >= 1
			...
			every_in_list = {
				...
				if = {
					limit = {
						...
						has_variable = zg361_b1_agenda_item_object_available
						has_variable = zg361_b1_agenda_item_owner
						has_variable = zg361_b1_agenda_item_subject
						has_variable = zg361_b1_agenda_item_cycle
						has_variable = zg361_b1_agenda_item_case
						has_variable = zg361_b1_agenda_item_state
						has_variable = zg361_b1_m137_receipt_serial
						...
						has_variable = zg361_b1_m357_external_absolute_grade
						has_variable = zg361_b1_m357_external_final_grade
						has_variable = zg361_b1_m357_external_forced_down
					...
					if = {
						...
						root = { set_variable = { name = zg361_b1_m360_work_abs_c value = 1 } }
					}
					if = {
						limit = {
							var:zg361_b1_case_owner = root
							...
							var:zg361_b1_agenda_item_owner = root
							...
							OR = {
								var:zg361_b1_agenda_item_state = 2
								var:zg361_b1_agenda_item_state = 3
							}
							var:zg361_b1_m137_receipt_serial = var:zg361_b1_case_serial
							var:zg361_b1_m357_external_receipt_owner = root
							var:zg361_b1_m357_external_receipt_subject = this
							var:zg361_b1_m357_external_receipt_cycle = var:zg361_b1_cycle_serial
							var:zg361_b1_m357_external_receipt_case = var:zg361_b1_case_serial
							var:zg361_b1_m357_external_receipt_state = 8
							var:zg361_b1_m357_external_result_case = var:zg361_b1_result_adapter_result_case
							var:zg361_b1_m357_external_absolute_grade = var:zg361_b1_absolute_grade
							var:zg361_b1_m357_external_final_grade = var:zg361_b1_final_grade
							var:zg361_b1_m357_external_forced_down = var:zg361_b1_forced_down
							var:zg361_b1_m357_external_receipt_id > 0
							var:zg361_b1_m357_external_receipt_hash > 0
							var:zg361_b1_result_adapter_result_owner = root
							var:zg361_b1_result_adapter_result_subject = this
							var:zg361_b1_result_adapter_result_cycle = var:zg361_b1_cycle_serial
							var:zg361_b1_result_adapter_b1_owner = root
						...
						root = {
							change_variable = { name = zg361_b1_m360_work_member_count add = 1 }
							change_variable = {
								name = zg361_b1_m360_work_member_hash
						...
						if = {
							...
							root = {
								change_variable = { name = zg361_b1_m360_work_forced_count add = 1 }
								change_variable = { name = zg361_b1_m360_work_m357_hash_sum add = prev.var:zg361_b1_m357_external_receipt_hash }
							}
						}
					}
					else = {
						root = {
							set_variable = { name = zg361_b1_m360_work_valid value = 0 }
							set_variable = { name = zg361_b1_m360_work_tuple_mismatch value = 1 }
						}
					}
				}
				else = {
					root = {
						set_variable = { name = zg361_b1_m360_work_valid value = 0 }
						set_variable = { name = zg361_b1_m360_work_missing_fields value = 1 }
					}
				}
			}
			if = {
				limit = {
					var:zg361_b1_m360_work_valid = 1
					var:zg361_b1_m360_work_member_count = var:zg361_b1_processing_n
					var:zg361_b1_m360_work_member_hash = var:zg361_b1_agenda_hash
				...
				set_variable = { name = zg361_b1_m360_work_slot value = 0 }
				ordered_in_list = {
					variable = zg361_b1_processing_subjects
...
__M360_FORCED_SLOT_WRITES__
				...
				remove_variable = zg361_b1_m360_work_slot
				set_variable = { name = zg361_b1_m360_source_status value = 1 }
				remove_variable = zg361_b1_m360_source_reason
				set_variable = { name = zg361_b1_m360_source_sealed value = 1 }
				set_variable = { name = zg361_b1_m360_source_available value = 1 }
			}
			else_if = {
				limit = {
					var:zg361_b1_m360_work_valid = 1
					...
					var:zg361_b1_m360_work_member_hash = var:zg361_b1_agenda_hash
					var:zg361_b1_m360_work_member_hash = var:zg361_b1_agenda_new_hash
					var:zg361_b1_m360_work_abs_c = 1
				}
				set_variable = { name = zg361_b1_m360_source_status value = 2 }
				set_variable = { name = zg361_b1_m360_source_reason value = 3 }
			}
			else_if = {
				limit = {
					var:zg361_b1_m360_work_valid = 1
					...
					var:zg361_b1_m360_work_member_hash = var:zg361_b1_agenda_new_hash
					var:zg361_b1_m360_work_abs_c = 0
					NOT = { var:zg361_b1_m360_work_forced_count = var:zg361_pending_325_n }
				}
				set_variable = { name = zg361_b1_m360_source_status value = 3 }
				set_variable = { name = zg361_b1_m360_source_reason value = 105 }
			}
			else_if = {
				limit = { var:zg361_b1_m360_work_valid = 1 }
				set_variable = { name = zg361_b1_m360_source_status value = 3 }
				set_variable = { name = zg361_b1_m360_source_reason value = 104 }
			}
		}
		else = { set_variable = { name = zg361_b1_m360_work_valid value = 0 } }
	}
	else = { set_variable = { name = zg361_b1_m360_work_valid value = 0 } }
	if = {
		limit = { has_variable = zg361_b1_m360_source_status }
		if = {
			limit = { var:zg361_b1_m360_source_status = 3 var:zg361_b1_m360_work_missing_fields = 1 }
			set_variable = { name = zg361_b1_m360_source_reason value = 102 }
		}
		else_if = {
			limit = { var:zg361_b1_m360_source_status = 3 var:zg361_b1_m360_work_tuple_mismatch = 1 }
			set_variable = { name = zg361_b1_m360_source_reason value = 103 }
		}
	}
	remove_variable = zg361_b1_m360_work_valid
	remove_variable = zg361_b1_m360_work_member_count
	remove_variable = zg361_b1_m360_work_member_hash
	remove_variable = zg361_b1_m360_work_forced_count
	remove_variable = zg361_b1_m360_work_agenda_closed_n
	remove_variable = zg361_b1_m360_work_abs_c
	remove_variable = zg361_b1_m360_work_missing_fields
	remove_variable = zg361_b1_m360_work_tuple_mismatch
	remove_variable = zg361_b1_m360_work_m357_hash_sum
}'''.replace("__M360_SOURCE_CLEANUP__", render_m360_source_cleanup()).replace(
        "__M360_FORCED_SLOT_WRITES__", render_m360_forced_slot_writes()
    )
...
# or owns a manager/subject callback represented by pending_open_n or
# oversight_return_status=1. Status 2 means the one-day skip-level callback has
# already returned: that callback itself restores state 7 and clears the
# publication block. R418 retained a production lineage after every one of its
# 132 weak Character rows had disappeared: both manager lists were empty, no
# callback remained, and the annual request kept polling the active serial
...
zg361_b1_initialize_subject_case_effect = {
	...
	if = {
		limit = {
			trigger_if = {
				limit = {
					has_variable = zg361_b1_band_order_object_available
					has_variable = zg361_b1_band_order_object_id
					has_variable = zg361_b1_band_order_object_owner
					has_variable = zg361_b1_band_order_object_subject
					has_variable = zg361_b1_band_order_object_cycle
					has_variable = zg361_b1_band_order_object_case
					has_variable = zg361_b1_band_order_object_state
				}
...
# R62 proved that `exists = this` remains true for these stale weak scopes.
# R492 also proved that a live Character can remain in another manager's
# persisted list after its one case namespace has moved to a different exact
# owner/cycle/case. Rebuild both manager-owned lists through the engine's
# Character-safe `is_alive = yes` filter and the exact active case tuple before
# the first delayed consumer reads any case field. Every removed entry is a
# real vacancy from this manager's frozen domain and must leave the same
# reconstructible counters as a death/C departure that the normal roster
# audit can still inspect.
zg361_b1_prune_unavailable_subjects_effect = {
	# A scripted effect preserves the caller chain's ROOT even when it is
	# invoked inside `var:<manager> = { ... }`.  Freeze THIS explicitly so list
...
zg361_b1_resolve_pending_subject_effect = {
	...
	if = {
		limit = {
			...
			# The case owner is stored as a weak Character reference.  A pending
			# deadline can fire after that manager has become unavailable; never
			# enter manager-owned counters/publication from such a reference.
			var:zg361_b1_case_owner = { is_alive = yes }
			var:zg361_b1_case_state = 7
			var:zg361_b1_case_active = 1
			var:zg361_b1_recusal_active = 0
		}
	...
	else_if = {
		...
		# The manager-owned case can no longer settle or publish.  Cancel only
		# this live subject-local object and release its still-live reserved peer;
		# do not invent manager counters on an unavailable Character scope.
		if = {
		...
		set_variable = { name = zg361_b1_pending_cancel_reason value = 2 }
		set_variable = { name = zg361_b1_pending_observation_recorded value = 0 }
		debug_log = "ZG361B1: pending object cancelled because manager is unavailable"
	}
}
...
# A final delayed reopen callback can outlive one weak Character row after the
# quota book and agenda were sealed.  The callback barrier has already consumed
# every ticket, so compact only the surviving state-7 domain: preserve every
# surviving grade, rebuild the agenda audit from that same processing list, and
# reseal the board/reward receipts before settlement.  This is not a new quota
# rebuild and therefore does not advance rebuild_generation.
zg361_b1_compact_final_survivor_domain_effect = {
	set_variable = { name = zg361_b1_final_survivor_compaction_valid value = 0 }
	zg361_b1_verify_frozen_quota_conservation_effect = yes
	if = {
		limit = {
			var:zg361_b1_roster_pruned_n >= 1
		...
		# The actual surviving grades are authoritative.  Do not rerank because a
		# death after seal is not performance evidence against another subject.
		set_variable = { name = zg361_pending_375_n value = var:zg361_b1_quota_recount_top }
		set_variable = { name = zg361_pending_35_n value = var:zg361_b1_quota_recount_middle }
		set_variable = { name = zg361_pending_325_n value = var:zg361_b1_quota_recount_bottom }
		change_variable = { name = zg361_b1_quota_book_version add = 1 }
		set_variable = { name = zg361_b1_quota_built_serial value = var:zg361_b1_manager_case_serial }
		set_variable = { name = zg361_b1_quota_roster_amendment_version value = var:zg361_b1_roster_audit_version }
		set_variable = { name = zg361_b1_quota_rebuilt_for_roster value = 1 }
		set_variable = { name = zg361_b1_roster_reopen_required value = 0 }
		set_variable = { name = zg361_b1_final_survivor_compaction_receipt_serial value = var:zg361_b1_manager_case_serial }

		# Rebuild every agenda-derived audit from the same pruned processing list.
		# Keep surviving processing_order values stable so downstream #360 hashes
		# the exact same identity domain.
		if = { limit = { has_variable_list = zg361_b1_agenda_subjects } clear_variable_list = zg361_b1_agenda_subjects }
		set_variable = { name = zg361_b1_final_survivor_n value = 0 }
		set_variable = { name = zg361_b1_agenda_n value = 0 }
		set_variable = { name = zg361_b1_agenda_reviewed_n value = 0 }
		set_variable = { name = zg361_b1_agenda_changed_n value = 0 }
		set_variable = { name = zg361_b1_agenda_review_minutes value = 0 }
		set_variable = { name = zg361_b1_agenda_skipped_n value = 0 }
		set_variable = { name = zg361_b1_agenda_hash value = { value = var:zg361_b1_manager_case_serial multiply = 1000 } }
		save_temporary_scope_as = zg361_b1_final_survivor_manager
		every_in_list = {
			variable = zg361_b1_processing_subjects
			if = {
				limit = {
					var:zg361_b1_case_owner = scope:zg361_b1_final_survivor_manager
					var:zg361_b1_case_subject = this
					var:zg361_b1_cycle_serial = scope:zg361_b1_final_survivor_manager.var:zg361_b1_manager_cycle_serial
					var:zg361_b1_case_serial = scope:zg361_b1_final_survivor_manager.var:zg361_b1_manager_case_serial
					var:zg361_b1_case_state = 7
					var:zg361_b1_case_active = 1
					var:zg361_b1_roster_included = 1
					has_variable = zg361_pending_grade
				...
				save_temporary_scope_as = zg361_b1_final_survivor_subject
				scope:zg361_b1_final_survivor_manager = {
					change_variable = { name = zg361_b1_final_survivor_n add = 1 }
					change_variable = {
						name = zg361_b1_agenda_hash
						add = { value = scope:zg361_b1_final_survivor_subject.var:zg361_b1_roster_frozen_order multiply = scope:zg361_b1_final_survivor_subject.var:zg361_b1_processing_order }
					}
				}
				if = {
					limit = { scope:zg361_b1_final_survivor_manager.var:zg361_b1_agenda_mode != 3 }
					scope:zg361_b1_final_survivor_manager = {
						add_to_variable_list = { name = zg361_b1_agenda_subjects target = scope:zg361_b1_final_survivor_subject }
						change_variable = { name = zg361_b1_agenda_n add = 1 }
					}
					if = {
						limit = { var:zg361_b1_agenda_item_state = 2 }
						scope:zg361_b1_final_survivor_manager = {
							change_variable = { name = zg361_b1_agenda_reviewed_n add = 1 }
							change_variable = { name = zg361_b1_agenda_review_minutes add = prev.var:zg361_b1_agenda_review_duration_minutes }
							if = { limit = { prev.var:zg361_b1_agenda_grade_changed = 1 } change_variable = { name = zg361_b1_agenda_changed_n add = 1 } }
						}
					}
					else_if = {
						limit = { var:zg361_b1_agenda_item_state = 3 }
						scope:zg361_b1_final_survivor_manager = { change_variable = { name = zg361_b1_agenda_skipped_n add = 1 } }
					}
				}
			}
		}
		...
		# Reseal both immutable receipts against the compacted domain.
		set_variable = {
			name = zg361_b1_sealed_board_hash
			value = { value = var:zg361_b1_manager_case_serial multiply = 100000 add = var:zg361_b1_quota_book_version }
		}
		...
		every_in_list = {
			...
			set_variable = { name = zg361_b1_reopen_sealed_grade value = var:zg361_pending_grade }
			save_temporary_scope_as = zg361_b1_final_survivor_hash_subject
			root = {
				change_variable = {
					name = zg361_b1_sealed_board_checksum
					add = { value = scope:zg361_b1_final_survivor_hash_subject.var:zg361_b1_agenda_order multiply = scope:zg361_b1_final_survivor_hash_subject.var:zg361_pending_grade }
				}
			}
		}
		change_variable = { name = zg361_b1_pending_reward_book_version add = 1 }
		...
		every_in_list = {
			...
			save_temporary_scope_as = zg361_b1_final_survivor_reward_subject
			root = { change_variable = { name = zg361_b1_reward_snapshot_checksum add = scope:zg361_b1_final_survivor_reward_subject.var:zg361_b1_pending_reward_due } }
			if = {
				limit = {
					var:zg361_b1_pending_state = 2
					var:zg361_b1_pending_reward_due = 25
		...
		if = {
			limit = {
				var:zg361_b1_final_survivor_n = var:zg361_b1_subject_n
				var:zg361_b1_final_survivor_n = var:zg361_b1_processing_n
				var:zg361_b1_quota_conservation_valid = 1
				trigger_if = {
					limit = { var:zg361_b1_agenda_mode != 3 }
			...
			set_variable = { name = zg361_b1_final_survivor_compaction_valid value = 1 }
			debug_log = "ZG361B1: final callback survivor domain compacted and resealed"
		}
		else = { debug_log = "ZG361B1: final callback survivor compaction failed; settlement withheld" }
	}
}
...
zg361_b1_resolve_reopen_batch_effect = {
	...
	if = {
		limit = { var:zg361_b1_roster_pruned_n >= 1 }
		zg361_b1_compact_final_survivor_domain_effect = yes
	}
	if = {
		limit = {
			var:zg361_b1_closure_state = 1
			...
			var:zg361_b1_reopen_processed_n = var:zg361_b1_reopen_batch_expected_n
			OR = {
				var:zg361_b1_roster_pruned_n = 0
				var:zg361_b1_final_survivor_compaction_valid = 1
			}
			var:zg361_b1_reopen_batch_object_available = 1
			var:zg361_b1_reopen_batch_owner = this
			var:zg361_b1_reopen_batch_subject = this
...
__RESULT_ADAPTER_PEER_SLOTS__
		...
		zg361_b1_publish_m360_cohort_source_effect = yes
		remove_character_flag = zg361_b1_cycle_active
...
zg361b1.201 = {
	...
	desc = {
		...
		first_valid = {
			triggered_desc = { trigger = { var:zg361_b1_shadow_grade = 3 } desc = zg361b1.201.grade_375 }
			triggered_desc = { trigger = { var:zg361_b1_shadow_grade = 2 } desc = zg361b1.201.grade_35 }
			triggered_desc = { trigger = { var:zg361_b1_shadow_grade = 1 } desc = zg361b1.201.grade_325 }
		}
	}
...
def render_english_localization() -> bytes:
    return localized(r'''
l_english:
 zg361b1.200.t:0 "How Will You State This Cycle's Work?"
 zg361b1.200.desc:0 "The evidence window is about to close. Your account will be sealed beside the mid-cycle record; the factual record itself will remain unchanged."
 zg361b1.200.a:0 "Submit the mid-cycle evidence unchanged."
 zg361b1.200.b:0 "Submit a self-rating raised by up to 15 points."
 zg361b1.200.c:0 "Submit a self-rating lowered by up to 15 points."
 zg361b1.201.t:0 "Provisional Assessment"
 zg361b1.201.desc:0 "Your manager has issued an early view of your standing. It differs from the sealed facts by [ROOT.Char.MakeScope.Var('zg361_b1_shadow_gap_magnitude').GetValue|0], and you have [ROOT.Char.MakeScope.Var('zg361_b1_shadow_deadline_days').GetValue|0] days to respond. No reward or final-rating place is assigned during this window."
 zg361b1.201.grade_375:0 "Current provisional rating: #P 3.75 — exceeds expectations#!."
 zg361b1.201.grade_35:0 "Current provisional rating: #V 3.5 — meets expectations#!."
 zg361b1.201.grade_325:0 "Current provisional rating: #N 3.25 — needs improvement#!."
 zg361b1.201.a:0 "Accept it with no calibration adjustment."
 zg361b1.201.b:0 "Check current results; adjust calibration by at most 10 points."
 zg361b1.126.t:0 "Your Rating Has Been Posted"
 zg361b1.126.desc:0 "Other pending cases no longer hold back your final rating; this is revision [ROOT.Char.MakeScope.Var('zg361_b1_local_publish_revision').GetValue|0] of your case. Rewards and penalties will still be applied at formal settlement, and no other official's pending case is disclosed here."
 ...
 zg361b1.126.initial:0 "This is the first settled revision of your case this cycle."
 zg361b1.126.appended:0 "The previously pending matter has been resolved, so your result has now been added to the board."
 zg361b1.126.reopened:0 "New evidence triggered another review; this revision replaces the previous result."
 zg361b1.126.a:0 "Note this rating and revision."
 zg361_scoreboard_detail_field_self_choice:0 "Self-Review Position"
 ...
 zg361_scoreboard_detail_field_shadow_grade:0 "Rating Before Mandatory Distribution"
 zg361_scoreboard_detail_field_shadow_response:0 "Response to Provisional Rating"
 zg361_scoreboard_detail_field_shadow_delta:0 "Change from Supplementary Evidence"
 zg361_scoreboard_detail_field_shadow_response_year:0 "Year of Response to Provisional Rating"
 zg361_scoreboard_detail_field_peer_n:0 "Sealed Peer Review Count"
 zg361_scoreboard_detail_field_peer_mean:0 "Peer Raw Mean"
 zg361_scoreboard_detail_field_peer_variance:0 "Peer Review Disagreement"
 zg361_scoreboard_detail_field_peer_normalized_score:0 "Credit-Weighted Peer Score"
 zg361_scoreboard_detail_field_peer_shape:0 "Peer Review Pattern"
 ...
 zg361_scoreboard_detail_field_evaluator_credit:0 "Evaluator's Past Reliability"
 zg361_scoreboard_detail_field_evaluator_sample_n:0 "Evaluator's Prior Review Count"
 zg361_scoreboard_detail_field_peer_use_mode:0 "How Peer Evidence Was Used"
 zg361_scoreboard_detail_field_peer_fatigue:0 "Peer Submission Fatigue"
 zg361_scoreboard_detail_field_calibration_score:0 "Score After Response to Provisional Rating"
 zg361_scoreboard_detail_field_calibration_score_before_shadow:0 "Score Before Response to Provisional Rating"
 zg361_scoreboard_detail_field_shadow_to_quota_delta:0 "Change from Provisional to Final Rating"
 zg361_scoreboard_detail_field_quota_snapshot:0 "Rating After Mandatory Distribution"
 zg361_scoreboard_detail_field_forced_down:0 "Forced Below Absolute Band"
 zg361_scoreboard_detail_field_roster_employment_state:0 "Employment Status at Publication"
 zg361_scoreboard_detail_field_leaver_route:0 "Departure Treatment"
 ...
 zg361_scoreboard_detail_field_leaver_quota_source:0 "Source of Departed Official's Low-Rating Slot"
 zg361_scoreboard_detail_field_leaver_effective_year:0 "Departure Effective Year"
 zg361_scoreboard_detail_field_leaver_receipt_state:0 "Departure Case Status"
 zg361_scoreboard_detail_field_b1_peer_sealed:0 "Peer Evidence Sealing Status"
 zg361_scoreboard_detail_field_b1_141_must_review_marker:0 "Superior Review Requirement"
 zg361_scoreboard_detail_field_b1_141_review_outcome:0 "Superior Review Result"
 zg361_scoreboard_detail_field_b1_142_pending_marker:0 "Pending Review Status"
 zg361_scoreboard_detail_field_b1_142_milestone:0 "Result Awaiting Evidence"
 zg361_scoreboard_detail_field_b1_142_deadline_cycle:0 "Review Deadline Cycle"
 zg361_scoreboard_detail_field_b1_142_current_final_unchanged:0 "Whether This Cycle's Final Rating Was Kept"
 zg361_scoreboard_detail_field_b1_142_next_cycle_evidence:0 "Whether New Evidence Was Moved to Next Cycle"
 zg361_scoreboard_detail_field_b1_143_reopen_result:0 "Reopened Review Result"
 zg361_scoreboard_detail_field_b1_143_reason_code:0 "Reason for Reopening"
 zg361_scoreboard_detail_field_b1_143_next_cycle_evidence:0 "Whether New Evidence Was Moved to Next Cycle"
 zg361_scoreboard_detail_field_b1_143_target_cycle:0 "Cycle in Which New Evidence Applies"
 zg361_scoreboard_detail_field_b1_144_dissent_marker:0 "Named Objection Status"
 zg361_scoreboard_detail_field_b1_144_review_outcome:0 "Named Objection Review Result"
 zg361_scoreboard_detail_field_b1_144_consensus_marker:0 "Consensus Record Status"
 zg361_scoreboard_detail_field_b1_145_formal_band:0 "Published Rating"
 zg361_scoreboard_detail_field_b1_145_within_middle_order:0 "Position Within Middle Band"
 zg361_scoreboard_detail_field_b1_145_opportunity_capacity:0 "Development Opportunity Places"
 zg361_scoreboard_detail_field_b1_145_opportunity_selected:0 "Selected for a Development Opportunity"
 zg361_scoreboard_detail_field_b1_145_coaching_selected:0 "Placed on the Coaching List"
 zg361_scoreboard_detail_field_b1_145_own_opportunity_selected:0 "Whether You Received a Development Opportunity"
 zg361_scoreboard_detail_field_b1_145_appeal_evidence_available:0 "Whether Evidence Supports an Appeal"
 zg361_scoreboard_detail_field_b1_145_blackbox_audit:0 "Whether the Result Was Adequately Explained"
 zg361_scoreboard_detail_value_unknown:0 "Status unavailable"
 zg361_scoreboard_detail_value_self_choice_pending:0 "Not yet submitted"
 zg361_scoreboard_detail_value_self_choice_honest:0 "Stated as recorded"
 zg361_scoreboard_detail_value_self_choice_emphasized:0 "Strongest account presented"
 zg361_scoreboard_detail_value_self_choice_conservative:0 "Contribution understated"
 zg361_scoreboard_detail_value_shadow_response_pending:0 "Awaiting response"
 zg361_scoreboard_detail_value_shadow_response_accepted:0 "Provisional rating accepted"
 zg361_scoreboard_detail_value_shadow_response_supplemented:0 "Supplementary evidence submitted"
 zg361_scoreboard_detail_value_peer_shape_no_evidence:0 "No usable peer evidence"
 zg361_scoreboard_detail_value_peer_shape_positive:0 "Broadly positive"
 zg361_scoreboard_detail_value_peer_shape_balanced:0 "Mixed but balanced"
 zg361_scoreboard_detail_value_peer_shape_negative:0 "Broadly negative"
 zg361_scoreboard_detail_value_peer_shape_divergent:0 "Sharply divided"
 zg361_scoreboard_detail_value_peer_reciprocity_risk_clear:0 "No reciprocity pattern found"
 zg361_scoreboard_detail_value_peer_reciprocity_risk_flagged:0 "Possible reciprocal praise"
 zg361_scoreboard_detail_value_peer_use_mode_unused:0 "Not used for calibration"
 zg361_scoreboard_detail_value_peer_use_mode_direct:0 "Used with the standard cap"
 zg361_scoreboard_detail_value_peer_use_mode_credit_weighted:0 "Weighted by evaluator reliability"
 zg361_scoreboard_detail_value_forced_down_no:0 "No"
 zg361_scoreboard_detail_value_forced_down_yes:0 "Yes"
 zg361_scoreboard_detail_value_roster_employment_state_active:0 "Still in post"
 zg361_scoreboard_detail_value_roster_employment_state_departed:0 "Departed during this review cycle"
 zg361_scoreboard_detail_value_leaver_route_evidence:0 "Rated on the frozen evidence"
 zg361_scoreboard_detail_value_leaver_route_gray_c:0 "Placed in the departed-official low band"
 zg361_scoreboard_detail_value_leaver_route_excluded:0 "Excluded from final distribution"
 zg361_scoreboard_detail_value_leaver_quota_source_natural_c:0 "Used an existing low-band slot"
 zg361_scoreboard_detail_value_leaver_quota_source_swapped_c:0 "Low-band slot reassigned from an active official"
 zg361_scoreboard_detail_value_leaver_quota_source_none_available:0 "No existing low-band slot was available"
 zg361_scoreboard_detail_value_leaver_receipt_state_departure_recorded:0 "Departure recorded; final treatment pending"
 zg361_scoreboard_detail_value_leaver_receipt_state_gray_c_posted:0 "Departed-official low rating posted"
 zg361_scoreboard_detail_value_leaver_receipt_state_closed_without_c:0 "Closed without forcing a low rating"
 zg361_scoreboard_detail_value_grade_reason_facts_match:0 "Final rating matches the factual band"
 zg361_scoreboard_detail_value_grade_reason_displaced_from_top:0 "Displaced from a top-band slot during calibration"
 zg361_scoreboard_detail_value_grade_reason_raised_to_top:0 "Raised into a top-band slot during calibration"
 zg361_scoreboard_detail_value_grade_reason_moved_to_low:0 "Moved into the low band during calibration"
 zg361_scoreboard_detail_value_grade_reason_removed_from_low:0 "Removed from a low-band slot during calibration"
 zg361_scoreboard_detail_value_grade_reason_quota_forced_down:0 "Mandatory distribution lowered the final rating"
 zg361_scoreboard_detail_value_grade_reason_newcomer_protected:0 "Newcomer protection prevented a low rating"
 zg361_scoreboard_detail_value_grade_reason_rank_raised:0 "Rank and quota raised the final rating"
 zg361_scoreboard_detail_value_grade_reason_small_cohort_neutral:0 "Small cohort returned the final rating to neutral"
 zg361_scoreboard_detail_value_grade_reason_departed_gray_c:0 "Departure policy assigned the low rating"
 zg361_scoreboard_detail_value_grade_reason_c_swapped_to_departed:0 "An active official's low-band slot was reassigned to the departed official"
 zg361_scoreboard_detail_value_delivery_method_pending:0 "Awaiting formal service"
 zg361_scoreboard_detail_value_delivery_method_acknowledged:0 "Acknowledged by the official"
 zg361_scoreboard_detail_value_delivery_method_acknowledged_with_objection:0 "Acknowledged with a recorded objection"
 zg361_scoreboard_detail_value_delivery_method_witnessed:0 "Served before witnesses after refusal"
 zg361_scoreboard_detail_value_delivery_method_automatic:0 "Published automatically"
 zg361_scoreboard_detail_value_delivery_method_departure_closure:0 "Closed automatically after departure"
 zg361_scoreboard_detail_value_appeal_open_closed:0 "Closed"
 zg361_scoreboard_detail_value_appeal_open_open:0 "Open"
 zg361_scoreboard_detail_value_appeal_outcome_undecided:0 "No ruling yet"
 zg361_scoreboard_detail_value_appeal_outcome_upheld:0 "Original rating upheld"
 zg361_scoreboard_detail_value_appeal_outcome_corrected:0 "Appeal granted; rating corrected"
 zg361_scoreboard_detail_value_salary_cut_active_inactive:0 "Not active"
 zg361_scoreboard_detail_value_salary_cut_active_active:0 "Active"
 zg361_scoreboard_detail_value_b1_peer_sealed_not_sealed:0 "Not yet sealed"
 zg361_scoreboard_detail_value_b1_peer_sealed_sealed:0 "Sealed"
 zg361_scoreboard_detail_value_b1_141_must_review_marker_not_required:0 "No superior review required"
 zg361_scoreboard_detail_value_b1_141_must_review_marker_required:0 "Superior review required"
 zg361_scoreboard_detail_value_b1_141_review_outcome_pending:0 "Awaiting final comparison"
 zg361_scoreboard_detail_value_b1_141_review_outcome_aligned:0 "Superior recommendation matched the final rating"
 zg361_scoreboard_detail_value_b1_141_review_outcome_diverged:0 "Superior recommendation differed from the final rating"
 zg361_scoreboard_detail_value_b1_142_pending_marker_none:0 "No pending review"
 zg361_scoreboard_detail_value_b1_142_pending_marker_pending:0 "Held pending a verifiable milestone"
 zg361_scoreboard_detail_value_b1_142_milestone_none:0 "No milestone pending"
 zg361_scoreboard_detail_value_b1_142_milestone_awaiting_evidence:0 "Awaiting the next verifiable result"
 zg361_scoreboard_detail_value_b1_142_current_final_unchanged_no:0 "No"
 zg361_scoreboard_detail_value_b1_142_current_final_unchanged_yes:0 "Yes"
 zg361_scoreboard_detail_value_b1_142_next_cycle_evidence_not_queued:0 "Not queued"
 zg361_scoreboard_detail_value_b1_142_next_cycle_evidence_queued:0 "Queued for the next review cycle"
 zg361_scoreboard_detail_value_b1_143_reopen_result_pending:0 "Reopen review pending"
 zg361_scoreboard_detail_value_b1_143_reopen_result_self:0 "This case was reopened"
 zg361_scoreboard_detail_value_b1_143_reopen_result_none:0 "No case met the reopen threshold"
 zg361_scoreboard_detail_value_b1_143_reopen_result_other:0 "Another case received the single reopen slot"
 zg361_scoreboard_detail_value_b1_143_reason_code_pending:0 "Awaiting review"
 zg361_scoreboard_detail_value_b1_143_reason_code_positive_evidence:0 "New positive evidence changed this case"
 zg361_scoreboard_detail_value_b1_143_reason_code_negative_evidence:0 "New adverse evidence changed this case"
 zg361_scoreboard_detail_value_b1_143_reason_code_no_qualifying_evidence:0 "No new evidence met the threshold"
 zg361_scoreboard_detail_value_b1_143_reason_code_another_case_selected:0 "A stronger change in another case took priority"
 zg361_scoreboard_detail_value_b1_143_next_cycle_evidence_not_queued:0 "Not queued"
 zg361_scoreboard_detail_value_b1_143_next_cycle_evidence_queued:0 "Queued for the next review cycle"
 zg361_scoreboard_detail_value_b1_144_dissent_marker_none:0 "No named dissent"
 zg361_scoreboard_detail_value_b1_144_dissent_marker_recorded:0 "Named dissent recorded"
 zg361_scoreboard_detail_value_b1_144_review_outcome_pending:0 "Independent review pending"
 zg361_scoreboard_detail_value_b1_144_review_outcome_accepted:0 "Dissent accepted and reflected in the result"
 zg361_scoreboard_detail_value_b1_144_review_outcome_rejected:0 "Dissent rejected after independent review"
 zg361_scoreboard_detail_value_b1_144_consensus_marker_not_sealed:0 "Not yet sealed"
 zg361_scoreboard_detail_value_b1_144_consensus_marker_sealed:0 "Consensus sealed"
 zg361_scoreboard_detail_value_b1_145_opportunity_selected_no:0 "No"
 zg361_scoreboard_detail_value_b1_145_opportunity_selected_yes:0 "Yes"
 zg361_scoreboard_detail_value_b1_145_coaching_selected_no:0 "No"
 zg361_scoreboard_detail_value_b1_145_coaching_selected_yes:0 "Yes"
 zg361_scoreboard_detail_value_b1_145_own_opportunity_selected_no:0 "No"
 zg361_scoreboard_detail_value_b1_145_own_opportunity_selected_yes:0 "Yes"
 zg361_scoreboard_detail_value_b1_145_appeal_evidence_available_no:0 "No"
 zg361_scoreboard_detail_value_b1_145_appeal_evidence_available_yes:0 "Yes"
 zg361_scoreboard_detail_value_b1_145_blackbox_audit_clear:0 "No opacity risk found"
 zg361_scoreboard_detail_value_b1_145_blackbox_audit_flagged:0 "Result lacks enough explanation"
''')
...
l_simp_chinese:
 zg361b1.200.t:0 "本期成果要怎样写入自评？"
 zg361b1.200.desc:0 "证据窗口即将关闭。你的自陈会与期中记录一同封存，但已经查明的事实不会因此改变。"
 zg361b1.200.a:0 "照实提交期中证据，不修正自评分。"
 zg361b1.200.b:0 "将自评分最多上调 15 点后提交。"
 zg361b1.200.c:0 "将自评分最多下调 15 点后提交。"
 zg361b1.201.t:0 "暂定考绩"
 zg361b1.201.desc:0 "上司已经给出对你本期表现的初步判断。这份判断与冻结事实相差 [ROOT.Char.MakeScope.Var('zg361_b1_shadow_gap_magnitude').GetValue|0]；你有 [ROOT.Char.MakeScope.Var('zg361_b1_shadow_deadline_days').GetValue|0] 日回应。窗口关闭前不会结算奖惩，也不会占用最终档位名额。"
 zg361b1.201.grade_375:0 "当前暂定档位：#P 3.75——超出预期#!。"
 zg361b1.201.grade_35:0 "当前暂定档位：#V 3.5——符合预期#!。"
 zg361b1.201.grade_325:0 "当前暂定档位：#N 3.25——待改进#!。"
 zg361b1.201.a:0 "接受暂定档位，不作校准修正。"
 zg361b1.201.b:0 "核验当前成果，校准分最多变动 10 点。"
 zg361b1.126.t:0 "你的考绩已经张榜"
 zg361b1.126.desc:0 "即使同组还有待决案卷，你的最终档位也不再延后；这是本案第 [ROOT.Char.MakeScope.Var('zg361_b1_local_publish_revision').GetValue|0] 次定稿。奖惩仍按正式结算时间入账，其他官员的待定内容不会在这里公开。"
 zg361b1.126.grade_375:0 "当前公示档位：#P 3.75#!。"
 zg361b1.126.grade_35:0 "当前公示档位：#V 3.5#!。"
 zg361b1.126.grade_325:0 "当前公示档位：#N 3.25#!。"
 zg361b1.126.initial:0 "这是本案在本期的首次定稿。"
 zg361b1.126.appended:0 "此前待决事项已经结清，你的结果现已补入榜册。"
 zg361b1.126.reopened:0 "新证据触发重新审理，本次定稿取代先前结果。"
 zg361b1.126.a:0 "记下本版档位与修订号。"
 zg361_scoreboard_detail_field_self_choice:0 "自评立场"
 ...
 zg361_scoreboard_detail_field_shadow_grade:0 "强制分布前暂定档"
 zg361_scoreboard_detail_field_shadow_response:0 "对暂定档的回应"
 zg361_scoreboard_detail_field_shadow_delta:0 "补充证据带来的调整"
 zg361_scoreboard_detail_field_shadow_response_year:0 "回应暂定评价的年份"
 zg361_scoreboard_detail_field_peer_n:0 "封存互评数"
 zg361_scoreboard_detail_field_peer_mean:0 "互评原始均值"
 zg361_scoreboard_detail_field_peer_variance:0 "互评意见分歧"
 zg361_scoreboard_detail_field_peer_normalized_score:0 "按信用加权互评分"
 zg361_scoreboard_detail_field_peer_shape:0 "互评意见分布"
 ...
 zg361_scoreboard_detail_field_evaluator_credit:0 "评价者过往可靠度"
 zg361_scoreboard_detail_field_evaluator_sample_n:0 "评价者过往评议次数"
 zg361_scoreboard_detail_field_peer_use_mode:0 "互评证据采用方式"
 zg361_scoreboard_detail_field_peer_fatigue:0 "互评提交疲劳"
 zg361_scoreboard_detail_field_calibration_score:0 "回应暂定评价后的得分"
 zg361_scoreboard_detail_field_calibration_score_before_shadow:0 "回应暂定评价前的得分"
 zg361_scoreboard_detail_field_shadow_to_quota_delta:0 "暂定档到最终档的变化"
 zg361_scoreboard_detail_field_quota_snapshot:0 "强制分布后的档位"
 zg361_scoreboard_detail_field_forced_down:0 "是否被压低至事实绝对档以下"
 zg361_scoreboard_detail_field_roster_employment_state:0 "公示时任职状态"
 zg361_scoreboard_detail_field_leaver_route:0 "离任案处理方式"
 zg361_scoreboard_detail_field_leaver_honest_grade:0 "离任者证据应得档"
 zg361_scoreboard_detail_field_leaver_final_grade:0 "离任者公示终档"
 zg361_scoreboard_detail_field_leaver_quota_source:0 "离任者末档名额来源"
 zg361_scoreboard_detail_field_leaver_effective_year:0 "离任生效年份"
 zg361_scoreboard_detail_field_leaver_receipt_state:0 "离任案结案状态"
 zg361_scoreboard_detail_field_b1_peer_sealed:0 "互评证据封存状态"
 zg361_scoreboard_detail_field_b1_141_must_review_marker:0 "上级复核要求"
 zg361_scoreboard_detail_field_b1_141_review_outcome:0 "上级复核结果"
 zg361_scoreboard_detail_field_b1_142_pending_marker:0 "待定复核状态"
 zg361_scoreboard_detail_field_b1_142_milestone:0 "待验成果"
 zg361_scoreboard_detail_field_b1_142_deadline_cycle:0 "复核截止周期"
 zg361_scoreboard_detail_field_b1_142_current_final_unchanged:0 "本轮最终档是否保持不变"
 zg361_scoreboard_detail_field_b1_142_next_cycle_evidence:0 "新证据是否排入下轮"
 zg361_scoreboard_detail_field_b1_143_reopen_result:0 "重开复核结果"
 zg361_scoreboard_detail_field_b1_143_reason_code:0 "重开复核理由"
 zg361_scoreboard_detail_field_b1_143_next_cycle_evidence:0 "新证据是否排入下轮"
 zg361_scoreboard_detail_field_b1_143_target_cycle:0 "新证据适用周期"
 zg361_scoreboard_detail_field_b1_144_dissent_marker:0 "具名异议状态"
 zg361_scoreboard_detail_field_b1_144_review_outcome:0 "具名异议复核结果"
 zg361_scoreboard_detail_field_b1_144_consensus_marker:0 "共识封存状态"
 zg361_scoreboard_detail_field_b1_145_formal_band:0 "公示档位"
 zg361_scoreboard_detail_field_b1_145_within_middle_order:0 "中档内部次序"
 zg361_scoreboard_detail_field_b1_145_opportunity_capacity:0 "发展机会名额"
 zg361_scoreboard_detail_field_b1_145_opportunity_selected:0 "是否获得发展机会"
 zg361_scoreboard_detail_field_b1_145_coaching_selected:0 "是否列入辅导名单"
 zg361_scoreboard_detail_field_b1_145_own_opportunity_selected:0 "本人是否获得发展机会"
 zg361_scoreboard_detail_field_b1_145_appeal_evidence_available:0 "是否具备申诉证据"
 zg361_scoreboard_detail_field_b1_145_blackbox_audit:0 "结果说明是否充分"
 zg361_scoreboard_detail_value_unknown:0 "状态暂不可辨"
 zg361_scoreboard_detail_value_self_choice_pending:0 "尚未提交"
 zg361_scoreboard_detail_value_self_choice_honest:0 "据实陈述"
 zg361_scoreboard_detail_value_self_choice_emphasized:0 "着重呈现最强成果"
 zg361_scoreboard_detail_value_self_choice_conservative:0 "保守陈述个人贡献"
 zg361_scoreboard_detail_value_shadow_response_pending:0 "等待回应"
 zg361_scoreboard_detail_value_shadow_response_accepted:0 "已接受暂定评价"
 zg361_scoreboard_detail_value_shadow_response_supplemented:0 "已补交新证据"
 zg361_scoreboard_detail_value_peer_shape_no_evidence:0 "没有可用互评证据"
 zg361_scoreboard_detail_value_peer_shape_positive:0 "整体偏正面"
 zg361_scoreboard_detail_value_peer_shape_balanced:0 "褒贬参半，分布均衡"
 zg361_scoreboard_detail_value_peer_shape_negative:0 "整体偏负面"
 zg361_scoreboard_detail_value_peer_shape_divergent:0 "意见明显分裂"
 zg361_scoreboard_detail_value_peer_reciprocity_risk_clear:0 "未发现互相吹捧迹象"
 zg361_scoreboard_detail_value_peer_reciprocity_risk_flagged:0 "疑有互相吹捧"
 zg361_scoreboard_detail_value_peer_use_mode_unused:0 "未纳入校准"
 zg361_scoreboard_detail_value_peer_use_mode_direct:0 "按常规上限纳入校准"
 zg361_scoreboard_detail_value_peer_use_mode_credit_weighted:0 "按评价者信用加权"
 zg361_scoreboard_detail_value_forced_down_no:0 "否"
 zg361_scoreboard_detail_value_forced_down_yes:0 "是"
 zg361_scoreboard_detail_value_roster_employment_state_active:0 "仍在任"
 zg361_scoreboard_detail_value_roster_employment_state_departed:0 "本轮考核期间离任"
 zg361_scoreboard_detail_value_leaver_route_evidence:0 "按冻结证据正常定档"
 zg361_scoreboard_detail_value_leaver_route_gray_c:0 "列入离任者末档"
 zg361_scoreboard_detail_value_leaver_route_excluded:0 "不纳入最终强制分布"
 zg361_scoreboard_detail_value_leaver_quota_source_natural_c:0 "使用原有末档名额"
 zg361_scoreboard_detail_value_leaver_quota_source_swapped_c:0 "由在任官员让出末档名额"
 zg361_scoreboard_detail_value_leaver_quota_source_none_available:0 "没有可用的原有末档名额"
 zg361_scoreboard_detail_value_leaver_receipt_state_departure_recorded:0 "离任已登记，等待最终处理"
 zg361_scoreboard_detail_value_leaver_receipt_state_gray_c_posted:0 "离任者末档已经公示"
 zg361_scoreboard_detail_value_leaver_receipt_state_closed_without_c:0 "未强制末档，案卷已结"
 zg361_scoreboard_detail_value_grade_reason_facts_match:0 "最终档与事实档一致"
 zg361_scoreboard_detail_value_grade_reason_displaced_from_top:0 "校准时被挤出最高档名额"
 zg361_scoreboard_detail_value_grade_reason_raised_to_top:0 "校准时被抬入最高档名额"
 zg361_scoreboard_detail_value_grade_reason_moved_to_low:0 "校准时被调入末档"
 zg361_scoreboard_detail_value_grade_reason_removed_from_low:0 "校准时被调出末档"
 zg361_scoreboard_detail_value_grade_reason_quota_forced_down:0 "强制分布将最终档下调"
 zg361_scoreboard_detail_value_grade_reason_newcomer_protected:0 "新人保护避免了本期末档"
 zg361_scoreboard_detail_value_grade_reason_rank_raised:0 "排名与配额将最终档上调"
 zg361_scoreboard_detail_value_grade_reason_small_cohort_neutral:0 "同组人数过少，最终档回归中档"
 zg361_scoreboard_detail_value_grade_reason_departed_gray_c:0 "离任政策将本案列入末档"
 zg361_scoreboard_detail_value_grade_reason_c_swapped_to_departed:0 "在任官员的末档名额改由离任者承担"
 zg361_scoreboard_detail_value_delivery_method_pending:0 "等待正式送达"
 zg361_scoreboard_detail_value_delivery_method_acknowledged:0 "当事人已经签收"
 zg361_scoreboard_detail_value_delivery_method_acknowledged_with_objection:0 "当事人保留异议后签收"
 zg361_scoreboard_detail_value_delivery_method_witnessed:0 "拒收后由见证人送达"
 zg361_scoreboard_detail_value_delivery_method_automatic:0 "已经自动公示"
 zg361_scoreboard_detail_value_delivery_method_departure_closure:0 "离任后自动结案"
 zg361_scoreboard_detail_value_appeal_open_closed:0 "已关闭"
 zg361_scoreboard_detail_value_appeal_open_open:0 "开放中"
 zg361_scoreboard_detail_value_appeal_outcome_undecided:0 "尚未裁决"
 zg361_scoreboard_detail_value_appeal_outcome_upheld:0 "维持原定档"
 zg361_scoreboard_detail_value_appeal_outcome_corrected:0 "申诉成立，定档已更正"
 zg361_scoreboard_detail_value_salary_cut_active_inactive:0 "未生效"
 zg361_scoreboard_detail_value_salary_cut_active_active:0 "生效中"
 zg361_scoreboard_detail_value_b1_peer_sealed_not_sealed:0 "尚未封存"
 zg361_scoreboard_detail_value_b1_peer_sealed_sealed:0 "已经封存"
 zg361_scoreboard_detail_value_b1_141_must_review_marker_not_required:0 "无需上级复核"
 zg361_scoreboard_detail_value_b1_141_must_review_marker_required:0 "必须由上级复核"
 zg361_scoreboard_detail_value_b1_141_review_outcome_pending:0 "等待与最终档核对"
 zg361_scoreboard_detail_value_b1_141_review_outcome_aligned:0 "上级建议与最终档一致"
 zg361_scoreboard_detail_value_b1_141_review_outcome_diverged:0 "上级建议与最终档不一致"
 zg361_scoreboard_detail_value_b1_142_pending_marker_none:0 "没有待定复核"
 zg361_scoreboard_detail_value_b1_142_pending_marker_pending:0 "等待下一项可核验成果"
 zg361_scoreboard_detail_value_b1_142_milestone_none:0 "没有待验里程碑"
 zg361_scoreboard_detail_value_b1_142_milestone_awaiting_evidence:0 "等待下一项可核验成果"
 zg361_scoreboard_detail_value_b1_142_current_final_unchanged_no:0 "否"
 zg361_scoreboard_detail_value_b1_142_current_final_unchanged_yes:0 "是"
 zg361_scoreboard_detail_value_b1_142_next_cycle_evidence_not_queued:0 "未排入下轮"
 zg361_scoreboard_detail_value_b1_142_next_cycle_evidence_queued:0 "已排入下一考核周期"
 zg361_scoreboard_detail_value_b1_143_reopen_result_pending:0 "等待重开复核"
 zg361_scoreboard_detail_value_b1_143_reopen_result_self:0 "本案已经重开"
 zg361_scoreboard_detail_value_b1_143_reopen_result_none:0 "没有案卷达到重开门槛"
 zg361_scoreboard_detail_value_b1_143_reopen_result_other:0 "唯一重开名额由另一案卷取得"
 zg361_scoreboard_detail_value_b1_143_reason_code_pending:0 "等待复核"
 zg361_scoreboard_detail_value_b1_143_reason_code_positive_evidence:0 "新的有利证据改变了本案"
 zg361_scoreboard_detail_value_b1_143_reason_code_negative_evidence:0 "新的不利证据改变了本案"
 zg361_scoreboard_detail_value_b1_143_reason_code_no_qualifying_evidence:0 "没有新证据达到重开门槛"
 zg361_scoreboard_detail_value_b1_143_reason_code_another_case_selected:0 "另一案卷的变化更显著，优先重开"
 zg361_scoreboard_detail_value_b1_143_next_cycle_evidence_not_queued:0 "未排入下轮"
 zg361_scoreboard_detail_value_b1_143_next_cycle_evidence_queued:0 "已排入下一考核周期"
 zg361_scoreboard_detail_value_b1_144_dissent_marker_none:0 "没有具名异议"
 zg361_scoreboard_detail_value_b1_144_dissent_marker_recorded:0 "具名异议已经登记"
 zg361_scoreboard_detail_value_b1_144_review_outcome_pending:0 "等待独立复核"
 zg361_scoreboard_detail_value_b1_144_review_outcome_accepted:0 "异议成立，结果已经调整"
 zg361_scoreboard_detail_value_b1_144_review_outcome_rejected:0 "独立复核后驳回异议"
 zg361_scoreboard_detail_value_b1_144_consensus_marker_not_sealed:0 "尚未封存"
 zg361_scoreboard_detail_value_b1_144_consensus_marker_sealed:0 "共识已经封存"
 zg361_scoreboard_detail_value_b1_145_opportunity_selected_no:0 "否"
 zg361_scoreboard_detail_value_b1_145_opportunity_selected_yes:0 "是"
 zg361_scoreboard_detail_value_b1_145_coaching_selected_no:0 "否"
 zg361_scoreboard_detail_value_b1_145_coaching_selected_yes:0 "是"
 zg361_scoreboard_detail_value_b1_145_own_opportunity_selected_no:0 "否"
 zg361_scoreboard_detail_value_b1_145_own_opportunity_selected_yes:0 "是"
 zg361_scoreboard_detail_value_b1_145_appeal_evidence_available_no:0 "否"
 zg361_scoreboard_detail_value_b1_145_appeal_evidence_available_yes:0 "是"
 zg361_scoreboard_detail_value_b1_145_blackbox_audit_clear:0 "未发现说明不足"
 zg361_scoreboard_detail_value_b1_145_blackbox_audit_flagged:0 "结果说明不足，已标记复核"
'''))
