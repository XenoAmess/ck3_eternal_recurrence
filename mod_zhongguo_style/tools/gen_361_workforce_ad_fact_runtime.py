...
LOCALIZATION_EN = {
    ...
    "offer.desc": "An appointment document has arrived. [ROOT.Var('zg361_wad_offer_source_owner').Char.GetShortUIName] submitted it without stating compensation, scope of authority, or relocation terms. Without those clauses you cannot knowingly accept it. Record the condition blocking appointment so the manager can issue a complete proposal.",
...
LOCALIZATION_CN = {
    ...
    "vote.t": "封存你自己的面试票",
    "vote.desc": "候选人 [zg361_wad_panel_subject_scope.GetShortUIName] 正在接受三席独立评议，你是当前执笔的评委。案前现有材料只能核对候选人与当前评委的实名身份，并未附上面试题目、回答、评分表或独立证明材料。你的这一票将写入三席合议记录，并直接影响候选人能否进入下一道录用环节。",
    "vote.a": "我的判断：建议推进录用",
    "vote.b": "我的判断：暂缓并继续审查",
    "vote.c": "我的判断：不建议推进",
    "offer.t": "这份录用邀约只能由你回答",
    "offer.desc": "任命文书已经送达。[ROOT.Var('zg361_wad_offer_source_owner').Char.GetShortUIName] 提交的方案没有列明报酬、职权范围或调任条件。缺少这些条款时，你无法在知情的前提下接受；只能如实标出阻止任命的原因，交由上司重新拟定完整提议。",
