...
class B1RuntimeFoundationTests(unittest.TestCase):
    ...
    def test_visible_b1_copy_keeps_decisions_on_buttons_and_facts_in_bodies(self) -> None:
        expected_zh = {
            ...
            "zg361b1.126.desc": "即使同组还有待决案卷，你的最终档位也不再延后；这是本案第 [ROOT.Char.MakeScope.Var('zg361_b1_local_publish_revision').GetValue|0] 次定稿。奖惩仍按正式结算时间入账，其他官员的待定内容不会在这里公开。",
        ...
        expected_en = {
            ...
            "zg361b1.126.desc": "Other pending cases no longer hold back your final rating; this is revision [ROOT.Char.MakeScope.Var('zg361_b1_local_publish_revision').GetValue|0] of your case. Rewards and penalties will still be applied at formal settlement, and no other official's pending case is disclosed here.",
    ...
    def test_delayed_review_prunes_unavailable_weak_subjects_before_reads(self) -> None:
        ...
        compact = top_level_block(
            self.effects, "zg361_b1_compact_final_survivor_domain_effect"
        )
        for target, recount in (
            ("zg361_pending_375_n", "zg361_b1_quota_recount_top"),
            ("zg361_pending_35_n", "zg361_b1_quota_recount_middle"),
            ("zg361_pending_325_n", "zg361_b1_quota_recount_bottom"),
        ):
            self.assertIn(f"name = {target} value = var:{recount}", compact)
        for token in (
            "clear_variable_list = zg361_b1_agenda_subjects",
            "name = zg361_b1_agenda_n value = 0",
            "name = zg361_b1_agenda_reviewed_n value = 0",
            ...
            "name = zg361_b1_final_survivor_compaction_valid value = 1",
        ):
        ...
        self.assertLess(
            ...
            reopen_resolver.index("zg361_b1_compact_final_survivor_domain_effect = yes"),
        )
        self.assertLess(
            reopen_resolver.index("zg361_b1_compact_final_survivor_domain_effect = yes"),
            reopen_resolver.index("var:zg361_b1_final_survivor_compaction_valid = 1"),
        )
