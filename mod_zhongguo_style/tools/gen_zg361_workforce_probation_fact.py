...
def localization_rows(language: str) -> list[str]:
    ...
    if english:
        ...
        desc = "Your probationary term has ended. The office is now testing the original appointment judgment against what actually happened in service."
        quality_pass = "The later review found that you met the requirements of the post."
        quality_mismatch = "Your improvement period ended below its requirements. The outcome is recorded as a role mismatch, not a normal departure."
        quality_attrition = "You left normally after the appointment. The departure payment and release of the post were both confirmed, so the outcome is recorded as ordinary attrition."
        quality_role_change = "You left the original role because the appointment changed. This is excluded from both performance failure and ordinary attrition."
        ...
        option = "Record the outcome and its follow-up review."
    else:
        title = "试用期结局回执"
        desc = "你的试任期已经结束。官署正用实际任职结果复核当初的录用判断。"
        quality_pass = "后续考核表明，你的任职表现达到了岗位要求。"
        quality_mismatch = "改进期结束时，你的表现仍未达到要求；这次结局记为岗位不匹配，不算正常离任。"
        quality_attrition = "你在任命后正常离任，离任补偿与职位释放均已确认；这次结局记为自然流失。"
        quality_role_change = "你因任职变动离开原岗位；这次结局既不算绩效失败，也不计入自然流失。"
        followup_evidence = "官署会把这份结论与当初面试采用的证据逐项对照，用来复核那次判断是否可靠。"
        followup_accountability = "官署会把这份结论归入最终任命决定者的责任记录。"
        option = "记下任职结局与后续复核归属。"
    return [
        f"l_{language}:",
