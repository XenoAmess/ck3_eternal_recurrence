...
# catalogue.
SCENE_CN = {
    242: "考核将近，[zg361_we_ab_subject.GetShortUIName] 已连续数日最后离席，成果记录却没有相应增长。[zg361_we_ab_owner.GetShortUIName] 今天必须封存本轮容量账，免得下一次考核继续把灯火通明当成功劳。",
    ...
    250: "最近一次会议坐满了人，真正提供信息和承担决定的人却没有单独记录，[zg361_we_ab_subject.GetShortUIName] 的贡献也混在出席名单里。[zg361_we_ab_owner.GetShortUIName] 必须在分功前补上证据。",
    ...
    261: "交付现场已经出现多层转包，实际执行者 [zg361_we_ac_subject.GetShortUIName] 与合同名单并不一致，责任边界也无人核清。[zg361_we_ac_owner.GetShortUIName] 必须在下一笔款项放行前封存真实执行链。",
    ...
    270: "关键岗、增长岗与普通岗留下了截然不同的误招和漏招代价，统一门槛正在同时制造空缺与错配。[zg361_we_ad_owner.GetShortUIName] 必须在继续评估 [zg361_we_ad_subject.GetShortUIName] 前冻结本轮风险偏好。",
    271: "内推名单送审时，[zg361_we_ad_subject.GetShortUIName] 与推荐人的亲属或利益关系尚未完整披露，推荐奖却已经被催付。[zg361_we_ad_owner.GetShortUIName] 必须在最终录用票与金币支出发生前处理回避。",
    272: "报价发出前，[zg361_we_ad_subject.GetShortUIName] 已通过面试，直属管理者因交付期限要求临时抬高职级，内部同岗倒挂尚未复核。[zg361_we_ad_owner.GetShortUIName] 必须同时守住预算与公平线。",
    273: "面试排期发生冲突，[zg361_we_ad_subject.GetShortUIName] 的履历已被两个团队同时锁定，最先接触者主张独占，另一方则握有更紧急的岗位。[zg361_we_ad_owner.GetShortUIName] 必须确认归属与发掘者功劳。",
    274: "答复期限将近，[zg361_we_ad_subject.GetShortUIName] 已书面接受唯一一次公平加价，原报价预留与公平上限均已封存。[zg361_we_ad_owner.GetShortUIName] 手中的任命回执仍未落定；若错过期限，本次接受将无法转为正式在岗记录。",
    275: "报价被拒后，[zg361_we_ad_subject.GetShortUIName] 离开了流程，原岗位名额仍被冻结，下一名候选尚未出现。[zg361_we_ad_owner.GetShortUIName] 必须在下一轮招聘前规定名额何时回收；一年后的检查会追究这次占用。",
    276: "回聘申请递交后，[zg361_we_ad_subject.GetShortUIName] 带来的新外部经历必须与旧档中的离职原因和冲突记录对照。[zg361_we_ad_owner.GetShortUIName] 必须在恢复正式名额前核对这段成长是否真的弥合差距。",
    277: "改进流程结束后，[zg361_we_ad_subject.GetShortUIName] 的管理者随即要求返还一个正式名额，但原工作是否仍存在尚未证明。[zg361_we_ad_owner.GetShortUIName] 必须在启动补招前核清岗位、成本和离场记录。",
    355: "新周期目标起草时，[zg361_we_al_subject.GetShortUIName] 上一轮的峰值被直接当作底线，其中不可重复的时机与临时资源尚未剔除。[zg361_we_al_owner.GetShortUIName] 必须分清能力与红利，一年后按今天的基线复核。",
    356: "封账前，[zg361_we_al_subject.GetShortUIName] 才申报一项早已完成的成果；下一轮目标因此可能建立在失真的时间线上。[zg361_we_al_owner.GetShortUIName] 必须固定真实完成日，一年后的审计会回看这笔记录。",
    360: "三组管理者已联名拒绝现有末位指标，名单、人数和各自配额已经封存，[zg361_we_al_subject.GetShortUIName] 正等待组织答复。[zg361_we_al_owner.GetShortUIName] 必须在本轮集体行动升级前回应，一年后的结算会核对今天承担的代价。",
    361: "多轮个案留下的临时规则彼此冲突，[zg361_we_al_subject.GetShortUIName] 所在组织即将进入新的长期周期。[zg361_we_al_owner.GetShortUIName] 必须在旧规则失效前签发统一文本；金币、默认政策与十年审计将从今天起算。",
}

SCENE_EN = {
    242: "With review approaching, [zg361_we_ab_subject.GetShortUIName] has been the last to leave for several nights, yet the output record has not risen with those hours. [zg361_we_ab_owner.GetShortUIName] must close the capacity ledger today before another review mistakes lit windows for achievement.",
    ...
    250: "The latest meeting filled every seat, but nobody recorded who supplied evidence or carried the decision; [zg361_we_ab_subject.GetShortUIName]'s contribution is buried in attendance. [zg361_we_ab_owner.GetShortUIName] must restore the evidence before credit is assigned.",
    ...
    261: "Several subcontracting layers now stand between the contract and the work. Actual executor [zg361_we_ac_subject.GetShortUIName] is absent from the named roster, and responsibility remains unverified. [zg361_we_ac_owner.GetShortUIName] must freeze the real executor chain before the next payment.",
    ...
    270: "Critical, growth, and ordinary posts have produced sharply different costs from bad hires and missed hires, while one threshold creates both vacancies and mismatch. [zg361_we_ad_owner.GetShortUIName] must freeze this cycle's risk preference before evaluating [zg361_we_ad_subject.GetShortUIName] further.",
    271: "As the referral list is reviewed, [zg361_we_ad_subject.GetShortUIName]'s kinship and financial ties to the referrer remain incomplete while the reward is already being demanded. [zg361_we_ad_owner.GetShortUIName] must resolve recusal before the final vote or gold payment.",
    272: "Before the offer is sent, [zg361_we_ad_subject.GetShortUIName] has passed the interview and the line manager seeks a higher grade for urgency, before internal pay inversion has been reviewed. [zg361_we_ad_owner.GetShortUIName] must settle budget and equity.",
    273: "Two teams have locked [zg361_we_ad_subject.GetShortUIName]'s record at once. The first contact claims ownership while the other team has the more urgent post. [zg361_we_ad_owner.GetShortUIName] must settle allocation and sourcing credit before interviews are scheduled.",
    274: "The response deadline is near. [zg361_we_ad_subject.GetShortUIName] has accepted the sole fair counteroffer in writing, and both the prior reserve and fairness cap are frozen. [zg361_we_ad_owner.GetShortUIName] still lacks the appointment receipt; missing the deadline would leave that acceptance without a formal occupied-post record.",
    275: "After the offer was rejected, [zg361_we_ad_subject.GetShortUIName] left the process while the original post remained frozen and no replacement candidate was ready. [zg361_we_ad_owner.GetShortUIName] must define release before recruiting resumes.",
    276: "After the return application arrived, [zg361_we_ad_subject.GetShortUIName]'s new outside experience must be compared with the still-valid departure reason and conflict record. [zg361_we_ad_owner.GetShortUIName] must verify that the growth closes those gaps before restoring formal headcount.",
    277: "After the improvement process ended, [zg361_we_ad_subject.GetShortUIName]'s manager immediately requested a replacement slot, although the underlying work has not been shown to remain. [zg361_we_ad_owner.GetShortUIName] must verify the post, cost, and exit record before backfilling.",
    355: "As the new target is drafted, [zg361_we_al_subject.GetShortUIName]'s last peak is being used as the floor before temporary resources and unrepeatable timing are removed. [zg361_we_al_owner.GetShortUIName] must separate capability from windfall; the one-year review will use today's baseline.",
    356: "Before close, [zg361_we_al_subject.GetShortUIName] has finally reported a result completed much earlier, threatening to build the next target on a false timeline. [zg361_we_al_owner.GetShortUIName] must fix the actual completion date; the one-year audit will revisit this record.",
    360: "Three manager cohorts have jointly refused the current bottom quota; their names, sizes, and allocations are sealed while [zg361_we_al_subject.GetShortUIName] awaits an answer. [zg361_we_al_owner.GetShortUIName] must respond before the collective action escalates, with a one-year settlement of today's cost.",
    361: "Temporary rules left by many cases now contradict one another as [zg361_we_al_subject.GetShortUIName]'s organization enters a new long cycle. [zg361_we_al_owner.GetShortUIName] must issue one text before the old rules expire; gold, defaults, and the ten-year audit start today.",
}
...
BUTTON_EN = {
    ...
    245: ("Record five extra days for settlement", "Reject claims and keep the deadline"),
    ...
    248: ("Record understaffing; set lighter liability", "Keep the target and double manager liability"),
    249: ("Book six meeting-hours", "Book twelve meeting-hours"),
    250: ("Credit one evidenced contributor", "Credit all three attendees"),
    ...
    253: ("Approve appeal repair without misconduct", "Reject appeal repair and record misconduct"),
    ...
    258: ("Lower the target and freeze controllable scope", "Keep the target and record governance risk"),
    ...
    261: ("Record a three-tier acyclic chain", "Record a five-tier risk chain"),
    262: ("Record both sides separately", "Let the host rate alone"),
    263: ("Guarantee a return choice", "Extend the loan for one more cycle"),
    ...
    268: ("Calibrate within a bound and require training", "Use a wider bound without training"),
...
DETAIL_OVERRIDE_CN = {
    ...
    (245, 1): "补录 5 日额外工时并设为待结算；下一道裁定再选择支付 15 金币或记入 5 日调休",
    (245, 2): "仍记录 5 日未结工时，但不予批准并标为影子加班；这项裁定不支付补偿",
    (246, 1): "立即支付 15 金币，结清本案登记的 5 日额外工时",
    (247, 2): "动员持续约一年，不设中途复盘或续批；一年后按本案记录复核",
    (248, 1): "记录已连续缺编 3 个周期，经理评价扣 5 分；这项裁定不新增编制，也不调整目标",
    (248, 2): "记录已连续缺编 3 个周期，经理评价扣 10 分；这项裁定不新增编制，也不调整目标",
    (249, 1): "从可用容量中扣除 6 小时并记为会议工时，同时冻结议程与决策责任人",
    (249, 2): "从可用容量中扣除 12 小时并记为会议工时，同时冻结议程与决策责任人",
    (250, 1): "登记 3 名与会者，仅 1 人凭证据记为贡献者",
    (250, 2): "登记 3 名与会者，3 人均记为贡献者且不要求贡献证据",
    (253, 1): "确认最低职责与申诉修复记录，不记失职；这项裁定不直接改写绩效档位",
    (253, 2): "拒绝申诉修复并记作失职；这项裁定不直接改写绩效档位",
    ...
    (256, 2): "把外部团队并入正式受评组，并挤出 1 名正式受评者；这项裁定不直接改写任何绩效档位",
    (257, 1): "预留 1 个正式名额，按公开统一门槛登记该候选，下一周期再完成转正",
    (257, 2): "预留 1 个正式名额，按经理提名登记该候选，下一周期再完成转正",
    (258, 1): "把目标下调 20 点并冻结可控范围，不改写正式绩效档位，也不另行记录治理风险",
    (258, 2): "目标不作调整并记录治理风险；这项裁定不补权限，也不直接改写正式绩效档位",
    (259, 2): "把本案责任全部记入供应商合同账，不处分甲方一线人员",
    (260, 1): "沿用已开立的补位合同类型，并立即冻结合同类型、责任归属与变更规则",
    (260, 2): "沿用已开立的补位合同类型，但不冻结责任归属与变更规则，后续变化仍无书面边界",
    (261, 1): "冻结真实执行者，执行链深度记为 3 并标为无环；这项裁定不记录差价或甲方批准",
    (261, 2): "冻结真实执行者，执行链深度记为 5 且未证明无环；这项裁定不发生扩容",
    (262, 2): "由接收方单独定档并认领全部成果，借出方不记功；成本只记一次",
    (263, 2): "保留原身份并把借调延长到下一周期；到期后自动转为返岗选择，不会无限续延",
    (265, 1): "依据 2 项既有证据逐层追责，并从供应商追回 5 金币；不冻结整条管理链",
    (265, 2): "不采信现有证据，先冻结管理链并使经理评价扣 5 分；这项裁定不追回金币",
    (266, 1): "预留 1 个正式名额，门槛维持 70，招聘紧急度记为 2；这项裁定不会立即录用",
    (266, 2): "预留 1 个正式名额，门槛降至 60，招聘紧急度记为 4；这项裁定不会立即录用",
    (267, 2): "由最高职级先定调，再让其余面试官形成共识；同时支付此前预留的 5 金币内推奖",
    (268, 1): "保留原始面试票，校准调整上限记为 20，并安排面试官训练",
    (268, 2): "保留原始面试票，校准调整上限放宽至 100，不安排训练，也不暂停任何人资格",
    (269, 1): "数轮后核对胜任、错配与流失证据，把结果追记到各证据维度对应的面试官名下",
    (271, 2): "预留 5 金币内推奖并允许推荐人参与录用；只有随后由推荐人参与定调，封票时才支付",
    (272, 1): "预留 10 金币，请求职级 5 并要求跨团队复核；临时溢价在下一周期结束",
    (272, 2): "预留 10 金币，请求职级 6 且不作跨团队复核；临时溢价在下一周期结束",
...
DETAIL_OVERRIDE_EN = {
    ...
    (245, 1): "Record five extra days as pending; the next ruling separately chooses a 15-gold payment or five days of leave",
    (245, 2): "Still record five unsettled days, but leave them unapproved and mark shadow overtime; this card pays no compensation",
    ...
    (248, 1): "Record three understaffed cycles and reduce the manager score by 5; this card neither adds headcount nor changes the target",
    (248, 2): "Record three understaffed cycles and reduce the manager score by 10; this card neither adds headcount nor changes the target",
    (249, 1): "Remove six hours from available capacity and book them as meeting time, while freezing the agenda and decision owner",
    (249, 2): "Remove twelve hours from available capacity and book them as meeting time, while freezing the agenda and decision owner",
    (250, 1): "Record three attendees but credit only one evidenced contributor",
    (250, 2): "Record three attendees and credit all three without requiring contribution evidence",
    (253, 1): "Record minimum-duty and appeal repair without misconduct; this card does not directly change a performance grade",
    (253, 2): "Reject appeal repair and record misconduct; this card does not directly change a performance grade",
    ...
    (256, 2): "Place the external team in the formal cohort and displace one formal member; this card does not directly write any performance grade",
    ...
    (258, 1): "Reduce the target by 20 and freeze controllable scope, without changing a formal grade or recording governance risk",
    (258, 2): "Leave the target unchanged and record governance risk; this card neither restores access nor directly changes a formal grade",
    ...
    (261, 1): "Freeze the actual executor, record chain depth 3, and mark it acyclic; this card records neither margins nor client approval",
    (261, 2): "Freeze the actual executor and record chain depth 5 without an acyclic finding; this card adds no capacity",
    (262, 2): "Let the host alone rate and claim the full result; the home team receives no credit and cost is booked once",
    (263, 2): "Preserve the prior identity and extend the loan to the next cycle; expiry automatically resolves to return instead of renewing indefinitely",
    (265, 1): "Use two existing evidence items to attribute liability by layer and recover 5 gold from the supplier; do not freeze the whole chain",
    (265, 2): "Record no accepted evidence, freeze the management chain, and reduce the manager score by 5; recover no gold on this card",
    (266, 1): "Reserve one formal slot, keep the threshold at 70, and record urgency 2; this card does not hire immediately",
    (266, 2): "Reserve one formal slot, lower the threshold to 60, and record urgency 4; this card does not hire immediately",
    ...
    (268, 1): "Preserve the raw votes, cap calibration adjustment at 20, and require interviewer training",
    (268, 2): "Preserve the raw votes, widen the adjustment bound to 100, require no training, and suspend nobody",
    ...
    (272, 1): "Reserve 10 gold, request level 5, and require cross-team review; the temporary premium ends next cycle",
    (272, 2): "Reserve 10 gold, request level 6 without cross-team review; the temporary premium ends next cycle",
...
def render_localization(language: str) -> bytes:
    ...
    for spec in sorted(MECHANISMS, key=lambda item: item.mid):
        ...
        visible_routes = (
            zip("a", routes[:1], route_details[:1])
            if spec.mid == 274
            else zip("abc", routes, route_details)
        )
        rows += [
            ...
            *(
                row
                for letter, label, detail in visible_routes
                for row in (
    ...
    handoff_cn = {
        ...
        2: (
            "交接第二关：耳闻不如随行",
            f"文书交付已经过去三十日。你是 [{PREFIX}_m264_handoff_subject_scope.GetShortUIName]，接任者还没有随你处置过一次真实事务；今日必须确认随行学习是否确实发生。",
            f"文书交付已经过去三十日。当事人 [{PREFIX}_m264_handoff_subject_scope.GetShortUIName] 未能亲自答复，接任者是否真正随行，只能由你 [{PREFIX}_m264_handoff_owner_scope.GetShortUIName] 依亲见事实裁定。",
            "带接任者完成一次随行学习",
            "不再安排随行学习",
            "我亲见随行完成，代为确认",
            "我未见随行完成，记录未成",
    ...
    handoff_en = {
        ...
        2: (
            ...
            f"Thirty days have passed since the record was delivered. You are [{PREFIX}_m264_handoff_subject_scope.GetShortUIName], yet the successor has not accompanied you through one real matter; whether that learning occurred must be settled today.",
            f"Thirty days have passed since the record was delivered. [{PREFIX}_m264_handoff_subject_scope.GetShortUIName] cannot answer personally, so only you, [{PREFIX}_m264_handoff_owner_scope.GetShortUIName], can judge from direct observation whether the successor truly accompanied the work.",
