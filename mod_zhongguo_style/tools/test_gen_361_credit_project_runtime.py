...
class RegistryAndGenerationTests(unittest.TestCase):
    ...
    def test_variable_reads_are_existence_gated(self) -> None:
        for path in (EFFECTS_PATH, EVENTS_PATH):
            with self.subTest(path=path):
                self.assertNotRegex(read(path), r"limit = \{ has_variable = [^}\n]+ var:")
...
class ReceiptAndConsumerTests(unittest.TestCase):
    ...
    def test_preflight_precedes_receipt_business_write_and_consumer(self) -> None:
        for spec in gen.MECHANISMS:
            for letter in "abc":
                route = block(self.effects, f"zg361_cp_m{spec.mid}_route_{letter}_effect")
                with self.subTest(mid=spec.mid, route=letter):
...
class RoleAndLedgerInvariantTests(unittest.TestCase):
    ...
    def test_project_slot_and_capacity_have_real_reserve_spend_release(self) -> None:
        ...
        for mid in (66, 132):
            combined = "".join(block(self.effects, f"zg361_cp_m{mid}_route_{letter}_effect") for letter in "abc")
    ...
    def test_signature_route_and_attention_are_three_different_ledgers(self) -> None:
        signed = "".join(block(self.effects, f"zg361_cp_m57_route_{letter}_effect") for letter in "abc")
        routed = "".join(block(self.effects, f"zg361_cp_m58_route_{letter}_effect") for letter in "abc")
        read = "".join(block(self.effects, f"zg361_cp_m55_route_{letter}_effect") for letter in "abc")
    ...
    def test_report_build_consumes_the_frozen_policy(self) -> None:
        ...
        for choice, letter in enumerate("abc", 1):
            ...
            self.assertIn("has_variable = zg361_cp_report_policy", event)
    ...
    def test_pip_dump_stop_loss_and_postmortem_have_real_consumers(self) -> None:
        transfer = block(self.effects, "zg361_cp_m130_route_c_effect")
        self.assertIn("transfer_pip_disclosed value = 0", transfer)
        self.assertIn("transfer_trial_success value = 0", transfer)
        self.assertIn("transfer_source_accountability value = 1", transfer)
        ...
        weak_stop = block(self.effects, "zg361_cp_m132_route_c_effect")
        self.assertIn("stop_judgement value = 3", weak_stop)
    ...
    def test_b2_is_read_only_optional_pip_input(self) -> None:
        history = "".join(block(self.effects, f"zg361_cp_m68_route_{letter}_effect") for letter in "abc")
