...
class CareerLearningCopyTests(unittest.TestCase):
    ...
    def test_subject_buttons_describe_only_the_players_response(self) -> None:
        chinese = entries(chinese=True)
        self.assertEqual(chinese["zg361_cl_m314_route_a"], "接受调任，按二十金安置方案赴任")
        ...
        self.assertEqual(chinese["zg361_cl_m321_route_b"], "退回名帖，终止往来但保留旧案")
    ...
    def test_digest_reports_in_world_matters_not_pipeline_mechanics(self) -> None:
        ...
        self.assertIn("六项会交由当事人亲自答复", chinese["zg361_cl_digest_desc"])
        self.assertIn("另有十六项不另开窗口", chinese["zg361_cl_digest_desc"])
        self.assertIn("十五项固定采用各案第一项合规处置", chinese["zg361_cl_digest_desc"])
        self.assertIn("保护工时一案另看实情", chinese["zg361_cl_digest_desc"])
        self.assertIn("无战事便不虚构危机", chinese["zg361_cl_digest_desc"])
