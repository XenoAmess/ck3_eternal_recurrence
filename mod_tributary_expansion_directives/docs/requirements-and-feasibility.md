...
AI 拒绝时，已经用于发号施令的 150 威望不退，但不会扣战争金；这代表一次公开要求遭到回绝。若等待回复期间朝贡关系、目标或宣战合法性发生变化而令交互失效，则退还威望、不扣战争金，也不保留冷却，避免让玩家为外部状态变化受罚。
...
- 同一朝贡国的 5 年冷却在有效发令时开始；朝贡国拒绝也不能绕过冷却，交互因外部状态变化失效时则取消冷却。
