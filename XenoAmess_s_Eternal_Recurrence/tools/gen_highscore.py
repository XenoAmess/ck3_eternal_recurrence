...
def gen_gui() -> str:
    lines = [
        ...
        '\t\ton_finish = "[GetVariableSystem.Clear(\'xar_ironman_terminal\')]"',
        "\t}",
        "",
        "\t# After a non-Ironman player confirms the result event, switch to",
        "\t# observer mode. Ironman uses the registered native-menu terminal.",
        "\tstate = {",
        '\t\tname = "xar_exit_to_menu"',
        '\t\ttrigger_when = "[And( EqualTo_string( GetPlayer.Custom(\'xar_quit_check\'), Localize(\'xar_quit_sentinel\') ), Not( IsIronmanEnabled ) )]"',
        '\t\ton_start = "[ExecuteConsoleCommand(\'observe\')]"',
        "\t}",
        "",
        "\t# The GetPlayer-gated parent resolves the persistent terminal signal",
        "\t# before exposing a player-independent interface flag to the modal.",
        "\tstate = {",
        '\t\tname = "xar_prepare_ironman_terminal"',
