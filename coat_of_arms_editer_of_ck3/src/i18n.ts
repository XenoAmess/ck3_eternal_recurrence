...
const zhCN = {
  ...
  vfsReceiptUnavailable: 'VFS 收据不可用',
  vfsScopeBase: '仅基础游戏',
  vfsScopeOverlay: '已解析覆盖层',
  vfsNativeScoped: '目录模组直接 DDS 胜者实机已验',
  vfsNativeUnverified: '原生胜者未验证',
  vfsReceiptSummary: 'VFS 收据：{scope} · {sources} 个源 · {conflicts} 个冲突 · 胜者集 {hash} · {evidence}',
  ...
  resumeFit: '继续',
  cancel: '取消',
  importFitCheckpointFile: '导入拟合 checkpoint',
  exportFitCheckpointFile: '导出拟合 checkpoint',
  portableCheckpointHelp: '便携 checkpoint 绑定输入、素材包、预算、模型与搜索游标的 SHA-256，可在 IndexedDB 配额失败或站点数据丢失时人工保全和恢复。',
  ...
  storageFailureQuota: '浏览器存储空间不足；请导出便携 checkpoint，释放站点空间后重试。',
  storageFailureBlocked: '浏览器存储被另一个标签页或旧连接阻止；请关闭其他编辑器标签页后重试，或先导出便携 checkpoint。',
  storageFailureUnavailable: '当前浏览器上下文不允许使用持久存储；请导出便携 checkpoint 作为恢复副本。',
  storageFailureAborted: '浏览器存储事务已中止；内存 checkpoint 仍可继续，请重试或导出便携 checkpoint。',
  storageFailureUnknown: '浏览器恢复槽操作失败（{detail}）；内存 checkpoint 仍可继续，请先导出便携 checkpoint。',
  checkpointStorageEstimate: 'checkpoint 估算 {checkpoint} · 浏览器剩余 {remaining}（仅估算，最终以事务结果为准）',
  checkpointStorageEstimateUnavailable: 'checkpoint 估算 {checkpoint} · 浏览器未提供剩余空间估算，最终以事务结果为准',
  ...
  complete: '完整',
  pruneContract: '剪枝合同',
  pruneContractValue: '96 / 230 / 512 总损失与边缘损失零累计退化；数值容差 1e-12',
  seamGate: '高分辨率接缝门禁',
  ...
  canonicalCandidateSnapshot: '当前候选 · 正式渲染快照',
  load: '载入',
  ...
  resolvedOverlayBoundary: '当前导入包携带启动配置和胜者集收据；网页只消费已解析结果，不访问本机 CK3。收据未覆盖的 VFS 情形仍按其声明显示。',
...
const en: Record<MessageKey, string> = {
  ...
  vfsReceiptUnavailable: 'VFS receipt unavailable',
  vfsScopeBase: 'base game only',
  vfsScopeOverlay: 'resolved overlay',
  vfsNativeScoped: 'directory-mod direct-DDS winner verified natively',
  vfsNativeUnverified: 'native winner unverified',
  vfsReceiptSummary: 'VFS receipt: {scope} · {sources} sources · {conflicts} conflicts · winner set {hash} · {evidence}',
  ...
  startLocalFit: 'Start local fit', pauseFit: 'Pause', resumeFit: 'Resume', cancel: 'Cancel',
  importFitCheckpointFile: 'Import fit checkpoint', exportFitCheckpointFile: 'Export fit checkpoint',
  portableCheckpointHelp: 'Portable checkpoints SHA-256-bind the input, asset pack, budget, model, and search cursor, providing a manual recovery path when IndexedDB quota or site-data retention fails.',
  compressBlocks: 'Safely merge adjacent equal-style blocks', exactPrune: 'Zero-regression fixed-point prune', cancelPrune: 'Cancel pruning',
  runStatus: 'Run status', progressStageNote: ' (progress is for the current search stage)', checkpointReady: 'Safe checkpoint: {lane} · native-tile cursor {completed}/{total}', checkpointPending: 'waiting for IndexedDB', checkpointSaved: 'persisted', checkpointFailed: 'persistence failed',
  storageFailureQuota: 'Browser storage quota is exhausted. Export the portable checkpoint, free site storage, and retry.',
  storageFailureBlocked: 'Browser storage is blocked by another tab or an old connection. Close other editor tabs and retry, or export the portable checkpoint first.',
  storageFailureUnavailable: 'Persistent storage is unavailable in this browser context. Export the portable checkpoint as the recovery copy.',
  storageFailureAborted: 'The browser storage transaction was aborted. The in-memory checkpoint remains resumable; retry or export it.',
  storageFailureUnknown: 'The browser recovery slot failed ({detail}). The in-memory checkpoint remains resumable; export it first.',
  checkpointStorageEstimate: 'Checkpoint estimate {checkpoint} · browser space remaining {remaining} (estimate only; the transaction result is authoritative)',
  checkpointStorageEstimateUnavailable: 'Checkpoint estimate {checkpoint} · the browser did not expose remaining space; the transaction result is authoritative',
  pruneProgress: 'Prune pass {pass} · {completed}/{total} · {candidates} candidates evaluated',
  ...
  pruneContract: 'Prune contract', pruneContractValue: '96 / 230 / 512 zero cumulative total/edge loss regression; numeric tolerance 1e-12', seamGate: 'High-resolution seam gate', allPassed: '96 / 230 / 512 all passed',
  ...
  nativeResult: 'Native result', nativeEvidence: 'Native evidence', editorPolicy: 'Editor policy', boundary: 'Boundary',
  ...
  candidateBoundary: 'One fit can produce one to three distinct quality/complexity Pareto candidates; these are not duplicate previews of one composition. The card marked Current composition shares the exact canonical render shown above. Losses are compared only when input SHA, scorer, renderer, resolution, and surface mask all match.',
  ...
  dominated: 'Dominated under the same contract', nonDominated: 'Non-dominated under the same contract', metricsUnbound: 'No comparable fit metrics', currentComposition: 'Current composition', canonicalCandidateSnapshot: 'Current candidate · canonical render snapshot',
  ...
  resolvedOverlayBoundary: 'This imported pack carries a launch-configuration and winner-set receipt. The page consumes only the resolved result and never accesses local CK3; uncovered VFS cases remain explicitly scoped by the receipt.',
...
const runtimeEnglish: Record<string, string> = {
  ...
  '已取消零退化剪枝；当前编辑结果保持不变': 'Zero-regression pruning cancelled; the current edit remains unchanged',
