...
const inputPath = resolve('../docs/coat-of-arms-fit-artifacts/xenoamess-hunter-v6-edge-refined/coat_of_arms.txt')
const artifactDirectory = resolve('test-results/reference-hunter-v6-compressed')
...
test('safely compresses hunter v6 adjacent equal-style blocks', async ({ page }) => {
  ...
  const result = await page.evaluate(async ({ source }) => {
    ...
    if (errors.length) throw new Error(`hunter v6 parse errors: ${JSON.stringify(errors)}`)
  ...
  expect(result.receipt.drawnInstancesAfter).toBe(1008)
  ...
  const report = {
    ...
    artifactVersion: 'xenoamess-hunter-v6-compressed',
    ...
    input: {
      path: '../xenoamess-hunter-v6-edge-refined/coat_of_arms.txt',
