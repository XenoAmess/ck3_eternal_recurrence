...
const inputPath = resolve('../docs/coat-of-arms-fit-artifacts/xenoamess-hunter-v4-compressed/coat_of_arms.txt')
const targetPath = resolve('../docs/coat-of-arms-fit-artifacts/xenoamess-hunter-v4/target.png')
const artifactDirectory = resolve('test-results/reference-hunter-prune')
const sha256 = (bytes: Uint8Array | string) => createHash('sha256').update(bytes).digest('hex').toUpperCase()

test('reaches an exact leave-one-out fixed point for hunter v4', async ({ page }) => {
  ...
  expect(result.receipt.drawnInstancesAfter + result.receipt.removedInstances).toBe(1000)
  ...
  const report = {
    ...
    artifactVersion: 'xenoamess-hunter-v4-pruned',
    predecessor: '../xenoamess-hunter-v4-compressed/',
    ...
    input: {
      path: '../xenoamess-hunter-v4-compressed/coat_of_arms.txt',
