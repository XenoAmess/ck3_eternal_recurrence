"""Verify the sealed Council controlled-replacement candidate without CK3."""
...
EXPECTED_SCHEMA = "xar.ck3.g2_m4_council_r696_controlled_sealed_prep_v1"
...
def verify(root: Path) -> dict[str, object]:
    ...
    if operator.get("schema") != "xar.ck3.g2_m4_council_r696_controlled_operator_v1":
        raise RuntimeError("operator config schema differs")
    if (operator.get("last_completed_round"), operator.get("suggested_new_round"),
            operator.get("round_allocated")) != ("R695", "R696", False):
        raise RuntimeError("round allocation template differs")
    if (operator.get("readiness_timeout_seconds"), operator.get("operation_timeout_seconds"),
    ...
    if operator.get("pipe") != r"\\.\pipe\xar_ck3_bridge_g2_m4_council_r696_replace_" + source_commit[:8]:
    ...
    if (manifest.get("cmake_options") != required_options or
            manifest.get("private_action_admitted") is not True or
            manifest.get("private_action_budget") != 1 or
            manifest.get("public_query_registered") is not False or
            manifest.get("public_action_registered") is not False):
        raise RuntimeError("controlled CMake/action admission differs")
    cache = root / "candidate-bin" / "CMakeCache.txt"
    if sha256(cache) != manifest.get("cmake_cache_sha256"):
        raise RuntimeError("controlled CMake cache differs")
    ...
    if manifest.get("expected_scene") != {
            ...
            "candidate_count": 11}:
        raise RuntimeError("R696 scene contract differs")
