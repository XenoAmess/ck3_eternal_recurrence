#!/usr/bin/env python3
"""Compile and run the private government source adapter in both modes."""
...
def compile_and_run(
    *, root: Path, shell: Path, output_root: Path, optimized: bool
    ...
    executable = output_root / f"government_source_adapter_{mode}.exe"
    compiler = [
        "cl.exe",
        "/nologo",
        "/std:c++20",
        ...
        f"/I{native / 'include'}",
        str(native / "src/government_runtime_adapter_observer_v1.cpp"),
        str(native / "src/government_runtime_adapter_source_adapter_v1.cpp"),
        str(native / "src/government_runtime_adapter_source_adapter_v1_test.cpp"),
        f"/Fe:{executable}",
    ]
    batch = output_root / f"build-source-adapter-{mode}.cmd"
    batch.write_text(
        "@echo off\n"
    ...
    if ran.returncode != 0:
        raise RuntimeError(
            f"{mode} source adapter test failed ({ran.returncode}):\n"
            f"{ran.stdout}\n{ran.stderr}"
        )
    return {
        "mode": mode,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xar-gov-source-adapter-") as temporary:
    ...
    print(
        json.dumps(
            {
                "status": "GREEN",
                "contract": "government_runtime_adapter_source_adapter_v1",
                "results": results,
            },
            sort_keys=True,
        )
