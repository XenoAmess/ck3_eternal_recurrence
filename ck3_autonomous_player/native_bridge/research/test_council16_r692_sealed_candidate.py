...
from prepare_council16_r692_sealed_candidate import (
...
class Council16CandidateTests(unittest.TestCase):
    ...
    def test_candidate_local_tree_and_commit_are_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                "b9b1228524e5f3922500b41dfecfb23b739d7993",
            )
    ...
    def test_round_pair_advances_without_cascading(self) -> None:
        self.assertEqual(
            advance_rounds("R690 -> R691; run_r691.py; live-r691"),
            "R691 -> R692; run_r692.py; live-r692",
        )
    ...
    def test_copy_uses_sealed_rows_without_live_or_control_extras(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            kept.write_text("{}\n", encoding="utf-8")
            live = source / "live-r691/report.json"
            ...
            self.assertFalse((output / "live-r691").exists())
    ...
    def test_text_bindings_use_the_new_root_pipe_and_harness(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            source = root / "g2-m4-council14-r691-source"
            output = root / "g2-m4-council16-r692-newhash"
            output.mkdir()
            target_commit = "1" * 40
            target_pipe = r"\\.\pipe\xar_ck3_bridge_g2_m4_council16_r692_1111111"
            python_template = (
                f"COUNCIL14 R690 R691 r691 {SOURCE_HARNESS_COMMIT} {SOURCE_PIPE}\n"
            )
            for name in ("run_r691.py", "invoke_r691.py", "verify_prep.py"):
                (output / name).write_text(python_template, encoding="utf-8")
            text_template = f"{source} R690 R691 r691 {SOURCE_HARNESS_COMMIT} {SOURCE_PIPE}\n"
            for name in (
                "execute-command.txt",
    ...
    def test_isolated_import_closure_from_temporary_staging(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            shutil.copyfile(
                research_root / "council16_candidate_runtime_identity.py",
                candidate_root / "candidate_runtime_identity.py",
            )
            shutil.copyfile(
                research_root / "council16_candidate_runtime_import_probe.py",
