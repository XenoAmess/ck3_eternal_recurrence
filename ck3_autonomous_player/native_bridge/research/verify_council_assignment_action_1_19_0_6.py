#!/usr/bin/env python3
"""Read-only verifier for the frozen CK3 1.19.0.6 council assignment ABI."""
...
def verify(executable: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if manifest.get("contract") != "council_assignment_action_1_19_0_6_abi":
    ...
    for source in manifest["source_files"]:
        source_path = game_root / str(source["path_from_game_root"])
        if source_path.stat().st_size != parse_integer(source["size"]):
            raise VerificationError(f"{source['name']}: source file size mismatch")
        if file_sha256(source_path) != str(source["sha256"]).upper():
            raise VerificationError(f"{source['name']}: source file SHA-256 mismatch")
        source_file_count += 1
    ...
    return {
        ...
        "source_files_verified": source_file_count,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=Path(__file__).with_name(
            "council_assignment_action_1_19_0_6_abi.json"
        ),
    )
