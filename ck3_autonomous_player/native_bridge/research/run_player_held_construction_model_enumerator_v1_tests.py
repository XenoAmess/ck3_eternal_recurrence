"""Compile the private read-only player-held model source in normal and optimized MSVC."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xar-player-held-model-") as name:
        build = Path(name)
        for mode, flags in (("normal", ["/Od"]), ("optimized", ["/O2"])):
            output = build / f"player-held-model-{mode}.exe"
            command = [
                ...
                str(NATIVE / "src/player_held_construction_model_enumerator_v1_test.cpp"),
                f"/Fe:{output}",
            ]
            ...
            print(f"player-held-model-{mode}: GREEN_W4WX")
    return 0
