#!/usr/bin/env python3
"""Verify the exact-build private active-scheme definition resolver evidence."""
...
def verify(contract: dict[str, Any], game_root: Path) -> list[str]:
    ...
    exe = game_root / build["exe_relative_path"]
    if not exe.is_file():
        return [f"missing exact executable: {exe}"]
    if exe.stat().st_size != build["exe_size"]:
        errors.append("EXE size mismatch")
    actual_exe_sha = sha256_file(exe)
    ...
    identity = contract["definition_identity"]
    expected_rtti = identity["rtti_name_ascii_nul"].encode("ascii") + b"\0"
    observed_rtti = image.read_rva(number(identity["rtti_name_rva"]), len(expected_rtti))
    if observed_rtti != expected_rtti:
        errors.append("CCharacterInteraction RTTI mismatch")
    elif hashlib.sha256(observed_rtti).hexdigest().upper() != identity["rtti_name_sha256"]:
        errors.append("CCharacterInteraction RTTI SHA mismatch")

    for slot in contract["image_pointer_slots"]:
        observed = struct.unpack("<Q", image.read_rva(number(slot["slot_rva"]), 8))[0]
        expected = image.image_base + number(slot["target_rva"])
        if observed != expected:
            errors.append(f"image pointer mismatch at {slot['slot_rva']}")

    if contract["database_layout"].get("capacity_offset") is not None:
        errors.append("unproven database capacity offset must remain null")
    if identity.get("native_generation_field") is not None:
        errors.append("unproven native definition generation must remain null")
    lifecycle = contract["lifecycle"]
    if lifecycle.get("pointer_cache") != "forbidden":
        errors.append("pointer caching must remain forbidden")
    if "not a native or monotonic" not in lifecycle.get("generation", ""):
        errors.append("resolver-owned generation boundary is missing")
    expected_keys = {
        "sway_interaction": ("sway", "0x5783F850"),
        "start_murder_interaction": ("murder", "0xDF3F9819"),
    }
    observed_keys = {
        row["key"]: (row["scheme_type"], row["murmur3_seed0_u32"])
        for row in contract["stable_keys"]
    }
    if observed_keys != expected_keys:
        errors.append("stable-key mapping/hash contract mismatch")
    return errors
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=Path(__file__).with_name(
            "active_scheme_definition_resolver_1_19_0_6_abi.json"
        ),
    )
    ...
    if errors:
        ...
        return 1
    print("GREEN: active-scheme definition resolver exact spans and source contract match")
    return 0
