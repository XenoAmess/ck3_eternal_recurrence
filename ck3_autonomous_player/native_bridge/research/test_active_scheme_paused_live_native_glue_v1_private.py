#!/usr/bin/env python3
"""Build and run the SCHEME9 native glue fixture in normal or -O mode."""
...
def vcvars64() -> Path | None:
    ...
    for variable in ("ProgramFiles", "ProgramFiles(x86)"):
        ...
        if visual_studio.is_dir():
            candidates.extend(
                visual_studio.glob("*/*/VC/Auxiliary/Build/vcvars64.bat")
            )
    if not candidates:
...
def main() -> int:
    ...
    sources = [
        ...
        "active_scheme_paused_live_native_glue_v1_private_test.cpp",
    ]
    optimization = "/O2 /DNDEBUG" if sys.flags.optimize else "/Od"
    label = "optimized-python" if sys.flags.optimize else "normal-python"
    with tempfile.TemporaryDirectory(prefix="scheme9-native-glue-") as raw_temp:
        temporary = Path(raw_temp)
        executable = temporary / "scheme9_native_glue_test.exe"
        command = " ".join(
            [
                "cl.exe",
        ...
        if environment is not None:
            lines.append(f"call {quoted(environment)} >nul")
            lines.append("if errorlevel 1 exit /b 97")
        lines.extend(
            [
                command,
    ...
    print(f"GREEN: active-scheme native glue {label} /W4 /WX fixture")
    return 0
