"""Seal a Council R695 gate-only candidate after Council27 master and DLL freeze."""
...
def prepare(
    ...
    if git(repo, "rev-parse", "HEAD") != source_commit:
        raise RuntimeError("R695 source checkout is not the frozen Council27 commit")
    if git(repo, "diff", "--name-only", "HEAD", "--", *RUNTIME_ROOTS,
           "ck3_autonomous_player/native_bridge"):
        raise RuntimeError("R695 Python/native source differs from frozen commit")
    ...
    if sha256(source_save) != EXPECTED_SAVE_SHA256:
        raise RuntimeError("R694 source save differs from the frozen scene")
    ...
    # R694's sealed prelaunch profile supplies the same mod/DLC/game-rule combination.
    ...
    if prior_manifest.get("schema") != "xar.ck3.g2_m4_council_r694_query_sealed_prep_v1":
        raise RuntimeError("prior R694 sealed profile identity differs")
    ...
    for row in profile_rows:
        ...
        if not source.is_file() or source.stat().st_size != row["size_bytes"] or sha256(source) != row["sha256"]:
            raise RuntimeError(f"R694 sealed prelaunch profile changed: {relative}")
    ...
    if sha256(target_save) != EXPECTED_SAVE_SHA256:
        raise RuntimeError("R695 target save differs before launch")
    ...
    for name in ("run_council_r695_final_gates.py", "verify_council_r695_gate_prep.py",
                 "council_r695_gate_input_contract.template.json"):
        copy_file(runner / name, output / name)
    operator = {
        "schema": "xar.ck3.g2_m4_council_r695_gate_operator_v1",
        "python": str(python),
        "game_dir": str(game),
        "pipe": r"\\.\pipe\xar_ck3_bridge_g2_m4_council_r695_gate_" + source_commit[:8],
        "old_round": "R694",
        "new_round": "R695",
    ...
    manifest: dict[str, object] = {
        "schema": "xar.ck3.g2_m4_council_r695_gate_sealed_prep_v1",
        ...
        "profile_lineage": "R694 frozen prelaunch DLC/mod/settings; own descriptor path; dynamic live files omitted",
...
def main() -> int:
    ...
    parser.add_argument("--r694-candidate", type=Path, required=True)
    ...
    manifest = prepare(
        args.repo, args.r694_candidate, args.build_release,
