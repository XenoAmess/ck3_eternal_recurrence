#!/usr/bin/env python3
"""Build and run the private construction native-submit adapter in two modes."""
...
def _compile_and_run(
    ...
    output = build / f"domain-construction-native-submit-adapter-{mode}.exe"
    ...
    command = [
        ...
        str(research / "domain_construction_native_submit_adapter_v1_test.cpp"),
        f"/Fe:{output}",
    ]
    ...
    print(f"domain-construction-native-submit-adapter-{mode}: GREEN_W4WX")
...
def run(root: Path, build_root: Path | None = None) -> None:
    ...
    if build_root is None:
        with tempfile.TemporaryDirectory(
            prefix="xar-domain-construction-native-submit-adapter-"
        ) as temporary:
