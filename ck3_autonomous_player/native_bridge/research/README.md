...
| `game.command.research-arm/query-tactical-daily-sentinel-v1` | exact-build production-live for narrow consumers | final daily-stage detour at `0x26D3E80` calls the original once, then checks an absolute deadline and bounded route/combat fingerprints before pausing through `0x346B910`; current G1 cold continuation has speed-3 committed-route and active-battle zero-pause arms, while the full-watch terminal speed-5 primitive has separate live evidence | command names remain research-prefixed, but ordinary speed-3 and committed-route speed-3 production selectors consume the primitive; speed 4 and double-4x crush remain closed, and date fallback cannot admit crush mode |
...
| daily final-stage tactical sentinel hook | exact-build production-live for narrow consumers | exact 15-byte anchor at `0x26D3E80`, original-once trampoline, bounded post-day evaluator and native pause wrapper at `0x346B910`; current G1 route arms ran 44 days and battle arms 39 days without an intermediate pause or running rich query | ordinary/route speed 3 and terminal primitive only; speed 4 and double-4x crush still require the balanced ten-arm live matrix |
