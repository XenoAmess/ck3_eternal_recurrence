...
| active event instance IDs | anchor-only | medium-high static | unsupported in pure native mode |
| select event option by numeric index | anchor-only | high static command layout; no live probe | unsupported in pure native mode |
| current/front-most event | unsupported | active vector is not proven UI ordering | unsupported in pure native mode |
| event option enumeration/text | unsupported | no repeatable option-list layout yet | unsupported in pure native mode |
...
`native-headless` must return unsupported for the rows marked unsupported or
anchor-only. `hybrid-fallback` may explicitly choose data-Mod or visual paths;
...
without a main-thread callback. A minimized CK3 live probe remains the final
acceptance for that threading conclusion.
...
event and consumes the option index. It remains anchor-only until active-event
selection/order and an actual minimized live command are validated.

## Next live acceptance (not performed in this research pass)
