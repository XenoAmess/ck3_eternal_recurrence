#!/usr/bin/env python3
"""Compile and run the private government bridge binder in both modes."""
...
def compile_and_run(
    *, root: Path, shell: Path, output_root: Path, optimized: bool
    ...
    executable = output_root / f"government_bridge_binder_{mode}.exe"
    sources = [
    ...
    batch = output_root / f"build-bridge-binder-{mode}.cmd"
    batch.write_text(
        "@echo off\n"
    ...
    if ran.returncode != 0:
        raise RuntimeError(
            f"{mode} bridge binder test failed ({ran.returncode}):\n"
            f"{ran.stdout}\n{ran.stderr}"
        )
    return {
        "mode": mode,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="xar-gov-bridge-binder-") as temporary:
    ...
    print(
        json.dumps(
            {
                "status": "GREEN",
                "contract": "government_runtime_adapter_bridge_binder_v1",
                "results": results,
            },
            sort_keys=True,
        )
