#!/usr/bin/env python3
"""Build and run the private construction semantic action core in two modes."""
...
def _compile_and_run(
    ...
    output = build / f"domain-construction-semantic-action-core-{mode}.exe"
    ...
    command = [
        ...
        str(research / "domain_construction_semantic_action_core_v1_test.cpp"),
        f"/Fe:{output}",
    ]
    ...
    print(f"domain-construction-semantic-action-core-{mode}: GREEN_W4WX")
...
def run(root: Path, build_root: Path | None = None) -> None:
    ...
    if build_root is None:
        with tempfile.TemporaryDirectory(
            prefix="xar-domain-construction-semantic-action-core-"
        ) as temporary:
