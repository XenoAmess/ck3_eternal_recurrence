#!/usr/bin/env python3
"""Build and run the private construction cost/legality decoder in two modes."""
...
def _compile_and_run(
    ...
    output = build / f"domain-construction-candidate-cost-legality-v1-{mode}.exe"
    ...
    command = [
        ...
        "/UNDEBUG",
        *mode_flags,
        str(research / "domain_construction_candidate_cost_legality_decoder_v1.cpp"),
        str(research / "domain_construction_candidate_cost_legality_decoder_v1_test.cpp"),
        f"/Fe:{output}",
    ]
    ...
    print(f"domain-construction-candidate-cost-legality-{mode}: GREEN_W4WX")
...
def run(root: Path, build_root: Path | None = None) -> None:
    ...
    if build_root is None:
        with tempfile.TemporaryDirectory(
            prefix="xar-domain-construction-cost-legality-"
        ) as temporary:
