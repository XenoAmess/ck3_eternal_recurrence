#!/usr/bin/env python3
"""Freeze DEV7's exact native-runtime construction observer seam."""
...
PATCH_RVA = 0x18D2948
CALL_RVA = 0x18D294F
CONTINUE_RVA = 0x18D2958
PRODUCER_RVA = 0x1921810
PATCH_ANCHOR = bytes.fromhex("488D5560488BCEE8BCEE0400488D45D0")
...
def _direct_target(rva: int, instruction: bytes) -> int:
    _require(len(instruction) == 5 and instruction[0] == 0xE8,
...
def check(root: Path, *, ck3_executable: Path | None = None) -> None:
    ...
    _require(capture["private_build"] is True and capture["advertised"] is False and capture["read_only"] is True,
             "private read-only fixture boundary drifted")
    _require(capture["capture"]["paused"] is False,
             "fixture must prove paused is recorded rather than required")
    _require(capture["raw_pointer_fields_persisted"] is False and capture["row_bytes_reinterpreted_after_capture"] is False,
    ...
    for row in capture["capture"]["rows"]:
        ...
        _require(int.from_bytes(raw[:8], "little", signed=True) == row["score_raw"], "capture score/raw bytes drifted")
        _require(f"{_fnv1a64(raw):016X}" == row["row_bytes_fnv1a64"], "capture row FNV drifted")
    ...
    _require(hashlib.sha256(image).hexdigest().upper() == EXPECTED_SHA256, "executable SHA-256 drifted")
    sections = _sections(image)
    _require(_at(image, sections, PATCH_RVA, len(PATCH_ANCHOR)) == PATCH_ANCHOR,
             "native-runtime callsite anchor drifted")
    _require(hashlib.sha256(PATCH_ANCHOR).hexdigest().upper() == "8534255F75D4595B57E2093C6F67AC196C7FF71D596C2122258DED90151F31A6",
             "native-runtime anchor digest drifted")
    _require(_direct_target(CALL_RVA, _at(image, sections, CALL_RVA, 5)) == PRODUCER_RVA,
             "native-runtime producer direct-call target drifted")
    _require(_at(image, sections, 0x18D2954, 4) == bytes.fromhex("488D45D0"),
             "post-call replay instruction drifted")
    _require(CONTINUE_RVA == 0x18D2958, "continue boundary drifted")
    _require(_at(image, sections, PRODUCER_RVA, 5) == bytes.fromhex("48895C2408"),
...
def main() -> int:
    ...
    check(args.root.resolve(), ck3_executable=args.ck3_executable.resolve() if args.ck3_executable else None)
    print("domain-construction-runtime-callsite-observer-source-contract: GREEN_STATIC")
    return 0
