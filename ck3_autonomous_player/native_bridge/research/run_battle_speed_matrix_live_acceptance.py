...
def _validate_speed_request(
    ...
    *,
    ...
    if mode == "battle-parity" and (1 not in speeds or len(speeds) < 2):
        raise ValueError(
            "battle-parity requires speed 1 plus at least one comparison speed"
        )
...
def _run(args: argparse.Namespace) -> tuple[dict[str, object], int]:
    ...
    try:
        ...
        for sample_index, speed in enumerate(schedule, start=1):
            ...
            restore_wall_ms = 0.0
            if args.mode == "battle-parity" and sample_index > 1:
            ...
            row: dict[str, object] = {
                "sample_index": sample_index,
                "requested_speed": speed,
                "requested_days": args.target_days,
                "restore_wall_ms": round(restore_wall_ms, 3),
                "starting_date_raw": raw_slice["starting_date_raw"],
                "target_date_raw": raw_slice["target_date_raw"],
                "first_target_observed_date_raw": raw_slice[
    ...
    summary = (
        _summarize_stop_envelope(rows, speeds)
        if args.mode == "stop-envelope"
        else _summarize_battle_parity(rows, speeds)
    )
