...
struct MailboxAccessProxyV1 {
  ZhongguoCareerHcWorkforceMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoCareerHcWorkforceMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoCareerHcWorkforceMailboxContextV1 *>(
                 &query);
}
...
bool ParseZhongguoCareerHcWorkforcePostconditionV1Step(
  ...
  return step == kZhongguoCareerHcWorkforcePostconditionV1Step;
}

bool ParseZhongguoCareerHcWorkforcePostconditionRequestV1(
    std::string_view json, ZhongguoCareerHcWorkforcePostconditionRequestV1 &output,
  ...
  if (!HasExactControlFields(json) || !ParseStringField(json, "type", type) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoCareerHcWorkforcePostconditionV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoCareerHcWorkforceMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoCareerHcWorkforceMailboxContextV1 *>(
          opaque_context);
  ...
  try {
    ...
    query->read_result = ReadZhongguoCareerHcWorkforcePostconditionV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoCareerHcWorkforcePostconditionResultV1::available &&
        query->result.status ==
    ...
    if ((typed_available || typed_unavailable) &&
      ...
      query->completion =
          ZhongguoCareerHcWorkforceMailboxCompletionV1::completed;
      return true;
    }
    query->completion =
        ZhongguoCareerHcWorkforceMailboxCompletionV1::
...
std::string_view ZhongguoCareerHcWorkforceFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main career-HC/workforce executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main career-HC/workforce boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main career-HC/workforce query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main career-HC/workforce query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main career-HC/workforce executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main career-HC/workforce ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main career-HC/workforce result is inconsistent"
               : "career-HC/workforce completion snapshot changed";
  }
  if (completion ==
      ZhongguoCareerHcWorkforceMailboxCompletionV1::frame_changed)
    return "career-HC/workforce application-main frame changed";
  return "application-main career-HC/workforce executor was rejected";
}
