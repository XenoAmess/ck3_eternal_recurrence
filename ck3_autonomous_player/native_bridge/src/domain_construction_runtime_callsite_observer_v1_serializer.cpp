...
std::string SerializeDomainConstructionRuntimeObserverV1(
  ...
  output += kDomainConstructionRuntimeObserverPrivateKeyV1;
  output += "\",\"artifact_stem\":\"";
  output += kDomainConstructionRuntimeObserverArtifactStemV1;
  output += "\",\"status\":\"";
  output += captured
      ? (diagnostics.offline_fixture
             ? "fixture-captured-private-raw"
             : "production-captured-private-raw")
      : "waiting-for-live-session-application-main";
  output += "\",\"exact_build\":{\"product_version\":\"1.19.0.6\","
            "\"executable_sha256\":\"";
  output += kDomainConstructionRuntimeObserverExecutableSha256V1;
  output += "\"},\"private_build\":true,\"advertised\":false,"
            "\"read_only\":true,\"installed\":";
  ...
  output += ",\"readiness\":{\"live_session_application_main_capture\":";
  ...
  output += ",\"rejected_session\":" +
      std::to_string(capture.rejected_session_count);
  ...
  output += ",\"paused\":";
  AppendBool(output, capture.last_paused);
  output += ",\"thread_id\":" +
  ...
  output += kDomainConstructionRuntimeObserverNextReverseEngineeringEntryV1;
  output += "\"}";
  return output;
}
