...
bool ExecutePlayerEpidemicTreatmentPresenceMailboxV1(
  ...
  auto *query = static_cast<PlayerEpidemicTreatmentMailboxContextV1 *>(opaque);
  ...
  if (mailbox.state.load(std::memory_order_acquire) !=
      ...
      mailbox.executor != &ExecutePlayerEpidemicTreatmentPresenceMailboxV1 ||
  ...
  try {
    ...
    query->result = ReadPlayerEpidemicTreatmentPresenceNativeV1(
