...
struct MailboxAccessProxyV1 {
  ZhongguoWorkforceOwnerMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoWorkforceOwnerMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoWorkforceOwnerMailboxContextV1 *>(
                 &query);
}
...
bool ParseZhongguoWorkforceOwnerSnapshotV1Step(
  ...
  return step == kZhongguoWorkforceOwnerSnapshotV1Step;
}
...
bool ParseZhongguoWorkforceOwnerSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) || !ParseStringField(json, "type", type) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoWorkforceOwnerSnapshotV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoWorkforceOwnerMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoWorkforceOwnerMailboxContextV1 *>(
          opaque_context);
  ...
  try {
    ...
    query->read_result = ReadZhongguoWorkforceOwnerSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoWorkforceOwnerResultV1::available &&
        query->result.status ==
            game::ZhongguoWorkforceOwnerStatusV1::available;
    const bool typed_unavailable =
        query->read_result ==
            game::ReadZhongguoWorkforceOwnerResultV1::unavailable &&
        query->result.status ==
            game::ZhongguoWorkforceOwnerStatusV1::unavailable &&
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.player_character_id ==
            query->expected_snapshot.played_character_id) {
      query->completion =
          ZhongguoWorkforceOwnerMailboxCompletionV1::completed;
      return true;
    }
    query->completion =
        ZhongguoWorkforceOwnerMailboxCompletionV1::
...
std::string_view ZhongguoWorkforceOwnerFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main workforce owner executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main workforce owner boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main workforce owner query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main workforce owner query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main workforce owner executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main workforce owner ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main workforce owner result is inconsistent"
               : "workforce owner completion snapshot changed";
  }
  if (completion ==
      ZhongguoWorkforceOwnerMailboxCompletionV1::frame_changed)
    return "workforce owner application-main frame changed";
  return "application-main workforce owner executor was rejected";
}
