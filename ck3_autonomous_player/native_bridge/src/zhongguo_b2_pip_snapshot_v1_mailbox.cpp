...
struct MailboxAccessProxyV1 {
  ZhongguoB2PipSnapshotMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoB2PipSnapshotMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor == &ExecuteZhongguoB2PipSnapshotMailboxQueryV1 &&
         mailbox.executor_context ==
             const_cast<ZhongguoB2PipSnapshotMailboxContextV1 *>(&query);
}
...
bool ParseZhongguoB2PipSnapshotV1Step(std::string_view step) noexcept {
  return step == kZhongguoB2PipSnapshotV1Step;
}
...
bool ParseZhongguoB2PipSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) ||
      ...
      request_id.size() > 256 || step != kZhongguoB2PipSnapshotV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoB2PipSnapshotMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoB2PipSnapshotMailboxContextV1 *>(opaque_context);
  if (query == nullptr || !IsExecutingExactMailboxSlot(*query, stamp) ||
    ...
    if (query != nullptr) {
      query->completion =
          ZhongguoB2PipSnapshotMailboxCompletionV1::infrastructure_rejected;
    }
    return false;
  }
  try {
    ...
    query->read_result = ReadZhongguoB2PipSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoB2PipSnapshotResultV1::available &&
        query->result.status ==
    ...
    const bool typed_unavailable =
        ...
        query->result.status ==
            game::ZhongguoB2PipSnapshotStatusV1::unavailable &&
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.case_kind == kZhongguoB2PipSnapshotV1CaseKind &&
      ...
      query->completion = ZhongguoB2PipSnapshotMailboxCompletionV1::completed;
      return true;
    }
    query->completion =
        ZhongguoB2PipSnapshotMailboxCompletionV1::infrastructure_rejected;
...
std::string_view ZhongguoB2PipSnapshotFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main ZhongGuo B2 PIP executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main ZhongGuo B2 PIP boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main ZhongGuo B2 PIP query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main ZhongGuo B2 PIP query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main ZhongGuo B2 PIP executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main ZhongGuo B2 PIP ticket mismatch";
    case MainThreadQueryWaitResultV1::completed: break;
    }
  }
  if (completion == ZhongguoB2PipSnapshotMailboxCompletionV1::completed) {
    return completion_snapshot_stable
               ? "application-main ZhongGuo B2 PIP result is inconsistent"
               : "ZhongGuo B2 PIP completion snapshot changed";
  }
  if (completion ==
      ZhongguoB2PipSnapshotMailboxCompletionV1::frame_changed) {
    return "ZhongGuo B2 PIP application-main frame changed";
  }
  return "application-main ZhongGuo B2 PIP executor was rejected";
}
