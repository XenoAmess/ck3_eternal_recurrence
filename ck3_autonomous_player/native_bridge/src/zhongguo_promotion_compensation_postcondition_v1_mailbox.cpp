...
struct MailboxAccessProxyV1 {
  ZhongguoPromotionCompensationMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoPromotionCompensationMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoPromotionCompensationMailboxContextV1 *>(
                 &query);
}
...
bool ParseZhongguoPromotionCompensationPostconditionV1Step(
  ...
  return step == kZhongguoPromotionCompensationPostconditionV1Step;
}
...
bool ParseZhongguoPromotionCompensationPostconditionRequestV1(
  ...
  if (!HasExactControlFields(json) || !ParseStringField(json, "type", type) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoPromotionCompensationPostconditionV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoPromotionCompensationMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoPromotionCompensationMailboxContextV1 *>(
          opaque_context);
  ...
  try {
    ...
    query->read_result = ReadZhongguoPromotionCompensationPostconditionV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoPromotionCompensationResultV1::available &&
        query->result.status ==
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.player_character_id ==
            query->expected_snapshot.played_character_id) {
      query->completion =
          ZhongguoPromotionCompensationMailboxCompletionV1::completed;
      return true;
    }
    query->completion =
        ZhongguoPromotionCompensationMailboxCompletionV1::
...
std::string_view ZhongguoPromotionCompensationFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main promotion/compensation executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main promotion/compensation boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main promotion/compensation query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main promotion/compensation query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main promotion/compensation executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main promotion/compensation ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main promotion/compensation result is inconsistent"
               : "promotion/compensation completion snapshot changed";
  }
  if (completion ==
      ZhongguoPromotionCompensationMailboxCompletionV1::frame_changed)
    return "promotion/compensation application-main frame changed";
  return "application-main promotion/compensation executor was rejected";
}
