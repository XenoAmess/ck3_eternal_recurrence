...
static_assert(sizeof(void *) == 8, "the startup DX11 draw guard is x64-only");
static_assert(sizeof(std::atomic<std::uint64_t>) == sizeof(std::uint64_t));
static_assert(kStartupDx11RenderContextDrawDirtyOffsetV1 == 0x1938);
static_assert(kStartupDx11RenderContextDrawShaderStateOffsetV1 == 0x1940);

std::atomic<StartupDx11RenderContextDrawGuardV1State *> g_active_guard{
    nullptr};

constexpr std::array<std::uint8_t,
                     kStartupDx11RenderContextDrawPatchBytesV1>
    kPatchAnchor{
...
// The exact wrapper through its first virtual draw call. The 15-byte unwind
// prologue alone does not prove the observed helper relationship, so production
// admission also freezes the helper call and the wrapper's vcall at +0x68.
constexpr std::array<std::uint8_t,
                     kStartupDx11RenderContextDrawProductionAnchorBytesV1>
    kProductionAnchor{
...
void AddFailure(StartupDx11RenderContextDrawGuardV1State &state,
                StartupDx11RenderContextDrawGuardFailureV1 failure) noexcept {
...
void EmitBytes(
    std::array<std::uint8_t, kStartupDx11RenderContextDrawStubBytesV1> &stub,
    std::size_t &cursor,
...
bool BuildStub(
    ...
    std::array<std::uint8_t, kStartupDx11RenderContextDrawStubBytesV1>
        &stub) noexcept {
  std::size_t cursor = 0;

  // test byte ptr [rcx+0x1938], 1; jz healthy
  EmitBytes(stub, cursor,
            {0xF6, 0x81, 0x38, 0x19, 0x00, 0x00, 0x01, 0x74, 0x19});
  // cmp qword ptr [rcx+0x1940], 0; jne healthy
  EmitBytes(stub, cursor,
            {0x48, 0x83, 0xB9, 0x40, 0x19, 0x00, 0x00, 0x00,
             0x75, 0x0F});

  // The live failure path must keep the dirty bit set. It writes only the
  // process-lifetime counter and returns before the original unwind prologue.
  EmitBytes(stub, cursor, {0x49, 0xBB}); // mov r11, &suppressed_count
  EmitU64(stub, cursor,
  ...
  // Healthy path: replay all four overwritten complete instructions.
  EmitBytes(stub, cursor,
...
void BuildPatch(
    ...
    std::array<std::uint8_t, kStartupDx11RenderContextDrawPatchBytesV1>
        &patch) noexcept {
  // mov r11, imm64; jmp r11; nop; nop. The 15-byte window comprises complete
  // unwind-prologue instructions and resumes at the first body instruction.
  patch[0] = 0x49;
  patch[1] = 0xBB;
...
bool Flush(StartupDx11RenderContextDrawGuardV1State &state,
  ...
  if (state.flush_instruction_cache == nullptr ||
    ...
    AddFailure(state, startup_dx11_render_context_draw_guard_failure_flush);
    return false;
  }
  return true;
}
...
TargetWriteResult WriteTargetTransaction(
  ...
  constexpr auto size = kStartupDx11RenderContextDrawPatchBytesV1;
  if (!SafeBytesEqual(target, expected, size)) {
    AddFailure(state,
               startup_dx11_render_context_draw_guard_failure_target_identity);
  ...
  if (!rollback_identity || !rollback_flushed || !rollback_protection) {
    AddFailure(state, startup_dx11_render_context_draw_guard_failure_rollback);
    return TargetWriteResult::rollback_unproven;
  }
  return TargetWriteResult::original_preserved;
}

bool ReleaseStub(StartupDx11RenderContextDrawGuardV1State &state) noexcept {
...
bool InstallStartupDx11RenderContextDrawGuardV1(
  ...
  if (!environment.exact_build_admitted || environment.module_base == 0) {
    AddFailure(state,
               startup_dx11_render_context_draw_guard_failure_exact_build);
    return false;
  }
  ...
  if (state.installed.load(std::memory_order_acquire) != 0 ||
    ...
    AddFailure(
        state,
        startup_dx11_render_context_draw_guard_failure_already_installed);
    return false;
  }

  StartupDx11RenderContextDrawGuardV1State *expected_active = nullptr;
  ...
  state.continue_target = Resolve(environment.continue_target_override,
                                  environment.module_base,
                                  kStartupDx11RenderContextDrawContinueRvaV1);
  ...
  const bool resolved = state.patch_target != 0 && state.continue_target != 0;
  if (!resolved ||
      state.continue_target !=
          state.patch_target + kStartupDx11RenderContextDrawPatchBytesV1 ||
      !ExactAnchorsMatch(state.patch_target, environment.offline_fixture)) {
    AddFailure(state, startup_dx11_render_context_draw_guard_failure_anchor);
  ...
  state.stub = virtual_alloc(state.memory_context,
                             kStartupDx11RenderContextDrawStubBytesV1,
                             MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  if (state.stub == nullptr) {
    AddFailure(state,
               startup_dx11_render_context_draw_guard_failure_allocation);
    ClearResolvedState(state);
    g_active_guard.store(nullptr, std::memory_order_release);
    return false;
  }
...
ReadStartupDx11RenderContextDrawGuardV1Diagnostics(
  ...
  StartupDx11RenderContextDrawGuardV1Diagnostics output{};
