...
bool FixtureAdmission(
    void *, DomainConstructionCandidateCaptureAdmissionV1 &output) noexcept {
...
DomainConstructionCandidateObserverEnvironmentV1 FixtureEnvironment(
  ...
  environment.post_call_global_slot_target_override =
      reinterpret_cast<std::uintptr_t>(target.remote_target) + 0x200;
...
void TestSuspendedNonCk3ExactHashRejectionAndFixtureTransaction() {
  ...
  DomainConstructionCandidateObserverStateV1 fixture{};
  assert(xar::bridge::InstallDomainConstructionCandidateObserverV1(
      fixture, FixtureEnvironment(target)));
  std::array<std::uint8_t, 17> installed{};
  assert(RemoteRead(&target,
                    reinterpret_cast<std::uintptr_t>(target.remote_target),
                    installed.data(), installed.size()));
  assert(installed[0] == 0xFF && installed[1] == 0x25);
  assert(xar::bridge::UninstallDomainConstructionCandidateObserverV1(
      fixture));
  std::array<std::uint8_t, 17> restored{};
  assert(RemoteRead(&target,
                    reinterpret_cast<std::uintptr_t>(target.remote_target),
