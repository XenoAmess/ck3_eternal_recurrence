...
struct MailboxAccessProxyV1 {
  ZhongguoCompensationAf5MailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoCompensationAf5MailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoCompensationAf5MailboxContextV1 *>(
                 &query);
}
...
bool ParseZhongguoCompensationAf5SnapshotV1Step(
  ...
  return step == kZhongguoCompensationAf5SnapshotV1Step;
}
...
bool ParseZhongguoCompensationAf5SnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) || !ParseStringField(json, "type", type) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoCompensationAf5SnapshotV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoCompensationAf5MailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoCompensationAf5MailboxContextV1 *>(
          opaque_context);
  ...
  try {
    ...
    query->read_result = ReadZhongguoCompensationAf5SnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoCompensationAf5ResultV1::available &&
        query->result.status ==
            game::ZhongguoCompensationAf5StatusV1::available;
    const bool typed_unavailable =
        query->read_result ==
            game::ReadZhongguoCompensationAf5ResultV1::unavailable &&
        query->result.status ==
            game::ZhongguoCompensationAf5StatusV1::unavailable &&
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.player_character_id ==
            query->expected_snapshot.played_character_id) {
      query->completion =
          ZhongguoCompensationAf5MailboxCompletionV1::completed;
      return true;
    }
    query->completion =
        ZhongguoCompensationAf5MailboxCompletionV1::
...
std::string_view ZhongguoCompensationAf5FailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main compensation AF5 executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main compensation AF5 boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main compensation AF5 query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main compensation AF5 query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main compensation AF5 executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main compensation AF5 ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main compensation AF5 result is inconsistent"
               : "compensation AF5 completion snapshot changed";
  }
  if (completion ==
      ZhongguoCompensationAf5MailboxCompletionV1::frame_changed)
    return "compensation AF5 application-main frame changed";
  return "application-main compensation AF5 executor was rejected";
}
