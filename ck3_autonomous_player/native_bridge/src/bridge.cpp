...
#endif
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_RUNTIME_OBSERVER_V1)
static xar::bridge::DomainConstructionRuntimeObserverStateV1
    g_domain_construction_runtime_observer_v1{};
#endif
...
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_RUNTIME_OBSERVER_V1)
bool ReadDomainConstructionRuntimeCaptureAdmissionV1(
    void *context,
    xar::bridge::DomainConstructionRuntimeCaptureAdmissionV1
        &admission) noexcept {
  auto *mailbox = static_cast<xar::ck3_11906::MainThreadQueryMailboxV1 *>(
      context);
  if (mailbox == nullptr ||
...
#endif
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_RUNTIME_OBSERVER_V1)
  const auto domain_construction_runtime_observer =
      xar::bridge::ReadDomainConstructionRuntimeObserverDiagnosticsV1(
          g_domain_construction_runtime_observer_v1);
#endif
#if defined(XAR_CK3_ENABLE_COUNCIL_COMPOSITION_CANDIDATE_OBSERVER_V1)
...
#endif
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_RUNTIME_OBSERVER_V1)
  result += ",\"g2_domain_construction_runtime_observer_enabled\":true";
#endif
#if defined(XAR_CK3_ENABLE_COUNCIL_COMPOSITION_CANDIDATE_OBSERVER_V1)
...
#undef XAR_APPEND_PHASE2_PRODUCER_HISTOGRAM_FIELD
  ...
  auto construction_runtime_json = xar::bridge::
      SerializeDomainConstructionRuntimeObserverV1(
          domain_construction_runtime_observer);
  if (!construction_runtime_json.empty() &&
      construction_runtime_json.back() == '}') {
    construction_runtime_json.pop_back();
  }
  result += construction_runtime_json;
#endif
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_CANDIDATE_OBSERVER_V1)
...
std::string PlayerFactionAlertsResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::PlayerFactionAlertsV1 &alerts) {
  const auto payload = xar::ck3_11906::SerializePlayerFactionAlertsV1(alerts);
  if (payload.empty()) return {};
  const std::string_view status =
      alerts.status == xar::game::PlayerFactionAlertsStatusV1::available
  ...
  result +=
      ",\"ok\":true,\"result\":{\"step\":"
      "\"query-player-faction-alerts-v1\",\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":" + Number(query_sequence) +
            ",\"snapshot_revision\":" +
            Number(alerts.snapshot_revision) +
            ",\"player_faction_alerts\":" + payload +
            ",\"player_faction_alerts_ready\":";
  result += alerts.readiness.alert_ready ? "true" : "false";
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}
...
std::string ZhongguoCaseSnapshotResultFrame(
  ...
  const auto payload =
      xar::ck3_11906::SerializeZhongguoCaseSnapshotV1(snapshot);
  if (payload.empty()) {
    return {};
  }
  const std::string_view status =
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-case-snapshot-v1\",";
  result += "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_case_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoB1CycleSnapshotResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoB1CycleSnapshotV1 &snapshot) {
  const auto payload =
      xar::ck3_11906::SerializeZhongguoB1CycleSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const auto status =
      snapshot.status == xar::game::ZhongguoB1CycleSnapshotStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,\"request_id\":";
  AppendJsonString(result, request_id);
  result += ",\"ok\":true,\"result\":{\"step\":\"query-zhongguo-b1-cycle-snapshot-v1\",\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":" + Number(query_sequence);
  result += ",\"snapshot_revision\":" + Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_b1_cycle_snapshot\":" + payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoResultCaseSnapshotResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoResultCaseSnapshotV1 &snapshot) {
  const auto payload =
      xar::ck3_11906::SerializeZhongguoResultCaseSnapshotV1(snapshot);
  if (payload.empty()) {
    return {};
  }
  const std::string_view status =
  ...
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_result_case_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoB2PipSnapshotResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoB2PipSnapshotV1 &snapshot) {
  const auto payload =
      xar::ck3_11906::SerializeZhongguoB2PipSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status == xar::game::ZhongguoB2PipSnapshotStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-b2-pip-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_b2_pip_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}
...
std::string ZhongguoCompensationAf5ResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoCompensationAf5SnapshotV1 &snapshot) {
  const auto payload = xar::ck3_11906::
      SerializeZhongguoCompensationAf5SnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status ==
              xar::game::ZhongguoCompensationAf5StatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-compensation-af5-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_compensation_af5_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoWorkforceOwnerResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoWorkforceOwnerSnapshotV1 &snapshot) {
  const auto payload = xar::ck3_11906::
      SerializeZhongguoWorkforceOwnerSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status ==
              xar::game::ZhongguoWorkforceOwnerStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-workforce-owner-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_workforce_owner_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoPromotionSourceProgressResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoPromotionSourceProgressV1 &progress) {
  const auto payload =
      xar::ck3_11906::SerializeZhongguoPromotionSourceProgressV1(progress);
  if (payload.empty()) return {};
  const auto available =
      progress.status ==
      xar::game::ZhongguoPromotionSourceProgressStatusV1::available;
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
      "\"request_id\":";
  AppendJsonString(result, request_id);
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-promotion-source-progress-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, available ? "available" : "unavailable");
  result += ",\"query_sequence\":" + Number(query_sequence);
  result += ",\"snapshot_revision\":" + Number(progress.snapshot_revision);
  result += ",\"zhongguo_promotion_source_progress\":" + payload;
  result += ",\"production_capability_advertised\":";
  result += xar::ck3_11906::
                    kZhongguoPromotionSourceProgressV1ProductionCapabilityAdvertised
                ? "true"
                : "false";
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
...
std::string ZhongguoReviewNowActionResultFrame(
    std::string_view request_id, std::uint64_t action_sequence,
    std::uint64_t snapshot_revision,
    const xar::game::ZhongguoReviewNowActionRequestV1 &request,
    const xar::game::ZhongguoReviewNowActionAckV1 &action) {
  if (action_sequence == 0 || snapshot_revision == 0) return {};
  const bool acknowledged = action.accepted && action.native_handled &&
                            !action.postcondition_verified;
  const auto payload = acknowledged
                           ? xar::ck3_11906::
                                 SerializeZhongguoReviewNowActionAckV1(action)
                           : std::string{};
  if ((acknowledged && payload.empty()) ||
      (!acknowledged && action.rejection_reason.empty())) return {};
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
      "\"request_id\":";
  AppendJsonString(result, request_id);
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"activate-zhongguo-review-now-v1\",\"accepted\":";
  result += acknowledged ? "true" : "false";
  result += ",\"status\":";
  AppendJsonString(result, acknowledged
                               ? "acknowledged_verification_pending"
                               : "unavailable");
  result += ",\"request_nonce\":";
  AppendJsonString(result, request.request_nonce);
  result += ",\"action_sequence\":" + Number(action_sequence);
  result += ",\"snapshot_revision\":" + Number(snapshot_revision);
  result += ",\"rejection_reason\":";
  if (acknowledged) result += "null";
  else AppendJsonString(result, action.rejection_reason);
  result += ",\"action_ack\":" + (acknowledged ? payload : "null");
  result += ",\"production_capability_advertised\":";
  result += xar::ck3_11906::
                    kZhongguoReviewNowActionV1ProductionCapabilityAdvertised
                ? "true"
                : "false";
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
...
std::string ZhongguoCareerHcWorkforceResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoCareerHcWorkforcePostconditionV1 &snapshot) {
  const auto payload = xar::ck3_11906::
      SerializeZhongguoCareerHcWorkforcePostconditionV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status == xar::game::
                             ZhongguoCareerHcWorkforcePostconditionStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-career-hc-workforce-postcondition-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_career_hc_workforce_postcondition\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}
...
std::string ZhongguoIncidentSnapshotResultFrame(
  ...
  const auto payload =
      xar::ck3_11906::SerializeZhongguoIncidentSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status == xar::game::ZhongguoIncidentSnapshotStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-incident-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_incident_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string ZhongguoManagerGovernanceSnapshotResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoManagerGovernanceSnapshotV1 &snapshot) {
  const auto payload =
      xar::ck3_11906::SerializeZhongguoManagerGovernanceSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status ==
              xar::game::ZhongguoManagerGovernanceSnapshotStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-manager-governance-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_manager_governance_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}
...
std::string ZhongguoScoreboardStateResultFrame(
  ...
  const auto payload =
      xar::ck3_11906::SerializeZhongguoScoreboardStateV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status == xar::game::ZhongguoScoreboardStateStatusV1::available
          ? "available"
          : "unavailable";
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-scoreboard-state-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_scoreboard_state\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}

std::string_view ZhongguoScoreboardActionName(
    xar::game::ZhongguoScoreboardActionV1 action) noexcept {
  using Action = xar::game::ZhongguoScoreboardActionV1;
  switch (action) {
  case Action::open:
    return "open";
  case Action::switch_managed:
    return "switch-managed";
  case Action::switch_received:
    return "switch-received";
  case Action::switch_system:
...
std::string ZhongguoScoreboardActionResultFrame(
    std::string_view request_id, std::uint64_t action_sequence,
    std::uint64_t snapshot_revision,
    const xar::game::ZhongguoScoreboardActionRequestV1 &request,
    const xar::game::ZhongguoScoreboardActionAckV1 &action) {
  const auto action_name = ZhongguoScoreboardActionName(request.action);
  if (action_name.empty() || action_sequence == 0 || snapshot_revision == 0) {
    return {};
  }
  const bool acknowledged =
      action.result == xar::game::ZhongguoScoreboardActionResultV1::
                           acknowledged_verification_pending &&
      action.accepted;
  const auto payload = acknowledged
                           ? xar::ck3_11906::
                                 SerializeZhongguoScoreboardActionAckV1(action)
                           : std::string{};
  if (acknowledged && payload.empty()) return {};
  if (!acknowledged && action.rejection_reason.empty()) return {};
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,"
      "\"request_id\":";
  AppendJsonString(result, request_id);
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"activate-zhongguo-scoreboard-v1\",\"accepted\":";
  result += acknowledged ? "true" : "false";
  result += ",\"status\":";
  AppendJsonString(result, acknowledged
                               ? "acknowledged_verification_pending"
  ...
  AppendJsonString(result, request.request_nonce);
  result += ",\"action\":";
  AppendJsonString(result, action_name);
  result += ",\"action_sequence\":";
  result += Number(action_sequence);
  result += ",\"snapshot_revision\":";
  ...
  if (acknowledged) {
    ...
    AppendJsonString(result, action.rejection_reason);
  }
  result += ",\"action_ack\":";
...
std::string ZhongguoWorkforceNormalExitSnapshotResultFrame(
    std::string_view request_id, std::uint64_t query_sequence,
    const xar::game::ZhongguoWorkforceNormalExitSnapshotV1 &snapshot) {
  const auto payload = xar::ck3_11906::
      SerializeZhongguoWorkforceNormalExitSnapshotV1(snapshot);
  if (payload.empty()) return {};
  const std::string_view status =
      snapshot.status == xar::game::
                             ZhongguoWorkforceNormalExitSnapshotStatusV1::
                                 available
          ? "available"
          : "unavailable";
  std::string result =
  ...
  result +=
      ",\"ok\":true,\"result\":{"
      "\"step\":\"query-zhongguo-workforce-normal-exit-snapshot-v1\","
      "\"accepted\":true,\"status\":";
  AppendJsonString(result, status);
  result += ",\"query_sequence\":";
  result += Number(query_sequence);
  result += ",\"snapshot_revision\":";
  result += Number(snapshot.snapshot_revision);
  result += ",\"zhongguo_workforce_normal_exit_snapshot\":";
  result += payload;
  result += ",\"backend_id\":\"native-headless\"}}";
  return result;
}
...
std::string PlayerEpidemicRecoveryResultFrame(
    ...
    const xar::ck3_11906::PlayerEpidemicRecoveryV1 &value) {
  const auto payload =
      xar::ck3_11906::SerializePlayerEpidemicRecoveryV1(value);
  if (payload.empty()) return {};
  std::string result =
      "{\"type\":\"command_result\",\"protocol_version\":1,\"request_id\":";
  AppendJsonString(result, request_id);
  ...
  result += ",\"observation_revision\":" + Number(observation_revision);
  result += ",\"snapshot_revision\":" + Number(value.snapshot_revision);
  result += ",\"player_epidemic_recovery\":" + payload;
  result += ",\"private_build\":true,\"read_only\":true,";
  result += "\"advertised\":false,\"backend_id\":\"native-headless\"}}";
  return result;
}
...
public:
  ...
  void MaybeInstallFrontend() noexcept {
    ...
    environment.permitted_executor_quinquevigintary =
        &xar::ck3_11906::
...
struct WorkerState {
  ...
  xar::ck3_11906::ZhongguoScoreboardProviderRevisionTrackerV1
      zhongguo_scoreboard_provider_revision_tracker{};
  std::uint64_t zhongguo_workforce_collective_snapshot_query_sequence = 0;
  std::uint64_t zhongguo_workforce_owner_query_sequence = 0;
  ...
  std::uint64_t zhongguo_compensation_af5_query_sequence = 0;
  std::uint64_t zhongguo_promotion_source_progress_query_sequence = 0;
  std::uint64_t zhongguo_review_now_action_sequence = 0;
  std::uint64_t zhongguo_projects_metrics_query_sequence = 0;
  std::uint64_t zhongguo_career_hc_workforce_query_sequence = 0;
...
std::string NewProviderSessionId() noexcept {
  try {
    ...
    for (std::size_t index = 0; index < random.size(); ++index) {
      result[index * 2] = hex[random[index] >> 4];
      result[index * 2 + 1] = hex[random[index] & 0x0F];
    }
    return result;
  } catch (...) {
    return {};
...
void RunConnectedSession(
  ...
  auto &player_faction_alerts_query_sequence =
      state.player_faction_alerts_query_sequence;
  ...
  auto &zhongguo_b1_cycle_snapshot_query_sequence =
      state.zhongguo_b1_cycle_snapshot_query_sequence;
  auto &zhongguo_result_case_snapshot_query_sequence =
      state.zhongguo_result_case_snapshot_query_sequence;
  auto &zhongguo_b2_pip_snapshot_query_sequence =
      state.zhongguo_b2_pip_snapshot_query_sequence;
  auto &zhongguo_incident_snapshot_query_sequence =
  ...
  auto &zhongguo_scoreboard_action_sequence =
      state.zhongguo_scoreboard_action_sequence;
  auto &zhongguo_scoreboard_provider_session_id =
      state.zhongguo_scoreboard_provider_session_id;
  auto &zhongguo_scoreboard_provider_revision_tracker =
      state.zhongguo_scoreboard_provider_revision_tracker;
  auto &zhongguo_workforce_collective_snapshot_query_sequence =
      state.zhongguo_workforce_collective_snapshot_query_sequence;
  auto &zhongguo_workforce_owner_query_sequence =
      state.zhongguo_workforce_owner_query_sequence;
  ...
  auto &zhongguo_compensation_af5_query_sequence =
      state.zhongguo_compensation_af5_query_sequence;
  auto &zhongguo_promotion_source_progress_query_sequence =
      state.zhongguo_promotion_source_progress_query_sequence;
  auto &zhongguo_review_now_action_sequence =
      state.zhongguo_review_now_action_sequence;
  auto &zhongguo_projects_metrics_query_sequence =
      state.zhongguo_projects_metrics_query_sequence;
  auto &zhongguo_career_hc_workforce_query_sequence =
      state.zhongguo_career_hc_workforce_query_sequence;
        ...
        if (!xar::bridge::JsonStringField(
          ...
          connected = xar::bridge::WriteFrame(
              pipe, CommandResultFrame(request_id, step, false,
                   ...
                   && ![&]() {
                     ...
                     return xar::ck3_11906::ParsePlayerEpidemicRecoveryStepV1(
                         step, title_id);
                   }()
#endif
#if defined(XAR_CK3_ENABLE_G2_MINOR_RELIGIOUS_WAR_DEFENDERS_PRIVATE_V1)
          ...
          if (!xar::ck3_11906::
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::
                  ZhongguoCareerHcWorkforceMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
              ...
              if (submit !=
                  xar::ck3_11906::MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main career-HC/workforce executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoCareerHcWorkforceV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoCareerHcWorkforceV1ExecutingWaitSliceMilliseconds);
                }
                xar::game::Snapshot completion_snapshot{};
                const bool completion_snapshot_stable =
                    wait == xar::ck3_11906::
                ...
                if (wait == xar::ck3_11906::
                                MainThreadQueryWaitResultV1::completed &&
                    query.completion == xar::ck3_11906::
                                            ZhongguoCareerHcWorkforceMailboxCompletionV1::
                                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoCareerHcWorkforceResultFrame(
                      request_id,
                      zhongguo_career_hc_workforce_query_sequence + 1,
                      query.result);
                  if (!response.empty())
                    ++zhongguo_career_hc_workforce_query_sequence;
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoCareerHcWorkforceFailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false, error);
                }
    ...
    defined(XAR_CK3_ENABLE_G2_M5_ALLIANCE_PROJECTION_PRIVATE_QUERY_V1)
          ...
          if (!xar::ck3_11906::
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::
                  PlayerFactionAlertsMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
                  BindPlayerFactionAlertsNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.campaign_root_environment = xar::ck3_11906::
                  BindCampaignRootNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request.expected_snapshot_revision = expected_revision;
              ...
              if (submit != xar::ck3_11906::
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kPlayerFactionAlertsV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kPlayerFactionAlertsV1ExecutingWaitSliceMilliseconds);
                }
                ...
                if (wait == xar::ck3_11906::
                                MainThreadQueryWaitResultV1::completed &&
                    query.completion == xar::ck3_11906::
                                            PlayerFactionAlertsMailboxCompletionV1::
                                                completed &&
                    completion_snapshot_stable) {
                  response = PlayerFactionAlertsResultFrame(
                      request_id,
                      player_faction_alerts_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                    ++player_faction_alerts_query_sequence;
                  }
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      PlayerFactionAlertsFailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
        ...
        } else if (step == xar::ck3_11906::kZhongguoCaseSnapshotV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoCaseSnapshotRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoCaseNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo case executor is unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                  error = "paused application-main boundary is not ready";
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoCaseSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoCaseSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                ...
                if (wait == xar::ck3_11906::
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoCaseSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoCaseSnapshotResultFrame(
                      request_id,
                      zhongguo_case_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                ...
                if (response.empty()) {
                  const auto error =
                      xar::ck3_11906::ZhongguoCaseSnapshotFailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false,
                                                error);
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoB1CycleSnapshotV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoB1CycleSnapshotRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoB1CycleSnapshotMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoB1CycleNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              query.expected_snapshot = current_snapshot;
              ...
              if (submit !=
                  xar::ck3_11906::MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo B1-cycle executor is unavailable";
                if (submit == xar::ck3_11906::MainThreadQuerySubmitResultV1::
                                  paused_main_thread_not_observed)
                  error = "paused application-main boundary is not ready";
                else if (submit == xar::ck3_11906::MainThreadQuerySubmitResultV1::
                                           mailbox_busy)
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoB1CycleSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::MainThreadQueryWaitResultV1::
                                   timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoB1CycleSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                xar::game::Snapshot completion_snapshot{};
                const bool stable =
                    wait == xar::ck3_11906::MainThreadQueryWaitResultV1::completed &&
                ...
                if (wait == xar::ck3_11906::MainThreadQueryWaitResultV1::completed &&
                    query.completion == xar::ck3_11906::
                                            ZhongguoB1CycleSnapshotMailboxCompletionV1::
                                                completed &&
                    stable) {
                  response = ZhongguoB1CycleSnapshotResultFrame(
                      request_id, zhongguo_b1_cycle_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty())
                    ++zhongguo_b1_cycle_snapshot_query_sequence;
                }
                if (response.empty()) {
                  response = CommandResultFrame(
                      request_id, step, false,
                      xar::ck3_11906::ZhongguoB1CycleSnapshotFailureMessageV1(
                          wait, query.completion, stable));
                }
                const auto reclaimed = xar::ck3_11906::ReclaimMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket);
                if (reclaimed != xar::ck3_11906::
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoResultCaseSnapshotV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoResultCaseSnapshotRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoResultCaseSnapshotMailboxContextV1
                  query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoResultNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo result-case executor is unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                  error = "paused application-main boundary is not ready";
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoResultCaseSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoResultCaseSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoResultCaseSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoResultCaseSnapshotResultFrame(
                      request_id,
                      zhongguo_result_case_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                ...
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoResultCaseSnapshotFailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false,
                                                error);
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoB2PipSnapshotV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoB2PipSnapshotRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoB2PipNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo B2 PIP executor is unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                  error = "paused application-main boundary is not ready";
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoB2PipSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoB2PipSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoB2PipSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoB2PipSnapshotResultFrame(
                      request_id,
                      zhongguo_b2_pip_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                ...
                if (response.empty()) {
                  const auto error =
                      xar::ck3_11906::ZhongguoB2PipSnapshotFailureMessageV1(
          ...
          if (!xar::ck3_11906::
            ...
            step == xar::ck3_11906::
                        kZhongguoCompensationAf5SnapshotV1Step) {
          xar::ck3_11906::ZhongguoCompensationAf5RequestV1 request{};
          std::int32_t requested_owner_character_id = -1;
          if (!xar::ck3_11906::
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::
                  ZhongguoCompensationAf5MailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
              ...
              const auto submit = xar::ck3_11906::TrySubmitMainThreadQueryV1(
                  g_main_thread_query_mailbox_v1,
                  &xar::ck3_11906::
                      ExecuteZhongguoCompensationAf5MailboxQueryV1,
                  &query, query.ticket);
              if (submit !=
                  xar::ck3_11906::MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main compensation AF5 executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoCompensationAf5V1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoCompensationAf5V1ExecutingWaitSliceMilliseconds);
                }
                xar::game::Snapshot completion_snapshot{};
                const bool completion_snapshot_stable =
                    wait == xar::ck3_11906::
                ...
                if (wait == xar::ck3_11906::
                                MainThreadQueryWaitResultV1::completed &&
                    query.completion == xar::ck3_11906::
                                            ZhongguoCompensationAf5MailboxCompletionV1::
                                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoCompensationAf5ResultFrame(
                      request_id,
                      zhongguo_compensation_af5_query_sequence + 1,
                      query.result);
                  if (!response.empty())
                    ++zhongguo_compensation_af5_query_sequence;
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoCompensationAf5FailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false, error);
                }
            ...
            step == xar::ck3_11906::
                        kZhongguoWorkforceOwnerSnapshotV1Step) {
          xar::ck3_11906::ZhongguoWorkforceOwnerRequestV1 request{};
          std::int32_t requested_owner_character_id = -1;
          if (!xar::ck3_11906::
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::
                  ZhongguoWorkforceOwnerMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
                  BindZhongguoWorkforceOwnerNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              ...
              const auto submit = xar::ck3_11906::TrySubmitMainThreadQueryV1(
                  g_main_thread_query_mailbox_v1,
                  &xar::ck3_11906::
                      ExecuteZhongguoWorkforceOwnerMailboxQueryV1,
                  &query, query.ticket);
              if (submit !=
                  xar::ck3_11906::MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main workforce owner executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoWorkforceOwnerV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoWorkforceOwnerV1ExecutingWaitSliceMilliseconds);
                }
                xar::game::Snapshot completion_snapshot{};
                const bool completion_snapshot_stable =
                    wait == xar::ck3_11906::
                ...
                if (wait == xar::ck3_11906::
                                MainThreadQueryWaitResultV1::completed &&
                    query.completion == xar::ck3_11906::
                                            ZhongguoWorkforceOwnerMailboxCompletionV1::
                                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoWorkforceOwnerResultFrame(
                      request_id,
                      zhongguo_workforce_owner_query_sequence + 1,
                      query.result);
                  if (!response.empty())
                    ++zhongguo_workforce_owner_query_sequence;
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoWorkforceOwnerFailureMessageV1(
                          wait, query.completion,
                          completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false, error);
                }
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                               ...
                               kZhongguoPromotionSourceProgressV1Step) {
          xar::ck3_11906::ZhongguoPromotionSourceProgressRequestV1 request{};
          std::int32_t requested_owner_character_id = -1;
          if (!xar::ck3_11906::ParseZhongguoPromotionSourceProgressRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::
                  ZhongguoPromotionSourceProgressMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
              ...
              if (submit !=
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoPromotionSourceProgressV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::MainThreadQueryWaitResultV1::
                                   timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoPromotionSourceProgressV1ExecutingWaitSliceMilliseconds);
                }
                xar::game::Snapshot completion_snapshot{};
                const bool completion_snapshot_stable =
                    wait == xar::ck3_11906::MainThreadQueryWaitResultV1::completed &&
                ...
                if (wait == xar::ck3_11906::MainThreadQueryWaitResultV1::completed &&
                    (query.completion == xar::ck3_11906::
                                             ZhongguoPromotionSourceMailboxCompletionV1::
                                                 completed_available ||
                     query.completion == xar::ck3_11906::
                                             ZhongguoPromotionSourceMailboxCompletionV1::
                                                 completed_unavailable) &&
                    completion_snapshot_stable) {
                  response = ZhongguoPromotionSourceProgressResultFrame(
                      request_id,
                      zhongguo_promotion_source_progress_query_sequence + 1,
                      query.result);
                  if (!response.empty())
                ...
                if (response.empty()) {
                  response = CommandResultFrame(
                      request_id, step, false,
                      xar::ck3_11906::ZhongguoPromotionSourceFailureMessageV1(
                          wait, query.completion, completion_snapshot_stable));
                }
                const auto reclaimed = xar::ck3_11906::ReclaimMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket);
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoIncidentSnapshotV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoIncidentSnapshotRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoIncidentSnapshotMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoIncidentNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              query.request = std::move(request);
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo incident executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoIncidentSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoIncidentSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                ...
                if (wait == xar::ck3_11906::
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoIncidentSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoIncidentSnapshotResultFrame(
                      request_id,
                      zhongguo_incident_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                ...
                if (response.empty()) {
                  const auto error =
                      xar::ck3_11906::ZhongguoIncidentSnapshotFailureMessageV1(
                          wait, query.completion, completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false,
                                                error);
                }
          ...
          if (!xar::ck3_11906::
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo manager-governance executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoManagerGovernanceSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoManagerGovernanceSnapshotResultFrame(
                      request_id,
                      zhongguo_manager_governance_snapshot_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                    ++zhongguo_manager_governance_snapshot_query_sequence;
                  }
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoManagerGovernanceSnapshotFailureMessageV1(
                          wait, query.completion, completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false, error);
                }
                const auto reclaimed =
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoScoreboardStateV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoScoreboardStateRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoScoreboardStateMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
              ...
              request.provider_session_id =
                  zhongguo_scoreboard_provider_session_id;
              request.provider_read_mode =
                  zhongguo_scoreboard_provider_session_id.empty()
                      ? xar::ck3_11906::
                            ZhongguoScoreboardProviderReadModeV1::unavailable
                      : xar::ck3_11906::
                            ZhongguoScoreboardProviderReadModeV1::
                                publish_observation;
              request.provider_revision_tracker =
                  &zhongguo_scoreboard_provider_revision_tracker;
              query.request = std::move(request);
              query.expected_snapshot = current_snapshot;
              ...
              if (submit != xar::ck3_11906::
                                MainThreadQuerySubmitResultV1::submitted) {
                std::string_view error =
                    "application-main ZhongGuo scoreboard state executor is "
                    "unavailable";
                if (submit == xar::ck3_11906::
                                  MainThreadQuerySubmitResultV1::
                                      paused_main_thread_not_observed) {
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoScoreboardStateV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoScoreboardStateV1ExecutingWaitSliceMilliseconds);
                }
                ...
                if (wait == xar::ck3_11906::
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoScoreboardStateMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoScoreboardStateResultFrame(
                      request_id,
                      zhongguo_scoreboard_state_query_sequence + 1,
                      query.result);
                  if (!response.empty()) {
                    ++zhongguo_scoreboard_state_query_sequence;
                  }
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoScoreboardStateFailureMessageV1(
                          wait, query.completion, completion_snapshot_stable);
                  response = CommandResultFrame(request_id, step, false,
                                                error);
                }
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoScoreboardActionV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoScoreboardActionRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoScoreboardActionMailboxContextV1
                  action{};
              ...
              action.environment =
                  xar::ck3_11906::BindZhongguoScoreboardNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              action.dispatch_environment = xar::ck3_11906::
                  BindZhongguoScoreboardActionDispatchEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(
                          GetModuleHandleW(nullptr)),
                      true);
              ...
              action.provider_revision_tracker =
                  &zhongguo_scoreboard_provider_revision_tracker;
              action.action_access.context = &action.dispatch_environment;
              action.action_access.dispatch =
                  &xar::ck3_11906::
                      DispatchZhongguoScoreboardActionNativeV1;
              // This exact shortcut-manager path may only acknowledge that
              // the event was submitted.  Its raw native boolean is not a
              // postcondition, and production capability remains unadvertised
              // until an independent provider-owned observation revision
              // proves the requested GUI state transition.
              ...
              if (submit != xar::ck3_11906::
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    ...
                    xar::ck3_11906::
                        kZhongguoScoreboardActionV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, action.ticket,
                      xar::ck3_11906::
                          kZhongguoScoreboardActionV1ExecutingWaitSliceMilliseconds);
                }
                ...
                if (wait == xar::ck3_11906::
                    ...
                    (action.completion ==
                         xar::ck3_11906::
                             ZhongguoScoreboardActionMailboxCompletionV1::
                                 completed_unavailable ||
                     action.completion ==
                         xar::ck3_11906::
                             ZhongguoScoreboardActionMailboxCompletionV1::
                    ...
                    completion_snapshot_stable) {
                  response = ZhongguoScoreboardActionResultFrame(
                      request_id, zhongguo_scoreboard_action_sequence + 1,
                      state_revision, action.request, action.result);
                  if (!response.empty()) {
                    ++zhongguo_scoreboard_action_sequence;
                  }
                }
                if (response.empty()) {
                  const auto error = xar::ck3_11906::
                      ZhongguoScoreboardActionFailureMessageV1(
                ...
                connected = xar::bridge::WriteFrame(pipe, response);
                   ...
                   xar::ck3_11906::kZhongguoReviewNowActionV1Step) {
          ...
          if (!xar::ck3_11906::ParseZhongguoReviewNowActionRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::ZhongguoReviewNowActionMailboxContextV1 action{};
              action.mailbox = &g_main_thread_query_mailbox_v1;
              action.bindings = xar::ck3_11906::BindCurrentProcess(true);
              action.environment = xar::ck3_11906::
                  BindZhongguoScoreboardNativeEnvironmentV1(
                      reinterpret_cast<std::uintptr_t>(GetModuleHandleW(nullptr)),
                      true);
              action.dispatch_environment = xar::ck3_11906::
              ...
              // A native handled result is dispatch ACK only.  The next
              // separately nonced paused progress query must advance its
              // query_sequence and expose the played-owner B1 witness.
              const auto submit = xar::ck3_11906::TrySubmitMainThreadQueryV1(
                  g_main_thread_query_mailbox_v1,
                  &xar::ck3_11906::ExecuteZhongguoPromotionSourceMailboxV1,
                  &action, action.ticket);
              if (submit !=
                ...
                std::string response;
                if (wait == xar::ck3_11906::MainThreadQueryWaitResultV1::completed &&
                    (action.completion == xar::ck3_11906::
                                              ZhongguoPromotionSourceMailboxCompletionV1::
                                                  completed_available ||
                     action.completion == xar::ck3_11906::
                                              ZhongguoPromotionSourceMailboxCompletionV1::
                                                  completed_unavailable) &&
                    completion_snapshot_stable) {
                  response = ZhongguoReviewNowActionResultFrame(
                      request_id, zhongguo_review_now_action_sequence + 1,
                      state_revision, action.request, action.result);
                  if (!response.empty()) ++zhongguo_review_now_action_sequence;
                }
                if (response.empty()) {
                  response = CommandResultFrame(
                      request_id, step, false,
                      xar::ck3_11906::ZhongguoPromotionSourceFailureMessageV1(
                          wait, action.completion, completion_snapshot_stable));
                }
                const auto reclaimed = xar::ck3_11906::ReclaimMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, action.ticket);
                if (reclaimed != xar::ck3_11906::
          ...
          if (!xar::ck3_11906::
            ...
            step == xar::ck3_11906::
                        kZhongguoWorkforceNormalExitSnapshotV1Step) {
          xar::ck3_11906::ZhongguoWorkforceNormalExitSnapshotRequestV1
              request{};
          if (!xar::ck3_11906::
            ...
            connected = xar::bridge::WriteFrame(
                ...
                CommandResultFrame(
                    request_id, step, false,
                    "ZhongGuo Workforce normal-exit snapshot revision is "
                    "stale"));
          } else {
            xar::game::Snapshot current_snapshot{};
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              connected = xar::bridge::WriteFrame(
                  ...
                  CommandResultFrame(
                      request_id, step, false,
                      "ZhongGuo Workforce normal-exit snapshot changed or is "
                      "not ready"));
            } else {
              xar::ck3_11906::
                  ZhongguoWorkforceNormalExitSnapshotMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment = xar::ck3_11906::
              ...
              if (submit != xar::ck3_11906::
                ...
                auto wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket,
                    xar::ck3_11906::
                        kZhongguoWorkforceNormalExitSnapshotV1QueuedWaitBudgetMilliseconds);
                while (wait == xar::ck3_11906::
                                   MainThreadQueryWaitResultV1::
                                       timeout_executor_already_running) {
                  wait = xar::ck3_11906::WaitForMainThreadQueryV1(
                      g_main_thread_query_mailbox_v1, query.ticket,
                      xar::ck3_11906::
                          kZhongguoWorkforceNormalExitSnapshotV1ExecutingWaitSliceMilliseconds);
                }
                    ...
                    query.completion ==
                        xar::ck3_11906::
                            ZhongguoWorkforceNormalExitSnapshotMailboxCompletionV1::
                                completed &&
                    completion_snapshot_stable) {
                  response = ZhongguoWorkforceNormalExitSnapshotResultFrame(
                      request_id,
          ...
          if (!xar::ck3_11906::ParseCurrentTimelineBlockerContextRequestV1(
            ...
            if (!previous_snapshot.has_value() || state_revision == 0 ||
              ...
              xar::ck3_11906::PlayerEpidemicRecoveryMailboxContextV1 query{};
              query.mailbox = &g_main_thread_query_mailbox_v1;
              query.bindings = xar::ck3_11906::BindCurrentProcess(true);
              query.environment =
                  xar::ck3_11906::BindZhongguoScoreboardNativeEnvironmentV1(
              ...
              query.expected_revision = expected_revision;
              query.expected_snapshot = current_snapshot;
              query.requested_title_id = requested_title_id;
              const auto submit = xar::ck3_11906::TrySubmitMainThreadQueryV1(
                  g_main_thread_query_mailbox_v1,
                  &xar::ck3_11906::ExecutePlayerEpidemicRecoveryMailboxV1,
                  &query, query.ticket);
              if (submit != xar::ck3_11906::
                ...
                std::string response;
                if (stable) {
                  response = PlayerEpidemicRecoveryResultFrame(
                      request_id, step,
                      player_epidemic_recovery_query_sequence + 1,
                      query.execution_stamp.pump_epoch, query.result);
                  if (!response.empty())
                    ++player_epidemic_recovery_query_sequence;
                }
                if (response.empty()) {
                  response = CommandResultFrame(
                      request_id, step, false,
                      "epidemic recovery query did not complete on stable paused frame");
                }
                const auto reclaimed = xar::ck3_11906::ReclaimMainThreadQueryV1(
                    g_main_thread_query_mailbox_v1, query.ticket);
                if (reclaimed != xar::ck3_11906::
...
#if defined(XAR_CK3_ENABLE_G2_DOMAIN_CONSTRUCTION_RUNTIME_OBSERVER_V1)
  {
    xar::bridge::DomainConstructionRuntimeObserverEnvironmentV1 environment{};
    environment.exact_build_admitted = true;
    environment.admitted_executable_sha256 = xar::bridge::
    ...
    environment.capture_admission_context =
        &g_main_thread_query_mailbox_v1;
    environment.capture_admission_probe =
        &ReadDomainConstructionRuntimeCaptureAdmissionV1;
    if (!xar::bridge::InstallDomainConstructionRuntimeObserverV1(
            g_domain_construction_runtime_observer_v1, environment)) {
      return FALSE;
    }
  }
#endif
