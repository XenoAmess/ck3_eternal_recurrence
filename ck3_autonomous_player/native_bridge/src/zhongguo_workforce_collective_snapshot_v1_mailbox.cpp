...
struct MailboxAccessProxyV1 {
  ZhongguoWorkforceCollectiveSnapshotMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoWorkforceCollectiveSnapshotMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoWorkforceCollectiveSnapshotMailboxContextV1 *>(
                 &query);
}
...
bool ParseZhongguoWorkforceCollectiveSnapshotV1Step(
  ...
  return step == kZhongguoWorkforceCollectiveSnapshotV1Step;
}
...
bool ParseZhongguoWorkforceCollectiveSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoWorkforceCollectiveSnapshotV1Step || revision == 0 ||
      owner == 0 ||
...
bool ExecuteZhongguoWorkforceCollectiveSnapshotMailboxQueryV1(
  ...
  auto *query = static_cast<
      ZhongguoWorkforceCollectiveSnapshotMailboxContextV1 *>(opaque_context);
  if (query == nullptr || !IsExecutingExactMailboxSlot(*query, stamp) ||
    ...
    if (query != nullptr) {
      query->completion = ZhongguoWorkforceCollectiveSnapshotMailboxCompletionV1::
  ...
  try {
    ...
    query->read_result = ReadZhongguoWorkforceCollectiveSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoWorkforceCollectiveSnapshotResultV1::available &&
        query->result.status ==
    ...
    if ((typed_available || typed_unavailable) &&
      ...
      query->completion =
          ZhongguoWorkforceCollectiveSnapshotMailboxCompletionV1::completed;
      return true;
    }
    query->completion = ZhongguoWorkforceCollectiveSnapshotMailboxCompletionV1::
        infrastructure_rejected;
...
std::string_view ZhongguoWorkforceCollectiveSnapshotFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main ZhongGuo Workforce collective executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main ZhongGuo Workforce collective boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main ZhongGuo Workforce collective query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main ZhongGuo Workforce collective query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main ZhongGuo Workforce collective executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main ZhongGuo Workforce collective ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main ZhongGuo Workforce collective result is inconsistent"
               : "ZhongGuo Workforce collective completion snapshot changed";
  }
  if (completion ==
      ZhongguoWorkforceCollectiveSnapshotMailboxCompletionV1::frame_changed) {
    return "ZhongGuo Workforce collective application-main frame changed";
  }
  return "application-main ZhongGuo Workforce collective executor was rejected";
}
