...
SavedEnvironment SavePipeEnvironment() {
  ...
  const DWORD copied = GetEnvironmentVariableW(
      kPipeEnvironment, saved.value.data(), static_cast<DWORD>(saved.value.size()));
...
bool StartSuspendedTarget(const std::filesystem::path& target_path,
  ...
  const BOOL created = CreateProcessW(
      target_path.c_str(), command_line.data(), nullptr, nullptr, FALSE,
      CREATE_SUSPENDED | CREATE_UNICODE_ENVIRONMENT, nullptr, nullptr, &startup,
      &target);
...
int wmain(int argc, wchar_t** argv) {
  ...
  std::thread connector([pipe, promise = std::move(connected_promise)]() mutable {
