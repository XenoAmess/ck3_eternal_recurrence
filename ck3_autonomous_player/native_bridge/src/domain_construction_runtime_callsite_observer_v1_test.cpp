...
using xar::bridge::DomainConstructionRuntimeRawRowV1;
...
struct AdmissionFixture {
  bool readable = true;
  DomainConstructionRuntimeCaptureAdmissionV1 value{};
};

bool ReadAdmission(
    void *context,
    DomainConstructionRuntimeCaptureAdmissionV1 &output) noexcept {
...
struct FixtureMemory {
  std::array<std::uint8_t, 16> target{kAnchor};
...
std::size_t CountU64(const std::array<std::uint8_t, Size> &bytes,
                     std::uint64_t value) {
  std::array<std::uint8_t, sizeof(value)> encoded{};
  std::memcpy(encoded.data(), &value, sizeof(value));
  std::size_t count = 0;
  auto cursor = bytes.begin();
  while ((cursor = std::search(cursor, bytes.end(), encoded.begin(),
...
template <std::size_t Size, std::size_t NeedleSize>
bool ContainsBytes(const std::array<std::uint8_t, Size> &bytes,
                   const std::array<std::uint8_t, NeedleSize> &needle) {
...
void SetScore(DomainConstructionRuntimeRawRowV1 &row,
              std::int64_t score) {
...
void TestDefaultOffAndExactHashGuard() {
  static_assert(!xar::bridge::
      kDomainConstructionRuntimeObserverInstalledByDefaultV1);
  ...
  const auto diagnostics = xar::bridge::
      ReadDomainConstructionRuntimeObserverDiagnosticsV1(state);
  assert((diagnostics.failure_flags & xar::bridge::
          domain_construction_runtime_observer_failure_exact_build) != 0);
}

void TestLiveSessionApplicationMainGateAndFullRawCopy(
  ...
  admission.value.session_live = true;
  ...
  assert(!xar::bridge::CaptureDomainConstructionRuntimePostCallV1(
      state, 0, 76, 1000));
  admission.value.session_live = false;
  assert(!xar::bridge::CaptureDomainConstructionRuntimePostCallV1(
      state, 0, 77, 1001));
  admission.value.session_live = true;

  std::array<DomainConstructionRuntimeRawRowV1, 2> rows{};
  ...
  assert(capture.rejected_session_count == 1);
  ...
  const std::string serialized = xar::bridge::
      SerializeDomainConstructionRuntimeObserverV1(diagnostics);
  ...
  assert(xar::bridge::UninstallDomainConstructionRuntimeObserverV1(state));
...
void TestExactTrampolineAndBoundedRows() {
  FixtureMemory memory{};
  AdmissionFixture admission{};
  admission.value = {12, true, false, 99, 1234};
  DomainConstructionRuntimeObserverStateV1 state{};
  const auto environment = Environment(memory, admission);
  assert(xar::bridge::InstallDomainConstructionRuntimeObserverV1(
      state, environment));
  assert(memory.target[0] == 0xFF && memory.target[1] == 0x25);
  assert(CountU64(memory.stub, environment.producer_target_override) == 1);
  assert(CountU64(memory.stub, environment.continue_target_override) == 1);
  constexpr std::array<std::uint8_t, 7> kReplayedCallSetup{
      0x48, 0x8D, 0x55, 0x60, 0x48, 0x8B, 0xCE};
  constexpr std::array<std::uint8_t, 4> kReplayedPostCall{
      0x48, 0x8D, 0x45, 0xD0};
  assert(ContainsBytes(memory.stub, kReplayedCallSetup));
  assert(ContainsBytes(memory.stub, kReplayedPostCall));

  std::array<DomainConstructionRuntimeRawRowV1, 70> rows{};
...
int main(int argc, char **argv) {
  ...
  TestLiveSessionApplicationMainGateAndFullRawCopy(argc > 1 ? argv[1] : nullptr);
