...
void AppendG2TrucePrivateCaptureV1(
  ...
  for (std::size_t slot = 0; slot < shape.scripted_candidates.size(); ++slot) {
    ...
    const int candidate_length = std::snprintf(
        ...
        "\"sole_child_status\":\"%.*s\","
        "\"sole_child\":\"0x%llX\","
        "\"sole_child_vtable_rva\":\"0x%llX\","
        "\"sole_child_children\":\"0x%llX\","
        ...
        "\"next_layer_status\":\"%.*s\","
        "\"next_layer_capture_limit\":%llu,"
        "\"next_layer_capture_completed\":%llu,"
        "\"next_layer_child_vtable_rvas\":["
        "\"0x%llX\",\"0x%llX\",\"0x%llX\","
        "\"0x%llX\",\"0x%llX\",\"0x%llX\"],"
        "\"next_layer_truce_match_count\":%llu,"
        "\"next_layer_truce_match_index\":%d,"
        "\"next_layer_truce_duration_script_value\":\"0x%llX\"}",
        ...
        static_cast<int>(candidate.sole_child_status.size()),
        candidate.sole_child_status.data(),
        static_cast<unsigned long long>(candidate.sole_child),
        static_cast<unsigned long long>(as_rva(candidate.sole_child_vtable)),
        static_cast<unsigned long long>(candidate.sole_child_children),
        ...
        static_cast<int>(candidate.next_layer_status.size()),
        candidate.next_layer_status.data(),
        static_cast<unsigned long long>(candidate.next_layer_capture_limit),
        static_cast<unsigned long long>(
            candidate.next_layer_capture_completed),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[0])),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[1])),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[2])),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[3])),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[4])),
        static_cast<unsigned long long>(
            as_rva(candidate.next_layer_child_vtables[5])),
        static_cast<unsigned long long>(
  ...
  const int length = std::snprintf(
      ...
      "\"next_layer_candidate_capture_completed\":%llu,"
      "\"next_layer_truce_match_count\":%llu,"
      ...
      static_cast<unsigned long long>(
          shape.next_layer_candidate_capture_completed),
...
constexpr std::int32_t kCbPrestigeFactorIdentifierId = 82;
...
constexpr std::size_t kUnitCurrentEdgeProgressOffset = 0x168;
...
struct RaiktorPrestigePreviewCapture {
  const Bindings *bindings = nullptr;
  EffectPreviewCollectorSlot8 original_slot8 = nullptr;
  LoadedEffectSlot58 original_slot58 = nullptr;
  void *collector = nullptr;
  ...
  std::int32_t factor_identifier_id = -1;
  std::int64_t factor_raw = 0;
  std::int64_t attacker_delta_raw = 0;
  std::size_t factor_rows = 0;
  std::size_t attacker_prestige_rows = 0;
  bool failed = false;
};
...
thread_local RaiktorGoldPreviewCapture *g_raiktor_gold_preview_capture =
    nullptr;
thread_local RaiktorPrestigePreviewCapture
    *g_raiktor_prestige_preview_capture = nullptr;
...
bool HasDeclarableWarReadBindings(const Bindings &bindings) noexcept {
  return bindings.enabled && bindings.game_state_slot != nullptr &&
         ...
         bindings.declare_war_interaction_offset != 0 &&
...
enum class PrepareDeclareWarContextResult {
  ready,
  validation_failed,
  unavailable,
};

PrepareDeclareWarContextResult PrepareDeclareWarContext(
    const Bindings &bindings, std::int32_t played_character_id,
    const DeclarableWarSnapshot &declaration, void *casus_belli_type,
    CharacterInteractionContextStorage &context_storage) noexcept {
  void *const interaction = ResolveCharacterInteraction(
      bindings, bindings.declare_war_interaction_offset);
  if (interaction == nullptr) {
    return PrepareDeclareWarContextResult::unavailable;
  }
  ...
  if (!declaration.target_title_ids.empty()) {
    bindings.append_native_int_array_range(
        native_target_titles, 0, declaration.target_title_ids.data(),
        declaration.target_title_ids.data() +
            declaration.target_title_ids.size());
  }
  std::vector<std::int32_t> copied_target_titles;
  if (!ReadNativeIntArray(native_target_titles, copied_target_titles) ||
      copied_target_titles != declaration.target_title_ids) {
    bindings.destroy_character_interaction_context(context);
    return PrepareDeclareWarContextResult::unavailable;
  }
  ...
  if (!bindings.validate_character_interaction_context(context, nullptr)) {
    bindings.destroy_character_interaction_context(context);
    return PrepareDeclareWarContextResult::validation_failed;
  }
  return PrepareDeclareWarContextResult::ready;
}
...
bool ReadDeclarableWarsForTargetInternal(
    ...
    std::int32_t played_character_id, std::int32_t target_character_id,
    std::vector<DeclarableWarSnapshot> &output) noexcept {
  ...
  for (std::int32_t index = 0; index < casus_belli_type_count; ++index) {
    ...
    for (auto &choice : choices) {
      ...
      const auto preparation = PrepareDeclareWarContext(
          bindings, played_character_id, choice, casus_belli_type,
          context_storage);
      if (preparation == PrepareDeclareWarContextResult::unavailable) {
        return false;
      }
      if (preparation ==
          PrepareDeclareWarContextResult::validation_failed) {
        continue;
      }
      bindings.destroy_character_interaction_context(
          context_storage.bytes.data());
      output.push_back(std::move(choice));
    }
  }
  return true;
...
void CaptureRaiktorPrestigePreviewRow(
    void *collector, const void *first_scope, const void *second_scope,
    const PreviewFixedPayload *payload, void *effect_node,
    void *forwarded_argument) noexcept {
  RaiktorPrestigePreviewCapture *const capture =
      g_raiktor_prestige_preview_capture;
  if (capture == nullptr || capture->bindings == nullptr ||
      capture->original_slot8 == nullptr) {
    return;
  }
  ...
  const auto effect_vtable =
      effect_node == nullptr ? std::uintptr_t{0}
                             : LoadAt<std::uintptr_t>(effect_node, 0x00);
  if (effect_vtable == bindings.prestige_effect_vtable) {
    std::int32_t character_id = -1;
    const bool valid_scope =
        ReadPreviewCharacterScope(bindings, first_scope, character_id);
    if (!valid_scope) {
      capture->failed = true;
    } else if (character_id == capture->attacker_character_id) {
      if (second_scope != nullptr || payload == nullptr ||
          payload->tag != 1 || forwarded_argument != nullptr ||
          capture->attacker_prestige_rows != 0) {
        capture->failed = true;
      } else {
        capture->attacker_delta_raw = payload->raw;
        ++capture->attacker_prestige_rows;
      }
    }
  }

  // Preserve every stock presentation callback.  Valid prestige rows for
  // the defender/allies and every unrelated effect family are outside this
  // deliberately attacker-only slice and are ignored only after forwarding.
  capture->original_slot8(collector, first_scope, second_scope, payload,
                          effect_node, forwarded_argument);
}

bool CaptureRaiktorPrestigeFactorFromWrapper(
    RaiktorPrestigePreviewCapture &capture, const void *wrapper) noexcept {
  if (wrapper == nullptr || capture.factor_identifier_id !=
                                kCbPrestigeFactorIdentifierId ||
      capture.factor_rows != 0) {
    return false;
  }
  ...
  for (std::int32_t index = 0; index < count; ++index) {
    ...
    if (capture.factor_rows != 0 ||
        LoadAt<std::uint16_t>(row, 0x08) != 1 ||
        LoadAt<std::uint16_t>(row, 0x0A) != 0 ||
        LoadAt<std::uint8_t>(row, 0x18) != 0) {
      return false;
    }
    capture.factor_raw = LoadAt<std::int64_t>(row, 0x10);
    ++capture.factor_rows;
  }
  return capture.factor_rows == 1;
}

void CaptureRaiktorPrestigeLoadedEffect(
  ...
  if (capture == nullptr || capture->original_slot58 == nullptr ||
      ...
      collector != capture->collector || mode != 0) {
    if (capture != nullptr) {
      capture->failed = true;
    }
    return;
  ...
  // Call the original Raiktor attacker-defeat visible root.  The wrapper's
  // temporary variable container remains valid until this trampoline returns,
  // so identifier 82 is read after all setup/resource callbacks but before
  // 0x3380170 tears the container down.
  capture->original_slot58(capture->original_loaded_effect, wrapper, mode,
                           collector);
  if (!capture->failed &&
      !CaptureRaiktorPrestigeFactorFromWrapper(*capture, wrapper)) {
    capture->failed = true;
  }
}

bool DryPreviewRaiktorPrestigeVisibleRoot(
    const Bindings &bindings, void *loaded_effect, void *effect_context,
    std::int32_t attacker_character_id,
    std::int32_t factor_identifier_id, std::int64_t &factor_raw,
    std::int64_t &attacker_delta_raw) noexcept {
  factor_raw = 0;
  attacker_delta_raw = 0;
  if (!bindings.enabled || loaded_effect == nullptr ||
      effect_context == nullptr || attacker_character_id <= 0 ||
      factor_identifier_id != kCbPrestigeFactorIdentifierId ||
      ResolveTermsCharacter(bindings, attacker_character_id) == nullptr ||
      bindings.construct_effect_preview_collector == nullptr ||
      bindings.destroy_effect_preview_collector == nullptr ||
      bindings.traverse_loaded_effect == nullptr ||
  ...
  constexpr std::size_t kPreviewVtableCloneSlots = 128;
  std::array<void *, kPreviewVtableCloneSlots> cloned_collector_vtable{};
  std::copy_n(original_collector_vtable,
              cloned_collector_vtable.size(),
              cloned_collector_vtable.begin());
  auto *const original_slot8 =
      reinterpret_cast<EffectPreviewCollectorSlot8>(
          original_collector_vtable[1]);
  cloned_collector_vtable[1] =
      reinterpret_cast<void *>(&CaptureRaiktorPrestigePreviewRow);

  constexpr std::size_t kLoadedEffectProxyVtableSlots = 12;
  std::array<void *, kLoadedEffectProxyVtableSlots> cloned_effect_vtable{};
  ...
  cloned_effect_vtable[11] =
      reinterpret_cast<void *>(&CaptureRaiktorPrestigeLoadedEffect);
  std::array<std::byte, sizeof(void *)> proxy_loaded_effect{};
  StoreAt(proxy_loaded_effect.data(), 0x00, cloned_effect_vtable.data());

  RaiktorPrestigePreviewCapture capture{};
  capture.bindings = &bindings;
  capture.original_slot8 = original_slot8;
  ...
  StoreAt(collector, 0x00, cloned_collector_vtable.data());
  g_raiktor_prestige_preview_capture = &capture;
  ...
  if (LoadAt<void **>(loaded_effect, 0x00) != original_effect_vtable ||
      capture.failed || capture.factor_rows != 1 ||
      capture.attacker_prestige_rows != 1 || capture.factor_raw < 0 ||
      capture.factor_raw >
          std::numeric_limits<std::int64_t>::max() / 10) {
    return false;
  }
  const auto multiplied = -capture.factor_raw * 10;
  constexpr std::int64_t kMinimumPrestigeDeltaRaw =
      -1'000LL * kFixedPointScale;
  const auto expected_delta =
      std::max(multiplied, kMinimumPrestigeDeltaRaw);
  if (capture.attacker_delta_raw != expected_delta) {
    return false;
  }
  factor_raw = capture.factor_raw;
  attacker_delta_raw = capture.attacker_delta_raw;
  return true;
}
...
bool ReadRaiktorSurrenderPrisonerReleasesOnce(
    const Bindings &bindings, void *game_state, void *war,
    void *casus_belli_type, std::int32_t war_id, std::int32_t date_raw,
    std::int32_t casus_belli_database_index,
    std::int32_t primary_attacker_character_id,
    std::int32_t primary_defender_character_id,
    std::int32_t claimant_character_id,
  ...
  output = {};
  if (game_state == nullptr || war == nullptr || casus_belli_type == nullptr ||
      war_id == -1 || primary_attacker_character_id <= 0 ||
      primary_defender_character_id <= 0 || claimant_character_id <= 0 ||
      primary_attacker_character_id == primary_defender_character_id) {
    return false;
  }
  void *const attacker = ResolveTermsCharacter(
      bindings, primary_attacker_character_id);
  void *const defender = ResolveTermsCharacter(
      bindings, primary_defender_character_id);
  if (attacker == nullptr || defender == nullptr || attacker == defender ||
      ResolveTermsCharacter(bindings, claimant_character_id) == nullptr) {
    return false;
  }

  RaiktorSurrenderPrisonerReleaseObservation candidate{};
  ...
  if (!ReadWarParticipantIds(
          bindings,
          static_cast<std::byte *>(war) + kWarAttackersOffset,
          candidate.attacker_participant_ids) ||
      !ReadWarParticipantIds(
          bindings,
          static_cast<std::byte *>(war) + kWarDefendersOffset,
          candidate.defender_participant_ids) ||
      std::find(candidate.attacker_participant_ids.begin(),
                candidate.attacker_participant_ids.end(),
                primary_attacker_character_id) ==
          candidate.attacker_participant_ids.end() ||
      std::find(candidate.defender_participant_ids.begin(),
                candidate.defender_participant_ids.end(),
                primary_defender_character_id) ==
          candidate.defender_participant_ids.end() ||
      std::any_of(
          candidate.attacker_participant_ids.begin(),
          candidate.attacker_participant_ids.end(),
          [&candidate](std::int32_t character_id) {
            return std::find(candidate.defender_participant_ids.begin(),
                             candidate.defender_participant_ids.end(),
                             character_id) !=
                   candidate.defender_participant_ids.end();
          }) ||
      !ReadPrimaryAndSuccessors(
          bindings, game_state, attacker,
          primary_attacker_character_id,
          candidate.attacker_release_candidate_ids) ||
      !ReadPrimaryAndSuccessors(
          bindings, game_state, defender,
          primary_defender_character_id,
          candidate.defender_release_candidate_ids) ||
      std::any_of(
          candidate.attacker_release_candidate_ids.begin(),
          candidate.attacker_release_candidate_ids.end(),
          [&candidate](std::int32_t character_id) {
            return std::find(
                       candidate.defender_release_candidate_ids.begin(),
                       candidate.defender_release_candidate_ids.end(),
                       character_id) !=
                   candidate.defender_release_candidate_ids.end();
          }) ||
      !AppendPrisonerReleases(
          bindings, candidate.attacker_release_candidate_ids,
          candidate.defender_participant_ids,
          candidate.release_pairs) ||
      !AppendPrisonerReleases(
          bindings, candidate.defender_release_candidate_ids,
          candidate.attacker_participant_ids,
          candidate.release_pairs) ||
      candidate.release_pairs.size() > 8U) {
    return false;
  }
  for (std::size_t index = 0; index < candidate.release_pairs.size();
       ++index) {
    for (std::size_t other = index + 1;
         other < candidate.release_pairs.size(); ++other) {
      if (candidate.release_pairs[index].jailer_character_id ==
              candidate.release_pairs[other].jailer_character_id &&
          candidate.release_pairs[index].prisoner_character_id ==
              candidate.release_pairs[other].prisoner_character_id) {
        return false;
      }
    }
  }
  ...
  candidate.full_participant_scan = true;
  candidate.primary_and_first_three_successors_scanned = true;
...
ReadRaiktorSurrenderPrisonerReleasesCore(
  ...
  if (!bindings.enabled || war_id == -1 ||
      ...
      bindings.character_storage_slot == nullptr ||
      bindings.get_character_primary_title == nullptr) {
    return ReadRaiktorSurrenderPrisonerReleasesResult::unavailable;
  }
  ...
  if (!before.paused) {
    return ReadRaiktorSurrenderPrisonerReleasesResult::requires_paused;
  }
  if (!before.has_played_character || !before.played_character_alive) {
    return ReadRaiktorSurrenderPrisonerReleasesResult::
        no_played_character;
  }
  ...
  if (war == nullptr) {
    return ReadRaiktorSurrenderPrisonerReleasesResult::war_not_found;
  }
  ...
  RaiktorSurrenderPrisonerReleaseObservation first{};
  RaiktorSurrenderPrisonerReleaseObservation second{};
  if (!ReadRaiktorSurrenderPrisonerReleasesOnce(
  ...
  if (*bindings.game_state_slot != game_state ||
      *bindings.jomini_state_slot != jomini_state ||
      ...
      !ReadRaiktorSurrenderPrisonerReleasesOnce(
          bindings, game_state, war, casus_belli_type, war_id,
...
bool ReadRaiktorPrestigeCharacter(
    const Bindings &bindings, void *character,
    std::int32_t character_id,
    game::WarExitCharacterFixedPointSnapshot &current_prestige) noexcept {
  current_prestige = {};
  if (character == nullptr || character_id <= 0 ||
      ResolveTermsCharacter(bindings, character_id) != character) {
    return false;
  ...
  current_prestige = {character_id,
                      {prestige_raw, kFixedPointScale}};
  return true;
}

bool ReadRaiktorSurrenderPrestigeOnce(
    const Bindings &bindings, void *game_state, void *war,
    void *casus_belli_type, std::int32_t war_id,
    std::int32_t date_raw, std::int32_t casus_belli_database_index,
    std::int32_t primary_attacker_character_id,
  ...
  RaiktorSurrenderPrestigeObservation candidate{};
  candidate.war_id = war_id;
  candidate.date_raw = date_raw;
  candidate.active_casus_belli_database_index =
      casus_belli_database_index;
  ...
  if (!ReadRaiktorPrestigeCharacter(
          bindings, attacker, primary_attacker_character_id,
          candidate.attacker_current_prestige)) {
    return false;
  }
  ...
  std::int64_t factor_raw = 0;
  std::int64_t attacker_delta_raw = 0;
  const bool previewed = DryPreviewRaiktorPrestigeVisibleRoot(
      bindings,
      static_cast<std::byte *>(casus_belli_type) +
          kWarEffectAttackerDefeatOffset,
      effect_context, primary_attacker_character_id,
      factor_identifier_id, factor_raw, attacker_delta_raw);
  const bool context_destroyed =
      DestroyWarEffectContext(bindings, effect_context);
  if (!previewed || !context_destroyed) {
    return false;
  ...
  game::WarExitCharacterFixedPointSnapshot prestige_after{};
  if (!ReadRaiktorPrestigeCharacter(
          bindings, attacker, primary_attacker_character_id,
          prestige_after) ||
      prestige_after != candidate.attacker_current_prestige) {
    return false;
  }
  candidate.cb_prestige_factor = {factor_raw, kFixedPointScale};
  candidate.attacker_prestige_delta = {
      primary_attacker_character_id,
      {attacker_delta_raw, kFixedPointScale}};
  ...
  candidate.exact_factor_and_attacker_delta_observed = true;
  output = std::move(candidate);
  return true;
}

ReadRaiktorSurrenderPrestigeResult ReadRaiktorSurrenderPrestigeCore(
    const Bindings &bindings, std::int32_t war_id,
  ...
  if (!bindings.enabled || war_id == -1 ||
      ...
      bindings.effect_preview_collector_vtable == 0 ||
      bindings.prestige_effect_vtable == 0) {
    return ReadRaiktorSurrenderPrestigeResult::unavailable;
  }
  ...
  if (!before.paused) {
    return ReadRaiktorSurrenderPrestigeResult::requires_paused;
  }
  if (!before.has_played_character || !before.played_character_alive) {
    return ReadRaiktorSurrenderPrestigeResult::no_played_character;
  }
  void *const game_state = *bindings.game_state_slot;
  void *const jomini_state = *bindings.jomini_state_slot;
  if (game_state == nullptr || jomini_state == nullptr) {
  ...
  void *const war = ResolveWar(bindings, game_state, war_id);
  if (war == nullptr) {
    return ReadRaiktorSurrenderPrestigeResult::war_not_found;
  }
  if (std::none_of(before.active_wars.begin(), before.active_wars.end(),
                   [war_id](const ActiveWarSnapshot &candidate) {
                     return candidate.war_id == war_id;
  ...
  const auto primary_defender_character_id = LoadAt<std::int32_t>(
      war, kWarPrimaryDefenderCharacterIdOffset);
  if (primary_attacker_character_id != before.played_character_id) {
    return ReadRaiktorSurrenderPrestigeResult::
        player_not_primary_attacker;
  }
  ...
  const auto factor_identifier_id =
      *bindings.cb_prestige_factor_identifier_id;
  if (factor_identifier_id != kCbPrestigeFactorIdentifierId) {
    return ReadRaiktorSurrenderPrestigeResult::unavailable;
  }
  const auto claimant_character_id =
      LoadAt<std::int32_t>(war, kWarClaimantCharacterIdOffset);
  void *const loaded_root =
      static_cast<std::byte *>(casus_belli_type) +
      kWarEffectAttackerDefeatOffset;
  auto **const loaded_root_vtable =
      LoadAt<void **>(loaded_root, 0x00);
  ...
  RaiktorSurrenderPrestigeObservation first{};
  RaiktorSurrenderPrestigeObservation second{};
  if (!ReadRaiktorSurrenderPrestigeOnce(
          bindings, game_state, war, casus_belli_type, war_id,
          before.date_raw, casus_belli_database_index,
          primary_attacker_character_id, primary_defender_character_id,
          claimant_character_id, factor_identifier_id, first)) {
    return ReadRaiktorSurrenderPrestigeResult::unavailable;
  }
  if (between_samples != nullptr) {
    between_samples();
  ...
  if (*bindings.game_state_slot != game_state ||
      ...
      *bindings.cb_prestige_factor_identifier_id !=
      ...
      !ReadRaiktorSurrenderPrestigeOnce(
          bindings, game_state, war, casus_belli_type, war_id,
          after.date_raw, casus_belli_database_index,
          primary_attacker_character_id, primary_defender_character_id,
          claimant_character_id, factor_identifier_id, second) ||
      first != second) {
    return ReadRaiktorSurrenderPrestigeResult::unavailable;
  }
...
bool ReadSnapshot(const Bindings &bindings, Snapshot &output) noexcept {
  ...
  if (output.has_played_character) {
    ...
    void *const extension =
        LoadAt<void *>(played_character, kCharacterExtensionOffset);
    output.played_character_stress_points =
        extension == nullptr
            ? 0
            : LoadAt<std::int32_t>(extension, kCharacterStressPointsOffset);
...
using RouteTimelineFailureStage =
    game::RouteContactTimelineFailureStage;

// Diagnostic-only: retain the earliest fail-closed gate reached by one
// projection.  Null callers preserve the pre-existing shared helper behavior,
// and every caller still returns false at the same short-circuit boundary.
bool RecordRouteTimelineFailure(
    RouteTimelineFailureStage *output,
    RouteTimelineFailureStage stage) noexcept {
  if (output != nullptr && *output == RouteTimelineFailureStage::none) {
    *output = stage;
  }
  return false;
}
...
bool CurrentEdgeSpeedIsPositive(const Bindings &bindings, void *unit,
                                 const void *path_storage,
                                 RouteTimelineFailureStage *failure_stage =
                                     nullptr) noexcept {
  bool shares_active_front = false;
  if (!PathSharesActiveRouteFront(unit, path_storage,
                                  shares_active_front)) {
    return RecordRouteTimelineFailure(failure_stage,
                                      RouteTimelineFailureStage::path_header);
  }
  ...
  if (bindings.read_unit_current_edge_speed == nullptr) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::current_edge_speed);
  }
  current_edge_speed = 0;
  if (bindings.read_unit_current_edge_speed(
...
bool IsZeroProgressCurrentEdgeSpeedBoundary(
  ...
  if (!PathSharesActiveRouteFront(unit, path_storage,
                                  shares_active_front) ||
      !shares_active_front ||
      LoadAt<std::int64_t>(unit, kUnitCurrentEdgeProgressOffset) != 0 ||
      LoadAt<std::int64_t>(unit, kUnitCachedCurrentEdgeSpeedOffset) != 0 ||
      bindings.read_unit_current_edge_speed == nullptr) {
    return false;
  }
  std::int64_t recalculated_speed = -1;
  return bindings.read_unit_current_edge_speed(
             unit, &recalculated_speed) == &recalculated_speed &&
         recalculated_speed == 0;
}

bool ValidateRouteEdgeTimingInputs(const Bindings &bindings,
  ...
  if (!ReadPathHeader(path_storage, path) || path.count <= 0) {
    return RecordRouteTimelineFailure(failure_stage,
                                      RouteTimelineFailureStage::path_header);
  }
  if (!ReadRouteTravelSpeeds(bindings, unit, land_speed, naval_speed)) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::route_speed_read);
  }
  ...
  if (origin_map_node == nullptr) {
    return RecordRouteTimelineFailure(failure_stage,
                                      RouteTimelineFailureStage::route_origin);
  }
  void *source_info =
  ...
  for (std::int32_t index = 0; index < path.count; ++index) {
    ...
    if (destination == nullptr ||
      ...
      return RecordRouteTimelineFailure(
          failure_stage, RouteTimelineFailureStage::route_adjacency);
    }
    ...
    if (source_is_land) {
      if (!destination_is_water && land_speed <= 0) {
        return RecordRouteTimelineFailure(
            failure_stage, RouteTimelineFailureStage::land_speed);
      }
    } else if (source_is_water) {
      if (!destination_is_land && naval_speed <= 0) {
        return RecordRouteTimelineFailure(
            failure_stage, RouteTimelineFailureStage::naval_speed);
      }
    }
...
bool ValidateRouteTimingInputs(const Bindings &bindings, void *game_state,
                               ...
                               RouteTimelineFailureStage *failure_stage =
                                   nullptr) noexcept {
  return ValidateRouteEdgeTimingInputs(bindings, game_state, unit,
...
bool ProjectPathTimeline(
  ...
  for (std::int32_t index = 0; index < full.count; ++index) {
    ...
    if (path_duration_raw == kRouteDurationFailureSentinel ||
      ...
      return RecordRouteTimelineFailure(
          failure_stage, RouteTimelineFailureStage::route_duration_value);
    }
    if (path_duration_raw < prior_path_duration_raw) {
      return RecordRouteTimelineFailure(
          failure_stage, RouteTimelineFailureStage::route_duration_order);
    }
...
bool ReadFirstActiveEdgeDuration(const Bindings &bindings, void *game_state,
  ...
  if (!ValidateRouteTimingInputs(bindings, game_state, unit, path_storage,
                                 current_province, failure_stage)) {
    return false;
  }
  if (bindings.read_route_travel_duration(
  ...
  if (duration_raw == kRouteDurationFailureSentinel ||
...
bool ReadFirstActiveEdgeDurationAtZeroProgress(
    const Bindings &bindings, void *game_state, void *unit,
    void *current_province, const void *path_storage,
    std::int32_t expected_front_province_id,
    std::int64_t &duration_raw,
    RouteTimelineFailureStage *failure_stage = nullptr) noexcept {
  duration_raw = -1;
  NativeMovePathPrefix active{};
  if (bindings.read_route_edge_duration == nullptr) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::invalid_input);
  }
  if (!ReadPathHeader(path_storage, active) || active.count <= 0) {
  ...
  if (!IsZeroProgressCurrentEdgeSpeedBoundary(bindings, unit,
                                              path_storage)) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::zero_progress_boundary);
  }
  if (!ValidateRouteEdgeTimingInputs(bindings, game_state, unit, path_storage,
                                     current_province, failure_stage)) {
    return false;
  }
  void *const first_info = LoadAt<void *>(active.province_infos, 0);
  ...
  if (bindings.read_route_edge_duration(unit, &duration_raw, 0) !=
          &duration_raw ||
      duration_raw < 0 || duration_raw == kRouteDurationFailureSentinel ||
      duration_raw > kMaximumProjectedRouteDurationRaw) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::edge_duration_read);
  }
...
bool ProjectZeroProgressActiveRouteBoundary(
    const Bindings &bindings, void *game_state, void *unit,
    const void *path_storage, void *current_province,
    std::int32_t date_raw, std::vector<std::int32_t> &province_ids,
    std::vector<std::int32_t> &arrival_date_raws,
    RouteTimelineFailureStage *failure_stage = nullptr) noexcept {
  province_ids.clear();
  arrival_date_raws.clear();
  ...
  const auto first_province_id =
      LoadAt<std::int32_t>(first_info, kUnitPathProvinceIdOffset);
  void *const first_province = ResolveProvince(game_state, first_province_id);
  std::int64_t first_edge_duration_raw = -1;
  std::int32_t first_arrival_date_raw = 0;
  if (first_province == nullptr) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::route_entry);
  }
  if (!ReadFirstActiveEdgeDurationAtZeroProgress(
          bindings, game_state, unit, current_province, path_storage,
          first_province_id, first_edge_duration_raw, failure_stage)) {
    return false;
  }
  if (!RouteDurationToDate(date_raw, first_edge_duration_raw,
                           first_arrival_date_raw)) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::arrival_date);
  }
  province_ids.push_back(first_province_id);
  arrival_date_raws.push_back(first_arrival_date_raw);
  if (active.count == 1) {
    return true;
  }

  auto *const tail_infos =
      static_cast<std::byte *>(active.province_infos) + sizeof(void *);
  void *const tail_front_info = LoadAt<void *>(tail_infos, 0);
  if (tail_front_info == nullptr ||
      LoadAt<std::int32_t>(tail_front_info,
                           kUnitPathProvinceIdOffset) == first_province_id) {
    province_ids.clear();
    arrival_date_raws.clear();
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::route_mismatch);
  }
  std::array<std::byte, 0x130> tail{};
  const auto tail_count = active.count - 1;
  StoreAt(tail.data(), 0x00, static_cast<void *>(tail_infos));
  StoreAt(tail.data(), 0x08, tail_count);
  StoreAt(tail.data(), 0x0C, tail_count);
  std::vector<std::int32_t> tail_ids;
  std::vector<std::int32_t> tail_arrivals;
  if (!ProjectPathTimeline(bindings, game_state, unit, tail.data(),
                           first_province, date_raw,
                           first_edge_duration_raw, tail_ids,
                           tail_arrivals, failure_stage)) {
    province_ids.clear();
    arrival_date_raws.clear();
    return false;
  }
  province_ids.insert(province_ids.end(), tail_ids.begin(), tail_ids.end());
  arrival_date_raws.insert(arrival_date_raws.end(), tail_arrivals.begin(),
                           tail_arrivals.end());
  if (province_ids.size() != arrival_date_raws.size()) {
    return RecordRouteTimelineFailure(
        failure_stage, RouteTimelineFailureStage::timeline_shape);
...
bool BuildActiveRouteTimeline(const Bindings &bindings, void *game_state,
  ...
  if (!snapshot.has_current_province) {
    return RecordRouteTimelineFailure(
        &failure_stage, RouteTimelineFailureStage::active_identity);
  }
  ...
  if (snapshot.route_province_ids.empty()) {
    ...
    if (!ReadPathHeader(path_storage, active) || active.count != 0) {
      return RecordRouteTimelineFailure(
          &failure_stage, RouteTimelineFailureStage::path_header);
    }
  ...
  bool projected =
      IsZeroProgressCurrentEdgeSpeedBoundary(bindings, unit, path_storage)
          ? ProjectZeroProgressActiveRouteBoundary(
                ...
                date_raw, output.route_province_ids,
                output.arrival_date_raws, &failure_stage)
          : ProjectPathTimeline(bindings, game_state, unit, path_storage,
  ...
  if (output.route_province_ids != snapshot.route_province_ids) {
    output.route_province_ids.clear();
    output.arrival_date_raws.clear();
    return RecordRouteTimelineFailure(
        &failure_stage, RouteTimelineFailureStage::route_mismatch);
...
RouteContactHorizonStatus BuildSubjectRouteTimeline(
    ...
    game::RouteTimelineSnapshot &output,
    game::RouteContactTimelinePathKind &failure_path_kind,
    RouteTimelineFailureStage &failure_stage) noexcept {
  output = {};
  failure_path_kind = game::RouteContactTimelinePathKind::none;
  failure_stage = RouteTimelineFailureStage::none;
  ...
  failure_path_kind = game::RouteContactTimelinePathKind::constructed;
  ...
  if (effective_origin_id != selected->current_province_id) {
    if (!ReadFirstActiveEdgeDuration(
            ...
            effective_origin_id, first_edge_duration_raw, &failure_stage)) {
  ...
  if (!ProjectPathTimeline(
          ...
          tail_ids, tail_arrivals, &failure_stage)) {
    return RouteContactHorizonStatus::timeline_unavailable;
  }
  if (tail_ids.empty() || tail_ids.back() != request.target_province_id) {
    RecordRouteTimelineFailure(
        &failure_stage, RouteTimelineFailureStage::route_mismatch);
    return RouteContactHorizonStatus::timeline_unavailable;
  }
...
RouteContactHorizonStatus ReadRouteContactHorizon(
  ...
  void *const game_state = *bindings.game_state_slot;
  game::RouteContactTimelinePathKind subject_path_kind =
      game::RouteContactTimelinePathKind::none;
  RouteTimelineFailureStage subject_failure_stage =
      RouteTimelineFailureStage::none;
  const auto subject_status = BuildSubjectRouteTimeline(
      bindings, game_state, before, request, output.subject_route,
      subject_path_kind, subject_failure_stage);
  if (subject_status != RouteContactHorizonStatus::available) {
    if (subject_status == RouteContactHorizonStatus::timeline_unavailable) {
      output.timeline_failure.role =
          game::RouteContactTimelineFailureRole::subject;
      output.timeline_failure.army_id = request.subject_army_id;
      output.timeline_failure.path_kind = subject_path_kind;
      output.timeline_failure.stage = subject_failure_stage;
    }
  ...
  for (const auto hostile_id : request.hostile_army_ids) {
    ...
    if (!BuildActiveRouteTimeline(bindings, game_state, before.date_raw,
      ...
      output.timeline_failure.role =
          game::RouteContactTimelineFailureRole::hostile;
      output.timeline_failure.army_id = hostile_id;
      output.timeline_failure.path_kind =
          game::RouteContactTimelinePathKind::hostile_active;
      output.timeline_failure.stage = hostile_failure_stage;
      output.hostile_routes.clear();
      output.status = RouteContactHorizonStatus::timeline_unavailable;
      return output.status;
    }
...
ReadDeclarableWarsResult ReadDeclarableWarsForTarget(
  ...
  if (attacker_character == nullptr ||
      !ReadDeclarableWarsForTargetInternal(
          ...
          target_character, current.played_character_id,
          target_character_id, choices)) {
    return ReadDeclarableWarsResult::unavailable;
  }
...
DeclareWarResult SubmitDeclareWar(
  ...
  CharacterInteractionContextStorage context_storage{};
  const auto preparation = PrepareDeclareWarContext(
      bindings, current.played_character_id, declaration,
      casus_belli_type, context_storage);
  if (preparation == PrepareDeclareWarContextResult::validation_failed) {
