...
struct MailboxAccessProxyV1 {
  ZhongguoManagerGovernanceSnapshotMailboxContextV1 *query = nullptr;
...
bool IsExecutingExactMailboxSlot(
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoManagerGovernanceSnapshotMailboxContextV1 *>(
                 &query);
}
...
ZhongguoBoundedAiManagerAuthorizationV1 ProxyAuthorizeBoundedAiManager(
    void *opaque, std::int32_t player_character_id,
    std::int32_t subject_character_id,
    std::int32_t owner_character_id) noexcept {
  const auto *proxy = static_cast<const MailboxAccessProxyV1 *>(opaque);
  if (proxy == nullptr || proxy->query == nullptr || proxy->stamp == nullptr ||
      proxy->query->access.authorize_bounded_ai_manager == nullptr ||
      !IsExecutingExactMailboxSlot(*proxy->query, *proxy->stamp)) {
    return ZhongguoBoundedAiManagerAuthorizationV1::dependency_unavailable;
  }
  return proxy->query->access.authorize_bounded_ai_manager(
      proxy->query->access.context, player_character_id, subject_character_id,
      owner_character_id);
}
...
bool ParseZhongguoManagerGovernanceSnapshotV1Step(
  ...
  return step == kZhongguoManagerGovernanceSnapshotV1Step;
}
...
bool ParseZhongguoManagerGovernanceSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) ||
      ...
      request_id.size() > 256 ||
      step != kZhongguoManagerGovernanceSnapshotV1Step || revision == 0 ||
      subject == 0 || owner == 0 ||
...
bool ExecuteZhongguoManagerGovernanceSnapshotMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoManagerGovernanceSnapshotMailboxContextV1 *>(
          opaque_context);
  ...
  try {
    ...
    access.authorize_bounded_ai_manager =
        query->access.authorize_bounded_ai_manager == nullptr
            ? nullptr
            : &ProxyAuthorizeBoundedAiManager;
    query->read_result = ReadZhongguoManagerGovernanceSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoManagerGovernanceSnapshotResultV1::available &&
        query->result.status ==
            game::ZhongguoManagerGovernanceSnapshotStatusV1::available &&
        query->result.readiness.case_identity_ready &&
        query->result.readiness.subject_binding_ready;
...
std::string_view ZhongguoManagerGovernanceSnapshotFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main ZhongGuo manager-governance executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main ZhongGuo manager-governance boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main ZhongGuo manager-governance query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main ZhongGuo manager-governance query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main ZhongGuo manager-governance executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main ZhongGuo manager-governance ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main ZhongGuo manager-governance result is inconsistent"
               : "ZhongGuo manager-governance completion snapshot changed";
  }
  if (completion ==
      ZhongguoManagerGovernanceSnapshotMailboxCompletionV1::frame_changed) {
    return "ZhongGuo manager-governance application-main frame changed";
  }
  return "application-main ZhongGuo manager-governance executor was rejected";
}
