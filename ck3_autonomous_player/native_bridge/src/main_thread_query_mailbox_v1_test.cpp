...
bool TestMailboxStateMachine() {
  ...
  // Production exposes thirteen exact typed identities, never a generic
  // callback slot. Every admitted identity executes normally; a fourteenth
  ...
  constexpr std::array<MainThreadQueryExecutorV1, 13> typed_executors{
      ...
      &ExecuteDuodenary, &ExecuteThirdenary};
...
bool TestSourceContract(int argc, char **argv) {
  ...
  constexpr std::array<std::string_view, 53> source_tokens{
  ...
  constexpr std::array<std::string_view, 61> contract_tokens{
  ...
  constexpr std::array<std::string_view, 71> bridge_tokens{
      "HeartbeatFrame",
