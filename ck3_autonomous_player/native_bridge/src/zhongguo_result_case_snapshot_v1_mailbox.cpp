...
struct MailboxAccessProxyV1 {
  ZhongguoResultCaseSnapshotMailboxContextV1 *query = nullptr;
  const MainThreadExecutionStampV1 *stamp = nullptr;
};

bool IsExecutingExactMailboxSlot(
    const ZhongguoResultCaseSnapshotMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor_context ==
             const_cast<ZhongguoResultCaseSnapshotMailboxContextV1 *>(&query);
}
...
bool HasExactControlFields(std::string_view json) noexcept {
  // The closed request object intentionally has no subject_character_id,
  // case_kind or variable_name aliases. The played character and case kind
  // are provider constants, never caller-selected scope.
...
bool ParseZhongguoResultCaseSnapshotV1Step(
    std::string_view step) noexcept {
  return step == kZhongguoResultCaseSnapshotV1Step;
}
...
bool ParseZhongguoResultCaseSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) ||
      ...
      request_id.size() > 256 || step != kZhongguoResultCaseSnapshotV1Step ||
      revision == 0 || owner == 0 ||
...
bool ExecuteZhongguoResultCaseSnapshotMailboxQueryV1(
    void *opaque_context,
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query = static_cast<ZhongguoResultCaseSnapshotMailboxContextV1 *>(
      opaque_context);
  if (query == nullptr || !IsExecutingExactMailboxSlot(*query, stamp) ||
    ...
    if (query != nullptr) {
      query->completion = ZhongguoResultCaseSnapshotMailboxCompletionV1::
  ...
  try {
    ...
    query->read_result = ReadZhongguoResultCaseSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoResultCaseSnapshotResultV1::available &&
        query->result.status ==
    ...
    const bool typed_unavailable =
        ...
        query->result.status ==
            game::ZhongguoResultCaseSnapshotStatusV1::unavailable &&
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.case_kind == kZhongguoResultCaseSnapshotV1CaseKind &&
...
std::string_view ZhongguoResultCaseSnapshotFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main ZhongGuo result-case executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main ZhongGuo result-case boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main ZhongGuo result-case query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main ZhongGuo result-case query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main ZhongGuo result-case executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main ZhongGuo result-case ticket mismatch";
  ...
  if (completion ==
    ...
    return completion_snapshot_stable
               ? "application-main ZhongGuo result-case result is inconsistent"
               : "ZhongGuo result-case completion snapshot changed";
  }
  if (completion ==
      ZhongguoResultCaseSnapshotMailboxCompletionV1::frame_changed) {
    return "ZhongGuo result-case application-main frame changed";
  }
  return "application-main ZhongGuo result-case executor was rejected";
}
