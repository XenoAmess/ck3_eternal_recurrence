...
struct MailboxAccessProxyV1 {
  ZhongguoIncidentSnapshotMailboxContextV1 *query = nullptr;
...
bool ParseProfile(std::string_view value,
                  game::ZhongguoIncidentProfileV1 &output) noexcept {
  if (value == "x") {
    output = game::ZhongguoIncidentProfileV1::x;
    return true;
  }
  if (value == "y") {
    output = game::ZhongguoIncidentProfileV1::y;
    return true;
  }
  if (value == "z") {
    output = game::ZhongguoIncidentProfileV1::z;
    return true;
  }
  return false;
}
...
bool IsExecutingExactMailboxSlot(
    const ZhongguoIncidentSnapshotMailboxContextV1 &query,
  ...
  return mailbox.state.load(std::memory_order_acquire) ==
         ...
         mailbox.executor == &ExecuteZhongguoIncidentSnapshotMailboxQueryV1 &&
         mailbox.executor_context ==
             const_cast<ZhongguoIncidentSnapshotMailboxContextV1 *>(&query);
}
...
bool HasExactControlFields(std::string_view json) noexcept {
  // There is no subject_character_id, case_kind or variable_name alias.
  // The paused player and fixed profile allowlists are provider-owned.
  constexpr std::array<std::string_view, 8> fields{
      "type", "protocol_version", "request_id", "step",
      "expected_revision", "owner_character_id", "profile",
      "request_nonce"};
  std::uint32_t seen = 0;
  std::size_t cursor = 0;
  SkipWhitespace(json, cursor);
...
bool ParseZhongguoIncidentSnapshotV1Step(std::string_view step) noexcept {
  return step == kZhongguoIncidentSnapshotV1Step;
}
...
bool ParseZhongguoIncidentSnapshotRequestV1(
  ...
  if (!HasExactControlFields(json) ||
      ...
      !ParseStringField(json, "profile", profile) ||
      !ParseStringField(json, "request_nonce", request_nonce) ||
      type != "execute_step" || protocol != 1 || request_id.empty() ||
      request_id.size() > 256 || step != kZhongguoIncidentSnapshotV1Step ||
      revision == 0 || owner == 0 ||
      owner > static_cast<std::uint64_t>(
                  std::numeric_limits<std::int32_t>::max()) ||
      !ParseProfile(profile, output.profile) || !ValidNonce(request_nonce)) {
    return false;
  }
...
bool ExecuteZhongguoIncidentSnapshotMailboxQueryV1(
    ...
    const MainThreadExecutionStampV1 &stamp) noexcept {
  auto *query =
      static_cast<ZhongguoIncidentSnapshotMailboxContextV1 *>(opaque_context);
  if (query == nullptr || !IsExecutingExactMailboxSlot(*query, stamp) ||
    ...
    if (query != nullptr) {
      query->completion =
          ZhongguoIncidentSnapshotMailboxCompletionV1::infrastructure_rejected;
    }
    return false;
  }
  try {
    ...
    query->read_result = ReadZhongguoIncidentSnapshotV1(
        query->environment, access, query->request, query->result);
    const bool typed_available =
        query->read_result ==
            game::ReadZhongguoIncidentSnapshotResultV1::available &&
        query->result.status ==
    ...
    const bool typed_unavailable =
        ...
        query->result.status ==
            game::ZhongguoIncidentSnapshotStatusV1::unavailable &&
    ...
    if ((typed_available || typed_unavailable) &&
        ...
        query->result.case_kind == kZhongguoIncidentSnapshotV1CaseKind &&
        query->result.profile == ProfileName(query->request.profile) &&
...
std::string_view ZhongguoIncidentSnapshotFailureMessageV1(
  ...
  if (wait != MainThreadQueryWaitResultV1::completed) {
    ...
    case MainThreadQueryWaitResultV1::executor_failed:
      return "application-main ZhongGuo incident executor failed";
    case MainThreadQueryWaitResultV1::infrastructure_failed:
      return "application-main ZhongGuo incident boundary drifted";
    case MainThreadQueryWaitResultV1::cancelled:
      return "application-main ZhongGuo incident query was cancelled";
    case MainThreadQueryWaitResultV1::timeout_cancelled_before_execution:
      return "application-main ZhongGuo incident query timed out";
    case MainThreadQueryWaitResultV1::timeout_executor_already_running:
      return "application-main ZhongGuo incident executor is still running";
    case MainThreadQueryWaitResultV1::ticket_mismatch:
      return "application-main ZhongGuo incident ticket mismatch";
    case MainThreadQueryWaitResultV1::completed:
      break;
    }
  }
  if (completion == ZhongguoIncidentSnapshotMailboxCompletionV1::completed) {
    return completion_snapshot_stable
               ? "application-main ZhongGuo incident result is inconsistent"
               : "ZhongGuo incident completion snapshot changed";
  }
  if (completion ==
      ZhongguoIncidentSnapshotMailboxCompletionV1::frame_changed) {
    return "ZhongGuo incident application-main frame changed";
  }
  return "application-main ZhongGuo incident executor was rejected";
}
