...
from .zhongguo_compensation_af5_snapshot_contract import (
...
from .zhongguo_workforce_owner_snapshot_contract import (
    QUERY_ZHONGGUO_WORKFORCE_OWNER_SNAPSHOT_V1_CAPABILITY,
    QUERY_ZHONGGUO_WORKFORCE_OWNER_SNAPSHOT_V1_STEP,
    normalize_native_zhongguo_workforce_owner_snapshot_v1,
    parse_query_zhongguo_workforce_owner_snapshot_v1_step,
    query_zhongguo_workforce_owner_snapshot_v1_step,
)
...
class GameplayBridgeService:
    ...
    def query_zhongguo_compensation_af5_snapshot_v1(
        self,
        request_nonce: str,
        *,
        expected_revision: int,
    ) -> dict[str, object]:
        """Read the paused player's correlated compensation AF5 receipt."""
        if (
            isinstance(expected_revision, bool)
            or not isinstance(expected_revision, int)
            or not 0 <= expected_revision <= 2**64 - 1
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo compensation AF5 query lacks one stable "
                "paused player binding"
            )
        step = query_zhongguo_compensation_af5_snapshot_v1_step(
            player_character_id, request_nonce
        )
        query = parse_query_zhongguo_compensation_af5_snapshot_v1_step(step)
        if query is None:  # pragma: no cover - builder/parser invariant
            raise AssertionError(
                "compensation AF5 query builder violated v1"
            )
        capabilities = self.capabilities().get("bridge_capabilities")
        if not (
            isinstance(capabilities, list)
            and QUERY_ZHONGGUO_COMPENSATION_AF5_SNAPSHOT_V1_CAPABILITY
            in capabilities
        ):
            raise UnsupportedStepError(
                "selected backend does not advertise the ZhongGuo "
                "compensation AF5 postcondition query"
            )
        result = self.execute_step(step, expected_revision=expected_revision)
        expected_keys = {
            "step", "accepted", "status", "query_sequence",
            "snapshot_revision", "zhongguo_compensation_af5_snapshot",
            "backend_id", "queried_snapshot_id", "queried_revision",
            "queried_native_revision", "queried_connection_generation",
        }
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo compensation AF5 backend result is not bound "
                "to the requested paused frame"
            )
        try:
            normalized = normalize_native_zhongguo_compensation_af5_snapshot_v1(
                result.get(
                    "zhongguo_compensation_af5_snapshot"
                ),
                expected_query=query,
                expected_snapshot_revision=native_revision,
                expected_date_raw=date_raw,
            ...
            raise BridgeUnavailableError(
                "ZhongGuo compensation AF5 result is malformed: "
                f"{error}"
            ) from error
        current = self.snapshot()
        current_diagnostics = current.get("diagnostics")
        ...
        if not (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo compensation AF5 query crossed its paused "
                "snapshot binding"
            )
        bridge_version = (
            diagnostics.get("bridge_version")
    ...
    def query_zhongguo_workforce_owner_snapshot_v1(
        ...
        *,
        ...
        """Read the paused player's correlated Workforce owner receipt."""
        if (
            isinstance(expected_revision, bool)
            or not isinstance(expected_revision, int)
            or not 0 <= expected_revision <= 2**64 - 1
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo Workforce owner query lacks one stable "
                "paused player binding"
            )
        step = query_zhongguo_workforce_owner_snapshot_v1_step(
            player_character_id, request_nonce
        )
        query = parse_query_zhongguo_workforce_owner_snapshot_v1_step(step)
        if query is None:  # pragma: no cover - builder/parser invariant
            raise AssertionError(
                "Workforce owner query builder violated v1"
            )
        capabilities = self.capabilities().get("bridge_capabilities")
        if not (
            isinstance(capabilities, list)
            and QUERY_ZHONGGUO_WORKFORCE_OWNER_SNAPSHOT_V1_CAPABILITY
            in capabilities
        ):
            raise UnsupportedStepError(
                "selected backend does not advertise the ZhongGuo "
                "Workforce owner postcondition query"
            )
        result = self.execute_step(step, expected_revision=expected_revision)
        expected_keys = {
            "step", "accepted", "status", "query_sequence",
            "snapshot_revision", "zhongguo_workforce_owner_snapshot",
            "backend_id", "queried_snapshot_id", "queried_revision",
            "queried_native_revision", "queried_connection_generation",
        }
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo Workforce owner backend result is not bound "
                "to the requested paused frame"
            )
        try:
            normalized = normalize_native_zhongguo_workforce_owner_snapshot_v1(
                result.get(
                    "zhongguo_workforce_owner_snapshot"
                ),
                expected_query=query,
                expected_snapshot_revision=native_revision,
                expected_date_raw=date_raw,
            ...
            raise BridgeUnavailableError(
                "ZhongGuo Workforce owner result is malformed: "
                f"{error}"
            ) from error
        current = self.snapshot()
        current_diagnostics = current.get("diagnostics")
        ...
        if not (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo Workforce owner query crossed its paused "
                "snapshot binding"
            )
        bridge_version = (
            diagnostics.get("bridge_version")
    ...
    def query_zhongguo_manager_governance_snapshot_v1(
        ...
        *,
        ...
        if snapshot.get("paused") is not True:
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance queries require a paused CK3 "
                "snapshot"
            )
        revision = snapshot.get("revision")
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance query lacks a valid public "
                "revision"
            )
        if expected_revision != revision:
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance revision mismatch: expected "
                f"{expected_revision}, current {revision}"
            )
        native_revision = snapshot.get("native_revision")
        date_raw = snapshot.get("date_raw")
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance query lacks a stable native "
                "snapshot binding"
            )
        played_character = snapshot.get("played_character")
        player_character_id = (
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance query lacks the played character "
                "identity"
            )
        step = query_zhongguo_manager_governance_snapshot_v1_step(
            subject_character_id, owner_character_id, request_nonce
        )
        query = parse_query_zhongguo_manager_governance_snapshot_v1_step(step)
        if query is None:  # pragma: no cover - builder/parser invariant
            raise AssertionError("manager-governance query builder violated v1")
        diagnostics = snapshot.get("diagnostics")
        hello = (
            diagnostics.get("hello") if isinstance(diagnostics, dict) else None
        )
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance query lacks its bridge connection "
                "binding"
            )
        bridge_capabilities = self.capabilities().get("bridge_capabilities")
        if not (
            ...
            raise UnsupportedStepError(
                "selected backend cannot query ZhongGuo manager governance"
            )
        result = self.execute_step(step, expected_revision=expected_revision)
        expected_result_keys = {
            "step",
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance backend returned a malformed "
                "result"
            )
        query_sequence = result.get("query_sequence")
        backend_id = result.get("backend_id")
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance result lacks its native query "
                "identity"
            )
        try:
            normalized = (
            ...
            raise BridgeUnavailableError(
                f"ZhongGuo manager-governance result is malformed: {error}"
            ) from error
        if result.get("status") != normalized["status"]:
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance envelope status disagrees with "
                "its frame"
            )
        if (
            result.get("queried_snapshot_id") != snapshot_id
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance result is bound to another "
                "snapshot"
            )
        provenance = normalized["provenance"]
        assert isinstance(provenance, dict)
        ...
        if (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance build mirror disagrees with "
                "bridge hello"
            )
        current = self.snapshot()
        current_diagnostics = current.get("diagnostics")
        ...
        if not (
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance query crossed its paused snapshot "
                "binding"
            )
        subject_binding = normalized["subject_binding"]
        ...
        try:
            return normalize_zhongguo_manager_governance_snapshot_v1_response(
                response,
                expected_query=query,
                expected_snapshot_id=snapshot_id,
                expected_revision=revision,
            ...
            raise BridgeUnavailableError(
                "ZhongGuo manager-governance response projection is "
                f"malformed: {error}"
            ) from error
    ...
    def calibrate_frontend_coat_of_arms_framebuffer_v2(
        self,
        calibration_id: str,
        phase: str,
    ) -> dict[str, object]:
        """Capture one of two reference-independent solid-state calibration frames."""

        if phase not in {"begin", "complete"}:
            raise ValueError("calibration phase must be begin or complete")
        before_route = self.query_frontend_gui_route_v1()
        if before_route.get("route") != "coat_of_arms_designer":
            raise BridgeUnavailableError(
        ...
        try:
            ...
            operation = getattr(
                self._coat_of_arms_framebuffer_calibrations_v2, phase
            )
    ...
    def compare_frontend_coat_of_arms_framebuffer_v2(
        self,
        calibration_id: str,
        reference_png_base64: str,
        reference_png_sha256: str,
    ) -> dict[str, object]:
        """Compare against one fixed, reference-independent calibrated surface."""
    ...
    def prepare_frontend_coat_of_arms_framebuffer_v1(self) -> dict[str, object]:
        ...
        if before_route.get("route") != "coat_of_arms_designer":
            raise BridgeUnavailableError(
                "coat-of-arms framebuffer preparation requires the native designer route"
            )
        before_tree = self.inspect_frontend_coat_of_arms_tree_v1()
        if not (
            before_tree.get("status") == "available"
            ...
            raise BridgeUnavailableError(
                "coat-of-arms framebuffer preparation requires a visible native page root"
            )
        try:
            before_binding = frontend_gui_route_binding_from_capabilities(
                self.capabilities()
            )
            preparation = prepare_ck3_framebuffer_capture_v1(
                before_binding["bridge_pid"]
            )
            after_binding = frontend_gui_route_binding_from_capabilities(
                self.capabilities()
            ...
            raise BridgeUnavailableError(
                f"coat-of-arms framebuffer preparation failed closed: {error}"
            ) from error
        after_route = self.query_frontend_gui_route_v1()
        after_tree = self.inspect_frontend_coat_of_arms_tree_v1()
        if not (
            ...
            and after_tree.get("root_available") is True
            and after_tree.get("scope_root_name") == "coat_of_arms_page"
            and preparation.get("bridgePid") == before_binding["bridge_pid"]
            and preparation.get("presentationOnly") is True
            and preparation.get("usesOcr") is False
            and preparation.get("usesKeyboard") is False
            and preparation.get("usesMouse") is False
        ):
            raise BridgeUnavailableError(
                "coat-of-arms route or native bridge binding changed during preparation"
            )
        return {
            **preparation,
            "route": "coat_of_arms_designer",
    ...
    def activate_frontend_pick_any_character_v1(self) -> dict[str, object]:
        """Open the ruler-selection lobby; the driver proves the route."""

        activate = getattr(
            self.driver, "activate_frontend_pick_any_character_v1", None
        )
        if not callable(activate):
            raise UnsupportedStepError(
                "selected backend has no native frontend pick-any-character action"
            )
        result = activate()
        if (
            not isinstance(result, dict)
    ...
    def query_player_epidemic_recovery_private_v1(
        ...
        """Private paused CE1 county readback; absent from public capabilities."""
        reader = getattr(
            self.driver, "query_player_epidemic_recovery_private_v1", None
        )
        if not callable(reader):
            raise UnsupportedStepError(
                "selected backend has no private epidemic recovery query"
            )
        return reader(
            expected_revision=expected_revision,
