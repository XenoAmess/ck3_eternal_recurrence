...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 246
    assert result["statistics"] == {
        "evidence": 246,
        "generated_definition_references": 176,
        "lexical_caller_candidate_references": 510,
        "manually_reviewed_analysis_source_references": 213,
        "observation_artifacts": 70,
        "observation_artifact_references": 72,
        "references": 971,
        "source_definitions": 176,
    }
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 246
...
def test_manifest_preserves_honest_source_provenance_for_all_indexed_events() -> None:
    ...
    assert provenance_counts == {
        "captured-observation-artifact": 72,
        "generated-definition-index": 176,
        "lexical-caller-candidate-not-proven-runtime-caller": 510,
        "manually-reviewed-analysis-source": 213,
    }
    lexical = [
        reference
        for reference in references
