...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 227
    assert result["statistics"] == {
        "evidence": 227,
        "generated_definition_references": 169,
        "lexical_caller_candidate_references": 500,
        "manually_reviewed_analysis_source_references": 192,
        "observation_artifacts": 60,
        "observation_artifact_references": 61,
        "references": 922,
        "source_definitions": 167,
    }
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 227
...
def test_manifest_preserves_honest_source_provenance_for_all_indexed_events() -> None:
    ...
    assert provenance_counts == {
        "captured-observation-artifact": 61,
        "generated-definition-index": 169,
        "lexical-caller-candidate-not-proven-runtime-caller": 500,
        "manually-reviewed-analysis-source": 192,
    }
    lexical = [
        reference
        for reference in references
