...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 234
    assert result["statistics"] == {
        "evidence": 234,
        "generated_definition_references": 172,
        "lexical_caller_candidate_references": 503,
        "manually_reviewed_analysis_source_references": 198,
        "observation_artifacts": 63,
        "observation_artifact_references": 64,
        "references": 937,
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 234
...
def test_manifest_preserves_honest_source_provenance_for_all_indexed_events() -> None:
    ...
    assert provenance_counts == {
        "captured-observation-artifact": 64,
        "generated-definition-index": 172,
        "lexical-caller-candidate-not-proven-runtime-caller": 503,
        "manually-reviewed-analysis-source": 198,
    }
    lexical = [
        reference
        for reference in references
