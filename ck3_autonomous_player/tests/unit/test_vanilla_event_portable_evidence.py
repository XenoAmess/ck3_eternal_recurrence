...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 250
    assert result["statistics"] == {
        "evidence": 250,
        "generated_definition_references": 177,
        "lexical_caller_candidate_references": 511,
        "manually_reviewed_analysis_source_references": 217,
        "observation_artifacts": 71,
        "observation_artifact_references": 74,
        "references": 979,
        "source_definitions": 179,
    }
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 250
