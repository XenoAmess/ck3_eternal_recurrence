...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 253
    assert result["statistics"] == {
        "evidence": 253,
        "generated_definition_references": 177,
        "lexical_caller_candidate_references": 511,
        "manually_reviewed_analysis_source_references": 223,
        "observation_artifacts": 72,
        "observation_artifact_references": 76,
        "references": 987,
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 253
