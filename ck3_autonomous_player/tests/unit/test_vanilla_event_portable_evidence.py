...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 269
    assert result["statistics"] == {
        "evidence": 269,
        "generated_definition_references": 182,
        "lexical_caller_candidate_references": 519,
        "manually_reviewed_analysis_source_references": 259,
        "observation_artifacts": 82,
        "observation_artifact_references": 97,
        "references": 1057,
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 269
