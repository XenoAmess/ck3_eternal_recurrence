...
def test_checked_in_bundle_is_complete_and_strictly_self_validating() -> None:
    ...
    assert result["validated_evidence"] == 256
    assert result["statistics"] == {
        "evidence": 256,
        "generated_definition_references": 177,
        "lexical_caller_candidate_references": 511,
        "manually_reviewed_analysis_source_references": 235,
        "observation_artifacts": 75,
        "observation_artifact_references": 82,
        "references": 1005,
    ...
    assert len(list((BUNDLE / "blobs").glob("*.gz"))) == 256
