...
def test_all_168_keys_are_stably_keyset_paginated() -> None:
    ...
    while True:
        ...
        assert result["total_matches"] == 170
        assert result["dataset_summary"]["total_events"] == 170
    ...
    assert len(found) == len(set(found)) == 170
...
def test_filters_search_and_counts_match_the_canonical_metadata() -> None:
    ...
    assert migration["total_matches"] == 170 - len(source_expected)
    assert migration["match_summary"]["source_reviewed_events"] == 0
    assert observed["total_matches"] == len(observation_expected)
    assert unobserved["total_matches"] == 170 - len(observation_expected)
