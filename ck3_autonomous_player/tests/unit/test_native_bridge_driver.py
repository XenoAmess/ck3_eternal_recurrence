...
class HostileSiegeProvinceProjectionTests(unittest.TestCase):
    def test_primary_defender_enemy_siege_survives_snapshot_and_history(self) -> None:
        raw_war = {
            ...
            "player_relative_war_score": -42,
            "war_objective_province_ids": [2610],
            "enemy_armies": [
                {
                    "army_id": 50331920,
                    "owner_character_id": 30097,
                    "controllable": False,
...
def _tactical_sentinel_status(
    *,
    ...
    army_count: int,
    combat_count: int = 1,
    completed_daily_ticks: int = 0,
    intermediate_pause_count: int = 0,
    trigger_flags: int = 0,
    trigger_reasons: list[str] | None = None,
    overshoot_days: int = -1,
    pause_wrapper_called: bool = False,
    pause_observed: bool = False,
    terminal_observed: bool = False,
    abnormal: bool = False,
) -> dict[str, object]:
    triggered = state in {"triggered", "failed"}
    trigger_date_raw = observed_date_raw if triggered else 0
    return {
        "state": state,
        "generation": generation,
        "starting_date_raw": starting_date_raw,
        "target_date_raw": target_date_raw,
        "last_observed_date_raw": observed_date_raw,
        "trigger_date_raw": trigger_date_raw,
        "speed": speed,
        "mode": mode,
        "army_count": army_count,
        "combat_count": combat_count,
        "completed_daily_ticks": completed_daily_ticks,
        "intermediate_pause_count": intermediate_pause_count,
        "trigger_flags": trigger_flags,
        "trigger_reasons": list(trigger_reasons or []),
        "signed_date_delta_from_target_raw": (
            trigger_date_raw - target_date_raw if triggered else 0
        ),
        "overshoot_days": overshoot_days,
        "pause_wrapper_called": pause_wrapper_called,
...
class NativeHeadlessGameplayDriverTests(unittest.TestCase):
    ...
    def test_pending_reply_rejects_an_unpaused_changed_request(self) -> None:
        endpoint = FakeEndpoint()
        driver = NativeHeadlessGameplayDriver(
            endpoint.pipe_name,
            endpoint=endpoint,
        ...
        def answer_unpaused(frame: dict[str, object]) -> None:
            if frame.get("type") != "execute_step":
                return
            endpoint.publish(
                {
            ...
            endpoint.publish(_snapshot(35, paused=False))

        endpoint.send_hook = answer_unpaused
        with self.assertRaisesRegex(
            BridgeUnavailableError, "postcondition is not paused"
        ):
            driver.execute_step("reject-pending-character-interaction")
    ...
    def test_r0033_raiktor_long_war_surrender_is_projected_and_typed(
        self,
    ) -> None:
        endpoint = FakeEndpoint()
        driver = NativeHeadlessGameplayDriver(
        ...
        war_id = 16_777_285
        active_war = _war(war_id=war_id, score=-3)
        endpoint.publish(
            _hello(
                "game.state.snapshot",
                "game.command.query-war-termination-options-N",
        ...
        def answer(frame: dict[str, object]) -> None:
            ...
            if step.startswith("query-war-termination-options-"):
                ...
                self.assertEqual(step, "surrender-war-16777285")
                result = {
                    "step": step,
                    "accepted": True,
                    "status": "submitted",
        ...
        endpoint.send_hook = answer
        driver.execute_step("query-war-termination-options-16777285")
        source = driver.take_snapshot()
        ready, reason, evidence = (
            native_driver_module._raiktor_long_war_surrender_readiness(
                source, war_id
            )
        )
        self.assertTrue(ready, reason)
        self.assertEqual(evidence["variant"], "raiktor_long_war_utility")
        ...
        mutations = {
            ...
            "wrong_cb": lambda row: row["war_termination_options"][0][
                "active_casus_belli_identity"
            ].__setitem__("canonical_key", "claim_cb"),
            "score_zero": lambda row: (
                row["active_wars"][0].__setitem__(
            ...
            "day_729": lambda row: row["war_termination_options"][
                0
            ].__setitem__("war_duration_days", 729),
            "native_rejected": lambda row: row["war_termination_options"][
                0
            ]["options"]["surrender"].__setitem__(
                "native_validator_passed", False
        ...
        self.assertIn(
            "surrender-war-16777285",
            driver.capabilities()["action_steps"],
        )
        submitted = driver.execute_step("surrender-war-16777285")
        action = submitted["war_termination_result"]
        self.assertEqual(action["status"], "submitted_pending")
        self.assertEqual(action["outcome"], "attacker_defeat")
        self.assertEqual(action["surrender_variant"], "raiktor_long_war_utility")
        self.assertEqual(action["casus_belli"]["canonical_key"], "raiktor_claim_cb")
        self.assertEqual(action["player_relative_war_score"], -3)
        self.assertEqual(action["war_duration_days"], 804)
        self.assertTrue(action["absolute_war_scores_observable"])
        self.assertEqual(action["attacker_war_score"], -3)
        self.assertEqual(action["defender_war_score"], 3)
        self.assertNotIn(
            "surrender-war-16777285",
            driver.capabilities()["action_steps"],
        )
    ...
    def test_battle_sentinel_composites_require_exact_capability_pair(
        self,
    ) -> None:
        self.assertTrue(
            is_life_advance_step(BATTLE_DECISION_EPOCH_ADVANCE_STEP)
        )
        self.assertTrue(is_life_advance_step(BATTLE_TERMINAL_CRUISE_STEP))
        self.assertFalse(is_life_advance_step([BATTLE_TERMINAL_CRUISE_STEP]))
        ...
        for extra in ((), (arm_capability,), (status_capability,)):
            with self.subTest(extra=extra):
                endpoint = FakeEndpoint()
                driver = NativeHeadlessGameplayDriver(
                    endpoint.pipe_name, endpoint=endpoint
                )
                endpoint.publish(_hello(*base, *extra))
                endpoint.publish(
                    _snapshot(
                        active_wars=[_war(allied_armies=[player])],
                        player_armies=[player],
                ...
                self.assertNotIn(BATTLE_TERMINAL_CRUISE_STEP, steps)

        endpoint = FakeEndpoint()
        driver = NativeHeadlessGameplayDriver(
            endpoint.pipe_name, endpoint=endpoint
        )
        endpoint.publish(_hello(*base, arm_capability, status_capability))
        endpoint.publish(
            _snapshot(
                active_wars=[_war(allied_armies=[player])],
                player_armies=[player],
    ...
    def test_decision_epoch_composite_uses_full_watch_set_and_no_pause(
        ...
        war = _war(allied_armies=players)
        endpoint.publish(
            _snapshot(
                1,
                date_raw=start,
        ...
        stop = start + 2 * 24
        final_status = _tactical_sentinel_status(
            state="triggered",
            generation=7,
            starting_date_raw=start,
            target_date_raw=target,
        ...
        def answer(frame: dict[str, object]) -> None:
            ...
            if step.startswith(
                ...
                result["tactical_daily_sentinel"] = arm_status
            elif step == "research-query-tactical-daily-sentinel-v1":
                result["tactical_daily_sentinel"] = final_status
            endpoint.publish(
                {
                    "type": "command_result",
                    "protocol_version": 1,
        ...
        self.assertEqual(wire_steps[0], "set-speed-3")
        self.assertEqual(
            wire_steps[1],
            "research-arm-tactical-daily-sentinel-v1-"
            f"{start}-to-{target}-speed-3-mode-decision-a-2-501-502",
        )
        self.assertEqual(
            wire_steps[2:],
            [
    ...
    def test_committed_route_speed_five_ab_stops_on_native_contact(
        ...
        final_status = _tactical_sentinel_status(
            ...
            army_count=1,
            combat_count=0,
            completed_daily_ticks=3,
            trigger_flags=(1 << 2) | (1 << 3),
            trigger_reasons=[
        ...
        contacted_war = _war(allied_armies=[contacted_player])
        ...
        self.assertEqual(
            result["armed_tactical_daily_sentinel"]["combat_count"], 0
        )
        self.assertEqual(
    ...
    def test_stationary_objective_hold_is_default_production_with_legacy_flag(
        ...
        war = _war(
            allied_armies=[player],
            war_objective_province_ids=[2_635],
            objective_province_states=[_objective_state(2_635)],
        )
    ...
    def test_stationary_objective_hold_runs_seven_days_without_polling(
        ...
        wars = [war, other_war_without_objective_states]
        endpoint.publish(
            _snapshot(
                1,
                date_raw=start,
        ...
        self.assertEqual(wire_steps[0], "set-speed-3")
        self.assertIn("-speed-3-mode-decision-", wire_steps[1])
        self.assertEqual(wire_steps.count("resume-map"), 1)
        self.assertNotIn("pause-map", wire_steps)
        ...
        self.assertEqual(
            result["war_objective_hold_admission"]["status"], "matched"
        )
        self.assertEqual(
            result["war_objective_hold_post_stop"]["status"], "matched"
        )
        self.assertFalse(result["exact_war_terminal_watch"])
        self.assertFalse(result["exact_active_war_set_watch"])
        self.assertEqual(
    ...
    def test_stationary_objective_hold_rejects_wrong_binding_or_busy_army(
        ...
        base_war = _war(
            61,
            allied_armies=[base_player],
            war_objective_province_ids=[2_635],
            objective_province_states=[_objective_state(2_635)],
        )
        active_player_siege = _active_siege(
            army_id=501,
            player=True,
            assault_observable=True,
            breach_level=1,
            ...
            can_start_assault=True,
            can_stop_assault=False,
            assault_daily_progress_raw=1_000,
            assault_daily_casualties=10,
        )
        active_player_assault = {
            **active_player_siege,
            "assault_in_progress": True,
            "can_start_assault": False,
            "can_stop_assault": True,
        ...
        cases: dict[str, tuple[list[dict[str, object]], dict[str, object], str]] = {
            ...
            "moving": (
                [
                    {
                        **base_player,
                        "move_target_province_id": 2_636,
                        "army_state": "moving",
                        "army_state_code": 7,
                        "route_province_ids": [2_636],
                    }
                ],
                base_war,
                war_objective_hold_sentinel_advance_step(
            ...
            "active_siege": (
                ...
                _war(
                    ...
                    objective_province_states=[
                        _objective_state(
                            2_635, active_siege=active_player_siege
                        )
                    ],
                ),
                war_objective_hold_sentinel_advance_step(
            ...
            "active_assault": (
                ...
                _war(
                    ...
                    objective_province_states=[
                        _objective_state(
                            2_635, active_siege=active_player_assault
                        )
                    ],
                ),
                war_objective_hold_sentinel_advance_step(
        ...
        for label, (players, war, step) in cases.items():
            with self.subTest(label=label):
                endpoint = FakeEndpoint()
                driver = NativeHeadlessGameplayDriver(
                    endpoint.pipe_name,
        ...
        endpoint.publish(
            _snapshot(
                date_raw=start,
                active_wars=[base_war],
                player_armies=[base_player],
            )
        )
        with self.assertRaisesRegex(BridgeUnavailableError, "1..7 whole days"):
            driver.execute_step(
                war_objective_hold_sentinel_advance_step(
                    61, 501, 2_635, start + 8 * 24
                )
            )
        self.assertFalse(
            any(
        ...
        scoped_step = war_objective_hold_sentinel_advance_step(
            61, 501, 2_635, target
        )
        with self.assertRaisesRegex(
            UnsupportedStepError, "request scope is inconsistent"
        ):
            driver._execute_battle_sentinel_advance(
                scoped_step,
                expected_revision=None,
                requested_scope="active_battle",
                requested_objective_hold=(61, 501, 2_635, target),
            )

    def test_committed_route_scope_rejects_mismatched_subject_and_watch_state(
        self,
    ) -> None:
        start = 53_256_000
        target_date = start + 45 * 24
        subject = _army(
            501,
            move_target_province_id=2635,
            army_state="moving",
        ...
        sibling_route = _army(
            ...
            army_state="moving",
            army_state_code=7,
            route_province_ids=[2645, 2635],
        )
        cases = {
            "sibling_cannot_replace_subject": (
                [
                    {
                        **subject,
                        "move_target_province_id": None,
            ...
            "watched_active_combat": (
                ...
                "rejects any watched active combat or retreat",
            ),
            "watched_retreat": (
                [
                    subject,
        ...
        for label, (players, step, message) in cases.items():
            with self.subTest(label=label):
                endpoint = FakeEndpoint()
                driver = NativeHeadlessGameplayDriver(
                    endpoint.pipe_name,
                ...
                endpoint.publish(
                    _snapshot(
                        date_raw=start,
                        active_wars=[_war(allied_armies=players)],
                        player_armies=players,
                    )
                )
                with self.assertRaisesRegex(BridgeUnavailableError, message):
                    driver.execute_step(step)
                self.assertFalse(
                    any(
                        frame.get("type") == "execute_step"
    ...
    def test_tactical_sentinel_stop_rejects_abnormal_generation_date_and_overshoot(
        self,
    ) -> None:
        start = 53_171_400
        target = start + 45 * 24
        ...
        valid = _tactical_sentinel_status(
            state="triggered",
            generation=9,
            starting_date_raw=start,
            target_date_raw=target,
        ...
        cases: dict[str, dict[str, object]] = {
            "abnormal": {
                **valid,
                ...
                "abnormal": True,
                "trigger_flags": 1 << 15,
                "trigger_reasons": ["evaluation_failure"],
            },
            "generation": {**valid, "generation": 10},
            "date": {
                **valid,
                "last_observed_date_raw": ending + 24,
                "trigger_date_raw": ending + 24,
                "signed_date_delta_from_target_raw": ending + 24 - target,
            },
            "overshoot": {**valid, "overshoot_days": 1},
        }
        for name, candidate in cases.items():
            with self.subTest(name=name):
                normalized = _normalize_tactical_daily_sentinel_status(
                    candidate
                )
                with self.assertRaises(BridgeUnavailableError):
                    _validate_tactical_daily_sentinel_stop(
    ...
    def test_active_battle_sentinel_arm_still_requires_positive_combat_count(
        self,
    ) -> None:
        start = 53_171_400
        target = start + 45 * 24
        arm_step = (
            "research-arm-tactical-daily-sentinel-v1-"
            f"{start}-to-{target}-speed-3-mode-decision-a-1-501"
        )
        ...
        with self.assertRaisesRegex(
            BridgeUnavailableError, "arm acknowledgement is inconsistent"
        ):
            _validate_tactical_daily_sentinel_arm(
                {
                    "step": arm_step,
    ...
    def test_committed_route_scope_detects_retreating_watched_sibling(
        self,
    ) -> None:
        moving = _army(
            501,
        ...
        self.assertTrue(
            _battle_sentinel_has_active_retreat(
                {"player_armies": [moving, retreating]}, (501, 502)
            )
        )
        self.assertFalse(
            _battle_sentinel_has_active_retreat(
                {"player_armies": [moving]}, (501,)
            )
        )
    ...
    def test_terminal_cruise_uses_speed_five_and_reports_terminal_stop(
        self,
    ) -> None:
        endpoint = FakeEndpoint()
        driver = NativeHeadlessGameplayDriver(
        ...
        endpoint.send_hook = answer
        result = driver.execute_step(BATTLE_TERMINAL_CRUISE_STEP)
        wire_steps = [
            str(frame["step"])
            for frame in endpoint.frames
            if frame.get("type") == "execute_step"
        ...
        self.assertEqual(wire_steps[0], "set-speed-5")
        self.assertIn("-speed-5-mode-terminal-a-1-501", wire_steps[1])
        self.assertEqual(wire_steps.count("resume-map"), 1)
        self.assertNotIn("pause-map", wire_steps)
        self.assertEqual(result["progress_status"], "postcondition")
        self.assertTrue(result["terminal_reached"])
        self.assertEqual(result["stop_kind"], "terminal")
        self.assertTrue(result["zero_intermediate_pause"])
        self.assertEqual(result["external_pause_count"], 0)

    def test_player_decision_boundary_cancels_exact_generation_before_return(
        self,
    ) -> None:
        cases = (
        ...
        for decision_kind, already_paused, expected_pause_count in cases:
            with self.subTest(
                ...
                player = _army(501, in_combat=True)
                war = _war(allied_armies=[player])
                event = {"instance_id": 47, "option_count": 8}
                pending = {
                    "instance_id": _SIGNED_PENDING_ID,
                ...
                self.assertEqual(wire_steps.count("pause-map"), expected_pause_count)
                self.assertIn(
                ...
                self.assertEqual(
                    result["player_decision_boundary_pause_count"],
                    expected_pause_count,
                )
                self.assertFalse(result["managed_failure_cleanup"]["attempted"])
                self.assertIsNone(result["final_screen"])
                if decision_kind == "active_event":
                    self.assertEqual(result["active_event"]["instance_id"], 47)
    ...
    def test_failed_sentinel_transaction_uses_managed_pause_cleanup(
        ...
        driver = NativeHeadlessGameplayDriver(
            ...
            endpoint=endpoint,
            command_timeout_seconds=0.05,
            life_advance_timeout_seconds=0.01,
        )
        endpoint.publish(
            _hello(
                "game.state.snapshot",
        ...
        endpoint.send_hook = answer
        with self.assertRaises(StepPostconditionError) as raised:
            driver.execute_step(BATTLE_DECISION_EPOCH_ADVANCE_STEP)
        wire_steps = [
            str(frame["step"])
            for frame in endpoint.frames
            if frame.get("type") == "execute_step"
