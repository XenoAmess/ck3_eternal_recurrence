...
from xar_autoplayer.bridge.zhongguo_workforce_owner_snapshot_contract import (
    QUERY_ZHONGGUO_WORKFORCE_OWNER_SNAPSHOT_V1_CAPABILITY as CAPABILITY,
    QUERY_ZHONGGUO_WORKFORCE_OWNER_SNAPSHOT_V1_STEP as STEP,
...
def _fixtures() -> dict[str, object]:
    return json.loads((PROJECT_ROOT / "tests/fixtures/zhongguo_workforce_owner_snapshot_v1.json").read_text())
...
class _Endpoint:
    pipe_name = r"\\.\pipe\xar_workforce_owner_snapshot_v1_fixture"
    ...
    def send(self, request) -> None:
        ...
        self.publish({
            ...
            "result": {
                ...
                "zhongguo_workforce_owner_snapshot": deepcopy(self.frame),
            },
        })
...
def _driver(*, supported: bool = True):
    ...
    endpoint.publish({
        "type": "hello", "protocol_version": 1, "bridge_version": "workforce-owner-fixture-v1",
...
class WorkforceOwnerSnapshotContractTests(unittest.TestCase):
    def test_step_round_trip(self) -> None:
        step = query_zhongguo_workforce_owner_snapshot_v1_step(147, "workforce.14")
        self.assertEqual(
            parse_query_zhongguo_workforce_owner_snapshot_v1_step(step),
            ZhongguoWorkforceOwnerSnapshotQueryV1(147, "workforce.14"),
        )

    def test_pre_action_and_all_terminal_frames_remain_observable(self) -> None:
        schema = json.loads((PROJECT_ROOT / "schemas/zhongguo-workforce-owner-snapshot-v1.schema.json").read_text())
        validator = Draft202012Validator(schema)
        for name, frame in _fixtures()["frames"].items():
            with self.subTest(frame=name):
                normalized = normalize_native_zhongguo_workforce_owner_snapshot_v1(
                    frame,
                    expected_query=ZhongguoWorkforceOwnerSnapshotQueryV1(147, "workforce.14"),
                ...
                self.assertEqual(normalized["terminal_kind"], {
                    "pre_action": "none", "success": "success",
                    "history_accruing": "history_accruing",
                    "not_applicable_without_m360_source": "not_applicable",
                }[name])
                self.assertNotEqual(normalized["player_character_id"], normalized["subject_character_id"])
                if name == "not_applicable_without_m360_source":
                    self.assertTrue(all(field["status"] == "unavailable" for field in normalized["workforce"]["source"].values()))
                    self.assertTrue(normalized["workforce"]["portfolio"]["terminal_na"]["value"])

    def test_service_reads_through_native_transport_without_an_active_event(self) -> None:
        driver, endpoint = _driver()
        self.addCleanup(driver.close)
        service = GameplayBridgeService(driver)
        self.assertTrue(driver.capabilities()["zhongguo_workforce_owner_snapshot_v1_query_supported"])
        ...
        for name, frame in _fixtures()["frames"].items():
            with self.subTest(frame=name):
                ...
                result = service.query_zhongguo_workforce_owner_snapshot_v1(
                    "workforce.14", expected_revision=revision
                )
                schema = json.loads((PROJECT_ROOT / "schemas/zhongguo-workforce-owner-snapshot-v1.schema.json").read_text())
                ...
                self.assertEqual(request["request_nonce"], "workforce.14")
    ...
    def test_service_requires_supported_same_frame_query(self) -> None:
        ...
        with self.assertRaises(UnsupportedStepError):
            service.query_zhongguo_workforce_owner_snapshot_v1(
                "workforce.14", expected_revision=service.snapshot()["revision"]
            )
...
class WorkforceOwnerSnapshotMcpTests(unittest.IsolatedAsyncioTestCase):
    async def test_mcp_query_reaches_native_service(self) -> None:
        from mcp import Client

        driver, endpoint = _driver()
        self.addCleanup(driver.close)
        endpoint.frame = _fixtures()["frames"]["not_applicable_without_m360_source"]
        async with Client(create_server(driver)) as client:
            listed = await client.list_tools()
            tool = next(t for t in listed.tools if t.name == "ck3_query_zhongguo_workforce_owner_snapshot_v1")
            self.assertEqual(set(tool.input_schema["properties"]), {"request_nonce", "expected_revision"})
            result = await client.call_tool(tool.name, {
                "request_nonce": "workforce.14", "expected_revision": driver.take_snapshot()["revision"],
            })
        self.assertFalse(result.is_error)
        self.assertEqual(len(endpoint.frames), 1)
        self.assertTrue(result.structured_content["terminal"])
        self.assertEqual(result.structured_content["terminal_kind"], "not_applicable")
