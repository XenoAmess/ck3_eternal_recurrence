...
class EpidemicEventRecordTests(unittest.TestCase):
    ...
    def test_default_registry_mcp_and_production_runtime_include_record(self) -> None:
        self.assertEqual(len(DEFAULT_VANILLA_EVENT_TIMELINE_CONTRACTS), 161)
        self.assertEqual(len(DEFAULT_VANILLA_EVENT_ANALYSIS), 161)
        self.assertEqual(len(DEFAULT_VANILLA_EVENT_OBSERVATIONS), 11)
        self.assertEqual(len(production.KNOWN_TIMELINE_INTERRUPTS), 300)
        self.assertIs(
