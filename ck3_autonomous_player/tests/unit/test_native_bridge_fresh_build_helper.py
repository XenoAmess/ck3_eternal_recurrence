...
class NativeBridgeFreshBuildHelperTests(unittest.TestCase):
    def test_source_contract_covers_the_reproduced_incremental_abi_failure(
        ...
        self.assertIn('return "cmake-detected-2052-utf8"', source)
    ...
    @unittest.skipUnless(os.name == "nt", "PowerShell helper targets Windows")
    def test_2052_only_toolchain_accepts_cmake_utf8_prefix_without_rewrite(
        self,
    ) -> None:
        powershell = shutil.which("powershell.exe")
        self.assertIsNotNone(powershell)
        with tempfile.TemporaryDirectory(prefix="xar-native-prefix-utf8-") as temporary:
            root = Path(temporary)
            build_dir = root / "configured-build"
            rules = build_dir / "CMakeFiles" / "rules.ninja"
            rules.parent.mkdir(parents=True)
            ...
            self.assertIn("MODE=cmake-detected-2052-utf8", result.stdout)
