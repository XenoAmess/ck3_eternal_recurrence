...
class VassalInteractionEventRecordTests(unittest.TestCase):
    ...
    def test_default_registry_mcp_and_runtime_include_record(self) -> None:
        self.assertEqual(len(DEFAULT_VANILLA_EVENT_TIMELINE_CONTRACTS), 192)
        self.assertEqual(len(DEFAULT_VANILLA_EVENT_ANALYSIS), 192)
        self.assertIs(
            DEFAULT_VANILLA_EVENT_OBSERVATIONS[EVENT_KEY],
            VANILLA_VASSAL_INTERACTION_OBSERVATIONS[EVENT_KEY],
        )
        self.assertEqual(len(production.KNOWN_TIMELINE_INTERRUPTS), 338)
        self.assertIs(
