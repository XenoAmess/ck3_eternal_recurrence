...
- 授予旧天子并设为主头衔；`0.4.0` 添加 `single_heir_succession_law`，同时保留 `always_follows_primary_heir`，使动态后朝产生明确主继承人。CK3 对空法理动态霸权仍会在死亡事务中暂时销毁 title holder，因此 title object 采用“销毁但不删除”的生命周期，并在次日把同一对象及其原忠臣封臣树交回已计算的主继承人。
...
`0.4.0` 在动态后朝授予完成、头衔已经有持有者之后添加 `single_heir_succession_law`，并继续设置 `always_follows_primary_heir`。这两项负责产生单一继承顺位并对齐角色主继承人。实机证明 CK3 `1.19.0.6` 仍会在死亡结算中判定空法理动态霸权无效；因此死亡 on_action 只冻结已经由引擎算出的主继承人、同一后朝 title object、直属忠臣封臣和九席 incumbent，并把 title object 设为“可销毁、不可删除”。次日隐藏事务把同一 title object 及原忠臣封臣树恢复给该继承人，并幂等补回缺失的九席。旧君的其他个人头衔始终由其角色继承法决定，本 mod 不抢救或重新分配这些头衔。

`rmtm_migrate_restoration_hegemonies_effect` 会在新游戏与读档初始化时枚举带耐久 marker 的既有后朝，为缺少继承法的头衔补法、重申主继承人对齐，并启用“销毁但不删除”的同对象恢复合同。迁移不按名称猜测、不重建动态头衔，也不改持有者、国号、纹章、首都、空法理或复辟进度；重复运行是幂等的。执行【宣称复辟】前则显式恢复“销毁即删除”，确保后朝 title object 真正终结。
...
2026-09-14 追加已知缺陷：二期验收只覆盖后朝创建、保存重载和复辟，没有真正执行旧天子死亡。现有动态后朝缺少原版同类 titular title 使用的 `single_heir_succession_law`，因此不得再把死亡后的 realm 连续性视为已验收；三期修复完成前，以本节和三期计划的限制为准。
