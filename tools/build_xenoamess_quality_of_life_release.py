#!/usr/bin/env python3
"""Build, verify, and reproduce the XenoAmess Quality of Life release."""
...
RUNTIME_FILES = frozenset(
    {
        ...
        "localization/german/xqol_l_german.yml",
        "localization/japanese/xqol_l_japanese.yml",
        "localization/korean/xqol_l_korean.yml",
        "localization/polish/xqol_l_polish.yml",
        "localization/russian/xqol_l_russian.yml",
        "localization/simp_chinese/xqol_l_simp_chinese.yml",
        "localization/spanish/xqol_l_spanish.yml",
    }
)
...
def normalize_workshop_item_id(value: str | None) -> str | None:
    ...
    if value in FORBIDDEN_WORKSHOP_ITEM_IDS:
        raise ValueError("XenoAmess Quality of Life must not reuse an existing Workshop item ID")
    return value
...
def release_source_errors(source: Path, *, allow_source_only_files: bool = True) -> list[str]:
    ...
    expected_directories = SOURCE_DIRECTORIES if allow_source_only_files else RUNTIME_DIRECTORIES
...
def release_identity(source: Path) -> dict[str, str]:
    ...
    if source != DEFAULT_SOURCE.resolve():
        raise ValueError("formal release requires the canonical XenoAmess Quality of Life source")
...
def build_release(source: Path, staging: Path, revision: str | None = None, version: str | None = None, workshop_item_id: str | None = None, versioned_sidecars: bool = False, git_tag: str | None = None) -> tuple[Path, Path, Path, dict[str, object]]:
    ...
    if errors:
        raise ValueError("invalid XenoAmess Quality of Life source:\n" + "\n".join(errors))
...
def check_reproducible(source: Path = DEFAULT_SOURCE, workshop_item_id: str | None = None, revision: str | None = None) -> dict[str, object]:
    ...
    if errors:
        raise ValueError("invalid XenoAmess Quality of Life source:\n" + "\n".join(errors))
    revision, workshop_item_id = _require_full_revision(revision or git_sha()), normalize_workshop_item_id(workshop_item_id)
    with tempfile.TemporaryDirectory(prefix="xqol-release-check-") as name:
        root = Path(name)
...
def main(argv: list[str] | None = None) -> int:
    ...
    modes.add_argument("--release", action="store_true", help="require clean product source and xqol-v<version> tag")
    ...
    try:
        ...
        print(f"XQOL RELEASE FAILED: {error}", file=sys.stderr)
        return 1
