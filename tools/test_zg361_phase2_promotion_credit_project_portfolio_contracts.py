#!/usr/bin/env python3
"""Focused tests for purpose-split credit/project portfolio contracts."""
...
import zg361_phase2_promotion_credit_project_portfolio_contracts as portfolio  # noqa: E402
...
class CreditProjectPortfolioContractTests(unittest.TestCase):
    def _r211_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = portfolio.CREDIT_PROJECT_PORTFOLIO_TIMELINE_CONTRACTS[
            "zg361cp.9050"
        ]
        ...
        event = {"event_instance_id": 134}
        return snapshot, event, context

    def test_r211_exact_frame_selects_itemized_full_tree_mode(self) -> None:
        contracts = portfolio.CREDIT_PROJECT_PORTFOLIO_TIMELINE_CONTRACTS
        self.assertEqual(set(contracts), {"zg361cp.9050"})
        contract = contracts["zg361cp.9050"]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.9050"],
            contract,
        )
        snapshot, event, context = self._r211_frame()
    ...
    def test_r211_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = portfolio.CREDIT_PROJECT_PORTFOLIO_TIMELINE_CONTRACTS[
            "zg361cp.9050"
        ]
        snapshot, event, context = self._r211_frame()
    ...
    def test_contract_is_not_inlined_in_production_entry(self) -> None:
        ...
        self.assertNotIn('    "zg361cp.9050": {', production_source)
