...
class PpBargainingContractTests(unittest.TestCase):
    ...
    def _r205_frame(self) -> tuple[
        ...
        return self._observed_frame(
            event_key="zg361pp.149",
            date_raw=53186856,
            event_instance_id=128,
        )

    def _r206_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        return self._observed_frame(
            event_key="zg361pp.150",
            date_raw=53186880,
            event_instance_id=129,
        )
    ...
    def test_r205_exact_frame_selects_non_destructive_continuation(self) -> None:
        ...
        self.assertEqual(set(contracts), {"zg361pp.149", "zg361pp.150"})
    ...
    def test_r206_exact_frame_selects_policy_debt_continuation(self) -> None:
        contract = bargaining.PP_BARGAINING_TIMELINE_CONTRACTS[
            "zg361pp.150"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361pp.150"],
            contract,
        )
        snapshot, event, context = self._r206_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
    ...
    def test_r206_frame_rejects_date_and_alias_drift(self) -> None:
        contract = bargaining.PP_BARGAINING_TIMELINE_CONTRACTS[
            "zg361pp.150"
        ]
        snapshot, event, context = self._r206_frame()
