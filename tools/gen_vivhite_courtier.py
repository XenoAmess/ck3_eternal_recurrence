#!/usr/bin/env python3
"""Generate standalone Vivhite courtier catalogs from pinned vanilla metadata."""
...
OUTPUTS = {
    MOD
    / "common/scripted_effects/ervc_generated_courtier_catalog_effects.txt": "effects",
    MOD
    / "common/scripted_triggers/ervc_generated_courtier_catalog_triggers.txt": "triggers",
    MOD
    / "common/script_values/ervc_generated_courtier_catalog_values.txt": "values",
}
...
def load_snapshot(path: Path = SNAPSHOT) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    require(
        len(traits) == EXPECTED_TRAIT_COUNT,
        f"snapshot has {len(traits)} traits, expected {EXPECTED_TRAIT_COUNT}",
    )
    seen: set[str] = set()
...
def generated_effects(
    ...
    lines.extend(["ervc_cc_rebuild_trait_catalogs_effect = {", "\troot = {"])
    for name in CATALOG_NAMES:
        lines.append(f"\t\tclear_variable_list = ervc_cc_catalog_{name}")
    lines.extend(["\t}", ""])
    for trait in union:
        key = trait["key"]
        lines.extend(
            [
                f"\ttrait:{key} = {{ save_scope_as = ervc_cc_catalog_entry }}",
                "\troot = {",
                "\t\tadd_to_variable_list = {",
                f"\t\t\tname = ervc_cc_catalog_{membership[key]}",
                "\t\t\ttarget = scope:ervc_cc_catalog_entry",
                "\t\t}",
                "\t}",
            ]
        )
    lines.extend(["}", "", "ervc_cc_remove_selected_trait_conflicts_effect = {"])
    for trait in union:
        key = trait["key"]
        ...
        lines.extend(
            [
                "\tif = {",
                f"\t\tlimit = {{ scope:ervc_cc_candidate_trait = trait:{key} }}",
            ]
        )
        for conflict in conflicts[key]:
            lines.append(
                "\t\tervc_cc_remove_selected_trait_effect = "
                f"{{ ervc_trait = trait:{conflict} }}"
            )
        lines.append("\t}")
    lines.extend(["}", ""])
    return "\n".join(lines)
...
def metadata_checks(trait: dict[str, object]) -> list[str]:
    checks = [f"scope:ervc_cc_candidate_trait = trait:{trait['key']}"]
    if trait["minimum_age"] is not None:
        checks.append(f"var:ervc_cc_age >= {trait['minimum_age']}")
    if trait["maximum_age"] is not None:
        checks.append(f"var:ervc_cc_age <= {trait['maximum_age']}")
    if trait["valid_sex"] == "male":
        checks.append("var:ervc_cc_female = 0")
    elif trait["valid_sex"] == "female":
        checks.append("var:ervc_cc_female = 1")
    return checks
...
def generated_triggers(
    ...
    lines.extend(["ervc_cc_candidate_trait_metadata_valid_trigger = {", "\tOR = {"])
    for trait in union:
        ...
        lines.append("\t\t}")
    lines.extend(["\t}", "}", "", "ervc_cc_candidate_replaces_selected_trait_trigger = {", "\tOR = {"])
    for trait in union:
        key = trait["key"]
        if not conflicts[key]:
            continue
        lines.extend(
            [
                "\t\tAND = {",
                f"\t\t\tscope:ervc_cc_candidate_trait = trait:{key}",
                "\t\t\tOR = {",
            ]
        )
        for conflict in conflicts[key]:
            lines.append(
                "\t\t\t\tervc_cc_trait_is_selected_trigger = "
                f"{{ ervc_trait = trait:{conflict} }}"
            )
        lines.extend(["\t\t\t}", "\t\t}"])
    lines.extend(["\t}", "}", "", "ervc_cc_selected_traits_compatible_trigger = {"])
    for left, right in conflict_pairs(union, conflicts):
        lines.extend(
            [
                "\tNAND = {",
                "\t\tervc_cc_trait_is_selected_trigger = "
                f"{{ ervc_trait = trait:{left} }}",
                "\t\tervc_cc_trait_is_selected_trigger = "
                f"{{ ervc_trait = trait:{right} }}",
                "\t}",
            ]
        )
    lines.extend(["}", ""])
...
def generated_values(union: list[dict[str, object]]) -> str:
    ...
    lines.extend(["ervc_cc_selected_trait_cost = {", "\tvalue = 0"])
    for trait in union:
        lines.extend(
            [
                "\tif = {",
                "\t\tlimit = { ervc_cc_trait_is_selected_trigger = "
                f"{{ ervc_trait = trait:{trait['key']} }} }}",
...
def render_all() -> tuple[dict[Path, str], dict[str, int], str, int]:
    ...
    require(
        len(pairs) == EXPECTED_CONFLICT_PAIR_COUNT,
        f"catalog has {len(pairs)} conflict pairs, expected "
        f"{EXPECTED_CONFLICT_PAIR_COUNT}",
    )
    rendered_by_kind = {
...
def main() -> int:
    ...
    print(
        f"{verb} Vivhite courtier catalogs: {count_text}, "
        f"union={sum(counts.values())}, "
