...
class CreditProjectGovernanceContractTests(unittest.TestCase):
    ...
    def _r235_frame(self) -> tuple[
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.129",
            "current_event_instance_id": 158,
            "date_raw": 53187552,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index) for index in range(3)],
        ...
        event = {"event_instance_id": 158}
        return snapshot, event, context

    def _r236_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.134"
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.134",
            "current_event_instance_id": 159,
            "date_raw": 53187576,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index) for index in range(3)],
        ...
        event = {"event_instance_id": 159}
        return snapshot, event, context

    def _r237_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.130"
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.130",
            "current_event_instance_id": 160,
            "date_raw": 53187600,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index) for index in range(3)],
    ...
    def test_r235_exact_frame_queues_without_consuming_slot(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.129"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.129"],
            contract,
        )
        snapshot, event, context = self._r235_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.129",
            contract=contract,
        )
    ...
    def test_r235_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.129"
        ]
        snapshot, event, context = self._r235_frame()
    ...
    def test_promotion_queue_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.129": {', production_source)

    def test_r236_exact_frame_keeps_metric_owner_on_subject(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.134"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.134"],
            contract,
        )
        snapshot, event, context = self._r236_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.134",
            contract=contract,
        )
    ...
    def test_r236_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.134"
        ]
        snapshot, event, context = self._r236_frame()
    ...
    def test_shared_metric_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.134": {', production_source)

    def test_r237_exact_frame_selects_clean_talent_transfer(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.130"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.130"],
            contract,
        )
        snapshot, event, context = self._r237_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.130",
            contract=contract,
        )
    ...
    def test_r237_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = governance.CREDIT_PROJECT_GOVERNANCE_TIMELINE_CONTRACTS[
            "zg361cp.130"
        ]
        snapshot, event, context = self._r237_frame()
    ...
    def test_talent_transfer_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.130": {', production_source)
