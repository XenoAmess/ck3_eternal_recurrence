#!/usr/bin/env python3
"""Focused tests for purpose-split PP feedback completion contracts."""
...
import zg361_phase2_promotion_pp_feedback_completion_contracts as completion  # noqa: E402
...
class PpFeedbackCompletionContractTests(unittest.TestCase):
    def _r210_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = completion.PP_FEEDBACK_COMPLETION_TIMELINE_CONTRACTS[
            "zg361pp.9001"
        ]
        ...
        context = {
            ...
            "event_definition_key": "zg361pp.9001",
            "current_event_instance_id": 133,
            "date_raw": 53186976,
        ...
        event = {"event_instance_id": 133}
        return snapshot, event, context

    def test_r210_exact_frame_selects_the_only_queue_release(self) -> None:
        contracts = completion.PP_FEEDBACK_COMPLETION_TIMELINE_CONTRACTS
        self.assertEqual(set(contracts), {"zg361pp.9001"})
        contract = contracts["zg361pp.9001"]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361pp.9001"],
            contract,
        )
        snapshot, event, context = self._r210_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361pp.9001",
            contract=contract,
        )

        self.assertTrue(all(checks.values()), checks)
        self.assertEqual(len(contract["saved_scope_name_sets"][0]), 54)
        self.assertEqual(len(contract["character_scopes"]), 20)
        self.assertEqual(len(contract["scope_types"]), 34)
        self.assertEqual(contract["selected_option_number"], 1)
        self.assertEqual(contract["selected_native_option_index"], 0)

    def test_r210_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = completion.PP_FEEDBACK_COMPLETION_TIMELINE_CONTRACTS[
            "zg361pp.9001"
        ]
        snapshot, event, context = self._r210_frame()
        ...
        subject = next(
            ...
            if row["name"] == "zg361_pp_completion_subject"
        )
        ...
        self.assertFalse(alias_checks["scope:zg361_pp_completion_subject"])
    ...
    def test_contract_is_not_inlined_in_production_entry(self) -> None:
        ...
        self.assertNotIn('    "zg361pp.9001": {', production_source)
