...
class PromotionSourceCheckpointRunnerTests(unittest.TestCase):
    ...
    def test_imperial_debate_is_repeatable_inside_the_bounded_timeline(self) -> None:
        contract = production._timeline_contract_for_window(
        ...
        self.assertNotIn("max_occurrences", contract)
        for date_raw in (53203368, 53211192, 53215920, 53236224):
            with self.subTest(date_raw=date_raw):
                self.assertTrue(
    ...
    def test_movement_petition_interrupt_uses_terminal_refusal(self) -> None:
        ...
        petitioner_recipient = copy.deepcopy(context)
        petitioner_recipient["saved_scopes"][-1] = character_scope(
            "province_change_recipient", 28664
        )
        petitioner_recipient_checks = production._known_interrupt_checks(
            snapshot={"date_raw": 53158008, "active_event": {"option_count": 3}},
            event={"event_instance_id": 20},
            context={
                **petitioner_recipient,
                "current_event_instance_id": 20,
                "date_raw": 53158008,
            },
            event_key=event_key,
        ...
        self.assertTrue(
            all(petitioner_recipient_checks.values()),
    ...
    def test_administrative_confirmation_interrupt_uses_terminal_refusal(self) -> None:
        ...
        for date_raw in (53204640, 53226000, 53246304, 53288040):
            with self.subTest(date_raw=date_raw):
                self.assertTrue(
