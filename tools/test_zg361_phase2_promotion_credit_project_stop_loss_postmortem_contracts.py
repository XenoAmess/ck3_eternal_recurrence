#!/usr/bin/env python3
"""Focused tests for R-domain stop-loss/postmortem contracts."""
...
import zg361_phase2_promotion_credit_project_stop_loss_postmortem_contracts as postmortem  # noqa: E402
...
class CreditProjectStopLossPostmortemContractTests(unittest.TestCase):
    def _r238_frame(self) -> tuple[
        ...
        context = {
            ...
            "event_definition_key": "zg361cp.132",
            "current_event_instance_id": 161,
            "date_raw": 53187624,
    ...
    def test_r238_exact_frame_releases_retained_capacity(self) -> None:
        ...
        snapshot, event, context = self._r238_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.132",
            contract=contract,
        )
    ...
    def test_r238_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = (
            postmortem.CREDIT_PROJECT_STOP_LOSS_POSTMORTEM_TIMELINE_CONTRACTS[
                "zg361cp.132"
            ]
