...
class CreditProjectResourceRaceContractTests(unittest.TestCase):
    ...
    def _r216_frame(self) -> tuple[
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.28",
            "current_event_instance_id": 139,
            "date_raw": 53187120,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index, index) for index in range(3)],
        ...
        event = {"event_instance_id": 139}
        return snapshot, event, context

    def _r217_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = race.CREDIT_PROJECT_RESOURCE_RACE_TIMELINE_CONTRACTS[
            "zg361cp.29"
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.29",
            "current_event_instance_id": 140,
            "date_raw": 53187144,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index, index) for index in range(3)],
    ...
    def test_r216_exact_frame_selects_audited_credit_reversal(self) -> None:
        contract = race.CREDIT_PROJECT_RESOURCE_RACE_TIMELINE_CONTRACTS[
            "zg361cp.28"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.28"],
            contract,
        )
        snapshot, event, context = self._r216_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.28",
            contract=contract,
        )
    ...
    def test_r216_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = race.CREDIT_PROJECT_RESOURCE_RACE_TIMELINE_CONTRACTS[
            "zg361cp.28"
        ]
        snapshot, event, context = self._r216_frame()
    ...
    def test_credit_claim_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.28": {', production_source)

    def test_r217_exact_frame_selects_zero_delayed_cost_metric(self) -> None:
        contract = race.CREDIT_PROJECT_RESOURCE_RACE_TIMELINE_CONTRACTS[
            "zg361cp.29"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.29"],
            contract,
        )
        snapshot, event, context = self._r217_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.29",
            contract=contract,
        )
    ...
    def test_r217_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = race.CREDIT_PROJECT_RESOURCE_RACE_TIMELINE_CONTRACTS[
            "zg361cp.29"
        ]
        snapshot, event, context = self._r217_frame()
    ...
    def test_metric_audit_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.29": {', production_source)
