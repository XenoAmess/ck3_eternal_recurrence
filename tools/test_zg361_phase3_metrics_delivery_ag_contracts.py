#!/usr/bin/env python3
"""Focused tests for exact Phase 3 metrics-delivery AG contracts."""
...
import zg361_phase3_metrics_delivery_ag_contracts as metrics_ag  # noqa: E402
...
class Phase3MetricsDeliveryAGContractTests(unittest.TestCase):
    def _r242_live_m301_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = metrics_ag.PHASE3_METRICS_DELIVERY_AG_TIMELINE_CONTRACTS[
            "zg361p3.301"
        ]
        ...
        context = {
            ...
            "event_definition_key": "zg361p3.301",
            "current_event_instance_id": 177,
            "date_raw": 53188056,
        ...
        event = {"event_instance_id": 177}
        return snapshot, event, context

    def test_all_ag_contracts_share_exact_schema_and_route_vector(self) -> None:
        contracts = metrics_ag.PHASE3_METRICS_DELIVERY_AG_TIMELINE_CONTRACTS
        schedule = metrics_ag.PHASE3_METRICS_DELIVERY_AG_ITEMIZED_SCHEDULE
        self.assertEqual(set(contracts), {event_key for event_key, _ in schedule})
        self.assertEqual(
            metrics_ag.PHASE3_METRICS_DELIVERY_AG_ROUTE_VECTOR,
            tuple((event_key, 1, 0) for event_key, _ in schedule),
        )
        for event_key, date_raw in schedule:
            with self.subTest(event_key=event_key):
                ...
                self.assertIs(
                    ...
                    metrics_ag.PHASE3_METRICS_DELIVERY_AG_CHARACTER_SCOPES,
                )
                self.assertEqual(
                    contract["saved_scope_name_sets"],
                    (metrics_ag.PHASE3_METRICS_DELIVERY_AG_SAVED_SCOPE_NAMES,),
                )
                self.assertEqual(len(contract["saved_scope_name_sets"][0]), 58)
                self.assertEqual(len(contract["character_scopes"]), 25)
    ...
    def test_r242_live_frozen_m301_frame_matches(self) -> None:
        contract = metrics_ag.PHASE3_METRICS_DELIVERY_AG_TIMELINE_CONTRACTS[
            "zg361p3.301"
        ]
        snapshot, event, context = self._r242_live_m301_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361p3.301",
            contract=contract,
        )

        self.assertTrue(all(checks.values()), checks)
        self.assertEqual(context["current_event_instance_id"], 177)

    def test_r242_live_m301_rejects_date_scope_and_alias_drift(self) -> None:
        contract = metrics_ag.PHASE3_METRICS_DELIVERY_AG_TIMELINE_CONTRACTS[
            "zg361p3.301"
        ]
        snapshot, event, context = self._r242_live_m301_frame()
        ...
        self.assertFalse(scope_checks["scope:zg361_p3_ag_case:type"])
        ...
        subject = next(
            ...
            if row["name"] == "zg361_p3_ag_subject"
        )
        ...
        self.assertFalse(alias_checks["scope:zg361_p3_ag_subject"])

    def test_ag_source_sequence_barriers_route_and_resource_are_frozen(self) -> None:
        schedule = metrics_ag.PHASE3_METRICS_DELIVERY_AG_ITEMIZED_SCHEDULE
        mids = tuple(int(event_key.rsplit(".", 1)[1]) for event_key, _ in schedule)
        barriers = dict(metrics_ag.PHASE3_METRICS_DELIVERY_AG_STAGE_BARRIERS)
        self.assertEqual(tuple(generator.DOMAIN_ORDER["ag"]), mids)
        self.assertEqual(generator.STAGE_LAST["ag"], barriers)
        ...
        effect_source = "\n".join(
            ...
            for path in sorted(
                (
                    ...
                    / "common"
                    / "scripted_effects"
                ).glob("zg361_phase3_ag_*_effects.txt")
            )
        )
        for index, mid in enumerate(mids):
            with self.subTest(mid=mid):
                ...
                if mid in barriers:
                    self.assertEqual(
                        route_a.count(
                            f"zg361_case_ag_advance_{barriers[mid]:02d}_effect"
                        ),
                        1,
                    )
                else:
                    self.assertNotIn("zg361_case_ag_advance_", route_a)
        ...
        m309_route_a = _script_block(
            effect_source,
            "zg361_p3_m309_route_a_effect = {",
        )
        self.assertLess(
            m309_route_a.index("zg361_p3_ag_management_capacity_remaining >= 10"),
            m309_route_a.index("zg361_case_kernel_record_operation_effect"),
        )
        self.assertIn(
            "change_variable = { name = zg361_p3_ag_management_capacity_remaining subtract = 10 }",
            m309_route_a,
        )
        self.assertIn(
            "change_variable = { name = zg361_p3_ag_management_capacity_used add = 10 }",
            m309_route_a,
        )

        m311_route_a = _script_block(
            effect_source,
            "zg361_p3_m311_route_a_effect = {",
        )
        self.assertIn(
            "trigger_event = { id = zg361p3.9002 days = 1 }",
            m311_route_a,
        )
        queue = _script_block(event_source, "zg361p3.9002 = {")
        self.assertIn("hidden = yes", queue)
        self.assertIn("zg361_p3_aj_launch_effect = yes", queue)

    def test_ag_contracts_are_registered_without_inline_copies(self) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        for event_key in metrics_ag.PHASE3_METRICS_DELIVERY_AG_TIMELINE_CONTRACTS:
