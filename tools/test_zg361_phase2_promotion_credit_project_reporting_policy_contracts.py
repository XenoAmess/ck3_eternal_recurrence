...
class CreditProjectReportingPolicyContractTests(unittest.TestCase):
    ...
    def _r223_frame(self) -> tuple[
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.59",
            "current_event_instance_id": 146,
            "date_raw": 53187288,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index, index) for index in range(3)],
    ...
    def _r225_frame(self) -> tuple[
        ...
        context = {
            ...
            "status": "available",
            "window_match_count": 1,
            "event_definition_key": "zg361cp.60",
            "current_event_instance_id": 148,
            "date_raw": 53187336,
            "root_scope": _character_scope("root", 32904)["scope"],
            "saved_scopes": scopes,
            "options": [_option(index, index) for index in range(3)],
    ...
    def test_r223_exact_frame_selects_low_loss_risk_timing(self) -> None:
        contract = reporting.CREDIT_PROJECT_REPORTING_POLICY_TIMELINE_CONTRACTS[
            "zg361cp.59"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.59"],
            contract,
        )
        snapshot, event, context = self._r223_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.59",
            contract=contract,
        )
    ...
    def test_r223_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = reporting.CREDIT_PROJECT_REPORTING_POLICY_TIMELINE_CONTRACTS[
            "zg361cp.59"
        ]
        snapshot, event, context = self._r223_frame()
    ...
    def test_risk_timing_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
    ...
    def test_r225_exact_frame_preserves_subject_idea_ownership(self) -> None:
        contract = reporting.CREDIT_PROJECT_REPORTING_POLICY_TIMELINE_CONTRACTS[
            "zg361cp.60"
        ]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361cp.60"],
            contract,
        )
        snapshot, event, context = self._r225_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361cp.60",
            contract=contract,
        )
    ...
    def test_r225_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = reporting.CREDIT_PROJECT_REPORTING_POLICY_TIMELINE_CONTRACTS[
            "zg361cp.60"
        ]
        snapshot, event, context = self._r225_frame()
    ...
    def test_idea_arbitration_contract_is_not_inlined_in_production_entry(
        self,
    ) -> None:
        production_source = (
            ROOT / "tools" / "zg361_phase2_promotion_source_production_entry.py"
        ).read_text(encoding="utf-8")
        self.assertNotIn('    "zg361cp.60": {', production_source)
