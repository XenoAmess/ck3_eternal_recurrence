#!/usr/bin/env python3
"""Build and verify the Celestial Commerce & Corruption Workshop release."""
...
PRODUCT_ID = "mod_celestial_commerce_corruption"
PRODUCT_TAG_PREFIX = "celestial-commerce-corruption-v"
...
RUNTIME_FILES = frozenset(
    {
        ...
        "localization/english/xccc_l_english.yml",
        "localization/french/xccc_l_french.yml",
        "localization/german/xccc_l_german.yml",
        "localization/japanese/xccc_l_japanese.yml",
        "localization/korean/xccc_l_korean.yml",
        "localization/polish/xccc_l_polish.yml",
        "localization/russian/xccc_l_russian.yml",
        "localization/simp_chinese/xccc_l_simp_chinese.yml",
        "localization/spanish/xccc_l_spanish.yml",
        "thumbnail.png",
    }
)
SOURCE_ONLY_FILES = frozenset({"README.md", "docs/compatibility-audit.md"})
...
def normalize_workshop_item_id(value: str | None) -> str | None:
    ...
    if value in FORBIDDEN_WORKSHOP_ITEM_IDS:
        raise ValueError(
            "Celestial Commerce & Corruption must not reuse an existing Workshop item ID"
        )
    return value
...
def release_identity(source: Path) -> dict[str, str]:
    if Path(source).resolve() != DEFAULT_SOURCE.resolve():
        raise ValueError("formal release requires the canonical Celestial Commerce & Corruption source")
...
def build_release(
    ...
    *,
    ...
    if errors:
        raise ValueError(
            "invalid Celestial Commerce & Corruption source:\n" + "\n".join(errors)
        )
...
def check_reproducible(source: Path = DEFAULT_SOURCE) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="xccc-release-check-") as name:
        root = Path(name)
        revision = git_sha()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"CELESTIAL COMMERCE & CORRUPTION RELEASE FAILED: {error}", file=sys.stderr)
        return 1
