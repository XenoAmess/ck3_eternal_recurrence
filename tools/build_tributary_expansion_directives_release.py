#!/usr/bin/env python3
"""Build and verify the Tributary Expansion Directives Workshop release."""
...
PRODUCT_ID = "mod_tributary_expansion_directives"
PRODUCT_TAG_PREFIX = "tributary-expansion-directives-v"
...
RUNTIME_FILES = frozenset(
    {
        ...
        "localization/french/ted_tributary_expansion_l_french.yml",
        "localization/german/ted_tributary_expansion_l_german.yml",
        "localization/japanese/ted_tributary_expansion_l_japanese.yml",
        "localization/korean/ted_tributary_expansion_l_korean.yml",
        "localization/polish/ted_tributary_expansion_l_polish.yml",
        "localization/russian/ted_tributary_expansion_l_russian.yml",
        "localization/simp_chinese/ted_tributary_expansion_l_simp_chinese.yml",
        "localization/spanish/ted_tributary_expansion_l_spanish.yml",
        "thumbnail.png",
    }
)
SOURCE_ONLY_FILES = frozenset({"README.md", "docs/requirements-and-feasibility.md"})
...
def _allowed_directories() -> frozenset[str]:
    result: set[str] = set()
    for relative in RUNTIME_FILES | SOURCE_ONLY_FILES:
...
def normalize_workshop_item_id(value: str | None) -> str | None:
    ...
    if value in FORBIDDEN_WORKSHOP_ITEM_IDS:
        raise ValueError(
            "Tributary Expansion Directives must not reuse an existing Workshop item ID"
        )
    return value
...
def release_source_errors(
    ...
    allowed_directories = ALLOWED_DIRECTORIES if allow_source_only_files else frozenset(
        directory
        for relative in RUNTIME_FILES
        for directory in _parent_directories(relative)
    )
    errors.extend(
        f"directory outside exact runtime allowlist: {relative}/"
        for relative in sorted(actual_directories - allowed_directories)
    )
...
def _parent_directories(relative: str) -> tuple[str, ...]:
    result: list[str] = []
    parent = PurePosixPath(relative).parent
    while parent != PurePosixPath("."):
...
def release_identity(source: Path) -> dict[str, str]:
    if Path(source).resolve() != DEFAULT_SOURCE.resolve():
        raise ValueError("formal release requires the canonical Tributary Expansion Directives source")
...
def build_release(
    ...
    *,
    ...
    if errors:
        raise ValueError(
            "invalid Tributary Expansion Directives source:\n" + "\n".join(errors)
        )
...
def check_reproducible(source: Path = DEFAULT_SOURCE) -> dict[str, object]:
    with tempfile.TemporaryDirectory(prefix="ted-release-check-") as name:
        root = Path(name)
        revision = git_sha()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"TRIBUTARY EXPANSION DIRECTIVES RELEASE FAILED: {error}", file=sys.stderr)
        return 1
