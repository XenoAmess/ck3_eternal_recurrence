#!/usr/bin/env python3
"""Unit tests for the XenoAmess Quality of Life release builder."""
...
import build_xenoamess_quality_of_life_release as release  # noqa: E402
...
class XqolReleaseTests(unittest.TestCase):
    @contextmanager
    def fixture(self):
        with tempfile.TemporaryDirectory(prefix="xqol-builder-test-") as name:
            root = Path(name)
            source = root / "source"
            source.mkdir()
            for relative in sorted(release.RUNTIME_FILES):
                ...
                if relative == "descriptor.mod":
                    ...
                    data = codecs.BOM_UTF8 + f"xqol_fixture_{path.stem} = {{}}\n".encode()
                else:
    ...
    def test_check_reproducible_and_versioned_sidecars(self):
        with self.fixture() as (root, source):
            ...
            self.assertEqual("mod_xenoamess_quality_of_life-v1.0.0.zip", archive.name)
            self.assertEqual("xqol-v1.0.0", details["git_tag"])
    ...
    def test_missing_assets_languages_and_any_extra_file_fail_closed(self):
        ...
        for relative in ("notes.md", "workshop/description.bbcode", "tools/helper.py", "events/xqol.txt", "__pycache__/cached.pyc"):
    ...
    def test_remote_id_and_both_existing_item_ids_are_rejected(self):
        for payload, message in ((b'remote_file_id="123"', "remote_file_id"), (b"3784706360", "3784706360"), (b"3787304042", "3787304042")):
            with self.subTest(payload=payload), self.fixture() as (_, source):
                path = source / "common/decisions/xqol_decisions.txt"
    ...
    def test_manifest_and_formal_release_identity_are_strict(self):
        with self.fixture() as (root, source):
            ...
            def clean_git(*args, **_kwargs):
                return "" if args[0] == "status" else "xqol-v1.0.0"
            ...
            self.assertEqual("xqol-v1.0.0", identity["git_tag"])
