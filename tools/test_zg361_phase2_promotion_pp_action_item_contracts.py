#!/usr/bin/env python3
"""Focused tests for purpose-split PP feedback action-item contracts."""
...
import zg361_phase2_promotion_pp_action_item_contracts as action_item  # noqa: E402
...
class PpActionItemContractTests(unittest.TestCase):
    def _r208_frame(self) -> tuple[
        dict[str, object], dict[str, object], dict[str, object]
    ]:
        contract = action_item.PP_ACTION_ITEM_TIMELINE_CONTRACTS["zg361pp.153"]
        ...
        context = {
            ...
            "event_definition_key": "zg361pp.153",
            "current_event_instance_id": 131,
            "date_raw": 53186928,
        ...
        event = {"event_instance_id": 131}
        return snapshot, event, context

    def test_r208_exact_frame_selects_policy_debt_continuation(self) -> None:
        contracts = action_item.PP_ACTION_ITEM_TIMELINE_CONTRACTS
        self.assertEqual(set(contracts), {"zg361pp.153"})
        contract = contracts["zg361pp.153"]
        self.assertIs(
            production.KNOWN_TIMELINE_INTERRUPTS["zg361pp.153"],
            contract,
        )
        snapshot, event, context = self._r208_frame()
        checks = production._known_interrupt_checks(
            snapshot=snapshot,
            event=event,
            context=context,
            event_key="zg361pp.153",
            contract=contract,
        )
    ...
    def test_r208_frame_rejects_date_scope_and_alias_drift(self) -> None:
        contract = action_item.PP_ACTION_ITEM_TIMELINE_CONTRACTS["zg361pp.153"]
        snapshot, event, context = self._r208_frame()

        wrong_date = copy.deepcopy(context)
        wrong_date["date_raw"] = 53186904
    ...
    def test_contract_is_not_inlined_in_production_entry(self) -> None:
        ...
        self.assertNotIn('    "zg361pp.153": {', production_source)
