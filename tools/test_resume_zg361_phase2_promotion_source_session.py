...
class RetainedRuntimeDiagnosticTests(unittest.TestCase):
    ...
    def test_named_event_pause_retains_session_without_promotion_capture(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            state, cell, pipe = self._arrange_source(root)
            for name in ("error.log", "debug.log"):
            ...
            artifacts = root / "artifacts"
            event = {
                "event_instance_id": 26,
                "definition_key": "zg361cp.26",
                "option_count": 2,
            }
            snapshot = {
                "snapshot_id": "native:126",
                "native_revision": 126,
                "revision": 2,
                "date_raw": entry.PRODUCT_TIMELINE_ORIGIN_DATE_RAW + 48,
                "map_ready": True,
                "paused": True,
                "speed": 5,
                "played_character": {"character_id": 29037},
            ...
            def pause_on_target(*args: object, **kwargs: object) -> dict[str, object]:
                evidence = kwargs["evidence_out"]
                evidence.update(
                    result="GREEN",
                    readiness="paused-real-zg361cp.26",
                    target_binding=dict(event),
                )
                return evidence
            ...
            with (
                ...
                mock.patch.object(
                    client,
                    "enter_promotion_source_checkpoint_v1",
                    side_effect=pause_on_target,
                ) as enter_target,
                mock.patch.object(
                    client, "capture_promotion_source_checkpoint_v2"
                ) as capture,
            ):
                report = client.run(
                    state_dir=state,
        ...
        self.assertTrue(report["target_pause_only"])
        self.assertEqual(
            report["target_pause"]["readiness"], "paused-real-zg361cp.26"
        )
        self.assertTrue(report["session_retained_for_next_client"])
        capture.assert_not_called()
        self.assertEqual(
            enter_target.call_args.kwargs["pause_on_event_definition_key"],
            "zg361cp.26",
        )
        self.assertTrue(
            enter_target.call_args.kwargs["stop_at_clean_review_boundary"]
        )
