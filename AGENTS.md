...
- 干活中学到的新知识**当场同步进 `docs/`**，不要等任务收尾：尤其是 Paradox 脚本语言
  ...
  启动器/工坊合并行为）的实证结论
- **把文件边界/单文件体量视为加载性能 RED 的首批实证变量。** 若候选在加载阶段超时或明显变慢、但没有对应的 material/parser
  错误，先保留原始 RED，再做语义不变的文件拆分 A/B；必须固定 exact build、profile、候选树与探针，记录拆分前后字节数、SHA、
  顶层定义数、各加载阶段耗时、日志与 cleanup，并证明按原顺序重组正文逐字节一致。拆分后 GREEN 只能表述为“文件边界/单文件体量
  很可能参与故障”，不得在没有更多证据时写成唯一根因。完整流程与 2026-09-03/04 B1 实证见 `docs/testing-workflow.md`。
- **B2 及后续新增/重构的 scripted effect 必须按用途分组拆文件。** 目标每个文件放 `1–10` 个顶层 effect，原则上不得超过 `20` 个；
  generator、静态测试和投影/发布清单都必须覆盖所有分片。确需超过 `20` 时，先在对应专题记录理由与实机证据。
