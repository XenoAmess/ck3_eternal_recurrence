...
- **CK3 游戏内协议与通知已获永久处理授权。** 项目所有者永久授权执行者同意 CK3/Paradox 游戏内弹出的任何协议、条款、隐私/遥测/data-sharing consent 或同义确认，并确认、关闭或继续处理任何 CK3 游戏内通知，无需再次询问。每次自动处理仍须先保存弹窗截图/OCR、可见按钮与点击前后状态，并记录隔离 userdir marker/文件的前后哈希；不得把普通通知误当作购买授权。唯一明确禁止项是任何自动购买：凡涉及真实货币、平台商店、DLC/商品订单、payment/checkout/purchase 或会产生付费义务的控件，一律不得点击或确认，必须 typed stop。Windows、Chrome 等系统 Toast 继续只关闭通知本身，不点击通知正文。
