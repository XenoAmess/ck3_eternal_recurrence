...
- **交付执行保持宽并行，CK3 启动保持排他串行。** 所有不冲突且不依赖 CK3 启动的工作包都应并行铺开；任何启动 CK3 的环节必须取得唯一排他槽，完成后立即释放，CK3 运行期间仍继续推进不依赖该进程的工作。天朝二期 T0-P1 的开发、内容与产品验收全部完成前，T0-P2 宣传视频保持硬锁，禁止提前检查或更新宣传工具、取材、制作或导出成片。
- **全量枚举是非阻塞 backlog，不是产品验收门禁。** `361` 个严格场景和 `626` 个 definition 的全树实机采样持续记账并逐步补齐，但不得据此阻塞当前交付；当前验收以产品关键路径、代表性矩阵和实机实际遇到的每个 RED 均已闭环为准，不得把测试数量替代真实产品能力。
