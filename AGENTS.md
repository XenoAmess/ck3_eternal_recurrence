...
- 本项目禁止任何 PowerShell：不得新增或执行 `.ps1` 脚本，不得在代码、文档、CI、测试、构建、验收或操作命令中调用 `powershell`、`pwsh`、PowerShell cmdlet 或 PowerShell 语法。
- 发现历史已有的 PowerShell 脚本或调用时，必须整改为 Python，并同步更新其调用方、文档与测试；不得继续保留 PowerShell 兼容入口。
- 此整改只针对 PowerShell；现有 Python 之外的其他语言保持不动，不得借机重写或迁移。

## 桌面截图坐标换算

- 从聊天图片预览、远程查看器、缩略图或任何非原始桌面画面读取坐标时，禁止假定屏幕分辨率、宽高比、预览尺寸、等比缩放或固定倍率；历史 `2560×1440 → 2048×1152` 的 `1.25` 只是一条旧实证，不得作为默认值复用。
- 任何桌面点击都必须先取得原始截图，读取该文件真实宽高和点击当时的 `pyautogui.size()`，并显式提供预览中实际图像内容矩形的 left/top/width/height；X/Y 两轴必须独立换算。统一使用 `tools/desktop_coordinate_map.py`，完整合同见 `docs/desktop-coordinate-mapping.md`。
- 使用换算脚本执行点击时必须传 `--receipt`；源截图尺寸与当前桌面尺寸不一致、预览内容矩形缺失、点位越界或点击后回执尺寸变化时一律拒绝点击。不得绕过脚本手算倍率或直接把预览像素交给桌面输入 API。
- 键盘操作前必须切换目标窗口到英文键盘布局并确认 `LANGID=0x0409`，同时确认目标窗口和控件焦点；不得只凭任务栏图标推断。文本输入优先剪贴板粘贴，随后从目标控件读回全文；不一致时可使用 UI Automation `ValuePattern.SetValue` 并再次读回，禁止仅凭按键 ACK 声称输入成功。
- 控件提供 UI Automation 语义操作时可以按 HWND、Name、AutomationId 操作，无需制造截图坐标；`Invoke`、`Select` 返回不代表业务状态已改变，必须读取实际选中项/文本/结果。鼠标兜底仍严格执行上述换算脚本。
...
- **后续视频交付使用 OneDrive 桌面客户端上传**（项目所有者于 2026-09-23 明确指定），单次只传输用户指定的视频文件。
  固定交付目录为 `C:/Users/1/OneDrive/CK3-War-AI-20260923/`；用户已明确采用“固定目录开启同步，后续直接放视频”的流程。
  复用该目录及已保存的选择同步设置，由客户端自动上传，不必每次重新配置或请求授权；不要把项目源码、素材目录或其他文件一起放入。
...
- **当前执行顺序与 `open_kaishek` 预验（2026-09-02 起）。** 天朝二期是当前最高优先级，直到正式发布闭环前不得被其他工作包抢占；G2 作为第二优先级持续并行推进；`Z:\workspace\open_kaishek` 作为两条主线的共同验收加速器持续演进。任何 CK3 验收步骤（静态/fixture、paused snapshot、启动、动作或后置检查）开始前，先判断该步骤是否存在可由 `open_kaishek` 的 parser、validator、IR、finite runtime 或 replay 覆盖的确定性子集；有则先运行离线预验，再进入 CK3。每次预验至少记录 `open_kaishek` commit、profile/version、CK3 exact build/EXE SHA-256、fixture/corpus ID 与 SHA-256、实际命令、结果和不支持项；没有可覆盖语义时记录 `not-applicable` 及原因，不为形式重复执行。预验只用于提前发现确定性问题，不能代替真实 CK3/MCP paused artifact、输入/动作或 GREEN；其 RED 必须与 CK3 capability RED 分开归类。
- **间歇性反思并补充并行工作（持续规则）。** 任务开始、任一工作包完成或进入阻塞、开始等待外部结果、CK3 排他槽取得/释放、以及计划或优先级发生变化时，都必须重新扫描一次当前目标，判断是否还有不依赖该阻点且文件/进程资源不冲突的工作应同时推进。扫描至少覆盖当前 P0、P1、伴随同步、测试/CI、文档/证据与后续门禁准备；发现可执行项就立即铺开，不得因为某一前置等待而让其他主线空闲。只有 CK3 启动环节排他串行；CK3 运行期间仍继续所有不依赖 CK3 的工作。该反思不是制造忙碌：禁止派生无需求依据的功能、重复已有结论、重跑无新观察面的测试或为了填满并发而扩张范围。并发拓扑发生实质调整时，在当日日报记录“新增/暂停/完成的工作包、原因和下一门槛”。
...
- **MCP-first 是默认执行顺序，不只是验收口号。** 每个游戏内动作先检查现有 MCP 能否直接完成或由多个 typed 能力组合完成；缺少阻断当前交付的动作时，优先补成可复用的通用 MCP 能力并做 native 后置读回。只有该 MCP 路线已有可复现实证证明当前无法施工时，才允许降级到官方 UI/桌面自动化，并在 artifact 中记录原因。不得因为旧验收合同曾禁止某个 helper、fixture 或证据捷径，就推导出产品 MCP 不应具备对应的通用能力；验收链是否调用通用能力应由收据单独、诚实记录。
...
- **CK3 游戏内协议与通知已获永久处理授权。** 项目所有者永久授权执行者同意 CK3/Paradox 游戏内弹出的任何协议、条款、隐私/遥测/data-sharing consent 或同义确认，并确认、关闭或继续处理任何 CK3 游戏内通知，无需再次询问。每次自动处理仍须先保存弹窗截图/OCR、可见按钮与点击前后状态，并记录隔离 userdir marker/文件的前后哈希；不得把普通通知误当作购买授权。唯一明确禁止项是使用真实货币在 Paradox、Steam 或其他外部平台自动购买：外部商店、DLC/商品订单、payment/checkout、真实货币价格或会产生现实付费义务的可执行控件一律不得点击或确认，必须 typed stop。CK3 内金币、威望、虔诚、宗族威望、影响力、正统性等游戏资源支出是正常玩法，不属于这项现实购买禁令；正文只提到购买而唯一动作是关闭/好的时可安全关闭。分类必须结合可执行按钮、外部商业上下文、真实货币和游戏内资源信号，冲突或仍有现实付款歧义时零点击并保留证据 typed stop。Windows、Chrome 等系统 Toast 继续只关闭通知本身，不点击通知正文。
...
- 干活中学到的新知识**当场同步进 `docs/`**，不要等任务收尾：尤其是 Paradox 脚本语言
  ...
  启动器/工坊合并行为）的实证结论
- **单文件过大一律视为必须修复的文件边界缺陷，不再讨论它是否会导致某一次故障，也不再等待加载性能 RED 才拆分。** scripted effect
  必须按用途分组拆文件，目标每个文件放 `1–10` 个顶层 effect，原则上不得超过 `20` 个；发现超限即拆分并补齐 generator、静态测试、
  投影与发布清单。确需超过 `20` 时，必须先在对应专题记录不可拆理由和同一精确文件的实机证据；没有这两项不得保留例外。历史 A/B
  与归因材料只作为冻结证据保留，不得继续派生“是否需要拆分”的讨论或未拆分候选。
